...
extern bool asthra_concurrency_create_thread_hybrid(asthra_concurrency_thread_t *thread, 
                                                   void *(*func)(void*), void *arg, 
                                                   const AsthraConcurrencyTaskSpawnOptions *options);
extern uint64_t asthra_concurrency_atomic_increment_counter_explicit(asthra_concurrency_atomic_counter_t *counter, 
                                                                    memory_order order);
...
static void* task_thread_wrapper(void *arg) {
    ...
    atomic_store_explicit(&handle->enhanced_state, ASTHRA_TASK_RUNNING, ASTHRA_MEMORY_ORDER_RELEASE);
    ...
    asthra_concurrency_atomic_increment_counter_explicit(&bridge->stats.tasks_completed, ASTHRA_MEMORY_ORDER_RELAXED);
...
AsthraConcurrencyTaskHandle* Asthra_spawn_task(AsthraConcurrencyTaskFunction func, void* args, 
    ...
    handle->base_handle.task_id = asthra_concurrency_atomic_increment_counter_explicit(&bridge->next_task_id, 
    ...
    AsthraConcurrencyTaskHandle *old_head = atomic_load_explicit(&bridge->task_registry, 
    ...
    asthra_concurrency_atomic_increment_counter_explicit(&bridge->stats.tasks_spawned, ASTHRA_MEMORY_ORDER_RELAXED);
...
AsthraResult Asthra_task_wait_timeout(AsthraConcurrencyTaskHandle* handle, uint64_t timeout_ms) {
    ...
    ASTHRA_CONCURRENCY_MUTEX_LOCK(&handle->result_mutex);
    int result = 0;
    while (!atomic_load_explicit(&handle->is_complete, ASTHRA_MEMORY_ORDER_ACQUIRE) && result == 0) {
        result = ASTHRA_CONCURRENCY_COND_TIMEDWAIT(&handle->completion_cond, 
                                                  &handle->result_mutex, &timeout);
    }
...
AsthraResult Asthra_task_cancel(AsthraConcurrencyTaskHandle* handle) {
    ...
    if (!atomic_load_explicit(&handle->is_complete, ASTHRA_MEMORY_ORDER_ACQUIRE)) {
        atomic_store_explicit(&handle->enhanced_state, ASTHRA_TASK_FAILED, ASTHRA_MEMORY_ORDER_RELEASE);
        atomic_store_explicit(&handle->is_complete, true, ASTHRA_MEMORY_ORDER_RELEASE);
        
        // Create a cancellation error result
        int error_code = ASTHRA_CONCURRENCY_ERROR_TASK_NOT_FOUND; // Use as cancellation error
        handle->base_handle.result = asthra_result_err(&error_code, sizeof(int), 
...
void Asthra_task_handle_free(AsthraConcurrencyTaskHandle* handle) {
    ...
    AsthraConcurrencyTaskHandle *current = atomic_load_explicit(&bridge->task_registry, 
                                                               ASTHRA_MEMORY_ORDER_ACQUIRE);
...
AsthraConcurrencyTaskHandleWithAwait* Asthra_spawn_task_with_handle(
    AsthraConcurrencyTaskFunction func, 
    void* args, 
    size_t args_size, 
    const AsthraConcurrencyTaskSpawnOptions* options) {
