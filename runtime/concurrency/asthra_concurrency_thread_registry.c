...
AsthraResult Asthra_register_c_thread(void) {
    ...
    asthra_concurrency_atomic_store_counter(&thread_data->registration_time, asthra_concurrency_get_timestamp_ms());
    thread_data->next = NULL;
    
    // Add to registry
    ASTHRA_CONCURRENCY_MUTEX_LOCK(&bridge->thread_registry.registry_mutex);
    AsthraConcurrencyThreadData *old_head = atomic_load_explicit(&bridge->thread_registry.threads, 
                                                                ASTHRA_MEMORY_ORDER_ACQUIRE);
    thread_data->next = old_head;
    atomic_store_explicit(&bridge->thread_registry.threads, thread_data, ASTHRA_MEMORY_ORDER_RELEASE);
    asthra_concurrency_atomic_increment_size(&bridge->thread_registry.thread_count);
    atomic_fetch_add_explicit(&bridge->thread_registry.total_registered, 1, ASTHRA_MEMORY_ORDER_RELAXED);
...
void Asthra_unregister_c_thread(void) {
    ...
    ASTHRA_CONCURRENCY_MUTEX_LOCK(&bridge->thread_registry.registry_mutex);
    AsthraConcurrencyThreadData *current = atomic_load_explicit(&bridge->thread_registry.threads, 
                                                               ASTHRA_MEMORY_ORDER_ACQUIRE);
...
AsthraConcurrencyThreadRegistryStats Asthra_get_thread_registry_stats(void) {
    ...
    stats.active_threads = asthra_concurrency_atomic_load_size(&bridge->thread_registry.thread_count);
    stats.total_registered = atomic_load_explicit(&bridge->thread_registry.total_registered, 
                                                 ASTHRA_MEMORY_ORDER_ACQUIRE);
    stats.is_shutdown = atomic_load_explicit(&bridge->thread_registry.shutdown, 
                                            ASTHRA_MEMORY_ORDER_ACQUIRE);
