...
#define ASTHRA_ATOMIC_COMPARE_EXCHANGE_WEAK_EXPLICIT(ptr, expected, desired, success_order, failure_order) \
    atomic_compare_exchange_weak_explicit(ptr, expected, desired, success_order, failure_order)
#define ASTHRA_ATOMIC_COMPARE_EXCHANGE_STRONG_EXPLICIT(ptr, expected, desired, success_order, failure_order) \
...
// =============================================================================
    ...
    #define ASTHRA_CONCURRENCY_COND_TIMEDWAIT(cond, mutex, timeout) (cnd_timedwait(cond, mutex, timeout) == thrd_success ? 0 : ETIMEDOUT)
    ...
    #define ASTHRA_CONCURRENCY_COND_TIMEDWAIT(cond, mutex, timeout) pthread_cond_timedwait(cond, mutex, timeout)
...
// C17 static assertions for concurrency assumptions
    ...
    #else
        #define ASTHRA_STATIC_ASSERT(expr, msg) typedef char asthra_static_assert_##__LINE__[(expr) ? 1 : -1]
    #endif
#endif
