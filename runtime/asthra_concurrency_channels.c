...
AsthraResult Asthra_channel_send(AsthraConcurrencyChannel* channel, 
    ...
    while (atomic_load(&channel->count) >= channel->buffer_capacity && !channel->is_closed) {
        int wait_result;
        if (has_timeout) {
            wait_result = ASTHRA_CONCURRENCY_COND_TIMEDWAIT(&channel->not_full, 
...
AsthraResult Asthra_channel_recv(AsthraConcurrencyChannel* channel, 
    ...
    while (atomic_load(&channel->count) == 0 && !channel->is_closed) {
        int wait_result;
        if (has_timeout) {
            wait_result = ASTHRA_CONCURRENCY_COND_TIMEDWAIT(&channel->not_empty, 
                                                           &channel->mutex, &timeout);
            if (wait_result == ETIMEDOUT) {
                ASTHRA_CONCURRENCY_MUTEX_UNLOCK(&channel->mutex);
                return asthra_concurrency_create_error_result(ASTHRA_CONCURRENCY_ERROR_TASK_TIMEOUT, 
...
bool Asthra_channel_get_stats(AsthraConcurrencyChannel* channel, 
    ...
    if (count) *count = atomic_load(&channel->count);
    if (capacity) *capacity = channel->buffer_capacity;
...
AsthraConcurrencySelectContext* Asthra_select_context_create(size_t max_channels) {
    ...
    if (!context->channels || !context->is_send_op || 
        !context->send_values || !context->recv_buffers) {
...
bool Asthra_select_add_recv(AsthraConcurrencySelectContext* context,
                           ...
                           void* buffer) {
    if (!context || !channel || !buffer) return false;
...
bool Asthra_select_add_send(AsthraConcurrencySelectContext* context,
                           AsthraConcurrencyChannel* channel,
                           const void* value) {
    if (!context || !channel || !value) return false;
...
int Asthra_select_execute(AsthraConcurrencySelectContext* context, 
                         uint64_t timeout_ms) {
    if (!context || context->channel_count == 0) return -2;
...
void Asthra_select_context_destroy(AsthraConcurrencySelectContext* context) {
    ...
    if (context->channels) free(context->channels);
    if (context->is_send_op) free(context->is_send_op);
    if (context->send_values) free(context->send_values);
    if (context->recv_buffers) free(context->recv_buffers);
...
AsthraConcurrencyChannelInfo Asthra_channel_get_info(AsthraConcurrencyChannel* channel) {
    ...
    if (info.buffer_capacity > 0) {
        info.utilization_percent = (double)info.current_count / (double)info.buffer_capacity * 100.0;
    }
