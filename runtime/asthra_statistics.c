...
AsthraGlobalStatistics asthra_statistics_get_snapshot(void) {
    ...
    snapshot.performance.compilation_time_ns = atomic_load(&g_stats.performance.compilation_time_ns);
    snapshot.performance.semantic_analysis_time_ns = atomic_load(&g_stats.performance.semantic_analysis_time_ns);
    snapshot.performance.code_generation_time_ns = atomic_load(&g_stats.performance.code_generation_time_ns);
    snapshot.performance.optimization_time_ns = atomic_load(&g_stats.performance.optimization_time_ns);
    ...
    snapshot.performance.total_execution_time_ns = atomic_load(&g_stats.performance.total_execution_time_ns);
    ...
    snapshot.concurrency.synchronization_operations = atomic_load(&g_stats.concurrency.synchronization_operations);
    ...
    snapshot.concurrency.race_conditions_detected = atomic_load(&g_stats.concurrency.race_conditions_detected);
...
void asthra_atomic_stats_update_allocation(size_t size) {
    if (!asthra_statistics_is_enabled()) return;
...
void asthra_atomic_stats_update_gc_collection(uint64_t collection_time_ns) {
    ...
    while (collection_time_ns > current_max) {
        if (atomic_compare_exchange_weak(&g_stats.gc.max_collection_time_ns, &current_max, collection_time_ns)) {
            break;
        }
    }
    ...
    while (collection_time_ns < current_min) {
        if (atomic_compare_exchange_weak(&g_stats.gc.min_collection_time_ns, &current_min, collection_time_ns)) {
            break;
        }
    }
...
void asthra_statistics_print_report(FILE* output, bool json_format) {
    ...
    if (stats.gc.collections_performed > 0) {
        fprintf(output, "  Average Time: %.2f ms\n", 
                (double)stats.gc.total_collection_time_ns / (double)stats.gc.collections_performed / 1000000.0);
...
char* asthra_statistics_export_json(void) {
    ...
    int len = snprintf(json, buffer_size,
        ...
        stats.gc.collections_performed, stats.gc.total_collection_time_ns,
        stats.gc.bytes_collected, stats.gc.objects_collected,
        stats.gc.min_collection_time_ns, stats.gc.max_collection_time_ns,
        stats.performance.files_compiled, stats.performance.lines_processed,
        stats.performance.symbols_created, stats.performance.compilation_time_ns,
        stats.performance.semantic_analysis_time_ns, stats.performance.code_generation_time_ns,
        stats.performance.optimization_time_ns,
        ...
        stats.concurrency.race_conditions_detected,
        stats.statistics_enabled ? "true" : "false",
