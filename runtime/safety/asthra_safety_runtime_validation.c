...
void asthra_safety_log_scheduler_event(AsthraSchedulerEvent event, const char *details) {
    ...
    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_CONCURRENCY,
              "Scheduler event %d at %lu: %s", event, log_entry.timestamp_ns, 
              details ? details : "");
}
...
void asthra_safety_log_c_task_interaction(uint64_t asthra_task_id, pthread_t c_thread_id, 
    ...
    snprintf(interaction.interaction_details, sizeof(interaction.interaction_details),
            "C-Asthra interaction: task=%lu, thread=%lu, type=%s, data_size=%zu",
            asthra_task_id, (unsigned long)c_thread_id, 
            interaction_type ? interaction_type : "unknown", data_size);
    
    // Log to the standard logging system
    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_CONCURRENCY,
              "C-Task interaction %lu: %s", interaction_id, interaction.interaction_details);
}
...
void asthra_safety_check_unhandled_results(void) {
    ...
    for (size_t i = 0; i < g_result_tracker_count; i++) {
        ...
        if (!tracker->was_handled && 
            (current_time - tracker->creation_timestamp_ns) > timeout_ns) {
            
            asthra_safety_report_violation(ASTHRA_VIOLATION_TYPE_SAFETY, ASTHRA_SAFETY_LEVEL_STANDARD,
                                         "Unhandled Result detected", 
                                         tracker->creation_location, 0, "unknown", 
