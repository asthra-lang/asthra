...
asthra_safety_verify_constant_time_operation(const char *operation_name, void (*operation)(void),
    ...
    snprintf(verification.timing_analysis, sizeof(verification.timing_analysis),
             "Min: %lu ns, Max: %lu ns, Avg: %lu ns, Variance: %.2f%%", min_time, max_time,
             verification.actual_duration_ns, verification.timing_variance * 100.0);
...
AsthraSafetyPerformanceMetrics asthra_safety_get_performance_metrics(void) {
    ...
    if (metrics.safety_check_count > 0) {
        metrics.average_check_time_ns =
            (double)metrics.total_check_time_ns / metrics.safety_check_count;
    }
