...
AsthraStringOperationValidation asthra_safety_validate_string_concatenation(AsthraString *strings,
    ...
    if (total_length > validation.max_safe_length) {
        validation.has_overflow_risk = true;
        snprintf(validation.validation_message, sizeof(validation.validation_message),
                 "Total length %zu exceeds safe limit %zu", total_length,
                 validation.max_safe_length);
    }
...
void asthra_safety_log_task_lifecycle_event(uint64_t task_id, AsthraTaskEvent event,
    ...
    if (g_task_event_count >= g_task_event_capacity) {
        ...
        AsthraTaskEvent *new_events =
            realloc(g_task_events, new_capacity * sizeof(AsthraTaskEvent));
        if (!new_events) {
    ...
    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_CONCURRENCY, "Task %lu: event %d - %s",
               task_id, event, details ? details : "");
}
...
void asthra_safety_log_scheduler_event(AsthraSchedulerEvent event, const char *details) {
    ...
    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_CONCURRENCY, "Scheduler: event %d - %s", event,
               details ? details : "");
}
...
int asthra_safety_register_result_tracker(AsthraResult result, const char *location) {
    ...
    if (result.tag == ASTHRA_RESULT_ERR) {
        ...
        snprintf(tracker->error_context, sizeof(tracker->error_context), "Ok result created at %s",
        ...
        if (!tracker->was_handled && tracker->result.tag == ASTHRA_RESULT_ERR) {
            ...
            if (age > stale_threshold) {
                asthra_safety_report_violation(
                    ...
                    "Unhandled error result detected", tracker->creation_location, 0,
                    "result_tracker", &tracker->result, sizeof(AsthraResult));
            }
        }
    }
...
bool asthra_safety_validate_string_encoding(const char *str, size_t length) {
    ...
    for (size_t i = 0; i < length;) {
        ...
        if (c < 0x80) {
            ...
            if (i + 3 >= length || (str[i + 1] & 0xC0) != 0x80 || (str[i + 2] & 0xC0) != 0x80 ||
                (str[i + 3] & 0xC0) != 0x80) {
                return false;
            }
            i += 4;
