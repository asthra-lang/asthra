...
int asthra_safety_register_ffi_pointer(void *ptr, size_t size, AsthraTransferType transfer, 
    ...
    if (g_ffi_pointer_count >= g_ffi_pointer_capacity) {
        ...
        AsthraFFIPointerTracker *new_pointers = realloc(g_ffi_pointers, 
                                                        new_capacity * sizeof(AsthraFFIPointerTracker));
        if (!new_pointers) {
...
int asthra_safety_unregister_ffi_pointer(void *ptr) {
    ...
    asthra_safety_report_violation(ASTHRA_VIOLATION_FFI_SAFETY, ASTHRA_SAFETY_LEVEL_STANDARD,
                                 "Attempting to unregister unknown FFI pointer", 
                                 __FILE__, __LINE__, __func__, ptr, sizeof(void*));
    return -1;
}
...
AsthraVariadicValidation asthra_safety_validate_variadic_call(void *func_ptr, AsthraVarArg *args, size_t arg_count,
    ...
    for (size_t i = 0; i < arg_count; i++) {
        if (args[i].type != expected_types[i]) {
            validation.is_valid = false;
            snprintf(validation.type_mismatch_details, sizeof(validation.type_mismatch_details),
                    "Type mismatch at argument %zu: expected %d, got %d", 
                    i, expected_types[i], args[i].type);
            return validation;
        }
    }
...
void asthra_safety_log_ffi_call(const char *function_name, void *function_ptr, void **args, size_t arg_count) {
    ...
    snprintf(call_log.call_details, sizeof(call_log.call_details),
            "FFI call %llu: %s(%p) with %zu arguments",
            (unsigned long long)call_id, function_name ? function_name : "unknown", 
            function_ptr, arg_count);
    
    // Log to the standard logging system
    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_FFI,
              "FFI call: %s", call_log.call_details);
}
