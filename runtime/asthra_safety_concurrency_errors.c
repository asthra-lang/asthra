...
void asthra_safety_log_task_lifecycle_event(uint64_t task_id, AsthraTaskEvent event,
    ...
    if (g_task_event_count >= g_task_event_capacity) {
        ...
        AsthraTaskLifecycleEvent *new_events =
            realloc(g_task_events, new_capacity * sizeof(AsthraTaskLifecycleEvent));
        if (!new_events) {
    ...
    const char *event_names[] = {"SPAWNED",   "STARTED", "SUSPENDED", "RESUMED",
                                 "COMPLETED", "FAILED",  "CANCELLED"};

    const char *event_name =
        (event < sizeof(event_names) / sizeof(event_names[0])) ? event_names[event] : "UNKNOWN";

    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_CONCURRENCY, "Task %llu event %s: %s", task_id,
...
static bool asthra_safety_validate_task_state_transition(uint64_t task_id,
                                                         AsthraTaskEvent new_event) {
    ...
    case ASTHRA_TASK_EVENT_SPAWNED:
        return (new_event == ASTHRA_TASK_EVENT_STARTED || new_event == ASTHRA_TASK_EVENT_CANCELLED);

    case ASTHRA_TASK_EVENT_STARTED:
        return (new_event == ASTHRA_TASK_EVENT_SUSPENDED ||
                new_event == ASTHRA_TASK_EVENT_COMPLETED || new_event == ASTHRA_TASK_EVENT_FAILED ||
                new_event == ASTHRA_TASK_EVENT_CANCELLED);

    case ASTHRA_TASK_EVENT_SUSPENDED:
        return (new_event == ASTHRA_TASK_EVENT_RESUMED || new_event == ASTHRA_TASK_EVENT_CANCELLED);
...
void asthra_safety_log_task_lifecycle_event_enhanced(uint64_t task_id, AsthraTaskEvent event,
    ...
    if (!asthra_safety_validate_task_state_transition(task_id, event)) {
        ...
        asthra_safety_report_violation(ASTHRA_VIOLATION_CONCURRENCY, ASTHRA_SAFETY_LEVEL_STANDARD,
                                       violation_msg, __FILE__, __LINE__, __func__, &task_id,
                                       sizeof(task_id));
    }
