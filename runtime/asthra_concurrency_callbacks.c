...
AsthraResult Asthra_enqueue_callback(AsthraConcurrencyCallbackFunction func, void *data,
    ...
    if (atomic_load_explicit(&bridge->global_callback_queue.shutdown,
    ...
    if (asthra_concurrency_atomic_load_size(&bridge->global_callback_queue.size) >=
        bridge->max_callbacks) {
    ...
    // Add to queue
    AsthraConcurrencyCallbackEntry *tail =
        atomic_load_explicit(&bridge->global_callback_queue.tail, ASTHRA_MEMORY_ORDER_ACQUIRE);
    if (!tail) {
        // Empty queue
        atomic_store_explicit(&bridge->global_callback_queue.head, entry,
                              ASTHRA_MEMORY_ORDER_RELEASE);
        atomic_store_explicit(&bridge->global_callback_queue.tail, entry,
                              ASTHRA_MEMORY_ORDER_RELEASE);
    } else {
        // Add to tail
    ...
    atomic_fetch_add_explicit(&bridge->global_callback_queue.enqueue_count, 1,
...
size_t Asthra_process_callbacks(size_t max_callbacks) {
    ...
    while (processed < max_callbacks &&
        ...
        // Remove from queue
        AsthraConcurrencyCallbackEntry *next =
            atomic_load_explicit(&entry->next, ASTHRA_MEMORY_ORDER_ACQUIRE);
        atomic_store_explicit(&bridge->global_callback_queue.head, next,
                              ASTHRA_MEMORY_ORDER_RELEASE);
        if (!next) {
            atomic_store_explicit(&bridge->global_callback_queue.tail, NULL,
                                  ASTHRA_MEMORY_ORDER_RELEASE);
        }
        asthra_concurrency_atomic_decrement_size(&bridge->global_callback_queue.size);
        atomic_fetch_add_explicit(&bridge->global_callback_queue.dequeue_count, 1,
                                  ASTHRA_MEMORY_ORDER_RELAXED);
...
void Asthra_clear_callback_queue(void) {
    ...
    AsthraConcurrencyCallbackEntry *current =
        atomic_load_explicit(&bridge->global_callback_queue.head, ASTHRA_MEMORY_ORDER_ACQUIRE);
    while (current) {
        AsthraConcurrencyCallbackEntry *next =
            atomic_load_explicit(&current->next, ASTHRA_MEMORY_ORDER_ACQUIRE);
        ...
        atomic_fetch_add_explicit(&bridge->global_callback_queue.drop_count, 1,
                                  ASTHRA_MEMORY_ORDER_RELAXED);
...
AsthraResult Asthra_enqueue_priority_callback(AsthraConcurrencyCallbackFunction func, void *data,
    ...
    if (insert_at_front) {
        ...
        AsthraConcurrencyCallbackEntry *head =
            atomic_load_explicit(&bridge->global_callback_queue.head, ASTHRA_MEMORY_ORDER_ACQUIRE);
...
AsthraConcurrencyCallbackQueueStats Asthra_get_callback_queue_stats(void) {
    ...
    stats.total_dropped = atomic_load_explicit(&bridge->global_callback_queue.drop_count,
                                               ASTHRA_MEMORY_ORDER_ACQUIRE);
    stats.is_shutdown =
        atomic_load_explicit(&bridge->global_callback_queue.shutdown, ASTHRA_MEMORY_ORDER_ACQUIRE);
    ...
    if (head) {
        ...
        uint64_t head_timestamp =
            atomic_load_explicit(&head->timestamp, ASTHRA_MEMORY_ORDER_ACQUIRE);
