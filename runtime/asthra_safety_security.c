...
int asthra_safety_check_stack_canary(void) {
    ...
    if (!g_thread_canary->is_active) {
        asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_ENHANCED,
                                       "Stack canary is not active", __FILE__, __LINE__, __func__,
                                       NULL, 0);
        return -1;
    }
    ...
    if (g_thread_canary->thread_id != pthread_self()) {
        asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_ENHANCED,
                                       "Stack canary thread mismatch", __FILE__, __LINE__, __func__,
                                       NULL, 0);
        return -1;
    }
    ...
    if (g_thread_canary->canary_value == 0) {
        asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_ENHANCED,
                                       "Stack canary value corrupted (zero)", __FILE__, __LINE__,
                                       __func__, NULL, 0);
        return -1;
    }
    ...
    if (stack_growth > 1024 * 1024) {
        ...
        snprintf(warning_msg, sizeof(warning_msg), "Large stack growth detected: %ld bytes",
                 (long)stack_growth);

        asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_STANDARD,
                                       warning_msg, __FILE__, __LINE__, __func__, &stack_growth,
                                       sizeof(stack_growth));
    }
...
static int asthra_safety_validate_memory_protection(void *ptr, size_t size) {
    ...
    if (!ptr || size == 0) {
        asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_STANDARD,
                                       "Invalid memory protection validation parameters", __FILE__,
                                       __LINE__, __func__, NULL, 0);
        return -1;
    }
...
static int asthra_safety_validate_pointer_alignment(void *ptr, size_t alignment) {
    ...
    if (addr % alignment != 0) {
        ...
        snprintf(violation_msg, sizeof(violation_msg), "Pointer %p not aligned to %zu bytes", ptr,
                 alignment);

        asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_STANDARD,
                                       violation_msg, __FILE__, __LINE__, __func__, &addr,
                                       sizeof(addr));
        return -1;
    }
...
static int asthra_safety_secure_zero_memory(void *ptr, size_t size) {
    ...
    for (size_t i = 0; i < size; i++) {
        if (check_ptr[i] != 0) {
            char violation_msg[256];
            snprintf(violation_msg, sizeof(violation_msg), "Secure zeroing failed at offset %zu",
                     i);

            asthra_safety_report_violation(ASTHRA_VIOLATION_SECURITY, ASTHRA_SAFETY_LEVEL_ENHANCED,
                                           violation_msg, __FILE__, __LINE__, __func__, &i,
                                           sizeof(i));
            return -1;
        }
    }
...
static void asthra_safety_audit_thread_security(void) {
    ...
    asthra_log(ASTHRA_LOG_DEBUG, ASTHRA_LOG_CATEGORY_GENERAL,
               "Security audit completed for thread %lu", (unsigned long)pthread_self());
}
