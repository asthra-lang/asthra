...
const AsthraSafetyConfig ASTHRA_SAFETY_CONFIG_DEBUG = {
    .level = ASTHRA_SAFETY_LEVEL_ENHANCED,
...
void asthra_safety_report_violation(AsthraViolationType type, AsthraSafetyLevel severity, 
    ...
    asthra_log(log_level, ASTHRA_LOG_CATEGORY_GENERAL,
              "SAFETY VIOLATION [%d]: %s at %s:%d in %s", 
              type, message, location, line, function);
    
    // In debug builds, we might want to abort on certain violations
    if (severity >= ASTHRA_SAFETY_LEVEL_ENHANCED && g_safety_config.level >= ASTHRA_SAFETY_LEVEL_ENHANCED) {
...
void asthra_safety_cleanup(void) {
    ...
    asthra_log(ASTHRA_LOG_INFO, ASTHRA_LOG_CATEGORY_GENERAL,
              "Asthra safety system cleaned up");
}
