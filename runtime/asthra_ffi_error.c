...
void* asthra_ffi_error_create_err_value(AsthraCFFIError* error) {
    ...
    AsthraCFFIError* err_value = asthra_ffi_error_create(error->code, 
                                                      error->subsystem ? 
                                                      error->subsystem->data : "unknown");
    
    if (err_value) {
        // Copy the source location
        asthra_ffi_error_set_location(err_value, 
                                    error->source_file ? 
                                    error->source_file->data : "unknown", 
                                    error->line);
    }
...
size_t asthra_ffi_error_to_string(AsthraCFFIError* error, 
    ...
    const char* subsystem_str = error->subsystem && error->subsystem->data ? 
                              error->subsystem->data : "unknown";
    const char* message_str = error->message && error->message->data ? 
                            error->message->data : "Unknown error";
    const char* file_str = error->source_file && error->source_file->data ? 
                         error->source_file->data : "unknown";
    
    size_t result = (size_t)snprintf(buffer, buffer_size, 
                  "FFI error (%s): [%d] %s at %s:%d",
                  subsystem_str, error->code, message_str, 
                  file_str, error->line);
