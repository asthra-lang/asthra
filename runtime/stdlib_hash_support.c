 ...
 */
static uint64_t siphash_internal(const uint8_t* data, size_t len, AsthraSipHashKey key, int c_rounds, int d_rounds) {
    ...
    if (ctx->buffer_len > 0) {
        ...
        if (ctx->buffer_len == 8) {
            ...
            int c_rounds = (ctx->variant == ASTHRA_SIPHASH_1_3) ? 1 : 
                          (ctx->variant == ASTHRA_SIPHASH_4_8) ? 4 : 2;
            for (int i = 0; i < c_rounds; ++i) {
...
uint64_t asthra_siphash_finalize(AsthraSipHashContext* ctx) {
    ...
    int d_rounds = (ctx->variant == ASTHRA_SIPHASH_1_3) ? 3 : 
                   (ctx->variant == ASTHRA_SIPHASH_4_8) ? 8 : 4;
    for (int i = 0; i < d_rounds; ++i) {
        sip_round(&ctx->v0, &ctx->v1, &ctx->v2, &ctx->v3);
    }
...
const char* asthra_hash_function_name(AshtraHashFunction function) {
    switch (function) {
        case ASTHRA_HASH_SIPHASH_2_4: return "SipHash-2-4";
        case ASTHRA_HASH_SIPHASH_1_3: return "SipHash-1-3";
        case ASTHRA_HASH_FNV_1A: return "FNV-1a";
        case ASTHRA_HASH_CITYHASH: return "CityHash";
...
const char* asthra_siphash_variant_name(AsthraSipHashVariant variant) {
    switch (variant) {
        case ASTHRA_SIPHASH_1_3: return "SipHash-1-3";
        case ASTHRA_SIPHASH_2_4: return "SipHash-2-4";
        case ASTHRA_SIPHASH_4_8: return "SipHash-4-8";
        default: return "Unknown";
    }
}
