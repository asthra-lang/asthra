...
AsthraTaskHandle asthra_spawn_task_with_stack(AsthraTaskFunction func, void *args, size_t args_size, size_t stack_size) {
    ...
    if (pthread_create(&task->thread, NULL, task_runner, task) != 0) {
        if (task_args) asthra_free(task_args, ASTHRA_ZONE_GC);
        asthra_free(task, ASTHRA_ZONE_GC);
        AsthraTaskHandle handle = {0};
        return handle;
    }
...
AsthraResult asthra_task_await(AsthraTaskHandle handle) {
    ...
    if (task->args) asthra_free(task->args, ASTHRA_ZONE_GC);
    asthra_free(task, ASTHRA_ZONE_GC);
...
void asthra_log_task_stats(void) {
    ...
    asthra_log(ASTHRA_LOG_INFO, ASTHRA_LOG_CATEGORY_CONCURRENCY,
               "Task Stats - Spawned: %zu, Completed: %zu",
               (size_t)stats.tasks_spawned, (size_t)stats.tasks_completed);
} 
