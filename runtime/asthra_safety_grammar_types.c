...
AsthraGrammarValidation asthra_safety_validate_grammar(const char *source_code, size_t code_length) {
    ...
    if (brace_count != 0 || paren_count != 0 || bracket_count != 0) {
        validation.result = ASTHRA_GRAMMAR_INVALID_SYNTAX;
        snprintf(validation.error_message, sizeof(validation.error_message),
                "Unmatched delimiters: braces=%d, parentheses=%d, brackets=%d",
...
AsthraTypeSafetyCheck asthra_safety_validate_result_type_usage(AsthraResult result, uint32_t expected_type_id) {
    ...
    check.actual_type_id = (result.tag == ASTHRA_RESULT_OK) ? result.data.ok.value_type_id : result.data.err.error_type_id;
    check.context = "Result type validation";
    
    uint32_t actual_type_id = (result.tag == ASTHRA_RESULT_OK) ? result.data.ok.value_type_id : result.data.err.error_type_id;
    if (actual_type_id != expected_type_id) {
        check.is_valid = false;
        snprintf(check.type_error_message, sizeof(check.type_error_message),
                "Type mismatch: expected type %u, got type %u", 
