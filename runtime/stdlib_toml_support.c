...
AsthraResult asthra_toml_table_set(AsthraTOMLTable *table, const char *key,
                                   AsthraTOMLValue *value) {
    if (!table || !key || !value) {
    ...
    if (table->count >= table->capacity) {
        ...
        AsthraTOMLValue *new_values =
            realloc(table->values, new_capacity * sizeof(AsthraTOMLValue));
...
AsthraTOMLConfig asthra_toml_default_config(void) {
    AsthraTOMLConfig config = {
        .preserve_order = true, .allow_mixed_arrays = false, .strict_mode = true, .max_depth = 64};
    return config;
}

AsthraTOMLConfig asthra_toml_lenient_config(void) {
    AsthraTOMLConfig config = {.preserve_order = false,
