...
AsthraStringOperationValidation asthra_safety_validate_string_concatenation(AsthraString *strings,
    ...
    validation.is_deterministic =
        !has_null_strings && !has_encoding_issues && !validation.has_overflow_risk;
...
AsthraBoundaryCheck asthra_safety_slice_bounds_check(AsthraSliceHeader slice, size_t index) {
    ...
    if (!slice.ptr) {
        ...
        asthra_safety_report_violation(ASTHRA_VIOLATION_MEMORY_SAFETY, ASTHRA_SAFETY_LEVEL_ENHANCED,
                                       check.error_details, __FILE__, __LINE__, __func__, &check,
                                       sizeof(check));
        return check;
    }
    ...
    if (slice.element_size == 0) {
        ...
        snprintf(check.error_details, sizeof(check.error_details), "Slice has zero element size");
    ...
    if (index > SIZE_MAX / slice.element_size) {
        check.is_out_of_bounds = true;
        snprintf(check.error_details, sizeof(check.error_details),
                 "Index %zu would cause arithmetic overflow with element size %zu", index,
                 slice.element_size);
...
AsthraMemoryLayoutValidation asthra_safety_validate_slice_header(AsthraSliceHeader slice) {
    ...
    // Overall validation
    validation.is_valid = validation.has_valid_pointer && validation.has_consistent_length &&
                          validation.has_correct_element_size && validation.has_valid_capacity &&
