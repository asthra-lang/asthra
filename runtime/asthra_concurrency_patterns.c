...
static void *worker_thread(void *arg) {
    ...
    while (!atomic_load_explicit(&pool->shutdown, memory_order_acquire)) {
        ...
        AsthraResult result =
            Asthra_channel_recv(pool->task_queue, &task_func, 100); // 100ms timeout

        if (asthra_result_is_ok(result)) {
            // Execute the task
            AsthraResult task_result =
                task_func(NULL, 0); // Simplified - real implementation would pass proper args
...
AsthraConcurrencyWorkerPool *Asthra_worker_pool_create(size_t num_workers, size_t queue_size,
                                                       const char *name) {
    if (num_workers == 0)
        return NULL;
    ...
    pool->task_queue =
        Asthra_channel_create(sizeof(AsthraConcurrencyTaskFunction), queue_size, pool->name);
    if (!pool->task_queue) {
        free(pool);
        return NULL;
    }
...
AsthraConcurrencyTaskHandleWithAwait *Asthra_worker_pool_submit(AsthraConcurrencyWorkerPool *pool,
...
void Asthra_worker_pool_shutdown(AsthraConcurrencyWorkerPool *pool, bool wait_for_completion) {
    if (!pool)
        return;
...
AsthraResult Asthra_worker_pool_get_stats(AsthraConcurrencyWorkerPool *pool, size_t *active_workers,
                                          size_t *queue_size, uint64_t *completed_tasks) {
...
void Asthra_worker_pool_dump_stats(AsthraConcurrencyWorkerPool *pool, FILE *output) {
    ...
    AsthraResult result =
        Asthra_worker_pool_get_stats(pool, &active_workers, &queue_size, &completed_tasks);
    if (asthra_result_is_err(result)) {
...
AsthraResult Asthra_pattern_fan_out(AsthraConcurrencyChannel *input_channel,
                                    AsthraConcurrencyChannel **output_channels, size_t num_outputs,
...
AsthraResult Asthra_pattern_fan_in(AsthraConcurrencyChannel **input_channels, size_t num_inputs,
...
AsthraResult Asthra_pattern_pipeline(AsthraConcurrencyTaskFunction *stages, size_t num_stages,
...
AsthraConcurrencyMultiplexer *Asthra_multiplexer_create(AsthraConcurrencyChannel **channels,
                                                        size_t channel_count,
                                                        AsthraConcurrencyTaskFunction processor) {
    if (!channels || channel_count == 0 || !processor)
        return NULL;
...
AsthraConcurrencyLoadBalancer *
Asthra_load_balancer_create(AsthraConcurrencyWorkerPool **pools, size_t pool_count,
                            AsthraConcurrencyLoadBalanceStrategy strategy) {
...
Asthra_load_balancer_submit(AsthraConcurrencyLoadBalancer *balancer,
                            AsthraConcurrencyTaskFunction func, void *args, size_t args_size) {
    if (!balancer || !func || !balancer->is_active)
        return NULL;
    ...
    case ASTHRA_LOAD_BALANCE_ROUND_ROBIN:
        selected_pool_index =
            atomic_fetch_add(&balancer->current_pool_index, 1) % balancer->pool_count;
        break;
    ...
    case ASTHRA_LOAD_BALANCE_LEAST_BUSY: {
        ...
        for (size_t i = 0; i < balancer->pool_count; i++) {
            size_t active_workers, queue_size;
            uint64_t completed_tasks;
            AsthraResult result = Asthra_worker_pool_get_stats(balancer->pools[i], &active_workers,
                                                               &queue_size, &completed_tasks);
