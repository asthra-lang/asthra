    ...
    if (data && data_size > 0) {
        if (data_size <= sizeof(uint64_t)) {
            ...
            if (variant.data.value_ptr) {
                ...
                ASTHRA_SET_ERROR(ASTHRA_ERROR_OUT_OF_MEMORY,
                                 "Failed to allocate memory for enum variant data");
...
AsthraEnumVariant Asthra_result_create_ok(void *value, size_t value_size, uint32_t value_type_id,
    ...
    return Asthra_enum_create_variant(ASTHRA_RESULT_TAG_OK, value, value_size, value_type_id,
                                      ownership);
}

AsthraEnumVariant Asthra_result_create_err(void *error, size_t error_size, uint32_t error_type_id,
                                           AsthraOwnershipHint ownership) {
    return Asthra_enum_create_variant(ASTHRA_RESULT_TAG_ERR, error, error_size, error_type_id,
                                      ownership);
}
...
AsthraEnumVariant Asthra_option_create_some(void *value, size_t value_size, uint32_t value_type_id,
                                            AsthraOwnershipHint ownership) {
    return Asthra_enum_create_variant(ASTHRA_OPTION_TAG_SOME, value, value_size, value_type_id,
                                      ownership);
}
...
size_t Asthra_enum_debug_string(AsthraEnumVariant *variant, char *buffer, size_t buffer_size) {
    ...
    int result =
        snprintf(buffer, buffer_size, "EnumVariant{tag=%u, size=%zu, type_id=%u, storage=%s}",
