...
AsthraFFIResult Asthra_slice_subslice(AsthraFFISliceHeader slice, size_t start, size_t end) {
    if (slice.magic != ASTHRA_SLICE_MAGIC) {
        return Asthra_result_err(4, // ASTHRA_FFI_ERROR_INVALID_SLICE
                                 "Invalid slice magic number", "Asthra_slice_subslice", NULL);
    }

    if (start > end || end > slice.len) {
        return Asthra_result_err(3, // ASTHRA_FFI_ERROR_BOUNDS_CHECK
                                 "Invalid subslice range", "Asthra_slice_subslice", NULL);
    }
    ...
    if (!subslice) {
        return Asthra_result_err(2, // ASTHRA_FFI_ERROR_OUT_OF_MEMORY
                                 "Failed to allocate subslice", "Asthra_slice_subslice", NULL);
    }
...
bool Asthra_slice_is_valid(AsthraFFISliceHeader slice) {
    return slice.magic == ASTHRA_SLICE_MAGIC && slice.element_size > 0 && slice.len <= slice.cap &&
...
AsthraFFIResult Asthra_slice_bounds_check(AsthraFFISliceHeader slice, size_t index) {
    if (slice.magic != ASTHRA_SLICE_MAGIC) {
        return Asthra_result_err(4, // ASTHRA_FFI_ERROR_INVALID_SLICE
                                 "Invalid slice magic number", "Asthra_slice_bounds_check", NULL);
    }

    if (index >= slice.len) {
        return Asthra_result_err(3, // ASTHRA_FFI_ERROR_BOUNDS_CHECK
                                 "Index out of bounds", "Asthra_slice_bounds_check", NULL);
    }
...
AsthraFFIResult Asthra_slice_get_element(AsthraFFISliceHeader slice, size_t index,
    ...
    if (!out_element) {
        return Asthra_result_err(1, // ASTHRA_FFI_ERROR_NULL_POINTER
                                 "Output element pointer is NULL", "Asthra_slice_get_element",
                                 NULL);
    }
...
AsthraFFIResult Asthra_slice_set_element(AsthraFFISliceHeader slice, size_t index,
    ...
    if (!element) {
        return Asthra_result_err(1, // ASTHRA_FFI_ERROR_NULL_POINTER
                                 "Element pointer is NULL", "Asthra_slice_set_element", NULL);
    }

    if (!slice.is_mutable) {
        return Asthra_result_err(5, // ASTHRA_FFI_ERROR_OWNERSHIP
                                 "Slice is not mutable", "Asthra_slice_set_element", NULL);
    }
