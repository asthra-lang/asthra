...
AsthraFFIResult Asthra_ownership_register(void *ptr, size_t size, AsthraOwnershipTransfer ownership,
    ...
    if (!ptr) {
        return Asthra_result_err(1, // ASTHRA_FFI_ERROR_NULL_POINTER
                                 "Pointer is NULL", "Asthra_ownership_register", NULL);
    }
    ...
    if (existing) {
        ...
        return Asthra_result_err(5, // ASTHRA_FFI_ERROR_OWNERSHIP
                                 "Pointer already registered", "Asthra_ownership_register", ptr);
    }
...
AsthraFFIResult Asthra_ownership_unregister(void *ptr) {
    if (!ptr) {
        return Asthra_result_err(1, // ASTHRA_FFI_ERROR_NULL_POINTER
                                 "Pointer is NULL", "Asthra_ownership_unregister", NULL);
    }
    ...
    if (!block) {
        pthread_mutex_unlock(&g_ffi_memory.mutex);
        return Asthra_result_err(5, // ASTHRA_FFI_ERROR_OWNERSHIP
                                 "Pointer not registered", "Asthra_ownership_unregister", ptr);
    }
...
AsthraFFIResult Asthra_ownership_transfer(void *ptr, AsthraOwnershipTransfer new_ownership) {
    if (!ptr) {
        return Asthra_result_err(1, // ASTHRA_FFI_ERROR_NULL_POINTER
                                 "Pointer is NULL", "Asthra_ownership_transfer", NULL);
    }
    ...
    if (!block) {
        pthread_mutex_unlock(&g_ffi_memory.mutex);
        return Asthra_result_err(5, // ASTHRA_FFI_ERROR_OWNERSHIP
                                 "Pointer not registered", "Asthra_ownership_transfer", ptr);
    }
    ...
    return Asthra_result_ok(old_ownership_ptr, sizeof(int), 0, ASTHRA_OWNERSHIP_TRANSFER_FULL);
}

AsthraFFIResult Asthra_ownership_query(void *ptr) {
    if (!ptr) {
        return Asthra_result_err(1, // ASTHRA_FFI_ERROR_NULL_POINTER
                                 "Pointer is NULL", "Asthra_ownership_query", NULL);
    }
    ...
    if (!block) {
        pthread_mutex_unlock(&g_ffi_memory.mutex);
        return Asthra_result_err(5, // ASTHRA_FFI_ERROR_OWNERSHIP
                                 "Pointer not registered", "Asthra_ownership_query", ptr);
    }
    ...
    if (!info) {
        pthread_mutex_unlock(&g_ffi_memory.mutex);
        return Asthra_result_err(2, // ASTHRA_FFI_ERROR_OUT_OF_MEMORY
                                 "Failed to allocate ownership info", "Asthra_ownership_query",
                                 NULL);
    }
    ...
    return Asthra_result_ok(info, sizeof(AsthraOwnershipInfo), 0, ASTHRA_OWNERSHIP_TRANSFER_FULL);
}
