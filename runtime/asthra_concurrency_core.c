...
uint64_t asthra_concurrency_atomic_increment_counter_explicit(asthra_concurrency_atomic_counter_t *counter, 
                                                             memory_order order) {
    return atomic_fetch_add_explicit(counter, 1, order);
}

// C17 atomic decrement with explicit memory ordering - Phase 1.2
uint64_t asthra_concurrency_atomic_decrement_counter_explicit(asthra_concurrency_atomic_counter_t *counter, 
                                                             memory_order order) {
    return atomic_fetch_sub_explicit(counter, 1, order);
}

// C17 atomic load with explicit memory ordering - Phase 1.2
uint64_t asthra_concurrency_atomic_load_counter_explicit(asthra_concurrency_atomic_counter_t *counter, 
                                                        memory_order order) {
...
bool asthra_concurrency_atomic_cas_ptr_explicit(_Atomic(void*) *ptr, void **expected, void *desired,
                                               memory_order success_order, memory_order failure_order) {
    return atomic_compare_exchange_weak_explicit(ptr, expected, desired, success_order, failure_order);
}
...
uint64_t asthra_concurrency_atomic_increment_counter(asthra_concurrency_atomic_counter_t *counter) {
    return asthra_concurrency_atomic_increment_counter_explicit(counter, ASTHRA_MEMORY_ORDER_RELAXED);
}

uint64_t asthra_concurrency_atomic_decrement_counter(asthra_concurrency_atomic_counter_t *counter) {
    return asthra_concurrency_atomic_decrement_counter_explicit(counter, ASTHRA_MEMORY_ORDER_RELAXED);
}
...
bool asthra_concurrency_atomic_cas_ptr(_Atomic(void*) *ptr, void **expected, void *desired) {
    return asthra_concurrency_atomic_cas_ptr_explicit(ptr, expected, desired, 
                                                     ASTHRA_MEMORY_ORDER_ACQ_REL, ASTHRA_MEMORY_ORDER_ACQUIRE);
}
...
AsthraResult asthra_concurrency_create_error_result(AsthraConcurrencyErrorCode code, const char *message) {
    AsthraFFIResult ffi_result = Asthra_result_err(code, message, "asthra_concurrency_bridge", NULL);
    
    // Convert FFI result to runtime result
    AsthraResult result;
    if (ffi_result.tag == ASTHRA_FFI_RESULT_ERR) {
        result = asthra_result_err(&ffi_result.data.err.error_code, sizeof(int), 
...
void Asthra_concurrency_bridge_cleanup(void) {
    ...
    atomic_store_explicit(&g_bridge.global_callback_queue.shutdown, true, ASTHRA_MEMORY_ORDER_RELEASE);
    ...
    AsthraConcurrencyTaskHandle *current = atomic_load_explicit(&g_bridge.task_registry, 
                                                               ASTHRA_MEMORY_ORDER_ACQUIRE);
    while (current) {
    ...
    AsthraConcurrencyThreadData *thread_data = atomic_load_explicit(&g_bridge.thread_registry.threads,
                                                                   ASTHRA_MEMORY_ORDER_ACQUIRE);
    while (thread_data) {
...
void Asthra_dump_concurrency_state(FILE* output) {
    ...
    fprintf(output, "Initialized: %s\n", atomic_load_explicit(&g_bridge.initialized, ASTHRA_MEMORY_ORDER_ACQUIRE) ? "Yes" : "No");
    ...
    fprintf(output, "Shutdown: %s\n", atomic_load_explicit(&g_bridge.global_callback_queue.shutdown, ASTHRA_MEMORY_ORDER_ACQUIRE) ? "Yes" : "No");
    
    fprintf(output, "\n=== Thread Registry ===\n");
    fprintf(output, "Registered Threads: %zu\n", asthra_concurrency_atomic_load_size(&g_bridge.thread_registry.thread_count));
    fprintf(output, "Registry Shutdown: %s\n", atomic_load_explicit(&g_bridge.thread_registry.shutdown, ASTHRA_MEMORY_ORDER_ACQUIRE) ? "Yes" : "No");
}
