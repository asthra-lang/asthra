...
void *asthra_ffi_alloc(size_t size, AsthraTransferType transfer) {
    AsthraMemoryZone zone = (transfer == ASTHRA_TRANSFER_FULL) ? ASTHRA_ZONE_MANUAL : ASTHRA_ZONE_GC;
...
static const char *log_category_names[] = {
    "GENERAL", "MEMORY", "GC", "FFI", "CONCURRENCY", "SECURITY", "PERFORMANCE"
};
...
AsthraSliceHeader asthra_slice_new(size_t element_size, size_t len, AsthraOwnershipHint ownership) {
    ...
    AsthraMemoryZone zone = (ownership == ASTHRA_OWNERSHIP_GC) ? ASTHRA_ZONE_GC : ASTHRA_ZONE_MANUAL;
...
size_t asthra_slice_get_len(AsthraSliceHeader slice) { return slice.len; }
size_t asthra_slice_get_cap(AsthraSliceHeader slice) { return slice.cap; }
size_t asthra_slice_get_element_size(AsthraSliceHeader slice) { return slice.element_size; }
