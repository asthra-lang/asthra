...
AsthraTestResult test_slice_header_structure(AsthraV12TestContext *ctx) {
    ...
    for (size_t i = 0; i < size_count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, slice->data != NULL,
                               "Slice data should not be NULL")) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, slice->capacity == capacity,
                               "Slice capacity should be %zu, got %zu", capacity, slice->capacity)) {
            test_slice_destroy(slice);
            return ASTHRA_TEST_FAIL;
        }
        
        if (!ASTHRA_TEST_ASSERT(&ctx->base, slice->element_size == element_size,
                               "Element size should be %zu, got %zu", element_size, slice->element_size)) {
            test_slice_destroy(slice);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_slice_header_operations(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, int_slice != NULL,
                           "Failed to create integer slice")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, value_ptr != NULL,
                               "Failed to get element at index %d", i)) {
            test_slice_destroy(int_slice);
            return ASTHRA_TEST_FAIL;
        }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, success,
                               "Failed to set element at index %d", i)) {
            test_slice_destroy(int_slice);
            return ASTHRA_TEST_FAIL;
        }
...
int main(void) {
    ...
    AsthraTestResult results[] = {
        test_slice_header_structure(&ctx),
