...
AsthraTestResult test_slice_pqc_sized_data(AsthraV12TestContext *ctx) {
    ...
    for (size_t i = 0; i < element_count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, success, "Failed to push large element %zu", i)) {
            free(pattern_buffer);
            test_slice_destroy(large_slice);
    ...
    for (size_t i = 0; i < element_count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, element != NULL, "Failed to get large element %zu",
                                i)) {
            free(pattern_buffer);
            test_slice_destroy(large_slice);
            return ASTHRA_TEST_FAIL;
...
AsthraTestResult test_slice_large_data_memory_safety(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !valid, "Bounds check should fail for index at capacity")) {
        // Cleanup
        for (size_t j = 0; j < stress_capacity; j++) {
            free(allocated_ptrs[j]);
        }
...
int main(void) {
    ...
    // Run tests
    AsthraTestResult results[] = {test_slice_pqc_sized_data(&ctx),
                                  test_slice_large_data_performance(&ctx),
                                  test_slice_large_data_memory_safety(&ctx)};

    // Print results
    const char *test_names[] = {"PQC-Sized Data", "Large Data Performance", "Memory Safety"};
