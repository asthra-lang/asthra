...
AsthraTestResult test_slice_header_memory_layout(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, element_distance == sizeof(double),
                            "Elements not contiguous: distance %td, expected %zu", element_distance,
                            sizeof(double))) {
        test_slice_destroy(slice);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_slice_predictable_memory_layout(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, struct_slice != NULL, "Failed to create struct slice")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, element->b == i * 2.5,
                                "Element %d field 'b': expected %.1f, got %.1f", i, i * 2.5,
                                element->b)) {
            test_slice_destroy(struct_slice);
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, element->c == 'A' + i,
                                "Element %d field 'c': expected '%c', got '%c'", i, 'A' + i,
                                element->c)) {
            test_slice_destroy(struct_slice);
            return ASTHRA_TEST_FAIL;
        }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, diff == sizeof(struct TestStruct),
                                "Non-contiguous memory layout: difference %td, expected %zu", diff,
...
AsthraTestResult test_slice_alignment_requirements(AsthraV12TestContext *ctx) {
    ...
    for (size_t i = 0; i < test_count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(
                &ctx->base, (data_addr % alignment) == 0,
...
AsthraTestResult test_slice_memory_efficiency(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, slice != NULL, "Failed to create efficiency test slice")) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, slice->capacity == capacity,
                            "Capacity should match requested: expected %zu, got %zu", capacity,
                            slice->capacity)) {
        test_slice_destroy(slice);
        return ASTHRA_TEST_FAIL;
    }
    ...
    for (size_t i = 0; i < capacity; i++) {
        ...
        if (!success)
            break; // Expected when reaching capacity
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, slice->len == capacity,
                            "Should have filled to capacity: expected %zu, got %zu", capacity,
                            slice->len)) {
        test_slice_destroy(slice);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestResult results[] = {
        test_slice_header_memory_layout(&ctx), test_slice_predictable_memory_layout(&ctx),
        test_slice_alignment_requirements(&ctx), test_slice_memory_efficiency(&ctx)};

    // Print results
    const char *test_names[] = {"Header Memory Layout", "Predictable Memory Layout",
                                "Alignment Requirements", "Memory Efficiency"};
