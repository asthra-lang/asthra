...
AsthraTestResult test_slice_bounds_checking_runtime(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, valid,
                               "Valid index %d should pass bounds check", i)) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, ptr != NULL,
                               "Valid access at index %d should succeed", i)) {
            test_slice_destroy(slice);
            return ASTHRA_TEST_FAIL;
        }
    }
    ...
    for (int i = 3; i < 10; i++) {
        bool valid = test_slice_bounds_check(slice, i);
        if (!ASTHRA_TEST_ASSERT(&ctx->base, !valid,
                               "Invalid index %d should fail bounds check", i)) {
            test_slice_destroy(slice);
            return ASTHRA_TEST_FAIL;
        }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !valid,
                           "Corrupted slice should fail bounds check")) {
        test_slice_destroy(slice);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_slice_bounds_checking_edge_cases(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, empty_slice != NULL,
                           "Failed to create empty slice")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !valid,
                           "Maximum size_t index should fail bounds check")) {
        test_slice_destroy(empty_slice);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, full_slice != NULL,
                           "Failed to create full slice")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, valid,
                           "Last valid index should pass bounds check")) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !valid,
                           "First invalid index should fail bounds check")) {
        test_slice_destroy(full_slice);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !valid,
                           "NULL slice should fail bounds check")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_slice_bounds_checking_performance(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, slice != NULL,
                           "Failed to create performance test slice")) {
...
int main(void) {
    ...
    AsthraTestResult results[] = {
        test_slice_bounds_checking_runtime(&context),
