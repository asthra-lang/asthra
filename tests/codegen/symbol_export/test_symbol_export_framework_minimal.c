...
typedef enum { SYMBOL_PRIVATE, SYMBOL_PUBLIC, SYMBOL_INTERNAL } SymbolVisibility;

// Simulate symbol types
typedef enum { SYMBOL_FUNCTION, SYMBOL_VARIABLE, SYMBOL_STRUCT, SYMBOL_METHOD } SymbolType;
...
DEFINE_TEST(test_public_symbol_export) {
    ...
    TEST_ASSERT_STR_EQ(func->mangled_name, "pub_main_function",
                       "Public function should have correct mangling");
...
DEFINE_TEST(test_private_symbol_export) {
    ...
    TEST_ASSERT_STR_EQ(func->mangled_name, "priv_internal_func",
                       "Private function should have correct mangling");
...
DEFINE_TEST(test_symbol_export_directives) {
    ...
    for (size_t i = 0; i < symbol_count; i++) {
        if (symbol_table[i].exported) {
            ...
            strncat(export_directives, directive,
                    sizeof(export_directives) - strlen(export_directives) - 1);
        }
    }

    // Verify export directives contain public symbols
    TEST_ASSERT(strstr(export_directives, "pub_Point_new") != NULL,
                "Export directives should contain public method");
    TEST_ASSERT(strstr(export_directives, "pub_Point_distance") != NULL,
                "Export directives should contain public method");
    TEST_ASSERT(strstr(export_directives, "priv_Point_internal_validate") == NULL,
                "Export directives should not contain private method");
...
RUN_TEST_SUITE(symbol_export_suite, RUN_TEST(test_public_symbol_export);
               RUN_TEST(test_private_symbol_export); RUN_TEST(test_mixed_visibility_export);
               RUN_TEST(test_symbol_export_directives); RUN_TEST(test_symbol_table_generation);
