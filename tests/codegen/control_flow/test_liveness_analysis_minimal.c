...
static AsthraTestMetadata test_metadata[] = {
    {"test_basic_liveness_analysis", __FILE__, __LINE__, "Test basic liveness analysis",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_control_flow_liveness", __FILE__, __LINE__, "Test liveness analysis with control flow",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_use_def_chains", __FILE__, __LINE__, "Test use-definition chains",
     ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
    {"test_live_intervals", __FILE__, __LINE__, "Test live interval computation",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}};

// Instruction types
typedef enum { INSTR_DEF, INSTR_USE, INSTR_BRANCH, INSTR_NOP } InstructionType;
...
static void add_instruction(MinimalBasicBlock *block, InstructionType type, int reg, int point) {
    if (block->instruction_count < 16) {
        block->instructions[block->instruction_count++] =
            (MinimalInstruction){.type = type, .virtual_reg = reg, .program_point = point};
    }
}
