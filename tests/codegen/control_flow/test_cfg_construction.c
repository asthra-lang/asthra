...
AsthraTestResult test_cfg_construction(AsthraTestContext* context) {
    ...
    // Note: The AssemblyInstruction struct uses flexible array members, so we need to allocate them individually
    ...
    instructions[0]->operands[1] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 10};
    ...
    instructions[1]->operands[1] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 5};
    ...
    instructions[3]->operands[1] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 1};
    ...
    instructions[5]->operands[1] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 0};
    ...
    if (!asthra_test_assert_bool(context, num_blocks == 4, "Expected 4 basic blocks, got %zu", num_blocks)) {
        cleanup_control_flow_analysis_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, num_successors == 2, "Entry block should have 2 successors, got %zu", num_successors)) {
        cleanup_control_flow_analysis_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_cfg_with_loops(AsthraTestContext* context) {
    ...
    instructions[0]->operand_count = 2;
    instructions[0]->operands[0] = (AssemblyOperand){.type = OPERAND_REGISTER, .data.reg = 0};
    instructions[0]->operands[1] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 0};
    ...
    instructions[1]->operand_count = 2;
    instructions[1]->operands[0] = (AssemblyOperand){.type = OPERAND_REGISTER, .data.reg = 0};
    instructions[1]->operands[1] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 10};
    ...
    instructions[3]->operands[2] = (AssemblyOperand){.type = OPERAND_IMMEDIATE, .data.immediate = 1};
    
    // Back edge: JMP loop_header
    instructions[4] = malloc(sizeof(AssemblyInstruction) + sizeof(AssemblyOperand));
    instructions[4]->type = INST_JMP;
    instructions[4]->operand_count = 1;
    instructions[4]->operands[0] = (AssemblyOperand){.type = OPERAND_LABEL, .data.label = "loop_header"};
