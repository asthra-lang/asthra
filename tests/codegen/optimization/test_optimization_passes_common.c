...
OptimizationResult test_optimizer_common_subexpression_elimination(Optimizer *opt,
    ...
    for (size_t i = 0; i < buffer->count; i++) {
        for (size_t j = i + 1; j < buffer->count; j++) {
            ...
            if (instr1->opcode == instr2->opcode && instr1->opcode == OP_ADD &&
...
OptimizationResult test_optimizer_peephole_optimization(Optimizer *opt, InstructionBuffer *buffer) {
    ...
    for (size_t i = 0; i < buffer->count - 1; i++) {
        ...
        if (instr1->opcode == OP_MOV && instr2->opcode == OP_MOV &&
            instr1->dst_reg == instr2->operand1.reg && instr2->dst_reg == instr1->operand1.reg) {
        ...
        // Eliminate ADD x, 0
        if (instr1->opcode == OP_ADD && instr1->operand2.type == OPERAND_IMMEDIATE &&
