...
AsthraTestResult test_constant_folding(AsthraTestContext* context) {
    ...
    instructions[0] = (Instruction){.opcode = OP_MOV, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 5}};
    instructions[1] = (Instruction){.opcode = OP_MOV, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 3}};
    instructions[2] = (Instruction){.opcode = OP_ADD, .operand1 = {.type = OPERAND_REGISTER}, .operand2 = {.type = OPERAND_REGISTER}};
    instructions[3] = (Instruction){.opcode = OP_MUL, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 2}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 4}};
    instructions[4] = (Instruction){.opcode = OP_SUB, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 10}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 3}};
    ...
    OptimizationResult result = test_optimizer_constant_folding(fixture->optimizer, fixture->instruction_buffer);
...
AsthraTestResult test_dead_code_elimination(AsthraTestContext* context) {
    ...
    OptimizationResult result = test_optimizer_dead_code_elimination(fixture->optimizer, fixture->instruction_buffer);
    ...
    if (!asthra_test_assert_bool(context, result.dead_code_eliminated == 3,
                                "Expected 3 dead instructions to be eliminated, got %d", result.dead_code_eliminated)) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, remaining_instructions == 3,
                                "Expected 3 live instructions to remain, got %zu", remaining_instructions)) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_common_subexpression_elimination(AsthraTestContext* context) {
    ...
    instructions[0] = (Instruction){.opcode = OP_MOV, .dst_reg = 0, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 5}};
    instructions[1] = (Instruction){.opcode = OP_MOV, .dst_reg = 1, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 3}};
    instructions[2] = (Instruction){.opcode = OP_ADD, .dst_reg = 2, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_REGISTER, .reg = 1}};  // r2 = r0 + r1
    instructions[3] = (Instruction){.opcode = OP_MOV, .dst_reg = 3, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 7}};
    instructions[4] = (Instruction){.opcode = OP_ADD, .dst_reg = 4, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_REGISTER, .reg = 1}};  // r4 = r0 + r1 (duplicate)
    instructions[5] = (Instruction){.opcode = OP_MUL, .dst_reg = 5, .operand1 = {.type = OPERAND_REGISTER, .reg = 2}, .operand2 = {.type = OPERAND_REGISTER, .reg = 3}};
    instructions[6] = (Instruction){.opcode = OP_ADD, .dst_reg = 6, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_REGISTER, .reg = 1}};  // r6 = r0 + r1 (duplicate)
    instructions[7] = (Instruction){.opcode = OP_SUB, .dst_reg = 7, .operand1 = {.type = OPERAND_REGISTER, .reg = 4}, .operand2 = {.type = OPERAND_REGISTER, .reg = 6}};
    ...
    OptimizationResult result = test_optimizer_common_subexpression_elimination(fixture->optimizer, fixture->instruction_buffer);
    if (!asthra_test_assert_bool(context, result.success, "Common subexpression elimination failed")) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, result.cse_eliminations >= 2,
                                "Expected at least 2 CSE eliminations, got %d", result.cse_eliminations)) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_loop_optimization(AsthraTestContext* context) {
    ...
    instructions[1] = (Instruction){.opcode = OP_MOV, .dst_reg = 1, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 10}};    // limit = 10
    instructions[2] = (Instruction){.opcode = OP_MOV, .dst_reg = 2, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 5}};     // invariant = 5
    // Loop start
    instructions[3] = (Instruction){.opcode = OP_CMP, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_REGISTER, .reg = 1}};  // cmp i, limit
    instructions[4] = (Instruction){.opcode = OP_JGE, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 9}};                   // jge end
    instructions[5] = (Instruction){.opcode = OP_MUL, .dst_reg = 3, .operand1 = {.type = OPERAND_REGISTER, .reg = 2}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 2}};  // invariant * 2 (should be hoisted)
    instructions[6] = (Instruction){.opcode = OP_ADD, .dst_reg = 4, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_REGISTER, .reg = 3}};     // i + (invariant * 2)
    instructions[7] = (Instruction){.opcode = OP_ADD, .dst_reg = 0, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 1}};  // i++
    instructions[8] = (Instruction){.opcode = OP_JMP, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 3}};                   // jmp loop_start
    // Loop end
    ...
    OptimizationResult result = test_optimizer_peephole_optimization(fixture->optimizer, fixture->instruction_buffer);
    ...
    if (!asthra_test_assert_bool(context, result.loop_invariant_motions >= 1,
                                "Expected at least 1 loop invariant motion, got %d", result.loop_invariant_motions)) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_strength_reduction(AsthraTestContext* context) {
    ...
    Instruction instructions[6];
    instructions[0] = (Instruction){.opcode = OP_MOV, .dst_reg = 0, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 7}};
    instructions[1] = (Instruction){.opcode = OP_MUL, .dst_reg = 1, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 2}};   // x * 2 -> x << 1
    instructions[2] = (Instruction){.opcode = OP_MUL, .dst_reg = 2, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 4}};   // x * 4 -> x << 2
    instructions[3] = (Instruction){.opcode = OP_MUL, .dst_reg = 3, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 8}};   // x * 8 -> x << 3
    instructions[4] = (Instruction){.opcode = OP_DIV, .dst_reg = 4, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 2}};   // x / 2 -> x >> 1
    instructions[5] = (Instruction){.opcode = OP_DIV, .dst_reg = 5, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 4}};   // x / 4 -> x >> 2
    ...
    OptimizationResult result = test_optimizer_strength_reduction(fixture->optimizer, fixture->instruction_buffer);
    ...
    if (!asthra_test_assert_bool(context, result.strength_reductions >= 4,
                                "Expected at least 4 strength reductions, got %d", result.strength_reductions)) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_peephole_optimization(AsthraTestContext* context) {
    ...
    Instruction instructions[8];
    instructions[0] = (Instruction){.opcode = OP_MOV, .dst_reg = 0, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 5}};
    instructions[1] = (Instruction){.opcode = OP_MOV, .dst_reg = 1, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}};       // mov r1, r0
    instructions[2] = (Instruction){.opcode = OP_MOV, .dst_reg = 2, .operand1 = {.type = OPERAND_REGISTER, .reg = 1}};       // mov r2, r1 (can be optimized to mov r2, r0)
    instructions[3] = (Instruction){.opcode = OP_ADD, .dst_reg = 3, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 0}};  // add r3, r0, 0 (can be mov r3, r0)
    instructions[4] = (Instruction){.opcode = OP_MUL, .dst_reg = 4, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 1}};  // mul r4, r0, 1 (can be mov r4, r0)
    instructions[5] = (Instruction){.opcode = OP_SUB, .dst_reg = 5, .operand1 = {.type = OPERAND_REGISTER, .reg = 0}, .operand2 = {.type = OPERAND_IMMEDIATE, .value = 0}};  // sub r5, r0, 0 (can be mov r5, r0)
    instructions[6] = (Instruction){.opcode = OP_JMP, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 7}};                   // jmp next
    ...
    if (!asthra_test_assert_bool(context, result.peephole_optimizations >= 3,
                                "Expected at least 3 peephole optimizations, got %d", result.peephole_optimizations)) {
        cleanup_optimization_passes_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
