...
static int test_return_statement_generation(void) {
    ...
    LLVMBasicBlockRef new_block = LLVMAppendBasicBlockInContext(g_backend_data->context, g_backend_data->current_function, "after_return");
...
static int test_void_return_statement(void) {
    ...
    LLVMBasicBlockRef new_block = LLVMAppendBasicBlockInContext(g_backend_data->context, g_backend_data->current_function, "after_void_return");
    LLVMPositionBuilderAtEnd(g_backend_data->builder, new_block);
...
static int test_variable_declaration(void) {
    ...
    TEST_ASSERT(LLVMGetInstructionOpcode(var_alloca) == LLVMAlloca, "Should be an alloca instruction");
...
static int test_unimplemented_statements(void) {
    ...
    continue_stmt->location = (SourceLocation){.filename = NULL, .line = 1, .column = 1, .offset = 0};
...
static int test_unknown_statement_type(void) {
    ...
    unknown_stmt->location = (SourceLocation){.filename = NULL, .line = 1, .column = 1, .offset = 0};
