...
static ASTNode* create_integer_literal(int64_t value) {
    ...
    node->location = (SourceLocation){.filename = "test.asthra", .line = 1, .column = 1, .offset = 0};
    node->ref_count = 1;
    return node;
}
...
static ASTNode* create_cast_expr(ASTNode *expression, TypeInfo *target_type_info) {
    ...
    cast_node->location = (SourceLocation){.filename = "test.asthra", .line = 1, .column = 1, .offset = 0};
...
static int test_integer_size_cast(void) {
    ...
    TEST_ASSERT(LLVMGetTypeKind(result_type) == LLVMIntegerTypeKind, "Result should be integer type");
...
static int test_cast_missing_expression(void) {
    ...
    cast_expr->location = (SourceLocation){.filename = "test.asthra", .line = 1, .column = 1, .offset = 0};
