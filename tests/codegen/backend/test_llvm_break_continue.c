...
static bool test_break_inside_loop() {
    ...
    // Create loop blocks
    LLVMBasicBlockRef continue_block = LLVMAppendBasicBlockInContext(data->context, data->current_function, "continue");
    LLVMBasicBlockRef break_block = LLVMAppendBasicBlockInContext(data->context, data->current_function, "break");
...
static bool test_nested_loops() {
    ...
    LLVMBasicBlockRef outer_continue = LLVMAppendBasicBlockInContext(data->context, data->current_function, "outer_continue");
    LLVMBasicBlockRef outer_break = LLVMAppendBasicBlockInContext(data->context, data->current_function, "outer_break");
    
    // Create inner loop blocks
    LLVMBasicBlockRef inner_continue = LLVMAppendBasicBlockInContext(data->context, data->current_function, "inner_continue");
    LLVMBasicBlockRef inner_break = LLVMAppendBasicBlockInContext(data->context, data->current_function, "inner_break");
    ...
    bool correct_inner = (current_loop && 
                         current_loop->continue_block == inner_continue &&
    ...
    bool correct_outer = (current_loop && 
                         current_loop->continue_block == outer_continue &&
...
static bool test_loop_context_management() {
    ...
    if (!current || current->continue_block != continue_block || current->break_block != break_block) {
        cleanup_test_backend(data);
        return false;
    }
