...
static AsthraTestMetadata test_metadata[] = {
    {"test_nested_struct_operations", __FILE__, __LINE__, "Test nested struct creation and access", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
...
int main(void) {
    ...
    results[0] = (TestResult){
        .test_name = "Nested Struct Operations",
    ...
    results[1] = (TestResult){
        .test_name = "Struct Inheritance Simulation",
    ...
    results[2] = (TestResult){
        .test_name = "Struct Method Dispatch",
    ...
    if (results[2].result == ASTHRA_TEST_PASS) {
        printf("   ✅ PASS: Virtual method tables, dispatch simulation, and signature validation\n");
        passed++;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        const char* priority = (results[i].severity == ASTHRA_TEST_SEVERITY_HIGH) ? "HIGH" : "MEDIUM";
        printf("  [%s] %s (%s): %s\n", 
               status, results[i].test_name, priority, results[i].description);
    }
