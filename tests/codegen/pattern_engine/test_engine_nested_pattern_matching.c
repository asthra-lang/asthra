 ...
 * Pattern Engine Tests - Nested Pattern Matching
 *
 * Copyright (c) 2024 Asthra Project
 * Licensed under the terms specified in LICENSE
 *
 * Test complex nested pattern matching with Option and Result types
 */

#include "test_pattern_engine_common.h"

// Test metadata
static AsthraTestMetadata test_metadata = {"test_nested_pattern_matching",
                                           __FILE__,
                                           __LINE__,
                                           "Test nested and complex pattern matching",
                                           ASTHRA_TEST_SEVERITY_HIGH,
                                           5000000000ULL,
                                           false,
                                           NULL};

static AsthraTestResult test_nested_pattern_matching(AsthraTestContext *context) {
    ...
    Pattern *some_42_pattern =
        create_pattern_constructor(&ctx, VARIANT_TAG_OPTION_SOME, "Some", int_pattern);
    Pattern *some_var_pattern =
    ...
    // Verify statistics
    if (complex_match->total_executions != 2 || complex_match->successful_matches != 2) {
        return ASTHRA_TEST_FAIL;
    }
...
// Main test runner
int main(void) {
    printf("=== Pattern Engine Tests - Nested Pattern Matching ===\n");
