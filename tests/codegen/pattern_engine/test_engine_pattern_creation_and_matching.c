 ...
 * Pattern Engine Tests - Basic Pattern Creation and Matching
 *
 * Copyright (c) 2024 Asthra Project
 * Licensed under the terms specified in LICENSE
 *
 * Test basic pattern creation and matching functionality
 */

#include "test_pattern_engine_common.h"

// Test metadata
static AsthraTestMetadata test_metadata = {"test_pattern_creation_and_matching",
                                           __FILE__,
                                           __LINE__,
                                           "Test pattern creation and basic matching",
                                           ASTHRA_TEST_SEVERITY_HIGH,
                                           5000000000ULL,
                                           false,
                                           NULL};

static AsthraTestResult test_pattern_creation_and_matching(AsthraTestContext *context) {
    ...
    if (!match_pattern(wildcard, int_val) || !match_pattern(wildcard, string_val) ||
    ...
    if (!match_pattern(var_x, int_val) || !match_pattern(var_x, string_val) ||
...
// Main test runner
int main(void) {
    printf("=== Pattern Engine Tests - Basic Pattern Creation and Matching ===\n");
