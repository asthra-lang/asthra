...
static AsthraTestMetadata test_metadata[] = {
    {"test_future_management", __FILE__, __LINE__, "Test future creation and state management",
...
static AsthraTestResult test_future_management(AsthraTestContext *context) {
    ...
    if (future1->state != ASYNC_STATE_PENDING || future2->state != ASYNC_STATE_PENDING ||
        future3->state != ASYNC_STATE_PENDING) {
        return ASTHRA_TEST_FAIL;
    }

    // Verify unique IDs
    if (future1->future_id == future2->future_id || future2->future_id == future3->future_id ||
    ...
    if (future1->state != ASYNC_STATE_COMPLETED || future1->result_value != &result_value ||
    ...
    // Verify statistics
    if (ctx.completed_futures != 1 || ctx.cancelled_futures != 1 || ctx.error_futures != 1) {
        return ASTHRA_TEST_FAIL;
    }
