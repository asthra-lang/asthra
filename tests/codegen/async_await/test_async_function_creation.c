...
// Test metadata
static AsthraTestMetadata test_metadata[] = {{"test_async_function_creation", __FILE__, __LINE__,
                                              "Test creating async functions and basic structure",
                                              ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false,
                                              NULL}};
...
static AsthraTestResult test_async_function_creation(AsthraTestContext *context) {
    ...
    if (async_func2->yield_count != 3 || async_func2->yield_points[0] != 100 ||
        async_func2->yield_points[1] != 200 || async_func2->yield_points[2] != 300) {
        return ASTHRA_TEST_FAIL;
    }
