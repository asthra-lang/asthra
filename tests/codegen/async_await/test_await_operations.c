...
static AsthraTestMetadata test_metadata[] = {
    {"test_await_operations", __FILE__, __LINE__, "Test await operations and future completion",
...
static AsthraTestResult test_await_operations(AsthraTestContext *context) {
    ...
    if (future2->continuation_count != 3 || future2->continuations[0] != 1001 ||
        future2->continuations[1] != 1002 || future2->continuations[2] != 1003) {
        return ASTHRA_TEST_FAIL;
    }
