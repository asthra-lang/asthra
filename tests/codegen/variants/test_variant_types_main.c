...
static void run_variant_test(const VariantTestInfo* test_info, VariantTestStats* stats, AsthraTestContext* context) {
...
static void calculate_statistics(VariantTestStats* stats) {
    ...
    stats->pass_rate = (stats->total_tests > 0) ? 
        (stats->passed_tests * 100.0) / stats->total_tests : 0.0;
    
    int total_critical = stats->critical_passed + stats->critical_failed;
    stats->critical_pass_rate = (total_critical > 0) ? 
        (stats->critical_passed * 100.0) / total_critical : 0.0;
}
