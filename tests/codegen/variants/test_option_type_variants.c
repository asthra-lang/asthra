...
static AsthraTestMetadata test_metadata[] = {
    {"test_option_type_variants", __FILE__, __LINE__, "Test Option<T> type variants (Some/None)", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}
};
...
static AsthraTestResult test_option_type_variants(AsthraTestContext* context) {
    ...
    // Verify Some variant
    if (some_var->tag != VARIANT_TAG_OPTION_SOME ||
        some_var->data.option_value != inner_int ||
        some_var->size != sizeof(VariantValue*) ||
        some_var->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Verify None variant
    if (none_var->tag != VARIANT_TAG_OPTION_NONE ||
        none_var->data.option_value != NULL ||
        none_var->size != 0 ||
        none_var->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Verify type classification
    if (!is_variant_option_type(some_var) ||
        !is_variant_option_type(none_var)) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (is_variant_option_type(inner_int) ||
        is_variant_result_type(some_var)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (unwrapped->tag != VARIANT_TAG_INT ||
        unwrapped->data.int_value != 100) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!double_nested || 
        double_nested->tag != VARIANT_TAG_OPTION_SOME ||
        double_nested->data.option_value != some_var) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Verify context state
    if (ctx.variant_count != 4 ||
        ctx.option_variants != 3 ||
        ctx.int_variants != 1) {
        return ASTHRA_TEST_FAIL;
    }
