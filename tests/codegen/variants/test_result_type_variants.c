...
static AsthraTestMetadata test_metadata[] = {
    {"test_result_type_variants", __FILE__, __LINE__, "Test Result<T,E> type variants (Ok/Error)", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}
};
...
static AsthraTestResult test_result_type_variants(AsthraTestContext* context) {
    ...
    if (ok_result->tag != VARIANT_TAG_RESULT_OK ||
        ...
        ok_result->data.result.error_value != NULL ||
        ok_result->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (error_result->tag != VARIANT_TAG_RESULT_ERROR ||
        ...
        error_result->data.result.error_value != error_message ||
        error_result->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Verify type classification
    if (!is_variant_result_type(ok_result) ||
        !is_variant_result_type(error_result)) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (is_variant_result_type(success_value) ||
        is_variant_option_type(ok_result)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (unwrap_result_ok(error_result) != NULL ||
        unwrap_result_error(ok_result) != NULL) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!complex_ok ||
        complex_ok->tag != VARIANT_TAG_RESULT_OK ||
        complex_ok->data.result.ok_value != some_int) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Verify context state
    if (ctx.variant_count != 7 ||
        ctx.result_variants != 3 ||
        ctx.string_variants != 2 ||
        ctx.option_variants != 1 ||
        ctx.int_variants != 1) {
        return ASTHRA_TEST_FAIL;
    }
