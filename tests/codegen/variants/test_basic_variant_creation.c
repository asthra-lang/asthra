...
static AsthraTestMetadata test_metadata[] = {
    {"test_basic_variant_creation", __FILE__, __LINE__, "Test basic variant type creation",
...
static AsthraTestResult test_basic_variant_creation(AsthraTestContext *context) {
    ...
    if (int_var->tag != VARIANT_TAG_INT || int_var->data.int_value != 42 ||
        int_var->size != sizeof(int) || int_var->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }

    // Verify float variant
    if (float_var->tag != VARIANT_TAG_FLOAT || float_var->data.float_value != 3.14159 ||
        float_var->size != sizeof(double) || float_var->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (string_var->tag != VARIANT_TAG_STRING ||
        ...
        string_var->size != strlen("hello world") || string_var->variant_id == 0) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!is_variant_primitive_type(int_var) || !is_variant_primitive_type(float_var) ||
        !is_variant_primitive_type(string_var)) {
        return ASTHRA_TEST_FAIL;
    }

    if (is_variant_option_type(int_var) || is_variant_result_type(float_var)) {
        return ASTHRA_TEST_FAIL;
    }

    // Verify context state
    if (ctx.variant_count != 3 || ctx.int_variants != 1 || ctx.float_variants != 1 ||
