...
bool is_variant_option_type(const VariantValue *variant) {
    return variant &&
           (variant->tag == VARIANT_TAG_OPTION_SOME || variant->tag == VARIANT_TAG_OPTION_NONE);
}

bool is_variant_result_type(const VariantValue *variant) {
    return variant &&
           (variant->tag == VARIANT_TAG_RESULT_OK || variant->tag == VARIANT_TAG_RESULT_ERROR);
}

bool is_variant_primitive_type(const VariantValue *variant) {
    return variant && (variant->tag == VARIANT_TAG_INT || variant->tag == VARIANT_TAG_FLOAT ||
                       variant->tag == VARIANT_TAG_STRING);
}
