...
static AsthraTestMetadata test_metadata[] = {
    {"test_interference_graph_construction", __FILE__, __LINE__, "Test interference graph construction", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_interference_graph_degree", __FILE__, __LINE__, "Test interference graph degree calculation", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_interference_graph_neighbors", __FILE__, __LINE__, "Test interference graph neighbor management", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
    {"test_live_range_interference", __FILE__, __LINE__, "Test live range interference detection", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}
};
...
static void add_interference_edge(MinimalInterferenceGraph* graph, int node1, int node2) {
    if (node1 >= 0 && node1 < graph->node_count && 
        node2 >= 0 && node2 < graph->node_count && 
        node1 != node2) {
...
static AsthraTestResult test_interference_graph_neighbors(AsthraTestContext* context) {
    ...
    if (!nodes_interfere(&graph, 0, 1) || !nodes_interfere(&graph, 0, 2) || !nodes_interfere(&graph, 0, 3)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (nodes_interfere(&graph, 1, 2) || nodes_interfere(&graph, 3, 4) || nodes_interfere(&graph, 4, 5)) {
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_live_range_interference(AsthraTestContext* context) {
    ...
    if (!nodes_interfere(&graph, 1, 0) || !nodes_interfere(&graph, 1, 2) || !nodes_interfere(&graph, 1, 5)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (nodes_interfere(&graph, 0, 2) || nodes_interfere(&graph, 0, 3) || nodes_interfere(&graph, 0, 4)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    // Range overlaps: (0-10)&(5-15), (0-10)&(8-14), (5-15)&(12-20), (5-15)&(8-14), (12-20)&(18-25), (12-20)&(8-14)
    if (total_edges != 6) {
        return ASTHRA_TEST_FAIL;
    }
