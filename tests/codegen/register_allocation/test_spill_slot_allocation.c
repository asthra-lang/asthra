...
AsthraTestResult test_spill_slot_reuse(AsthraTestContext* context) {
    ...
    LiveInterval interval1 = {.virtual_reg = 0, .start = 0, .end = 10, .physical_reg = REG_UNASSIGNED};
    LiveInterval interval2 = {.virtual_reg = 1, .start = 15, .end = 25, .physical_reg = REG_UNASSIGNED};
    
    // Allocate spill slots with lifetime information
    int slot1 = spill_manager_allocate_slot_with_lifetime(fixture->spill_manager, &interval1, SPILL_TYPE_GENERAL);
    int slot2 = spill_manager_allocate_slot_with_lifetime(fixture->spill_manager, &interval2, SPILL_TYPE_GENERAL);
