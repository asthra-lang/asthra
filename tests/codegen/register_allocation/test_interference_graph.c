...
AsthraTestResult test_interference_graph_construction(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_bool(context, interference_graph_has_edge(fixture->interference_graph, 0, 1),
                                "Interference edge (0,1) should exist")) {
        cleanup_graph_coloring_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, interference_graph_has_edge(fixture->interference_graph, 1, 2),
                                "Interference edge (1,2) should exist")) {
        cleanup_graph_coloring_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, interference_graph_has_edge(fixture->interference_graph, 0, 2),
    ...
    if (!asthra_test_assert_bool(context, interference_graph_has_edge(fixture->interference_graph, 1, 0),
