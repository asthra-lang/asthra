...
AsthraTestResult test_spill_code_generation(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_bool(context, spill_slot >= 0, "Spill slot allocation should succeed")) {
    ...
    bool spill_result = spill_manager_generate_spill_code(fixture->spill_manager, 
                                                         fixture->instruction_buffer,
                                                         0, spill_slot, 1);  // vreg 0, slot, at position 1
...
AsthraTestResult test_reload_code_generation(AsthraTestContext* context) {
    ...
    size_t initial_instruction_count = instruction_buffer_get_count(fixture->instruction_buffer);
    bool reload_result = spill_manager_generate_reload_code(fixture->spill_manager,
                                                           fixture->instruction_buffer,
                                                           0, spill_slot, 2, 1);  // vreg 0, slot, at position 2, to physical reg 1
...
AsthraTestResult test_spill_reload_optimization(AsthraTestContext* context) {
    ...
    spill_manager_generate_spill_code(fixture->spill_manager, fixture->instruction_buffer,
                                     0, spill_slot, 1);
    spill_manager_generate_reload_code(fixture->spill_manager, fixture->instruction_buffer,
                                      0, spill_slot, 2, 1);
    ...
    if (!asthra_test_assert_bool(context, instruction_count_after_opt <= instruction_count_before_opt,
