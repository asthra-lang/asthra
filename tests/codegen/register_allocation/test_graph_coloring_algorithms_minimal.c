...
static AsthraTestMetadata test_metadata[] = {
    {"test_basic_graph_coloring", __FILE__, __LINE__, "Test basic graph coloring algorithm",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_graph_coloring_bipartite", __FILE__, __LINE__, "Test bipartite graph coloring",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_graph_coloring_spilling", __FILE__, __LINE__,
     "Test graph coloring with register spilling", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false,
     NULL},
    {"test_interference_graph_construction", __FILE__, __LINE__,
     "Test interference graph construction", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false,
     NULL}};
...
static AsthraTestResult test_interference_graph_construction(AsthraTestContext *context) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int existing_reg = 0; existing_reg < next_physical_reg; existing_reg++) {
            ...
            for (int j = 0; j < i; j++) {
                if (registers[j].physical_reg == existing_reg && registers[i].interferes_with[j]) {
                    conflicts = true;
                    break;
                }
            }
