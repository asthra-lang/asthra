...
AsthraTestResult test_linear_scan_basic_allocation(AsthraTestContext* context) {
    ...
    intervals[0] = (LiveInterval){.virtual_reg = 0, .start = 0, .end = 10, .physical_reg = REG_UNASSIGNED};
    intervals[1] = (LiveInterval){.virtual_reg = 1, .start = 5, .end = 15, .physical_reg = REG_UNASSIGNED};
    intervals[2] = (LiveInterval){.virtual_reg = 2, .start = 12, .end = 20, .physical_reg = REG_UNASSIGNED};
    intervals[3] = (LiveInterval){.virtual_reg = 3, .start = 18, .end = 25, .physical_reg = REG_UNASSIGNED};
    ...
    if (!asthra_test_assert_bool(context, intervals[0].physical_reg == intervals[2].physical_reg,
...
AsthraTestResult test_linear_scan_overlapping_lifetimes(AsthraTestContext* context) {
    ...
    intervals[0] = (LiveInterval){.virtual_reg = 0, .start = 0, .end = 20, .physical_reg = REG_UNASSIGNED};
    intervals[1] = (LiveInterval){.virtual_reg = 1, .start = 5, .end = 25, .physical_reg = REG_UNASSIGNED};
    intervals[2] = (LiveInterval){.virtual_reg = 2, .start = 10, .end = 30, .physical_reg = REG_UNASSIGNED};
    intervals[3] = (LiveInterval){.virtual_reg = 3, .start = 15, .end = 35, .physical_reg = REG_UNASSIGNED};
    intervals[4] = (LiveInterval){.virtual_reg = 4, .start = 20, .end = 40, .physical_reg = REG_UNASSIGNED};
    intervals[5] = (LiveInterval){.virtual_reg = 5, .start = 25, .end = 45, .physical_reg = REG_UNASSIGNED};
    
    // Perform linear scan allocation
    bool result = register_allocator_linear_scan(fixture->allocator, intervals, 6);
    if (!asthra_test_assert_bool(context, result, "Linear scan allocation with overlapping lifetimes failed")) {
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    for (int i = 0; i < 6; i++) {
        if (!asthra_test_assert_bool(context, intervals[i].physical_reg != REG_UNASSIGNED,
                                    "Virtual register %d should be assigned a physical register", i)) {
            cleanup_linear_scan_fixture(fixture);
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_linear_scan_register_pressure(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_bool(context, result, "Linear scan allocation under register pressure failed")) {
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_linear_scan_lifetime_holes(AsthraTestContext* context) {
    ...
    LiveInterval intervals[4];
    intervals[0] = (LiveInterval){.virtual_reg = 0, .start = 0, .end = 10, .physical_reg = REG_UNASSIGNED};   // 0-10
    intervals[1] = (LiveInterval){.virtual_reg = 1, .start = 15, .end = 25, .physical_reg = REG_UNASSIGNED};  // 15-25 (gap from 10-15)
    intervals[2] = (LiveInterval){.virtual_reg = 2, .start = 5, .end = 20, .physical_reg = REG_UNASSIGNED};   // 5-20 (overlaps both)
    intervals[3] = (LiveInterval){.virtual_reg = 3, .start = 12, .end = 18, .physical_reg = REG_UNASSIGNED};  // 12-18 (fits in gap)
    
    // Perform linear scan allocation
    bool result = register_allocator_linear_scan(fixture->allocator, intervals, 4);
    if (!asthra_test_assert_bool(context, result, "Linear scan allocation with lifetime holes failed")) {
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_linear_scan_precolored_registers(AsthraTestContext* context) {
    ...
    intervals[0] = (LiveInterval){.virtual_reg = 0, .start = 0, .end = 10, .physical_reg = 0};  // Precolored to reg 0
    intervals[1] = (LiveInterval){.virtual_reg = 1, .start = 5, .end = 15, .physical_reg = REG_UNASSIGNED};
    intervals[2] = (LiveInterval){.virtual_reg = 2, .start = 8, .end = 18, .physical_reg = 1};  // Precolored to reg 1
    intervals[3] = (LiveInterval){.virtual_reg = 3, .start = 12, .end = 22, .physical_reg = REG_UNASSIGNED};
    intervals[4] = (LiveInterval){.virtual_reg = 4, .start = 16, .end = 26, .physical_reg = REG_UNASSIGNED};
    
    // Perform linear scan allocation
    bool result = register_allocator_linear_scan(fixture->allocator, intervals, 5);
    if (!asthra_test_assert_bool(context, result, "Linear scan allocation with precolored registers failed")) {
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, intervals[1].physical_reg != 0 && intervals[1].physical_reg != 1,
...
AsthraTestResult test_linear_scan_performance(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_bool(context, result, "Linear scan allocation of large interval set failed")) {
        free(intervals);
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, elapsed_time < 1.0,
                                "Linear scan allocation should complete in reasonable time, took %f seconds", elapsed_time)) {
        free(intervals);
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, assigned_count >= num_intervals / 2,
                                "Should assign at least 50%% of intervals, assigned %d/%d", assigned_count, num_intervals)) {
        free(intervals);
        cleanup_linear_scan_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    AsthraTestSuite suite = {
        ...
        .tests = {
            { "Basic Linear Scan Allocation", test_linear_scan_basic_allocation },
