...
static AsthraTestMetadata test_metadata[] = {
    {"test_basic_register_allocation", __FILE__, __LINE__, "Test basic register allocation",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_spill_handling", __FILE__, __LINE__, "Test register spill handling",
     ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_interference_graph", __FILE__, __LINE__, "Test interference graph construction",
     ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
    {"test_register_coalescing", __FILE__, __LINE__, "Test register coalescing optimization",
     ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}};
...
static AsthraTestResult test_basic_register_allocation(AsthraTestContext *context) {
    const char *test_source = "fn test_registers(void) {\n"
                              "    let a = 5;\n"
                              "    let b = 10;\n"
                              "    let c = a + b;\n"
                              "}";
...
static AsthraTestResult test_spill_handling(AsthraTestContext *context) {
    const char *test_source = "fn test_spill(void) {\n"
...
static AsthraTestResult test_interference_graph(AsthraTestContext *context) {
    const char *test_source = "fn test_interference(void) {\n"
                              "    let x = 1;\n"
                              "    if (x > 0) {\n"
                              "        let y = x + 1;\n"
                              "        x = y * 2;\n"
...
static AsthraTestResult test_register_coalescing(AsthraTestContext *context) {
    const char *test_source = "fn test_coalescing(void) {\n"
                              "    let a = 10;\n"
...
// Main test runner
int main(void) {
    AsthraTestFunction tests[] = {test_basic_register_allocation, test_spill_handling,
                                  test_interference_graph, test_register_coalescing};
