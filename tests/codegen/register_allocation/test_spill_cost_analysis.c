...
AsthraTestResult test_spill_cost_calculation(AsthraTestContext *context) {
    ...
    LiveInterval high_use_interval = {.virtual_reg = 0,
                                      .start = 0,
                                      .end = 100,
    ...
    LiveInterval low_use_interval = {.virtual_reg = 1,
                                     .start = 0,
                                     .end = 100,
                                     .physical_reg = REG_UNASSIGNED,
                                     .use_count = 2,
    ...
    double high_use_cost =
        spill_manager_calculate_spill_cost(fixture->spill_manager, &high_use_interval);
    double low_use_cost =
        spill_manager_calculate_spill_cost(fixture->spill_manager, &low_use_interval);
...
AsthraTestResult test_spill_candidate_selection(AsthraTestContext *context) {
    ...
    intervals[0] =
        (LiveInterval){.virtual_reg = 0, .start = 0, .end = 50, .use_count = 10, .def_count = 2};
    intervals[1] = (LiveInterval){.virtual_reg = 1,
                                  .start = 10,
                                  .end = 60,
                                  .use_count = 2,
                                  .def_count = 1}; // Low use - good candidate
    intervals[2] =
        (LiveInterval){.virtual_reg = 2, .start = 20, .end = 70, .use_count = 25, .def_count = 3};
    intervals[3] = (LiveInterval){.virtual_reg = 3,
                                  .start = 30,
                                  .end = 80,
                                  .use_count = 1,
                                  .def_count = 1}; // Very low use - best candidate
    intervals[4] =
        (LiveInterval){.virtual_reg = 4, .start = 40, .end = 90, .use_count = 15, .def_count = 2};
...
AsthraTestResult test_loop_aware_spill_cost(AsthraTestContext *context) {
    ...
    LiveInterval outside_loop = {.virtual_reg = 0,
                                 .start = 0,
                                 .end = 50,
                                 .physical_reg = REG_UNASSIGNED,
                                 .use_count = 5,
    ...
    LiveInterval inside_loop = {
        ...
        .use_count = 5,
        .def_count = 1,
        .loop_depth = 2 // Nested 2 levels deep
    };

    // Calculate spill costs with loop awareness
    double outside_cost =
        spill_manager_calculate_spill_cost_with_loops(fixture->spill_manager, &outside_loop);
    double inside_cost =
        spill_manager_calculate_spill_cost_with_loops(fixture->spill_manager, &inside_loop);

    // Interval inside loop should have much higher spill cost
    if (!asthra_test_assert_bool(
            context, inside_cost > outside_cost * 4, // Loop penalty should be significant
            "Interval inside loop should have much higher spill cost")) {
        cleanup_spill_handling_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    AsthraTestSuite suite = {
        ...
        .tests = {{"Spill Cost Calculation", test_spill_cost_calculation},
