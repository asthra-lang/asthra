...
static AsthraTestMetadata test_metadata[] = {
    {"test_basic_spill_handling", __FILE__, __LINE__, "Test basic register spill handling", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_spill_cost_analysis", __FILE__, __LINE__, "Test spill cost analysis", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_spill_code_generation", __FILE__, __LINE__, "Test spill code generation", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_reload_optimization", __FILE__, __LINE__, "Test register reload optimization", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}
};
...
static AsthraTestResult test_spill_cost_analysis(AsthraTestContext* context) {
    ...
    if (candidates[0].spill_cost != 15) return ASTHRA_TEST_FAIL;
    if (candidates[1].spill_cost != 60) return ASTHRA_TEST_FAIL;
    if (candidates[2].spill_cost != 115) return ASTHRA_TEST_FAIL;
    if (candidates[3].spill_cost != 225) return ASTHRA_TEST_FAIL;
    
    // Verify ordering (lower cost should be preferred for spilling)
    if (candidates[0].spill_cost >= candidates[1].spill_cost) return ASTHRA_TEST_FAIL;
    if (candidates[1].spill_cost >= candidates[2].spill_cost) return ASTHRA_TEST_FAIL;
    if (candidates[2].spill_cost >= candidates[3].spill_cost) return ASTHRA_TEST_FAIL;
...
static AsthraTestResult test_spill_code_generation(AsthraTestContext* context) {
    ...
    if (spill_code[0].type != INSTR_STORE || 
        spill_code[0].src_reg != physical_reg ||
        spill_code[0].stack_offset != stack_slot) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Verify reload instruction
    if (spill_code[2].type != INSTR_LOAD ||
        spill_code[2].dst_reg != physical_reg ||
