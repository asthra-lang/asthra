...
AsthraTestResult test_optimization_validation(AsthraTestContext *context) {
    ...
    if (dead_instructions == 0) {
        printf(
            "ERROR: No dead instructions found. Expected nop instructions to be marked as dead.\n");
        printf("Total instructions: %d\n", ctx.instruction_count);
        for (int i = 0; i < ctx.instruction_count; i++) {
            printf("  [%d] %s (ref_count=%d, is_dead=%d)\n", i, ctx.instructions[i].instruction,
                   ctx.instructions[i].reference_count, ctx.instructions[i].is_dead);
        }
    ...
    if (constant_instructions == 0) {
        printf("ERROR: No constant folding found. Expected 'add $5, %%ecx' to be marked as "
               "constant.\n");
        for (int i = 0; i < ctx.instruction_count; i++) {
