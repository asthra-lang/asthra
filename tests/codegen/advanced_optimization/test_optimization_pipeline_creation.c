...
AsthraTestResult test_optimization_pipeline_creation(AsthraTestContext* context) {
    ...
    for (int i = 0; i < ctx.pass_count; i++) {
        if (ctx.passes[i].type == OPT_DEAD_CODE_ELIMINATION) has_dead_code = true;
        if (ctx.passes[i].type == OPT_CONSTANT_FOLDING) has_constant_fold = true;
        if (ctx.passes[i].type == OPT_COMMON_SUBEXPRESSION) has_cse = true;
    }
