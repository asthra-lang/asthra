...
AsthraTestResult test_variable_debug_info(AsthraTestContext* context) {
    ...
    var_info[0] = (ELFVariableDebugInfo){.name = "x", .type = "int", .location = VAR_LOCATION_REGISTER, .reg = 0, .scope_start = 0x1000, .scope_end = 0x1020};
    var_info[1] = (ELFVariableDebugInfo){.name = "y", .type = "float", .location = VAR_LOCATION_STACK, .stack_offset = -8, .scope_start = 0x1008, .scope_end = 0x1030};
    var_info[2] = (ELFVariableDebugInfo){.name = "ptr", .type = "*int", .location = VAR_LOCATION_MEMORY, .memory_address = 0x2000, .scope_start = 0x1010, .scope_end = 0x1040};
...
AsthraTestResult test_variable_scope_tracking(AsthraTestContext* context) {
    ...
    bool result = elf_writer_lookup_variables_at_address(fixture->elf_writer, 0x1180, &found_vars, &var_count);
    if (!asthra_test_assert_bool(context, result, "Failed to lookup variables at address 0x1180")) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    if (!asthra_test_assert_bool(context, var_count == 4, "Expected 4 variables at address 0x1180, got %zu", var_count)) {
        free(found_vars);
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    result = elf_writer_lookup_variables_at_address(fixture->elf_writer, 0x1050, &found_vars, &var_count);
    if (!asthra_test_assert_bool(context, result, "Failed to lookup variables at address 0x1050")) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    if (!asthra_test_assert_bool(context, var_count == 1, "Expected 1 variable at address 0x1050, got %zu", var_count)) {
        free(found_vars);
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_variable_lifetime_tracking(AsthraTestContext* context) {
    ...
    ELFVariableDebugInfo var_info[] = {
        // Variable starts in register
        {.name = "var_x", .type = "int", .location = VAR_LOCATION_REGISTER, .reg = 0, .scope_start = 0x1000, .scope_end = 0x1010},
        // Same variable moves to stack
        {.name = "var_x", .type = "int", .location = VAR_LOCATION_STACK, .stack_offset = -4, .scope_start = 0x1010, .scope_end = 0x1020},
        // Same variable moves to different register
        {.name = "var_x", .type = "int", .location = VAR_LOCATION_REGISTER, .reg = 2, .scope_start = 0x1020, .scope_end = 0x1030}
    };
    ...
    result = elf_writer_lookup_variable_by_name_and_address(fixture->elf_writer, "var_x", 0x1005, &found_var);
    if (!asthra_test_assert_bool(context, result, "Failed to lookup var_x at 0x1005")) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    if (!asthra_test_assert_bool(context, found_var->location == VAR_LOCATION_REGISTER && found_var->reg == 0,
    ...
    result = elf_writer_lookup_variable_by_name_and_address(fixture->elf_writer, "var_x", 0x1015, &found_var);
    if (!asthra_test_assert_bool(context, result, "Failed to lookup var_x at 0x1015")) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    if (!asthra_test_assert_bool(context, found_var->location == VAR_LOCATION_STACK && found_var->stack_offset == -4,
...
int main(void) {
    ...
    AsthraTestFunction tests[] = {
        test_variable_debug_info,
        test_variable_scope_tracking,
        test_variable_location_encoding,
        test_variable_lifetime_tracking
    };
    
    AsthraTestMetadata metadata[] = {
        {"test_variable_debug_info", __FILE__, __LINE__, "test_variable_debug_info", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_variable_scope_tracking", __FILE__, __LINE__, "test_variable_scope_tracking", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_variable_location_encoding", __FILE__, __LINE__, "test_variable_location_encoding", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_variable_lifetime_tracking", __FILE__, __LINE__, "test_variable_lifetime_tracking", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}
    };
