...
AsthraTestResult test_relocation_validation(AsthraTestContext *context) {
    ...
    ELFRelocation invalid_reloc = {.offset = 0x1000,
                                   .symbol_index = 1,
                                   .type = 999, // Invalid type
                                   .addend = 0};

    bool result =
        elf_relocation_manager_add_relocation(fixture->relocation_manager, &invalid_reloc);
    ...
    ELFRelocation invalid_symbol_reloc = {.offset = 0x1000,
                                          .symbol_index = UINT32_MAX, // Invalid symbol index
                                          .type = R_X86_64_PC32,
                                          .addend = 0};

    result =
        elf_relocation_manager_add_relocation(fixture->relocation_manager, &invalid_symbol_reloc);
    if (!asthra_test_assert_bool(context, !result,
                                 "Should reject relocation with invalid symbol index")) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    ELFRelocation valid_reloc = {
        .offset = 0x1000, .symbol_index = 1, .type = R_X86_64_PC32, .addend = -4};
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = {.name = "ELF Relocation Validation Tests",
                                    .description =
                                        "Test ELF relocation validation and error handling",
    ...
    AsthraTestFunction tests[] = {test_relocation_validation};

    AsthraTestMetadata metadata[] = {{"test_relocation_validation", __FILE__, __LINE__,
                                      "test_relocation_validation", ASTHRA_TEST_SEVERITY_MEDIUM,
