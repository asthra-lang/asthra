...
AsthraTestResult test_relocation_entry_generation(AsthraTestContext* context) {
    ...
    relocations[0] = (ELFRelocation){.offset = 0x1008, .symbol_index = 1, .type = R_X86_64_PC32, .addend = -4};
    relocations[1] = (ELFRelocation){.offset = 0x1010, .symbol_index = 2, .type = R_X86_64_PLT32, .addend = -4};
    relocations[2] = (ELFRelocation){.offset = 0x1018, .symbol_index = 3, .type = R_X86_64_64, .addend = 0};
    
    for (int i = 0; i < 3; i++) {
        bool result = elf_relocation_manager_add_relocation(fixture->relocation_manager, &relocations[i]);
    ...
    if (!asthra_test_assert_bool(context, relocation_count == 3,
                                "Expected 3 relocations, got %zu", relocation_count)) {
...
AsthraTestResult test_relocation_processing(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_pointer(context, fixture->test_ast, "Failed to parse relocation source")) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    
    // Process relocations
    bool result = elf_relocation_manager_process_ast(fixture->relocation_manager, fixture->test_ast);
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = {
        .name = "ELF Basic Relocation Tests",
        .description = "Test basic ELF relocation entry generation and processing",
