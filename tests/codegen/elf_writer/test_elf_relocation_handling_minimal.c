...
static AsthraTestMetadata test_metadata[] = {
    {"test_relocation_section_creation", __FILE__, __LINE__, "Test relocation section creation", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_symbol_relocation", __FILE__, __LINE__, "Test symbol relocation handling", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_dynamic_relocations", __FILE__, __LINE__, "Test dynamic relocation processing", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
    {"test_relocation_validation", __FILE__, __LINE__, "Test relocation validation and linking", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}
};
...
static void add_section(MinimalElfRelocationContext* ctx, const char* name, uint32_t type, uint64_t size) {
...
static void add_symbol(MinimalElfRelocationContext* ctx, const char* name, uint64_t value, uint8_t type, uint8_t bind, uint16_t section) {
...
static void add_relocation(MinimalElfRelocationContext* ctx, uint64_t offset, uint32_t symbol_idx, uint32_t type, int64_t addend) {
...
static AsthraTestResult test_dynamic_relocations(AsthraTestContext* context) {
    ...
    for (int i = 0; i < ctx.section_count; i++) {
        ...
        if (section->type == SHT_DYNSYM) has_dynsym = true;
        if (section->type == SHT_STRTAB && strstr(section->name, "dynstr")) has_dynstr = true;
        if (section->type == SHT_RELA && strstr(section->name, ".rela.dyn")) has_rela_dyn = true;
        if (section->type == SHT_RELA && strstr(section->name, ".rela.plt")) has_rela_plt = true;
        if (strcmp(section->name, ".got") == 0) has_got = true;
        if (strcmp(section->name, ".plt") == 0) has_plt = true;
    }
    ...
    for (int i = 0; i < ctx.relocation_count; i++) {
        ...
        if (type == R_X86_64_PLT32) plt_count++;
        if (type == R_X86_64_GOT32) got_count++;
    }
