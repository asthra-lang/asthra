...
AsthraTestResult test_section_header_generation(AsthraTestContext *context) {
    ...
    sections[0] = (ELFSection){
        .name = ".text", .type = SHT_PROGBITS, .flags = SHF_ALLOC | SHF_EXECINSTR, .size = 1024};
    sections[1] = (ELFSection){
        .name = ".data", .type = SHT_PROGBITS, .flags = SHF_ALLOC | SHF_WRITE, .size = 512};
    sections[2] = (ELFSection){
        .name = ".bss", .type = SHT_NOBITS, .flags = SHF_ALLOC | SHF_WRITE, .size = 256};
    ...
    for (int i = 0; i < 4; i++) {
        bool result =
            elf_writer_generate_section_header(fixture->elf_writer, &sections[i], &headers[i]);
...
AsthraTestResult test_program_header_generation(AsthraTestContext *context) {
    ...
    segments[0] = (ELFSegment){.type = PT_LOAD,
                               .flags = PF_R | PF_X,
                               .vaddr = 0x400000,
                               .paddr = 0x400000,
                               .filesz = 1024,
                               .memsz = 1024};
    segments[1] = (ELFSegment){.type = PT_LOAD,
                               .flags = PF_R | PF_W,
                               .vaddr = 0x600000,
                               .paddr = 0x600000,
                               .filesz = 512,
    ...
    for (int i = 0; i < 2; i++) {
        bool result =
            elf_writer_generate_program_header(fixture->elf_writer, &segments[i], &headers[i]);
    ...
    for (size_t i = 0; i < section_count; i++) {
        ...
        if (section_align > 0) {
            if (!asthra_test_assert_bool(context, (section_offset % section_align) == 0,
                                         "Section %zu offset not aligned to %lu bytes", i,
                                         section_align)) {
                cleanup_elf_writer_fixture(fixture);
                return ASTHRA_TEST_FAIL;
            }
...
AsthraTestResult test_string_table_generation(AsthraTestContext *context) {
    ...
    const char *test_strings[] = {".text", ".data",  ".bss",   ".symtab", ".strtab",
                                  "main",  "printf", "malloc", NULL};
    ...
    for (int i = 0; test_strings[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_bool(context, string_offsets[i] != 0 || i == 0,
                                     "Failed to add string '%s' to string table",
                                     test_strings[i])) {
            cleanup_elf_writer_fixture(fixture);
            return ASTHRA_TEST_FAIL;
        }
    ...
    bool result =
        elf_writer_generate_string_table(fixture->elf_writer, &string_table, &string_table_size);
...
int main(void) {
    ...
    AsthraTestFunction tests[] = {test_elf_header_generation, test_section_header_generation,
                                  test_program_header_generation, test_elf_file_layout_validation,
                                  test_string_table_generation};

    AsthraTestMetadata metadata[] = {
        {"test_elf_header_generation", __FILE__, __LINE__, "test_elf_header_generation",
         ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
        {"test_section_header_generation", __FILE__, __LINE__, "test_section_header_generation",
         ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
        {"test_program_header_generation", __FILE__, __LINE__, "test_program_header_generation",
         ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
        {"test_elf_file_layout_validation", __FILE__, __LINE__, "test_elf_file_layout_validation",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_string_table_generation", __FILE__, __LINE__, "test_string_table_generation",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}};
