...
AsthraTestResult test_dwarf_compilation_unit(AsthraTestContext *context) {
    ...
    ELFCompilationUnit cu_info = {.source_filename = "test.asthra",
...
AsthraTestResult test_function_debug_info(AsthraTestContext *context) {
    ...
    func_info[0] = (ELFFunctionDebugInfo){.name = "main",
                                          .return_type = "int",
                                          .low_pc = 0x1000,
                                          .high_pc = 0x1100,
    ...
    ELFParameterDebugInfo params[2] = {{.name = "argc", .type = "int", .location = DW_OP_reg0},
                                       {.name = "argv", .type = "**char", .location = DW_OP_reg1}};

    func_info[1] = (ELFFunctionDebugInfo){.name = "helper",
                                          .return_type = "void",
                                          .low_pc = 0x1100,
                                          .high_pc = 0x1200,
                                          .frame_base = DW_OP_fbreg,
...
AsthraTestResult test_type_debug_info(AsthraTestContext *context) {
    ...
    type_info[0] = (ELFTypeDebugInfo){
        .name = "int", .tag = DW_TAG_base_type, .encoding = DW_ATE_signed, .byte_size = 4};

    type_info[1] = (ELFTypeDebugInfo){
        .name = "float", .tag = DW_TAG_base_type, .encoding = DW_ATE_float, .byte_size = 4};
    ...
    // Array type
    type_info[3] = (ELFTypeDebugInfo){.name = "int[10]",
...
AsthraTestResult test_complex_type_debug_info(AsthraTestContext *context) {
    ...
    ELFTypeDebugInfo struct_type = {.name = "Point3D",
    ...
    ELFTypeDebugInfo union_type = {.name = "Value",
...
AsthraTestResult test_dwarf_expression_generation(AsthraTestContext *context) {
    ...
    struct {
        ...
        {"complex_expr",
         {.op_count = 4, .ops = {DW_OP_reg1, DW_OP_const1u, DW_OP_plus, DW_OP_deref}}}};

    for (int i = 0; i < 4; i++) {
        bool result = elf_writer_add_dwarf_expression(fixture->elf_writer, test_expressions[i].name,
...
AsthraTestResult test_dwarf_abbreviation_table(AsthraTestContext *context) {
    ...
    ELFFunctionDebugInfo func_info = {.name = "test_func",
                                      .return_type = "int",
                                      .low_pc = 0x1000,
                                      .high_pc = 0x1100,
                                      .frame_base = DW_OP_fbreg,
    ...
    bool has_subprogram_abbrev =
        elf_writer_has_abbreviation(fixture->elf_writer, DW_TAG_subprogram);
    ...
    if (!asthra_test_assert_bool(context,
                                 has_cu_abbrev && has_subprogram_abbrev && has_base_type_abbrev,
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = {
        ...
        .description =
            "Test ELF DWARF compilation unit, function, and type debug information generation",
    ...
    AsthraTestFunction tests[] = {test_dwarf_compilation_unit,
    ...
    AsthraTestMetadata metadata[] = {
        {"test_dwarf_compilation_unit", __FILE__, __LINE__, "test_dwarf_compilation_unit",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_function_debug_info", __FILE__, __LINE__, "test_function_debug_info",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_type_debug_info", __FILE__, __LINE__, "test_type_debug_info",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_complex_type_debug_info", __FILE__, __LINE__, "test_complex_type_debug_info",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_dwarf_expression_generation", __FILE__, __LINE__, "test_dwarf_expression_generation",
         ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
        {"test_dwarf_abbreviation_table", __FILE__, __LINE__, "test_dwarf_abbreviation_table",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}};
