...
void test_metadata_population(void) {
    ...
    if (test_writer->asthra_metadata) {
        TEST_ASSERT(test_writer->asthra_metadata->ffi_metadata != NULL,
                    "FFI metadata structure created");
        TEST_ASSERT(test_writer->asthra_metadata->gc_metadata != NULL,
                    "GC metadata structure created");
        TEST_ASSERT(test_writer->asthra_metadata->security_metadata != NULL,
                    "Security metadata structure created");
        TEST_ASSERT(test_writer->asthra_metadata->pattern_metadata != NULL,
                    "Pattern metadata structure created");
        TEST_ASSERT(test_writer->asthra_metadata->string_metadata != NULL,
                    "String metadata structure created");
        TEST_ASSERT(test_writer->asthra_metadata->slice_metadata != NULL,
                    "Slice metadata structure created");
        TEST_ASSERT(test_writer->asthra_metadata->concurrency_metadata != NULL,
                    "Concurrency metadata structure created");
    }
}
