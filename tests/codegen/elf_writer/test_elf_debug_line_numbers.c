...
AsthraTestResult test_line_number_info(AsthraTestContext *context) {
    ...
    line_entries[0] =
        (ELFLineNumberEntry){.address = 0x1000, .line = 1, .column = 1, .file_index = 0};
    line_entries[1] =
        (ELFLineNumberEntry){.address = 0x1008, .line = 2, .column = 5, .file_index = 0};
    line_entries[2] =
        (ELFLineNumberEntry){.address = 0x1010, .line = 3, .column = 1, .file_index = 0};
    line_entries[3] =
        (ELFLineNumberEntry){.address = 0x1018, .line = 4, .column = 1, .file_index = 0};
...
AsthraTestResult test_line_number_lookup(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(context, line == 20,
                                 "Expected line 20 for approximate lookup, got %u", line)) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_multi_file_line_numbers(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(context, file_index == 1, "Expected file index 1, got %u",
                                 file_index)) {
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestFunction tests[] = {test_line_number_info, test_line_number_ordering,
                                  test_line_number_lookup, test_multi_file_line_numbers};

    AsthraTestMetadata metadata[] = {
        {"test_line_number_info", __FILE__, __LINE__, "test_line_number_info",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_line_number_ordering", __FILE__, __LINE__, "test_line_number_ordering",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_line_number_lookup", __FILE__, __LINE__, "test_line_number_lookup",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_multi_file_line_numbers", __FILE__, __LINE__, "test_multi_file_line_numbers",
         ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}};
