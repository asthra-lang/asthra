...
void test_elf_writer_lifecycle(void) {
    ...
    if (writer) {
        ...
        TEST_ASSERT(writer->config.generate_debug_info == true,
                    "Debug info generation enabled by default");
        TEST_ASSERT(writer->config.optimize_metadata == true,
                    "Metadata optimization enabled by default");
        TEST_ASSERT(writer->config.validate_elf_structure == true,
                    "ELF validation enabled by default");
        TEST_ASSERT(writer->config.enable_all_asthra_sections == true,
                    "All Asthra sections enabled by default");
