...
static AsthraTestMetadata test_metadata[] = {
    {"test_debug_section_creation", __FILE__, __LINE__, "Test debug section creation and structure", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_debug_info_generation", __FILE__, __LINE__, "Test debug info generation", ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
    {"test_debug_line_numbers", __FILE__, __LINE__, "Test debug line number information", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
    {"test_debug_variable_info", __FILE__, __LINE__, "Test debug variable information", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}
};
...
static void add_debug_section(MinimalElfDebugContext* ctx, const char* name, uint32_t type, uint64_t size) {
...
static void add_debug_line(MinimalElfDebugContext* ctx, uint32_t line, uint64_t addr, const char* file) {
...
static void add_debug_variable(MinimalElfDebugContext* ctx, const char* name, uint64_t addr, uint32_t type) {
...
static AsthraTestResult test_debug_line_numbers(AsthraTestContext* context) {
    ...
    for (int i = 0; i < ctx.line_count; i++) {
        if (strcmp(ctx.lines[i].file_name, "main.c") == 0) has_main_c = true;
        if (strcmp(ctx.lines[i].file_name, "helper.c") == 0) has_helper_c = true;
    }
...
static AsthraTestResult test_debug_variable_info(AsthraTestContext* context) {
    ...
    for (int i = 0; i < ctx.variable_count; i++) {
        if (strcmp(ctx.variables[i].name, "x") == 0) has_x = true;
        if (strcmp(ctx.variables[i].name, "message") == 0) has_message = true;
    }
    ...
    for (int i = 0; i < ctx.variable_count; i++) {
        if (ctx.variables[i].type == 1) has_type_1 = true;
        if (ctx.variables[i].type == 2) has_type_2 = true;
    }
