...
AsthraTestResult test_relocation_section_management(AsthraTestContext *context) {
    ...
    bool result = elf_relocation_manager_generate_section_table(
        fixture->relocation_manager, ".text", &text_reloc_table, &text_reloc_count);
    ...
    if (!asthra_test_assert_size_eq(context, text_reloc_count, 2,
                                    ".text should have 2 relocations")) {
        free(text_reloc_table);
    ...
    if (!asthra_test_assert_size_eq(context, data_reloc_count, 2,
                                    ".data should have 2 relocations")) {
        free(text_reloc_table);
        free(data_reloc_table);
        cleanup_elf_writer_fixture(fixture);
        return ASTHRA_TEST_FAIL;
...
int main(void) {
    ...
    AsthraTestFunction tests[] = {test_relocation_section_management};

    AsthraTestMetadata metadata[] = {{"test_relocation_section_management", __FILE__, __LINE__,
