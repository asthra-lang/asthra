...
AsthraTestResult test_relocation_types(AsthraTestContext *context) {
    ...
    } test_relocations[] = {{0x1000, 1, R_X86_64_64, 0, "64-bit absolute"},
    ...
    for (size_t i = 0; i < test_count; i++) {
        ELFRelocation reloc = {.offset = test_relocations[i].offset,
        ...
        if (!asthra_test_assert_bool(context, result, "Failed to add %s relocation",
                                     test_relocations[i].description)) {
    ...
    for (size_t i = 0; i < relocation_count; i++) {
        if (!asthra_test_assert_bool(context,
                                     relocation_table[i].r_offset == test_relocations[i].offset,
                                     "Relocation %zu offset mismatch: expected 0x%lx, got 0x%lx", i,
                                     test_relocations[i].offset, relocation_table[i].r_offset)) {
            free(relocation_table);
            cleanup_elf_writer_fixture(fixture);
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_bool(
                context, ELF64_R_TYPE(relocation_table[i].r_info) == test_relocations[i].type,
                "Relocation %zu type mismatch: expected %u, got %lu", i, test_relocations[i].type,
                ELF64_R_TYPE(relocation_table[i].r_info))) {
            free(relocation_table);
            cleanup_elf_writer_fixture(fixture);
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_bool(context,
                                     relocation_table[i].r_addend == test_relocations[i].addend,
                                     "Relocation %zu addend mismatch: expected %ld, got %ld", i,
                                     test_relocations[i].addend, relocation_table[i].r_addend)) {
            free(relocation_table);
            cleanup_elf_writer_fixture(fixture);
            return ASTHRA_TEST_FAIL;
        }
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = {.name = "ELF Relocation Types Tests",
    ...
    AsthraTestFunction tests[] = {test_relocation_types};

    AsthraTestMetadata metadata[] = {{"test_relocation_types", __FILE__, __LINE__,
                                      "test_relocation_types", ASTHRA_TEST_SEVERITY_HIGH,
