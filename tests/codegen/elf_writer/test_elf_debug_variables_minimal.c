...
static inline bool elf_writer_add_variable_debug_info(ELFWriter *writer,
                                                      ELFVariableDebugInfo *var) {
...
static inline bool elf_writer_lookup_variables_at_address(ELFWriter *writer, unsigned long addr,
                                                          ELFVariableDebugInfo **vars,
                                                          size_t *count) {
    if (!writer || !vars || !count)
        return false;
...
DEFINE_TEST(test_variable_debug_info) {
    ...
    ELFVariableDebugInfo var_info[3] = {{.name = "x",
                                         .type = "int",
...
DEFINE_TEST(test_asthra_variable_types) {
    ...
    ELFVariableDebugInfo var_info[] = {// Option<T> variable
                                       {.name = "maybe_value",
                                        .type = "Option<i32>",
                                        .location = VAR_LOCATION_STACK,
                                        .stack_offset = -8,
...
RUN_TEST_SUITE(elf_debug_variables_minimal, RUN_TEST(test_variable_debug_info);
               RUN_TEST(test_variable_scope_tracking); RUN_TEST(test_variable_location_encoding);
