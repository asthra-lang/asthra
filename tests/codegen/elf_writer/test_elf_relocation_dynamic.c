...
AsthraTestResult test_dynamic_relocation_support(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < 3; i++) {
        bool result = elf_relocation_manager_add_relocation(fixture->relocation_manager,
                                                            &dynamic_relocations[i]);
    ...
    if (!asthra_test_assert_size_eq(context, dyn_reloc_count, 3,
                                    "Expected 3 dynamic relocations")) {
    ...
    if (!asthra_test_assert_bool(context,
                                 ELF64_R_TYPE(dyn_reloc_table[0].r_info) == R_X86_64_JUMP_SLOT,
    ...
    if (!asthra_test_assert_bool(context,
                                 ELF64_R_TYPE(dyn_reloc_table[1].r_info) == R_X86_64_GLOB_DAT,
    ...
    if (!asthra_test_assert_bool(context,
                                 ELF64_R_TYPE(dyn_reloc_table[2].r_info) == R_X86_64_RELATIVE,
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = {.name = "ELF Dynamic Relocation Tests",
                                    .description =
                                        "Test ELF dynamic relocation support and dynamic linking",
    ...
    AsthraTestFunction tests[] = {test_dynamic_relocation_support};

    AsthraTestMetadata metadata[] = {{"test_dynamic_relocation_support", __FILE__, __LINE__,
