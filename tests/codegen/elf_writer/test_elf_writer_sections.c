...
void test_section_creation(void) {
    ...
    if (success) {
        ...
        for (size_t i = 0; i < test_writer->section_count; i++) {
            ...
            if (strcmp(section->name, ".symtab") == 0)
                has_symtab = true;
            if (strcmp(section->name, ".strtab") == 0)
                has_strtab = true;
            if (strcmp(section->name, ".shstrtab") == 0)
                has_shstrtab = true;
        }
    ...
    if (success) {
        ...
        for (size_t i = 0; i < test_writer->section_count; i++) {
            ELFSection *section = test_writer->sections[i];
            if (strcmp(section->name, ".Asthra.ffi") == 0)
                has_ffi = true;
            if (strcmp(section->name, ".Asthra.gc") == 0)
                has_gc = true;
            if (strcmp(section->name, ".Asthra.security_meta") == 0)
                has_security = true;
            if (strcmp(section->name, ".Asthra.pattern_matching") == 0)
                has_pattern = true;
            if (strcmp(section->name, ".Asthra.string_ops") == 0)
                has_string = true;
            if (strcmp(section->name, ".Asthra.slice_meta") == 0)
                has_slice = true;
            if (strcmp(section->name, ".Asthra.concurrency") == 0)
