...
AsthraTestResult test_live_variable_analysis(AsthraTestContext *context) {
    ...
    instructions[0] = (Instruction){.opcode = OP_MOV,
                                    .dst_reg = 0,
                                    .operand1 = {.type = OPERAND_IMMEDIATE, .value = 10}}; // x = 10
    instructions[1] = (Instruction){.opcode = OP_MOV,
                                    .dst_reg = 1,
                                    .operand1 = {.type = OPERAND_IMMEDIATE, .value = 20}}; // y = 20
    instructions[2] = (Instruction){
        .opcode = OP_ADD,
        .dst_reg = 2,
        .operand1 = {.type = OPERAND_REGISTER, .reg = 0},
        .operand2 = {.type = OPERAND_REGISTER, .reg = 1}}; // z = x + y (x, y live here)
    instructions[3] = (Instruction){
        .opcode = OP_MOV,
        .dst_reg = 0,
        .operand1 = {.type = OPERAND_IMMEDIATE, .value = 30}}; // x = 30 (old x dies, new x born)
    instructions[4] = (Instruction){
        .opcode = OP_MUL,
        .dst_reg = 3,
        .operand1 = {.type = OPERAND_REGISTER, .reg = 0},
        .operand2 = {.type = OPERAND_REGISTER, .reg = 2}}; // w = x * z (x, z live here)
    instructions[5] = (Instruction){
        .opcode = OP_ADD,
        .dst_reg = 4,
        .operand1 = {.type = OPERAND_REGISTER, .reg = 3},
        .operand2 = {.type = OPERAND_IMMEDIATE, .value = 5}}; // result = w + 5 (w live here)
...
AsthraTestResult test_live_variables_with_control_flow(AsthraTestContext *context) {
    ...
    instructions[1] = (Instruction){.opcode = OP_MOV,
                                    .dst_reg = 1,
                                    .operand1 = {.type = OPERAND_IMMEDIATE, .value = 20}}; // y = 20
    instructions[2] = (Instruction){.opcode = OP_JZ,
                                    .operand1 = {.type = OPERAND_IMMEDIATE, .value = 5}}; // jz else
    instructions[3] =
        (Instruction){.opcode = OP_ADD,
                      .dst_reg = 2,
                      .operand1 = {.type = OPERAND_REGISTER, .reg = 0},
                      .operand2 = {.type = OPERAND_IMMEDIATE, .value = 5}}; // z = x + 5 (x used)
    instructions[4] = (Instruction){
        .opcode = OP_JMP, .operand1 = {.type = OPERAND_IMMEDIATE, .value = 6}}; // jmp merge
    instructions[5] =
        (Instruction){.opcode = OP_ADD,
                      .dst_reg = 2,
                      .operand1 = {.type = OPERAND_REGISTER, .reg = 1},
                      .operand2 = {.type = OPERAND_IMMEDIATE, .value = 10}}; // z = y + 10 (y used)
    instructions[6] =
        (Instruction){.opcode = OP_MUL,
                      .dst_reg = 3,
                      .operand1 = {.type = OPERAND_REGISTER, .reg = 2},
                      .operand2 = {.type = OPERAND_IMMEDIATE, .value = 2}}; // w = z * 2 (z used)
    ...
    if (!asthra_test_assert_bool(context, success,
                                 "Live variable analysis with control flow failed")) {
        cleanup_data_flow_analysis_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
