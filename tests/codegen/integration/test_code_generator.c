...
static bool test_instruction_creation(void) {
    ...
    if (mov_imm->operands[0].type != OPERAND_REGISTER ||
        mov_imm->operands[0].data.reg != ASTHRA_REG_RAX) {
        printf("FAIL: MOV immediate instruction has incorrect destination operand\n");
        free(mov_imm);
        return false;
    }

    if (mov_imm->operands[1].type != OPERAND_IMMEDIATE ||
        mov_imm->operands[1].data.immediate != 42) {
