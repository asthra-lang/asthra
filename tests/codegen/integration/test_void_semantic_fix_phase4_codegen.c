...
static void free_parsed_program(ParsedProgram* program) {
    if (!program) return;
    if (program->ast) ast_free_node(program->ast);
    if (program->analyzer) semantic_analyzer_destroy(program->analyzer);
    free(program);
}

static bool generate_and_validate_code(ParsedProgram* program, const char* expected_pattern) {
    if (!program || !program->ast || !program->analyzer) return false;
    ...
    bool emit_success = code_generator_emit_assembly(generator, output_buffer, sizeof(output_buffer));
    const char* generated_code = emit_success ? output_buffer : NULL;
    bool contains_pattern = (expected_pattern == NULL) || 
...
static bool test_function_parameter_codegen(void) {
    ...
    CODEGEN_TEST_ASSERT(codegen_success == true, "Function with 'none' parameters should generate valid assembly");
...
static bool test_struct_content_codegen(void) {
    ...
    CODEGEN_TEST_ASSERT(codegen_success == true, "Struct-related function should generate valid assembly");
...
static bool test_function_call_codegen(void) {
    ...
    CODEGEN_TEST_ASSERT(codegen_success == true, "Function call with 'none' should generate valid assembly");
...
static bool test_pattern_matching_codegen(void) {
    ...
    CODEGEN_TEST_ASSERT(codegen_success == true, "Pattern matching with 'none' should generate valid C code");
...
static bool test_extern_function_codegen(void) {
    ...
    CODEGEN_TEST_ASSERT(codegen_success == true, "Extern function calls should generate parameterless C calls");
...
static bool test_annotation_codegen(void) {
    ...
    CODEGEN_TEST_ASSERT(codegen_success == true, "Annotations should generate appropriate C attributes");
...
static bool test_complex_integration_codegen(void) {
    ...
    bool codegen_success = generate_and_validate_code(ast, NULL); // Just check it generates
    CODEGEN_TEST_ASSERT(codegen_success == true, "Complex integration should generate valid C code");
...
static bool test_performance_critical_codegen(void) {
    ...
    bool codegen_success = generate_and_validate_code(ast, NULL); // Just check it generates
    CODEGEN_TEST_ASSERT(codegen_success == true, "Performance-critical code should generate optimized C");
...
int main(void) {
    ...
    printf("Success Rate: %.1f%%\n", 
           codegen_test_results.total > 0 ? 
           (float)codegen_test_results.passed / codegen_test_results.total * 100.0f : 0.0f);
