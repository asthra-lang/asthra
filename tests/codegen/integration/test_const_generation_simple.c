...
void test_global_symbol_table(void) {
    ...
    bool success = ffi_add_global_const_symbol((FFIAssemblyGenerator *)&mock_gen, "TEST_CONST",
                                               "int32_t", true);
    assert(success);
    ...
    success = ffi_add_global_const_symbol((FFIAssemblyGenerator *)&mock_gen, "PRIVATE_CONST",
                                          "float", false);
    assert(success);
