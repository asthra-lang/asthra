...
static void add_compilation_unit(const char* filename) {
    if (linker.unit_count >= 10) return;
...
DEFINE_TEST(test_linking_coordination) {
    ...
    TEST_ASSERT_STR_EQ(linker.executable_name, "test_program", "Executable name should be set correctly");
...
DEFINE_TEST(test_symbol_resolution) {
    ...
    for (size_t i = 0; i < linker.unit_count; i++) {
        for (size_t j = 0; j < linker.units[i].symbol_count; j++) {
            if (strcmp(linker.units[i].symbols[j], "main") == 0) main_found = true;
            if (strcmp(linker.units[i].symbols[j], "add") == 0) add_found = true;
            if (strcmp(linker.units[i].symbols[j], "print_debug") == 0) print_debug_found = true;
        }
    }
...
DEFINE_TEST(test_object_file_generation) {
    ...
    for (size_t i = 0; i < linker.unit_count; i++) {
        ...
        TEST_ASSERT(object_len > filename_len, "Object file name should be longer than source file name");
        TEST_ASSERT(strstr(linker.units[i].object_file, ".o") != NULL, "Object file should have .o extension");
    }
...
RUN_TEST_SUITE(multifile_coordination_suite,
    RUN_TEST(test_multiple_compilation_units);
    RUN_TEST(test_compilation_sequence);
    RUN_TEST(test_linking_coordination);
    RUN_TEST(test_symbol_resolution);
    RUN_TEST(test_object_file_generation);
