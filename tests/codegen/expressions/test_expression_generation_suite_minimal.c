...
DEFINE_TEST(test_arithmetic_expression_generation) {
    ...
    const char* expressions[] = {
        "a + b", "x - y", "m * n", "p / q", "value % mod", "(a + b) * c", "x + y + z"
    };
...
DEFINE_TEST(test_logical_expression_generation) {
    ...
    const char* expressions[] = {
        "a && b", "x || y", "!condition", "a && b || c", "!(x && y)", "flag1 && flag2 && flag3"
    };
...
DEFINE_TEST(test_comparison_expression_generation) {
    ...
    const char* expressions[] = {
        "a == b", "x != y", "m < n", "p > q", "val <= limit", "score >= threshold"
    };
...
DEFINE_TEST(test_call_expression_generation) {
    ...
    const char* expressions[] = {
        "func()", "add(a, b)", "calc(x, y, z)", "obj.method()", "obj.method(arg)", "nested(inner(value))"
    };
...
DEFINE_TEST(test_unary_expression_generation) {
    ...
    const char* expressions[] = {
        "-value", "+number", "!flag", "~bits", "*pointer", "&variable"
    };
...
DEFINE_TEST(test_bitwise_expression_generation) {
    ...
    const char* expressions[] = {
        "a & b", "x | y", "m ^ n", "value << 2", "data >> 4", "~mask"
    };
...
DEFINE_TEST(test_complex_expression_generation) {
    ...
    const char* expressions[] = {
        "(a + b) * (c - d)", "func(x + y, z * w)", "obj.method(a, b).result", 
        "array[index + offset]", "condition ? true_val : false_val", "a && b || c && d"
    };
...
RUN_TEST_SUITE(expression_generation_suite_tests,
    RUN_TEST(test_arithmetic_expression_generation);
