...
ASTNode *parse_and_analyze_expression(const char *expr_str, SemanticAnalyzer *analyzer) {
    ...
    // Navigate: program -> declarations -> main function -> body -> let statement -> init
    // expression
    if (program->type != AST_PROGRAM || !program->data.program.declarations ||
    ...
    for (size_t i = 0; i < program->data.program.declarations->count; i++) {
        ...
        if (decl->type == AST_FUNCTION_DECL && strcmp(decl->data.function_decl.name, "main") == 0) {
            main_func = decl;
            break;
        }
    }
    ...
    for (int i = statements->count - 2; i >= 0; i--) { // -2 to skip the return statement
        ...
        if (stmt->type == AST_LET_STMT && strcmp(stmt->data.let_stmt.name, "result") == 0 &&
