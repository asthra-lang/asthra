...
AsthraTestResult test_generate_break_statements(AsthraTestContext *context) {
    ...
    ASTNode *ast =
        parse_and_verify_source(context, source, "test_break.asthra", "break statement test");
    if (!ast) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_generate_continue_statements(AsthraTestContext *context) {
    ...
    // Parse and analyze the source
    ASTNode *ast =
        parse_and_verify_source(context, source, "test_continue.asthra", "continue statement test");
    if (!ast) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_generate_nested_control_flow(AsthraTestContext *context) {
    ...
    // Parse and analyze the source
    ASTNode *ast =
        parse_and_verify_source(context, source, "test_nested.asthra", "nested control flow test");
    if (!ast) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestSuiteConfig config =
        create_statement_test_suite_config("Control Flow Statement Generation Tests",
                                           "Test code generation for control flow statements "
                                           "including if, while, for, break, and continue",
                                           stats);

    AsthraTestFunction tests[] = {
        test_generate_if_statements,       test_generate_if_else_statements,
        test_generate_while_loops,         test_generate_for_loops,
        test_generate_break_statements,    test_generate_continue_statements,
