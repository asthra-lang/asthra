...
static inline bool is_expression_returning_value(const char *expr) {
    ...
    if (strstr(expr, "func1(") || strstr(expr, "func2(") || strstr(expr, "+ ") ||
    ...
    if (strstr(expr, ".scale(") || strstr(expr, "method1(") || strstr(expr, "method2(")) {
        return true;
    }
 ...
 */
static inline bool transform_if_else_for_void_context(const char *input, char *output,
                                                      size_t output_size) {
    ...
    if (strstr(input, "action1()") || strstr(input, "action2()") || strstr(input, "process()") ||
        strstr(input, "update()")) {
 ...
 */
static inline void wrap_fragment_for_expression_context(const char *fragment, char *output,
                                                        size_t output_size) {
    ...
    if (strstr(fragment, "let") && strstr(fragment, "=") && strstr(fragment, "if") &&
        strstr(fragment, "else")) {
...
static const TestPatternMapping expression_patterns[] = {
    ...
    {"if (c) { return 1; }", "if (c) { 1 } else { 0 }",
     "If expression should return values, not use return statements"},

    // Function calls
    {"compute_value();", "let _ = compute_value();",
     "Non-void function results must be used or explicitly discarded"},

    {"void_func();", "void_func();", "Void function calls can be used as statements"},
    ...
    // Loops
    {"for i in arr { process(i); }", "for i in arr { process(i); }",
