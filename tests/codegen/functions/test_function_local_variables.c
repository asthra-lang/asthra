...
AsthraTestResult test_generate_nested_scope_variables(AsthraTestContext *context) {
    ...
    const char *source = "package test;\n\npub fn nested_scope(x: i32) -> i32 { let mut outer: i32 "
                         "= 10; if (true) { let inner: i32 = 20; } return 30; }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast,
                                    "Failed to parse function with nested scopes")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_generate_mutable_local_variables(AsthraTestContext *context) {
    ...
    const char *source = "package test;\n\npub fn mutate_locals(x: i32) -> i32 { let mut count: "
                         "i32 = 0; let mut sum: i32 = 0; return sum; }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast,
                                    "Failed to parse function with mutable locals")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, analyze_result,
                                 "Failed to analyze mutable locals program")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    // For now, just verify that we can create and initialize the backend
...
AsthraTestResult test_generate_local_variable_initialization(AsthraTestContext *context) {
    ...
    const char *source = "package test;\n\npub fn init_locals(x: i32) -> i32 { let a: i32 = 10; "
                         "let b: i32 = x; let c: i32 = 30; return a; }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast,
                                    "Failed to parse function with initialized locals")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    bool analyze_result = semantic_analyze_program(fixture->analyzer, fixture->test_ast);
    if (!asthra_test_assert_bool(context, analyze_result,
                                 "Failed to analyze initialized locals program")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    // For now, just verify that we can create and initialize the backend
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = create_local_var_test_suite_config(
        "Function Local Variables Tests", "Test code generation for local variables in functions",
        stats);

    AsthraTestFunction tests[] = {
        test_generate_basic_local_variables, test_generate_nested_scope_variables,
        test_generate_mutable_local_variables, test_generate_local_variable_initialization};

    AsthraTestMetadata metadata[] = {{"test_generate_basic_local_variables", __FILE__, __LINE__,
                                      "test_generate_basic_local_variables",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
                                     {"test_generate_nested_scope_variables", __FILE__, __LINE__,
                                      "test_generate_nested_scope_variables",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
                                     {"test_generate_mutable_local_variables", __FILE__, __LINE__,
                                      "test_generate_mutable_local_variables",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
                                     {"test_generate_local_variable_initialization", __FILE__,
                                      __LINE__, "test_generate_local_variable_initialization",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}};
