...
static CodeGenTestFixture *setup_codegen_fixture(void) {
    ...
    printf("DEBUG: Creating backend (LLVM only)\n");
...
AsthraTestResult test_generate_stack_aligned_prologue(AsthraTestContext *context) {
    ...
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast,
                                    "Failed to parse aligned function")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = create_prologue_test_suite_config(
        "Function Prologue/Epilogue Generation Tests",
        "Test code generation for function prologue and epilogue", stats);

    AsthraTestFunction tests[] = {test_generate_basic_prologue_epilogue,
                                  test_generate_void_function_prologue_epilogue,
                                  test_generate_stack_aligned_prologue};

    AsthraTestMetadata metadata[] = {{"test_generate_basic_prologue_epilogue", __FILE__, __LINE__,
                                      "test_generate_basic_prologue_epilogue",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
                                     {"test_generate_void_function_prologue_epilogue", __FILE__,
                                      __LINE__, "test_generate_void_function_prologue_epilogue",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL},
                                     {"test_generate_stack_aligned_prologue", __FILE__, __LINE__,
                                      "test_generate_stack_aligned_prologue",
                                      ASTHRA_TEST_SEVERITY_HIGH, 5000000000ULL, false, NULL}};
