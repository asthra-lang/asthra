...
AsthraTestResult test_generate_multiple_return_paths(AsthraTestContext* context) {
    ...
    const char* source = "package test;\n\npub fn classify(x: i32) -> i32 { if (true) { return 1; } else { return 0; } }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast, "Failed to parse function with multiple returns")) {
...
AsthraTestResult test_generate_if_expression_flow(AsthraTestContext* context) {
    ...
    const char* source = "package test;\n\npub fn find_positive(x: i32) -> i32 { if (true) { return 42; } else { return 0; } }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast, "Failed to parse function with if expression")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    bool analyze_result = semantic_analyze_program(fixture->analyzer, fixture->test_ast);
    if (!asthra_test_assert_bool(context, analyze_result, "Failed to analyze if expression program")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    // For now, just verify that we can create and initialize the backend
...
AsthraTestResult test_generate_conditional_logic_flow(AsthraTestContext* context) {
    ...
    const char* source = "package test;\n\npub fn handle_code(code: i32) -> i32 { if (code == 200) { return 1; } else { return 0; } }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast, "Failed to parse function with conditional logic")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    // Analyze and generate the whole program
    bool analyze_result = semantic_analyze_program(fixture->analyzer, fixture->test_ast);
    if (!asthra_test_assert_bool(context, analyze_result, "Failed to analyze conditional program")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    // For now, just verify that we can create and initialize the backend
...
AsthraTestResult test_generate_nested_control_flow(AsthraTestContext* context) {
    ...
    const char* source = "package test;\n\npub fn nested_check(x: i32, y: i32) -> i32 { if (x > 0) { if (y > 0) { return 1; } else { return 2; } } else { return 3; } }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast, "Failed to parse function with nested control flow")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    bool analyze_result = semantic_analyze_program(fixture->analyzer, fixture->test_ast);
    if (!asthra_test_assert_bool(context, analyze_result, "Failed to analyze nested control flow program")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    // For now, just verify that we can create and initialize the backend
...
int main(void) {
    ...
    AsthraTestMetadata metadata[] = {
        {"test_generate_multiple_return_paths", __FILE__, __LINE__, "test_generate_multiple_return_paths", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_generate_if_expression_flow", __FILE__, __LINE__, "test_generate_if_expression_flow", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_generate_conditional_logic_flow", __FILE__, __LINE__, "test_generate_conditional_logic_flow", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
        {"test_generate_nested_control_flow", __FILE__, __LINE__, "test_generate_nested_control_flow", ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}
    };
