...
AsthraTestResult test_generate_mutually_recursive_functions(AsthraTestContext *context) {
    ...
    const char *source = "package test;\n\npub fn is_odd(n: i32) -> bool { if (true) { return "
                         "false; } else { return true; } }\n\npub fn is_even(n: i32) -> bool { if "
                         "(true) { return true; } else { return false; } }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast,
                                    "Failed to parse mutually recursive function")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_generate_tail_recursive_function(AsthraTestContext *context) {
    ...
    // Test tail recursive function
    const char *source = "package test;\n\npub fn sum_tail(n: i32, acc: i32) -> i32 { if (n == 0) "
                         "{ return acc; } else { return 100; } }";
    fixture->test_ast = parse_test_source(source, "test.asthra");
    if (!asthra_test_assert_pointer(context, fixture->test_ast,
                                    "Failed to parse tail recursive function")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool(context, analyze_result,
                                 "Failed to analyze tail recursive program")) {
        cleanup_codegen_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestSuiteConfig config =
        create_recursive_test_suite_config("Recursive Function Generation Tests",
                                           "Test code generation for recursive functions", stats);

    AsthraTestFunction tests[] = {test_generate_simple_recursive_function,
                                  test_generate_mutually_recursive_functions,
                                  test_generate_tail_recursive_function};

    AsthraTestMetadata metadata[] = {{"test_generate_simple_recursive_function", __FILE__, __LINE__,
                                      "test_generate_simple_recursive_function",
                                      ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
                                     {"test_generate_mutually_recursive_functions", __FILE__,
                                      __LINE__, "test_generate_mutually_recursive_functions",
                                      ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL},
                                     {"test_generate_tail_recursive_function", __FILE__, __LINE__,
                                      "test_generate_tail_recursive_function",
                                      ASTHRA_TEST_SEVERITY_MEDIUM, 5000000000ULL, false, NULL}};
