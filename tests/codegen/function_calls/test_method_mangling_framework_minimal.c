...
static char* mangle_method_name(const char* struct_name, const char* method_name, bool is_associated) {
...
DEFINE_TEST(test_associated_method_mangling) {
    ...
    TEST_ASSERT_STR_EQ(mangled, "Point_associated_new", "Associated method should be mangled correctly");
    free(mangled);
    
    // Test another associated method
    mangled = mangle_method_name("Rectangle", "create", true);
    TEST_ASSERT_STR_EQ(mangled, "Rectangle_associated_create", "Another associated method should be mangled correctly");
    free(mangled);
...
DEFINE_TEST(test_instance_method_mangling) {
    ...
    TEST_ASSERT_STR_EQ(mangled, "Point_instance_distance", "Instance method should be mangled correctly");
    free(mangled);
    
    // Test another instance method
    mangled = mangle_method_name("Vector", "magnitude", false);
    TEST_ASSERT_STR_EQ(mangled, "Vector_instance_magnitude", "Another instance method should be mangled correctly");
    free(mangled);
...
DEFINE_TEST(test_mangling_with_special_characters) {
    ...
    TEST_ASSERT_STR_EQ(mangled, "Http_Client_associated_send_request", "Underscore handling should work");
    free(mangled);
    
    // Test mixed case handling
    mangled = mangle_method_name("XMLParser", "parseDocument", false);
    TEST_ASSERT_STR_EQ(mangled, "XMLParser_instance_parseDocument", "Mixed case handling should work");
    free(mangled);
...
DEFINE_TEST(test_mangling_uniqueness) {
    ...
    TEST_ASSERT(strcmp(mangled1, mangled2) != 0, "Associated and instance methods should have different manglings");
    TEST_ASSERT(strcmp(mangled1, mangled3) != 0, "Different methods should have different manglings");
...
RUN_TEST_SUITE(method_mangling_suite,
    RUN_TEST(test_associated_method_mangling);
    RUN_TEST(test_instance_method_mangling);
    RUN_TEST(test_function_name_mangling);
    RUN_TEST(test_mangling_with_special_characters);
    RUN_TEST(test_mangling_uniqueness);
