...
DEFINE_TEST(test_struct_field_access_generation) {
    ...
    const char *accesses[] = {"obj.field",      "struct_var.member", "instance.property",
                              "ptr->field",     "data->value",       "nested.inner.field",
                              "array[0].member"};
...
DEFINE_TEST(test_struct_instantiation_generation) {
    ...
    const char *instantiations[] = {"Point { x: 1, y: 2 }",
                                    "Empty {}",
                                    "Person { name: \"John\", age: 30 }",
                                    "Complex { field1: value, field2: other.field }",
                                    "Nested { inner: Inner { value: 42 } }",
...
DEFINE_TEST(test_struct_method_generation) {
    ...
    const char *methods[] = {"obj.method()",
                             "instance.process(arg)",
                             "data.calculate(x, y)",
                             "point.distance_to(other)",
                             "obj.method1().method2()",
                             "Type::static_method()",
                             "obj.complex_method(a + b, func())"};
...
DEFINE_TEST(test_struct_assignment_generation) {
    ...
    const char *assignments[] = {"obj.field = value",         "instance.member = other.field",
                                 "ptr->field = expression",   "nested.inner.value = computed",
                                 "array[index].field = data", "struct_var = other_struct"};
...
DEFINE_TEST(test_struct_copy_move_generation) {
    ...
    const char *operations[] = {"copy(struct_instance)",     "move(temporary_struct)",
                                "deep_copy(complex_struct)", "clone(original)",
                                "duplicate(source)",         "transfer(from_location)"};
...
DEFINE_TEST(test_struct_complex_types_generation) {
    ...
    const char *complex_types[] = {"container.items[0]",
                                   "obj.callback(arg)",
                                   "matrix[i][j]",
                                   "tree.left.right.value",
                                   "graph.nodes[id].neighbors",
                                   "config.settings.advanced.optimization"};
...
RUN_TEST_SUITE(struct_generation_suite_tests, RUN_TEST(test_struct_field_access_generation);
               RUN_TEST(test_struct_instantiation_generation);
               RUN_TEST(test_struct_method_generation); RUN_TEST(test_struct_assignment_generation);
