...
DEFINE_TEST(test_deep_copy_nested_structs) {
    printf("Testing deep copy of nested structs...\n");

    CodeGenerator *generator = code_generator_create(0, 0);
    TEST_ASSERT_NOT_NULL(generator, "Code generator creation");

    // Test deep copy: dest = deep_copy(nested_struct);
    ASTNode *ast = parse_struct_copy_move("dest = deep_copy(nested_struct);");
    TEST_ASSERT_NOT_NULL(ast, "Parse deep copy of nested structs");

    bool result = code_generate_statement(generator, ast);
    TEST_ASSERT(result, "Generate deep copy code");

    printf("  ✓ Deep copy of nested structs code generation successful\n");
...
RUN_TEST_SUITE(struct_copy_move_tests, RUN_TEST(test_struct_copy_assignment);
               RUN_TEST(test_struct_initialization_copy); RUN_TEST(test_struct_move_semantics);
               RUN_TEST(test_struct_return_copy); RUN_TEST(test_deep_copy_nested_structs);)
