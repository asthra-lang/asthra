...
static AsthraTestResult test_option_pattern_matching_basic(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool_eq(context, compile_and_verify_option_patterns(source, true), true,
...
static AsthraTestResult test_option_pattern_matching_blocks(AsthraTestContext *context) {
    const char *source = "package test;\n"
                         ...
                         "    let mut result: i32 = 0;\n"
                         "    match opt { Option.Some(x) => { let temp: i32 = x; result = temp * "
                         "2; }, Option.None => { result = 0; } }\n"
                         "    return result;\n"
                         "}\n";
...
static AsthraTestResult test_option_pattern_matching_nested(AsthraTestContext *context) {
    const char *source =
        ...
        "    let mut result: i32 = 0;\n"
        "    match opt { Option.Some(inner) => { match inner { Option.Some(value) => { result = "
        "value; }, Option.None => { result = -1; } } }, Option.None => { result = 0; } }\n"
        "    return result;\n"
        "}\n";
...
static AsthraTestResult test_option_pattern_matching_wildcard(AsthraTestContext *context) {
    const char *source = "package test;\n"
                         ...
                         "    match opt { Option.Some(_) => { result = true; }, Option.None => { "
                         "result = false; } }\n"
                         "    return result;\n"
                         "}\n";
...
static AsthraTestResult test_result_pattern_matching(AsthraTestContext *context) {
    const char *source = "package test;\n"
                         ...
                         "    let mut result: i32 = 0;\n"
                         "    match res { Result.Ok(value) => { result = value; }, Result.Err(_) "
                         "=> { result = -1; } }\n"
                         "    return result;\n"
                         "}\n";
...
AsthraTestSuite *create_option_pattern_codegen_test_suite(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create("Option Pattern Matching Codegen Tests",
    ...
    asthra_test_suite_add_test(suite, "test_option_pattern_matching_basic",
                               "Basic Option pattern matching", test_option_pattern_matching_basic);
    ...
    asthra_test_suite_add_test(suite, "test_option_if_let_pattern", "Option if-let patterns",
