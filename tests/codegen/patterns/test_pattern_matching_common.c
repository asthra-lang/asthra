 ...
 * Pattern Matching Engine Tests - Common Implementation
 *
 * Copyright (c) 2024 Asthra Project
 * Licensed under the terms specified in LICENSE
 *
 * Implementation of shared utilities for pattern matching engine tests
 */
...
void add_match_case(MatchExpression *match, Pattern *pattern,
    ...
    if (pattern->type == PATTERN_CONSTRUCTOR || pattern->type == PATTERN_LITERAL) {
        VariantTag tag =
            (pattern->type == PATTERN_CONSTRUCTOR) ? pattern->expected_tag : pattern->literal->tag;
