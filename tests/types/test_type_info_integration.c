...
static bool test_type_info_creation(void) {
    ...
    TypeInfo *string_type = type_info_create_primitive("string", PRIMITIVE_INFO_STRING, sizeof(void*));
...
static bool test_type_descriptor_conversion(void) {
    ...
    TEST_ASSERT(converted_descriptor->data.primitive.primitive_kind == PRIMITIVE_I32, "Converted back primitive kind");
    
    // Test that they're equivalent
    TEST_ASSERT(type_descriptor_equals(i32_descriptor, converted_descriptor), "Round-trip conversion preserves equality");
