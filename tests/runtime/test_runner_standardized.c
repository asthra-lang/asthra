...
AsthraTestResult adapter_memory_zones(AsthraTestContext *context) {
    ...
    if (result) g_runtime_stats.memory_tests_passed++;
    return result ? ASTHRA_TEST_PASS : ASTHRA_TEST_FAIL;
}
...
void runtime_analysis_callback(AsthraTestSuite *suite, AsthraTestStatistics *stats) {
    ...
    printf("Test Results: %zu/%zu passed (%.1f%%)\n", 
           stats->passed_tests, stats->total_tests, stats->pass_rate);
    ...
    if (g_runtime_stats.error_tests_passed >= 1 && g_runtime_stats.observability_tests_passed >= 1) {
...
int main(void) {
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "runtime_initialization", "Runtime initialization functionality",
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "runtime_custom_config", "Runtime custom configuration",
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "memory_zones", "Memory zone management",
        adapter_memory_zones,
        &(AsthraTestMetadata){
            .severity = ASTHRA_TEST_SEVERITY_CRITICAL,
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "gc_operations", "Garbage collection operations",
        adapter_gc_operations,
        &(AsthraTestMetadata){
            .severity = ASTHRA_TEST_SEVERITY_CRITICAL,
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "slice_operations", "Slice operations and management",
        adapter_slice_operations,
        &(AsthraTestMetadata){
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "string_operations", "String operations and management",
        adapter_string_operations,
        &(AsthraTestMetadata){
            .severity = ASTHRA_TEST_SEVERITY_HIGH,
            .timeout_ns = 5000000000ULL,
            .skip = false
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "result_pattern_matching", "Result type and pattern matching",
    ...
    asthra_test_suite_add_test_with_metadata(suite,
        "task_system", "Task system functionality",
        adapter_task_system,
        &(AsthraTestMetadata){
