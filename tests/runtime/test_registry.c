...
static TestRegistryEntry test_registry[] = {
    ...
    // String Operations Tests
    {"Deterministic Behavior", "String Operations", test_string_deterministic_behavior, false, false},
    {"Performance Characteristics", "String Operations", test_string_performance_characteristics, true, false},
    ...
    {"SliceHeader Memory Layout", "Slice Management", test_slice_header_memory_layout, false, false},
    {"Bounds Checking Runtime", "Slice Management", test_slice_bounds_checking_runtime, false, true},
    {"Bounds Checking Edge Cases", "Slice Management", test_slice_bounds_checking_edge_cases, false, true},
    {"Bounds Checking Performance", "Slice Management", test_slice_bounds_checking_performance, true, false},
    {"Predictable Memory Layout", "Slice Management", test_slice_predictable_memory_layout, false, false},
    ...
    {"Large Data Memory Safety", "Slice Management", test_slice_large_data_memory_safety, false, true},
    ...
    {"Memory Safety Bounds Checking", "Enhanced FFI", test_ffi_memory_safety_bounds_checking, false, true},
    {"Memory Safety Leak Prevention", "Enhanced FFI", test_ffi_memory_safety_leak_prevention, false, true},
    {"Memory Safety Double Free Prevention", "Enhanced FFI", test_ffi_memory_safety_double_free_prevention, false, true},
    ...
    {"Spawn with C Functions", "Concurrency", test_concurrency_spawn_with_c_functions, false, false},
    {"C Threads Calling Asthra", "Concurrency", test_concurrency_c_threads_calling_asthra, false, false},
    ...
    {"Memory Disclosure Prevention", "Security", test_security_memory_disclosure_prevention, false, true},
    
    // Integration Tests
    {"Concurrent Text Processing", "Integration", test_integration_concurrent_text_processing, false, false},
    {"Concurrent Slice Management", "Integration", test_integration_concurrent_slice_management, false, false},
    {"Pattern Matching with FFI", "Integration", test_integration_pattern_matching_with_ffi, false, false},
    {"Security with Concurrency", "Integration", test_integration_security_with_concurrency, false, true},
    {"Performance Comprehensive", "Integration", test_integration_performance_comprehensive, true, false},
};
...
void test_summary_add_failure(TestSummary *summary, const char *test_name, const char *message) {
    if (!summary || !test_name || !message) return;
    
    summary->failure_messages = realloc(summary->failure_messages, 
                                       (summary->failure_count + 1) * sizeof(char*));
    
    if (!summary->failure_messages) return;
    ...
    if (summary->failure_messages[summary->failure_count]) {
        snprintf(summary->failure_messages[summary->failure_count], msg_len, 
                 "%s: %s", test_name, message);
