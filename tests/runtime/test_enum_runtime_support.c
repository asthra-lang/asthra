...
static bool test_enum_variant_creation(void) {
    ...
    int32_t value = 42;
    AsthraEnumVariant variant = Asthra_enum_create_variant(
        0, &value, sizeof(value), 1, ASTHRA_OWNERSHIP_GC);
...
static bool test_enum_variant_large_data(void) {
    ...
    AsthraEnumVariant variant = Asthra_enum_create_variant(
        1, large_data, sizeof(large_data), 2, ASTHRA_OWNERSHIP_GC);
...
static bool test_enum_tag_checking(void) {
    ...
    int32_t value = 100;
    AsthraEnumVariant variant = Asthra_enum_create_variant(
        5, &value, sizeof(value), 1, ASTHRA_OWNERSHIP_GC);
...
static bool test_result_ok_operations(void) {
    ...
    AsthraEnumVariant result = Asthra_result_create_ok(
        &success_value, sizeof(success_value), 1, ASTHRA_OWNERSHIP_GC);
...
static bool test_result_err_operations(void) {
    ...
    AsthraEnumVariant err_variant = Asthra_result_create_err(
        error_msg, strlen(error_msg) + 1, 2, ASTHRA_OWNERSHIP_GC);
...
static bool test_option_some_operations(void) {
    ...
    AsthraEnumVariant option = Asthra_option_create_some(
        &some_value, sizeof(some_value), 3, ASTHRA_OWNERSHIP_GC);
...
static bool test_pattern_matching(void) {
    ...
    int32_t value = 42;
    AsthraEnumVariant result = Asthra_result_create_ok(
        &value, sizeof(value), 1, ASTHRA_OWNERSHIP_GC);
    
    // Set up pattern matching arms
    int match_result = 0;
    AsthraEnumMatchArm arms[] = {
        { ASTHRA_RESULT_TAG_OK, ok_callback, &match_result },
...
static bool test_pattern_matching_wildcard(void) {
    ...
    int32_t value = 42;
    AsthraEnumVariant variant = Asthra_enum_create_variant(
        999, &value, sizeof(value), 1, ASTHRA_OWNERSHIP_GC);
...
static bool test_error_handling(void) {
    ...
    // Test type safety
    int32_t value = 42;
    AsthraEnumVariant err_result = Asthra_result_create_err(
        &value, sizeof(value), 1, ASTHRA_OWNERSHIP_GC);
...
static bool test_result_option_integration(void) {
    ...
    AsthraEnumVariant option_some = Asthra_option_create_some(
        &some_value, sizeof(some_value), 1, ASTHRA_OWNERSHIP_GC);
    
    // Create Result.Ok(Option.Some(42))
    AsthraEnumVariant result_ok = Asthra_result_create_ok(
        &option_some, sizeof(option_some), 4, ASTHRA_OWNERSHIP_GC);
...
int main(void) {
    ...
    // Basic enum operations tests
    if (!test_enum_variant_creation()) all_tests_passed = false;
    if (!test_enum_variant_large_data()) all_tests_passed = false;
    if (!test_enum_tag_checking()) all_tests_passed = false;
    
    // Result operations tests
    if (!test_result_ok_operations()) all_tests_passed = false;
    if (!test_result_err_operations()) all_tests_passed = false;
    
    // Option operations tests
    if (!test_option_some_operations()) all_tests_passed = false;
    if (!test_option_none_operations()) all_tests_passed = false;
    
    // Pattern matching tests
    if (!test_pattern_matching()) all_tests_passed = false;
    if (!test_pattern_matching_wildcard()) all_tests_passed = false;
    
    // Error handling tests
    if (!test_error_handling()) all_tests_passed = false;
    
    // Integration tests
    if (!test_result_option_integration()) all_tests_passed = false;
