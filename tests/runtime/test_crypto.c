...
int test_cryptographic_primitives(void) {
    ...
    TEST_ASSERT(asthra_constant_time_memcmp(data1, data2, 5) == 0,
                "Constant-time memcmp should return 0 for equal data");
    TEST_ASSERT(asthra_constant_time_memcmp(data1, data3, 5) != 0,
                "Constant-time memcmp should return non-zero for different data");
