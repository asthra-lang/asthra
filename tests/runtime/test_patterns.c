...
int test_result_pattern_matching(void) {
    ...
    // Test Ok result
    int ok_value = 42;
    AsthraResult ok_result =
        asthra_result_ok(&ok_value, sizeof(int), ASTHRA_TYPE_I32, ASTHRA_OWNERSHIP_C);
    ...
    AsthraResult err_result =
        asthra_result_err(err_msg, strlen(err_msg) + 1, ASTHRA_TYPE_STRING, ASTHRA_OWNERSHIP_C);
    ...
    int match_result = 0;
    AsthraMatchArm arms[] = {{.pattern = ASTHRA_MATCH_OK,
                              .expected_type_id = ASTHRA_TYPE_I32,
                              .handler = ok_handler,
                              .context = &match_result},
                             {.pattern = ASTHRA_MATCH_ERR,
                              .expected_type_id = 0,
                              .handler = err_handler,
                              .context = &match_result}};
