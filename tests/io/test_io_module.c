...
static AsthraTestResult test_stream_access_functions(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_EQ(context, stdout_handle, stdout,
                          "stdout handle should match system stdout");
    ASTHRA_TEST_ASSERT_EQ(context, stderr_handle, stderr,
                          "stderr handle should match system stderr");
...
static AsthraTestResult test_string_conversion_functions(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_EQ(context, asthra_str.len, strlen(test_cstr),
                          "Asthra string length should match C string length");
    ASTHRA_TEST_ASSERT_STR_EQ(context, asthra_str.data, test_cstr,
                              "Asthra string content should match C string");
    ...
    ASTHRA_TEST_ASSERT_STR_EQ(context, converted_cstr, test_cstr,
                              "Converted C string should match original");
...
static AsthraTestResult test_utf8_validation_concept(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_EQ(context, (int)invalid_continuation[1], 0x28,
                          "Invalid continuation test data correct");
...
static AsthraTestResult test_error_handling_concept(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_NE(context, IO_ERROR_INVALID_UTF8, IO_ERROR_READ_ERROR,
                          "Different errors should have different values");
...
static AsthraTestResult test_memory_safety_for_io_operations(AsthraTestContext *context) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ASTHRA_TEST_ASSERT_NOT_NULL(context, strstr(strings[i].data, "I/O string"),
                                    "String should contain expected content");
    }
...
int main(void) {
    ...
    asthra_test_suite_add_test(suite, "stream_access_functions", "Test stream access functions",
                               test_stream_access_functions);
    asthra_test_suite_add_test(suite, "string_conversion_functions",
                               "Test string conversion functions",
                               test_string_conversion_functions);
    asthra_test_suite_add_test(suite, "println_functions", "Test println functions",
                               test_println_functions);

    // Concept validation tests
    asthra_test_suite_add_test(suite, "utf8_validation_concept", "Test UTF-8 validation concept",
                               test_utf8_validation_concept);
    asthra_test_suite_add_test(suite, "error_handling_concept", "Test error handling concept",
                               test_error_handling_concept);
    asthra_test_suite_add_test(suite, "buffer_management_concept", "Test buffer management concept",
                               test_buffer_management_concept);

    // Integration tests
    asthra_test_suite_add_test(suite, "runtime_initialization_for_io",
                               "Test runtime initialization for I/O",
                               test_runtime_initialization_for_io);
    asthra_test_suite_add_test(suite, "memory_safety_for_io_operations",
                               "Test memory safety for I/O operations",
                               test_memory_safety_for_io_operations);
