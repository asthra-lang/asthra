...
AsthraTestResult test_slice_creation_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; slice_creation_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid slice creation syntax: %s", slice_creation_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, indices_valid,
                               "Invalid indices in slice creation: %s", slice_creation_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_slice_len_access_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; slice_len_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid slice length syntax: %s", slice_len_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, semantics_valid,
                               "Invalid slice length semantics: %s", slice_len_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_slice_array_literal_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; array_literal_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid array literal syntax: %s", array_literal_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, elements_valid,
                               "Invalid elements in array literal: %s", array_literal_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {
        .name = "Grammar Slice Syntax Tests",
        .file = __FILE__,
        .line = __LINE__,
        .function = "main",
