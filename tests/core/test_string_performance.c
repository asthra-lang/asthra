...
AsthraTestResult test_string_concatenation_performance(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, avg_duration_ms < 1.0,
                           "Concatenation performance too slow: %.3f ms average", avg_duration_ms)) {
...
static const size_t performance_test_count = sizeof(performance_test_cases) / sizeof(performance_test_cases[0]);
...
AsthraTestResult run_string_performance_test_suite(void) {
    ...
    printf("Success Rate: %.1f%%\n", 
           performance_test_count > 0 ? (double)passed_tests / performance_test_count * 100.0 : 0.0);
