...
AsthraTestResult test_pattern_match_statement_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; match_syntax_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid match syntax: %s", match_syntax_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, patterns_valid,
                               "Invalid patterns in match statement: %s", match_syntax_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_pattern_if_let_constructs(AsthraTestContext *ctx) {
    ...
    for (int i = 0; if_let_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid if-let syntax: %s", if_let_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, pattern_valid,
                               "Invalid pattern in if-let: %s", if_let_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_pattern_destructuring_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; destructuring_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid destructuring syntax: %s", destructuring_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, pattern_valid,
                               "Invalid pattern in destructuring: %s", destructuring_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_pattern_result_type_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; result_type_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid,
                               "Invalid Result type syntax: %s", result_type_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, type_valid,
                               "Invalid Result type parameters: %s", result_type_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
