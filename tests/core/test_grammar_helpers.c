...
bool grammar_test_parse_semantic_tag(const char *tag_expression) {
    ...
    const char *known_tags[] = {"#[ownership(gc)]",      "#[ownership(c)]", "#[transfer_full]",
                                "#[security_sensitive]", "#[borrowed]",     NULL};
...
bool grammar_test_validate_semantic_tag(const char *tag_expression) {
    ...
    if (!grammar_test_parse_semantic_tag(tag_expression))
        return false;

    // Mock: additional semantic validation
    // Check that tag is applied to appropriate context (simplified)
    bool has_type =
        strstr(tag_expression, "int") != NULL || strstr(tag_expression, "char") != NULL ||
        strstr(tag_expression, "void") != NULL || strstr(tag_expression, "uint8_t") != NULL;
...
bool grammar_test_parse_ffi_annotation(const char *annotation) {
    ...
    const char *ffi_patterns[] = {"extern \"C\"", "#[ffi_safe]",          "// variadic removed",
                                  "#[c_compat]",  "#[ownership(pinned)]", NULL};
...
bool grammar_test_validate_ffi_annotation(const char *annotation) {
    ...
    if (!grammar_test_parse_ffi_annotation(annotation))
        return false;

    // Mock: basic semantic validation
    // Check that annotation appears in appropriate context
    bool has_function = strstr(annotation, "int ") != NULL || strstr(annotation, "void ") != NULL ||
                        strstr(annotation, "(") != NULL;

    bool has_struct = strstr(annotation, "struct ") != NULL;
    bool has_variable = strstr(annotation, "buffer") != NULL || strstr(annotation, "[") != NULL;
...
bool grammar_test_type_args_populated(const char *expression) {
    ...
    if (!grammar_test_has_type_arguments(expression))
        return false;
