...
AsthraTestResult test_memory_safety_concepts(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_EQ(context, (int)test_buffer[0], (int)'A', "Buffer should be filled correctly");
    ASTHRA_TEST_ASSERT_EQ(context, (int)test_buffer[99], (int)'\0', "Buffer should be null terminated");
...
AsthraTestResult test_error_handling_concepts(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_EQ(context, (int)success_result.tag, (int)RESULT_OK, "Success result should have OK tag");
    asthra_test_assert_int_eq(context, success_result.value, 42, "Success result should have correct value");
    
    ASTHRA_TEST_ASSERT_EQ(context, (int)error_result.tag, (int)RESULT_ERROR, "Error result should have ERROR tag");
    ASTHRA_TEST_ASSERT_NOT_NULL(context, error_result.error_message, "Error result should have message");
...
int main(void) {
    ...
    asthra_test_suite_add_test(suite, "basic_grammar", 
        "Test basic grammar functionality concepts", test_basic_grammar_functionality);
    asthra_test_suite_add_test(suite, "string_operations", 
        "Test basic string operations", test_string_operations_basic);
    asthra_test_suite_add_test(suite, "pattern_matching", 
        "Test basic pattern matching concepts", test_pattern_matching_concepts);
    asthra_test_suite_add_test(suite, "memory_safety", 
        "Test basic memory safety concepts", test_memory_safety_concepts);
    asthra_test_suite_add_test(suite, "type_system", 
        "Test basic type system concepts", test_type_system_basics);
    asthra_test_suite_add_test(suite, "error_handling", 
        "Test basic error handling concepts", test_error_handling_concepts);
