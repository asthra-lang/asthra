    ...
    for (int i = 0; test_cases[i][0] != NULL; i++) {
        ...
        for (int trial = 0; trial < 5; trial++) {
            ...
            if (!ASTHRA_TEST_ASSERT(context, ts1 != NULL && ts2 != NULL,
                                    "Failed to create test strings for case %d, trial %d", i,
                                    trial)) {
                if (ts1)
            ...
            if (!ASTHRA_TEST_ASSERT(context, result->length == strlen(expected),
                                    ...
                                    "expected %zu, got %zu",
                                    i, trial, strlen(expected), result->length)) {
                test_string_destroy(ts1);
                test_string_destroy(ts2);
                test_string_destroy(result);
                return ASTHRA_TEST_FAIL;
...
AsthraTestResult test_string_deterministic_interpolation(AsthraTestContext *context) {
    ...
    const char *templates[][4] = {{"Hello, ", "John", "! You have ", " messages."},
                                  {"User ", "admin", " logged in at ", "12:00"},
                                  {"Processing ", "100", " items in ", " seconds"},
                                  {NULL, NULL, NULL, NULL}};

    const char *variables[][2] = {{"5"}, {"2023-12-01"}, {"3.14"}, {NULL}};
    ...
    for (int template_idx = 0; templates[template_idx][0] != NULL; template_idx++) {
        for (int var_idx = 0; variables[var_idx][0] != NULL; var_idx++) {
            ...
            char expected[256];
            snprintf(expected, sizeof(expected), "%s%s%s%s", templates[template_idx][0],
                     templates[template_idx][1], templates[template_idx][2], variables[var_idx][0]);
                        ...
                        if (temp2) {
                            ...
                            if (trial == 0) {
                                ...
                                if (!ASTHRA_TEST_ASSERT(context,
                                                        strcmp(result->data, first_result) == 0,
                                                        "Interpolation result not deterministic "
                                                        "for template %d, var %d",
...
AsthraTestResult test_string_cross_platform_consistency(AsthraTestContext *context) {
    ...
    const char *unicode_strings[] = {"Hello, 世界", // Mixed ASCII and Chinese
    ...
    for (int i = 0; unicode_strings[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, ts != NULL, "Failed to create Unicode string %d", i)) {
            return ASTHRA_TEST_FAIL;
        }
    ...
    const char *mixed_parts[][2] = {
        {"ASCII", "世界"}, {"🌟", "text"}, {"café", "モンド"}, {NULL, NULL}};
    ...
    const char *line_endings[] = {"Line 1\nLine 2",   // Unix
                                  "Line 1\r\nLine 2", // Windows
    ...
    for (int i = 0; line_endings[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, ts != NULL, "Failed to create line ending test string %d",
                                i)) {
            return ASTHRA_TEST_FAIL;
        }
...
static const size_t deterministic_test_count =
    sizeof(deterministic_test_cases) / sizeof(deterministic_test_cases[0]);
...
AsthraTestResult run_string_deterministic_test_suite(void) {
    ...
    printf("Passed: %d\n", passed_tests);
    printf("Failed: %d\n", failed_tests);
    printf("Success Rate: %.1f%%\n", deterministic_test_count > 0
                                         ? (double)passed_tests / deterministic_test_count * 100.0
                                         : 0.0);
