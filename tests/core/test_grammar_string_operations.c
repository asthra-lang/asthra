...
AsthraTestResult test_string_concatenation_syntax(AsthraTestContext *ctx) {
    ...
    for (int i = 0; concat_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid, "Invalid string concatenation syntax: %s",
                                concat_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, type_valid, "Invalid types in string concatenation: %s",
                                concat_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_string_edge_case_parsing(AsthraTestContext *ctx) {
    ...
    for (int i = 0; edge_case_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, syntax_valid, "Invalid string edge case syntax: %s",
                                edge_case_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(ctx, handled_correctly, "Edge case not handled correctly: %s",
                                edge_case_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
