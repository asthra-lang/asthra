...
AsthraTestResult test_grammar_postfix_expr_disambiguation(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, parse_success,
                               "Failed to parse postfix expression: %s", test_cases[i])) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, disambiguation_correct,
                               "Postfix expression disambiguation failed for: %s", test_cases[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_grammar_semantic_tags_parsing(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; semantic_tag_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, tag_parsed,
                               "Failed to parse semantic tag: %s", semantic_tag_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(&ctx->base, tag_valid,
                               "Invalid semantic tag: %s", semantic_tag_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_grammar_ffi_annotations_parsing(AsthraV12TestContext *ctx) {
    ...
    const char *ffi_annotation_tests[] = {
        ...
        "// variadic functions removed - use explicit overloads", // Variadic functions no longer supported
    ...
    for (int i = 0; ffi_annotation_tests[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, annotation_parsed,
                               "Failed to parse FFI annotation: %s", ffi_annotation_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }

        if (!ASTHRA_TEST_ASSERT(&ctx->base, annotation_valid,
                               "Invalid FFI annotation: %s", ffi_annotation_tests[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_grammar_postfix_double_colon_restriction(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; invalid_postfix_patterns[i] != NULL; i++) {
        ...
        bool has_postfix_error = (error_msg && strstr(error_msg, "postfix") != NULL && 
                                 strstr(error_msg, "::") != NULL);
...
AsthraTestResult test_grammar_valid_associated_function_calls(AsthraV12TestContext *ctx) {
    ...
    const char *valid_associated_patterns[] = {
        "Vec::new()",                         // Simple associated function
    ...
    for (int i = 0; valid_associated_patterns[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, parse_success,
                               "Should parse valid :: usage: %s", 
                               valid_associated_patterns[i])) {
            return ASTHRA_TEST_FAIL;
        }
        
        // Verify AST structure is correct (AssociatedFuncCall)
        bool is_associated_call = grammar_test_is_associated_function_call(valid_associated_patterns[i]);
...
AsthraTestResult test_grammar_generic_type_associated_functions(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; generic_associated_patterns[i] != NULL; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, parse_success,
                               "Should parse generic type :: usage: %s", 
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, has_type_args,
                               "Should capture type arguments for: %s", 
                               generic_associated_patterns[i])) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_grammar_disambiguation_ai_generation(AsthraV12TestContext *ctx) {
    ...
    const char *ai_confusion_cases[] = {
        // These should now be unambiguously invalid
        "42::method()",
        "func()::chain()",
        "expr::transform()",
