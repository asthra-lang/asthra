...
AsthraTestResult test_string_gc_interaction(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, strings != NULL,
                           "Failed to allocate test string array")) {
    ...
    for (int i = 0; i < num_strings; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, strings[i] != NULL,
                               "Failed to create string %d", i)) {
...
AsthraTestResult test_string_memory_management(AsthraTestContext *context) {
        ...
        if (strings) {
            ...
            for (int j = 0; j < batch_size; j++) {
                ...
                if (!ASTHRA_TEST_ASSERT(context, strings[j] != NULL,
                                       "Memory allocation failed at iteration %d, string %d", i, j)) {
                    // Cleanup on failure
                    for (int k = 0; k < j; k++) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, final_memory == initial_memory,
                           "Memory leak detected: initial=%zu, final=%zu", 
...
AsthraTestResult test_string_large_operations(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, large_buffer != NULL,
    ...
    if (!ASTHRA_TEST_ASSERT(context, large_string != NULL,
                           "Failed to create large string")) {
        free(large_buffer);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(context, result != NULL,
                           "Failed to concatenate large string")) {
