...
AsthraTestResult test_result_exhaustive_matching(AsthraV12TestContext *ctx) {
    ...
    if (err_result.is_ok) {
        ...
        if (!ASTHRA_TEST_ASSERT(
                &ctx->base, strcmp(err_result.data.error_message, "Test error") == 0,
...
AsthraTestResult test_result_all_cases_handled(AsthraV12TestContext *ctx) {
    ...
    TestResult test_cases[] = {test_result_ok(1), test_result_ok(100), test_result_err("Error 1"),
                               test_result_err("Error 2"), test_result_ok(0)};
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, ok_count == 3, "Expected 3 Ok cases, got %d", ok_count)) {
        return ASTHRA_TEST_FAIL;
    }

    if (!ASTHRA_TEST_ASSERT(&ctx->base, err_count == 2, "Expected 2 Err cases, got %d",
                            err_count)) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_result_nested_matching(AsthraV12TestContext *ctx) {
    ...
    // Create test cases
    NestedResult nested_ok_ok = {.outer_is_ok = true,
                                 .outer_data.inner_result = test_result_ok(42)};

    NestedResult nested_ok_err = {.outer_is_ok = true,
                                  .outer_data.inner_result = test_result_err("Inner error")};

    NestedResult nested_err = {.outer_is_ok = false, .outer_data.outer_error = "Outer error"};
    ...
    if (nested_ok_ok.outer_is_ok) {
        if (nested_ok_ok.outer_data.inner_result.is_ok) {
            if (!ASTHRA_TEST_ASSERT(&ctx->base,
                                    nested_ok_ok.outer_data.inner_result.data.ok_value == 42,
    ...
    if (nested_ok_err.outer_is_ok) {
        if (!nested_ok_err.outer_data.inner_result.is_ok) {
            if (!ASTHRA_TEST_ASSERT(&ctx->base,
                                    strcmp(nested_ok_err.outer_data.inner_result.data.error_message,
                                           "Inner error") == 0,
    ...
    if (!nested_err.outer_is_ok) {
        if (!ASTHRA_TEST_ASSERT(&ctx->base,
                                strcmp(nested_err.outer_data.outer_error, "Outer error") == 0,
...
AsthraTestResult test_result_complex_patterns(AsthraV12TestContext *ctx) {
    ...
    TestResult results[] = {test_result_ok(-5),   // Should be handled specially
    ...
    for (size_t i = 0; i < result_count; i++) {
        ...
        if (result.is_ok) {
            ...
            if (value < 0) {
                ...
                // Handle zero
                if (!ASTHRA_TEST_ASSERT(&ctx->base, value == 0, "Expected zero value")) {
                    return ASTHRA_TEST_FAIL;
                }
            } else if (value > 100) {
                // Handle large values
