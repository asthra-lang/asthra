...
AsthraTestResult test_json_guidance_generation(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(context, strstr(json, "\"schema_version\"") != NULL, "Missing schema_version")) {
        free(json);
        ai_lint_result_destroy(result);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, strstr(json, "\"AI_IDIOM_001\"") != NULL, "Missing rule_id")) {
        free(json);
        ai_lint_result_destroy(result);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, strstr(json, "\"statistics\"") != NULL, "Missing statistics")) {
        free(json);
        ai_lint_result_destroy(result);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_json_rule_catalog(AsthraTestContext *context) {
    ...
    // Basic validation
    if (!asthra_test_assert_bool(context, strstr(json, "\"TEST_RULE\"") != NULL, "Missing test rule")) {
        free(json);
        ai_linter_destroy(linter);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, strstr(json, "\"rule_count\"") != NULL, "Missing rule_count")) {
        free(json);
        ai_linter_destroy(linter);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_json_fix_suggestions(AsthraTestContext *context) {
    ...
    // Basic validation
    if (!asthra_test_assert_bool(context, strstr(json, "\"AI_IDIOM_002\"") != NULL, "Missing rule_id")) {
        free(json);
        ai_lint_result_destroy(result);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, strstr(json, "\"fix_suggestions\"") != NULL, "Missing fix_suggestions")) {
        free(json);
        ai_lint_result_destroy(result);
        return ASTHRA_TEST_FAIL;
    }
