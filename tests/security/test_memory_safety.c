...
AsthraTestResult test_security_memory_zeroing(AsthraV12TestContext *ctx) {
    ...
    if (!asthra_verify_secure_memory_zeroing(sensitive_buffer, buffer_size)) {
        if (!ASTHRA_TEST_ASSERT(&ctx->base, false, "Secure memory zeroing failed")) {
            free(sensitive_buffer);
            return ASTHRA_TEST_FAIL;
        }
    }
...
AsthraTestResult test_security_memory_protection(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, bounds_check_passed, "Valid memory access failed")) {
    ...
    if (!ASTHRA_TEST_ASSERT(
            &ctx->base, protected_buffer[0] == 0xAA && protected_buffer[protected_size - 1] == 0x55,
...
AsthraTestResult test_security_overflow_detection(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !overflow_detected, "False positive overflow detection")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_security_heap_protection(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, heap_buffer != NULL, "Failed to allocate heap buffer")) {
        return ASTHRA_TEST_ERROR;
    }
    ...
    struct {
        ...
        uint32_t magic;
        uint8_t *data;
    } heap_metadata = {.size = heap_size, .magic = 0xCAFEBABE, .data = heap_buffer};
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base,
                            heap_metadata.magic == 0xCAFEBABE && heap_metadata.size == heap_size &&
...
int main(void) {
    ...
    // Create test context
    AsthraV12TestMetadata metadata = {.base = {.name = "Memory Safety Tests",
                                               .file = __FILE__,
                                               .line = __LINE__,
                                               .function = "main",
