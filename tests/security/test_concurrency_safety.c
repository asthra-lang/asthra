...
void test_task_lifecycle_logging(void) {
    ...
    asthra_safety_log_task_lifecycle_event(12345, ASTHRA_TASK_EVENT_COMPLETED, "Test task completed");
...
void test_scheduler_event_logging(void) {
    ...
    asthra_safety_log_scheduler_event(ASTHRA_SCHEDULER_EVENT_TASK_QUEUED, "Task queued for execution");
    asthra_safety_log_scheduler_event(ASTHRA_SCHEDULER_EVENT_WORKER_STARTED, "Worker thread started");
