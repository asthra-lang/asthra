...
AsthraTestResult test_security_statistical_randomness(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, ones_ratio > 0.45 && ones_ratio < 0.55,
                           "Frequency test failed: ones ratio %.3f (should be 0.45-0.55)", ones_ratio)) {
        free(random_data);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_security_cryptographic_primitives(AsthraV12TestContext *ctx) {
    ...
    for (size_t i = 0; i < key_size - 3; i++) {
        if (key_material[i] == key_material[i + 1] &&
            key_material[i] == key_material[i + 2] &&
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, !has_pattern,
                           "Key material shows obvious patterns")) {
        free(key_material);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraV12TestMetadata metadata = {
        ...
        .ai_feedback_notes = "Validates cryptographic randomness quality",
