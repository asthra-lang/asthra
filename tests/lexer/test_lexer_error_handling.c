...
AsthraTestResult test_lexer_unterminated_string(AsthraTestContext *context) {
    const char *test_cases[] = {"\"unterminated string", "\"string with\nnewline",
                                "\"string with \\", // Unterminated escape
                                "'unterminated char",    "\"string with\rtab",
                                "'char with\nnewline",   "\"escape at end\\",
                                "'escape at end\\",      NULL};
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for input: %s",
                                        input)) {
...
AsthraTestResult test_lexer_invalid_character(AsthraTestContext *context) {
    const char *test_cases[] = {"\x01", // Control character
                                "\x02", // STX character
                                "\x03", // ETX character
                                "\x7F", // DEL character
                                "\x1B", // ESC character
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for invalid character test")) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_bool(
                context, is_valid_response,
                "Invalid character should produce error or EOF, got token type %d", token.type)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_invalid_number_format(AsthraTestContext *context) {
    const char *test_cases[] = {"0x",    // Incomplete hex
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for invalid number: %s", input)) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_invalid_escape_sequences(AsthraTestContext *context) {
    const char *test_cases[] = {"\"\\q\"",         // Invalid escape character
...
AsthraTestResult test_lexer_buffer_overflow(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(
            context, is_valid_response,
            "Long identifier should produce identifier or error, got token type %d", token.type)) {
        token_free(&token);
        lexer_destroy(lexer);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    } test_cases[] = {{test_lexer_unterminated_string, "test_lexer_unterminated_string"},
