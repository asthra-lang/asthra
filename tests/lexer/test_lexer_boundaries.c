...
AsthraTestResult test_lexer_single_character(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for single character: %s", input)) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_large_input(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_pointer(context, large_input, "Failed to allocate memory for large input test")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_size_eq(context, tokens_processed, token_count,
                                   "Should have processed %zu tokens, got %zu", token_count, tokens_processed)) {
        lexer_destroy(lexer);
        free(large_input);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_lexer_whitespace_only_input(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        do {
            token = lexer_next_token(lexer);
            if (token.type != TOKEN_WHITESPACE && token.type != TOKEN_NEWLINE && token.type != TOKEN_EOF) {
