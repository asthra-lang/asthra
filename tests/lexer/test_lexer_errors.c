...
AsthraTestResult test_lexer_null_literals(AsthraTestContext *context) {
    const char *null_keywords[] = {"null", "nil", "None", "undefined", NULL};
    ...
    for (int i = 0; null_keywords[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for null keyword: %s", keyword)) {
            return ASTHRA_TEST_FAIL;
        }

        Token token = lexer_next_token(lexer);

        // Should be either an error token, identifier, or specific keyword tokens depending on
        // language design
        bool is_valid = (token.type == TOKEN_ERROR || token.type == TOKEN_IDENTIFIER ||
                         token.type == TOKEN_BOOL_FALSE || // null might be parsed as false
                         token.type == TOKEN_BOOL_TRUE || token.type == TOKEN_EOF);

        if (!asthra_test_assert_bool(
                context, is_valid,
                "Null keyword '%s' should be handled properly, got token type %d", keyword,
                token.type)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
...
AsthraTestResult test_lexer_malformed_literals(AsthraTestContext *context) {
    const char *malformed_cases[] = {"\"unterminated string", // Unterminated string
    ...
    for (int i = 0; malformed_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for malformed case: %s", input)) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_bool(context, has_error, "Expected error for malformed input: %s",
                                     input)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
...
AsthraTestResult test_lexer_invalid_escapes(AsthraTestContext *context) {
    const char *invalid_escapes[] = {"'\\z'",       // Invalid escape character
    ...
    for (int i = 0; invalid_escapes[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for invalid escape: %s", input)) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        bool handled = (token.type == TOKEN_ERROR || token.type == TOKEN_CHAR ||
                        token.type == TOKEN_STRING || lexer_get_error(lexer) != NULL);
...
AsthraTestResult test_lexer_error_recovery(AsthraTestContext *context) {
    ...
    for (int i = 0; error_recovery_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for error recovery: %s", input)) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        bool has_valid_tokens =
            (first_token.type != TOKEN_ERROR || second_token.type != TOKEN_ERROR ||
...
AsthraTestResult test_lexer_literal_boundaries(AsthraTestContext *context) {
    ...
    } boundary_cases[] = {{"''", "Empty character literal"},
    ...
    for (int i = 0; boundary_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(
                context, lexer, "Failed to create lexer for boundary case: %s", description)) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        bool handled = (token.type == TOKEN_ERROR || token.type == TOKEN_EOF ||
                        token.type == TOKEN_CHAR || token.type == TOKEN_STRING ||
                        token.type == TOKEN_INTEGER || token.type == TOKEN_FLOAT ||
                        token.type == TOKEN_DOT || token.type == TOKEN_IDENTIFIER);

        if (!asthra_test_assert_bool(context, handled, "Boundary case should be handled: %s (%s)",
                                     description, input)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
...
int main(void) {
    ...
    } test_cases[] = {{test_lexer_boolean_literals, "test_lexer_boolean_literals"},
