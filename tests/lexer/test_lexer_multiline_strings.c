...
static AsthraTestResult test_lexer_raw_multiline_strings(AsthraTestContext *context) {
    const struct {
        ...
        {"r\"\"\"String with \\n literal backslash\"\"\"", "String with \\n literal backslash"},
        {"r\"\"\"Path: C:\\Users\\Name\\Documents\"\"\"", "Path: C:\\Users\\Name\\Documents"},
        {"r\"\"\"Regex: \\d+\\.\\d+\"\"\"", "Regex: \\d+\\.\\d+"},
        // Empty raw multi-line string
        {"r\"\"\"\"\"\"", ""},
        // Raw string with quotes inside
        {"r\"\"\"He said \"Hello\" to me\"\"\"", "He said \"Hello\" to me"},
        {NULL, NULL}};
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for raw string: %s",
                                       input);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_lexer_processed_multiline_strings(AsthraTestContext *context) {
    const struct {
        ...
        {"\"\"\"Backslash: \\\\\"\"\"", "Backslash: \\"},
        {"\"\"\"Null\\0Terminated\"\"\"", "Null\0Terminated"},
        {"\"\"\"Carriage\\rReturn\"\"\"", "Carriage\rReturn"},
        {"\"\"\"Single quote: \\'\"\"\"", "Single quote: '"},
        // Empty processed multi-line string
        {"\"\"\"\"\"\"", ""},
        // Mixed escape sequences
        {"\"\"\"Mixed\\tEscape\\nSequences\\\\Here\"\"\"", "Mixed\tEscape\nSequences\\Here"},
        {NULL, NULL}};
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer,
                                       "Failed to create lexer for processed string: %s", input);
            return ASTHRA_TEST_FAIL;
        }
        ...
        // Verify string value
        if (!asthra_test_assert_string_eq(context, token.data.string.value, expected,
                                          "Processed string value mismatch for input '%s'",
                                          input)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
...
static AsthraTestResult test_lexer_multiline_backward_compatibility(AsthraTestContext *context) {
    const struct {
        const char *input;
        const char *expected_value;
    } test_cases[] = {// Regular single-line strings should still work
                      {"\"hello\"", "hello"},
                      {"\"world\"", "world"},
                      {"\"\"", ""},
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer,
                                       "Failed to create lexer for regular string: %s", input);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_lexer_multiline_string_errors(AsthraTestContext *context) {
    const char *error_cases[] = {// Unterminated raw multi-line strings
                                 "r\"\"\"hello", "r\"\"\"hello world", "r\"\"\"multiline\ncontent",
                                 // Unterminated processed multi-line strings
                                 "\"\"\"hello", "\"\"\"hello world", "\"\"\"multiline\ncontent",
                                 NULL};
    ...
    for (int i = 0; error_cases[i] != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for error case: %s",
                                       input);
            return ASTHRA_TEST_FAIL;
        }
...
int main(void) {
    ...
    const struct {
        ...
        {test_lexer_multiline_backward_compatibility,
         "test_lexer_multiline_backward_compatibility"},
