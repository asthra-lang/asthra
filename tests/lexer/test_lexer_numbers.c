...
AsthraTestResult test_lexer_integer_literals(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for integer: %s", input);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_float_literals(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for float: %s", input);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_numeric_edge_cases(AsthraTestContext *context) {
    ...
    if (token.type != TOKEN_FLOAT && token.type != TOKEN_INTEGER && token.type != TOKEN_ERROR) {
        asthra_test_assert_bool(context, false, "Hex float should parse as numeric literal or error");
        token_free(&token);
        lexer_destroy(lexer);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_lexer_numeric_formats(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for %s: %s", description, input)) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_numeric_bounds(AsthraTestContext *context) {
    const struct {
        ...
        {"2147483647", "Max 32-bit int", true},
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (should_succeed) {
            ...
            if (!asthra_test_assert_bool(context, is_numeric,
                                        "Expected numeric token for %s: %s", description, input)) {
                token_free(&token);
                lexer_destroy(lexer);
                return ASTHRA_TEST_FAIL;
            }
...
int main(void) {
    ...
    const struct {
        ...
        {test_lexer_integer_literals, "test_lexer_integer_literals"},
