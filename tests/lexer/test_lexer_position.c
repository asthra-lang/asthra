...
AsthraTestResult test_lexer_line_column_tracking(AsthraTestContext *context) {
    ...
    do {
        ...
        if (current_expectation < expectation_count) {
            ...
            // Check line number
            if (!asthra_test_assert_int_eq(context, token.location.line, expectations[current_expectation].expected_line,
            ...
            if (token.location.column > 0) {
                if (!asthra_test_assert_int_eq(context, token.location.column, expectations[current_expectation].expected_column,
                                              "Column number mismatch for token %zu", current_expectation)) {
                    token_free(&token);
                    lexer_destroy(lexer);
                    return ASTHRA_TEST_FAIL;
                }
...
AsthraTestResult test_lexer_source_location_accuracy(AsthraTestContext *context) {
    ...
    do {
        ...
        if (token.location.filename) {
            if (!asthra_test_assert_string_eq(context, token.location.filename, "test_source_location.ast",
        ...
        if (!asthra_test_assert_bool(context, token.location.line >= 1 && token.location.line <= 3,
                                    "Line number should be between 1 and 3, got %d", token.location.line)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_line_endings(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_int_eq(context, max_line_seen, expected_lines,
                                      "Expected %d lines for '%s', got %d", 
                                      expected_lines, description, max_line_seen)) {
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
        }
