...
AsthraTestResult test_token_data_integrity(AsthraTestContext *context) {
    ...
    if (diff < 0) diff = -diff;
    ASTHRA_TEST_ASSERT_TRUE(context, diff < 0.000001,
                           "Float token data should be preserved");
...
int main(void) {
    ...
    const struct {
        ...
        {test_token_create_and_destroy, "test_token_create_and_destroy"},
