...
AsthraTestResult test_lexer_compound_operators(AsthraTestContext *context) {
    ...
    const struct {
        ...
        {"> >=", TOKEN_GREATER_THAN,
         TOKEN_GREATER_EQUAL}, // Note: >= is greater-equal, not right-shift
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        ASTHRA_TEST_ASSERT_NOT_NULL(context, lexer,
                                    "Failed to create lexer for compound operator: %s", input);
        ...
        ASTHRA_TEST_ASSERT_TRUE(context, first_token.type == first_expected,
                                "Expected first token type %d, got %d", first_expected,
                                first_token.type);
        ...
        ASTHRA_TEST_ASSERT_TRUE(context, second_token.type == second_expected,
                                "Expected second token type %d, got %d", second_expected,
                                second_token.type);
...
AsthraTestResult test_lexer_operator_disambiguation(AsthraTestContext *context) {
    ...
    } test_cases[] = {       // Test grammar-compliant operator sequences
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        ASTHRA_TEST_ASSERT_NOT_NULL(context, lexer,
                                    "Failed to create lexer for disambiguation test: %s", input);
...
AsthraTestResult test_lexer_complex_operator_sequences(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        ASTHRA_TEST_ASSERT_NOT_NULL(context, lexer, "Failed to create lexer for test: %s",
                                    description);
        ...
        if (!asthra_test_assert_int_eq(context, token_count, expected_count,
                                       "Expected %d tokens for '%s', got %d", expected_count,
...
AsthraTestResult test_lexer_operator_precedence_tokens(AsthraTestContext *context) {
    ...
    do {
        ...
        if (token.type == TOKEN_PLUS || token.type == TOKEN_MINUS || token.type == TOKEN_MULTIPLY ||
            token.type == TOKEN_DIVIDE || token.type == TOKEN_MODULO) {
            ASTHRA_TEST_ASSERT_TRUE(context, operator_index < 5, "Too many operators found");
    ...
    if (!asthra_test_assert_int_eq(context, operator_index, 5, "Expected 5 operators, found %d",
                                   operator_index)) {
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    asthra_test_suite_add_test(suite, "test_lexer_compound_operators",
                               "Test compound operator recognition", test_lexer_compound_operators);
