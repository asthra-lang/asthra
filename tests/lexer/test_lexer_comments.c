...
AsthraTestResult test_lexer_single_line_comments(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for comment test: %s", input)) {
        ...
        do {
            ...
            if (token.type != TOKEN_COMMENT && token.type != TOKEN_WHITESPACE && token.type != TOKEN_NEWLINE) {
                break;
            }
            token_free(&token);
...
AsthraTestResult test_lexer_multi_line_comments(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for multi-line comment test")) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (should_be_valid) {
            ...
            bool is_valid = (token.type == TOKEN_COMMENT || token.type == TOKEN_EOF || token.type == TOKEN_LET);
...
AsthraTestResult test_lexer_nested_comments(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for nested comment test")) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_bool(context, is_valid,
                                    "Nested comment should be handled correctly, got token type %d",
                                    token.type)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
...
AsthraTestResult test_lexer_comment_edge_cases(AsthraTestContext *context) {
    const struct {
        ...
        {"// single /* multi-line start\n still in single-line", "Multi-line start in single-line", true},
        {NULL, NULL, false}
    };
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for test: %s", description)) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_comment_content_types(AsthraTestContext *context) {
    const char* test_cases[] = {
        "/* comment with numbers: 123 456 789 */",
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for comment content test")) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_comment_token_interaction(AsthraTestContext *context) {
    const struct {
        ...
        {"let/* comment */x", TOKEN_LET, TOKEN_IDENTIFIER},
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer, "Failed to create lexer for comment interaction test")) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        } while (second_token.type == TOKEN_COMMENT || second_token.type == TOKEN_WHITESPACE || second_token.type == TOKEN_NEWLINE);
