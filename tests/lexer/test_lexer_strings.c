...
static AsthraTestResult test_lexer_string_literals(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for string: %s", input);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_lexer_character_literals(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for character: %s", input);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_lexer_escape_sequences(AsthraTestContext *context) {
    ...
    for (int i = 0; test_cases[i].input != NULL; i++) {
        ...
        if (!lexer) {
            asthra_test_assert_pointer(context, lexer, "Failed to create lexer for escape sequence: %s", input);
            return ASTHRA_TEST_FAIL;
        }
...
int main(void) {
    ...
    const struct {
        ...
        {test_lexer_string_literals, "test_lexer_string_literals"},
