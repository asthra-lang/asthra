...
AsthraTestResult test_lexer_unicode_identifiers(AsthraTestContext *context) {
    const char *test_cases[] = {"café",      // Latin with accent
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for Unicode identifier")) {
        ...
        if (!asthra_test_assert_bool(
                context, is_valid,
                "Unicode identifier should be handled properly, got token type %d", token.type)) {
            token_free(&token);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_unicode_strings(AsthraTestContext *context) {
    const char *test_cases[] = {"\"Hello, 世界!\"",
                                "\"Здравствуй мир\"",
                                "\"مرحبا بالعالم\"",
                                "\"🌍🌎🌏\"",             // Emoji
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for Unicode string")) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_unicode_escapes(AsthraTestContext *context) {
    const char *test_cases[] = {"\"\\u0041\"",     // Unicode escape for 'A'
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for Unicode escape")) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        bool is_valid =
            (token.type == TOKEN_STRING || token.type == TOKEN_CHAR || token.type == TOKEN_ERROR);
        if (!asthra_test_assert_bool(context, is_valid,
...
AsthraTestResult test_lexer_mixed_ascii_unicode(AsthraTestContext *context) {
    const char *test_cases[] = {"let café = \"naïve\";",
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(context, lexer,
                                        "Failed to create lexer for mixed Unicode test")) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_bool(context, token_count > 1,
                                     "Should have processed multiple tokens, got %d",
                                     token_count)) {
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestResult test_lexer_unicode_normalization(AsthraTestContext *context) {
    ...
    } test_cases[] = {{"é", "e\u0301", "e with acute accent"},
                      {"ñ", "n\u0303", "n with tilde"},
    ...
    for (int i = 0; test_cases[i].composed != NULL; i++) {
        ...
        if (!asthra_test_assert_pointer(
                context, lexer1, "Failed to create lexer for composed Unicode: %s", description)) {
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (!asthra_test_assert_pointer(context, lexer2,
                                        "Failed to create lexer for decomposed Unicode: %s",
                                        description)) {
            token_free(&token1);
        ...
        if (!asthra_test_assert_bool(context, composed_valid && decomposed_valid,
                                     "Both composed and decomposed forms should be handled for %s",
                                     description)) {
            token_free(&token1);
            token_free(&token2);
            lexer_destroy(lexer1);
...
int main(void) {
    ...
    } test_cases[] = {{test_lexer_unicode_identifiers, "test_lexer_unicode_identifiers"},
