...
static void add_ai_annotation_to_mock_node(MockASTNode *node, const char *annotation) {
    ...
    node->ai_annotations =
        realloc(node->ai_annotations, sizeof(char *) * (node->ai_annotation_count + 1));
...
static void free_mock_node(MockASTNode *node) {
    ...
    if (node->ai_annotations)
        free(node->ai_annotations);
...
static bool test_ai_annotation_conflict_detection(void) {
    ...
    AIAnnotation annotation1 = {.type = AI_ANNOTATION_CONFIDENCE,
                                .value.confidence_score = 0.8f,
                                .target_node = NULL,
                                .is_processed = false};

    AIAnnotation annotation2 = {.type = AI_ANNOTATION_CONFIDENCE,
    ...
    AIAnnotation annotation3 = {.type = AI_ANNOTATION_TODO,
    ...
    AIAnnotation annotation4 = {.type = AI_ANNOTATION_HYPOTHESIS,
                                .value.text_value = "First hypothesis",
                                .target_node = NULL,
                                .is_processed = false};

    AIAnnotation annotation5 = {.type = AI_ANNOTATION_HYPOTHESIS,
...
static bool test_ai_annotation_metadata_extraction(void) {
    ...
    AIAnnotation security_annotation = {.type = AI_ANNOTATION_SECURITY,
    ...
    TEST_ASSERT_STR_EQ("security", metadata.category,
                       "Security annotation category should be 'security'");
    ...
    AIAnnotation optimize_annotation = {.type = AI_ANNOTATION_OPTIMIZE,
    ...
    TEST_ASSERT_STR_EQ("performance", optimize_metadata.category,
                       "Optimize annotation category should be 'performance'");
    TEST_ASSERT(optimize_metadata.priority != NULL, "Optimize annotation should have priority");
    TEST_ASSERT_STR_EQ("medium", optimize_metadata.priority,
                       "Optimize annotation priority should be 'medium'");

    // Test TODO annotation metadata
    AIAnnotation todo_annotation = {.type = AI_ANNOTATION_TODO,
    ...
    TEST_ASSERT_STR_EQ("maintainability", todo_metadata.category,
                       "TODO annotation category should be 'maintainability'");
    ...
    if (optimize_metadata.category)
        free(optimize_metadata.category);
    if (optimize_metadata.priority)
        free(optimize_metadata.priority);
    if (todo_metadata.category)
        free(todo_metadata.category);
    if (todo_metadata.priority)
        free(todo_metadata.priority);
...
static bool test_ai_annotation_parameter_validation(void) {
    ...
    AIAnnotationParam valid_confidence = {.type = ANNOTATION_PARAM_STRING,
    ...
    AIAnnotationParam valid_hypothesis = {.type = ANNOTATION_PARAM_STRING,
                                          .name = strdup("hypothesis"),
                                          .value.string_value =
                                              strdup("This function validates user input")};
    ...
    AIAnnotationParam invalid_hypothesis = {.type = ANNOTATION_PARAM_STRING,
                                            .name = strdup("hypothesis"),
