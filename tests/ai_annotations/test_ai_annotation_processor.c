...
static bool test_processor_creation_and_destruction(void) {
    ...
    TEST_ASSERT_FLOAT_EQ(0.5f, config->confidence_threshold, 0.01f,
...
static bool test_processor_configuration(void) {
    ...
    AIProcessorConfig *config = ai_annotation_processor_get_config(processor);
    TEST_ASSERT_FLOAT_EQ(0.8f, config->confidence_threshold, 0.01f,
                         "Confidence threshold should be updated");
    free_ai_processor_config(config);
    ...
    config = ai_annotation_processor_get_config(processor);
    TEST_ASSERT_FLOAT_EQ(0.7f, config->confidence_threshold, 0.01f,
...
static bool test_compilation_report_creation(void) {
    ...
    TEST_ASSERT_FLOAT_EQ(0.0f, report->overall_confidence, 0.01f,
    ...
    TEST_ASSERT_EQ(0, report->human_reviewed_functions,
                   "Initial human reviewed functions should be 0");
    ...
    TEST_ASSERT_FLOAT_EQ(0.5f, report->test_coverage_score, 0.01f,
                         "Default test coverage score should be 0.5");
    TEST_ASSERT_FLOAT_EQ(0.5f, report->security_score, 0.01f,
                         "Default security score should be 0.5");
    TEST_ASSERT_FLOAT_EQ(0.5f, report->maintainability_score, 0.01f,
                         "Default maintainability score should be 0.5");
...
static bool test_report_generation(void) {
    ...
    TEST_ASSERT(strstr(text_report, "Optimize loop performance") != NULL,
                "Report should contain suggestions");
    TEST_ASSERT(strstr(text_report, "Validate user input") != NULL,
                "Report should contain security concerns");
    ...
    TEST_ASSERT(strstr(json_report, "\"file_path\": \"test_file.asthra\"") != NULL,
                "JSON should contain file path");
    TEST_ASSERT(strstr(json_report, "\"overall_confidence\": 0.85") != NULL,
                "JSON should contain confidence");
    TEST_ASSERT(strstr(json_report, "\"improvement_suggestions\"") != NULL,
                "JSON should contain suggestions section");
    TEST_ASSERT(strstr(json_report, "\"quality_scores\"") != NULL,
                "JSON should contain quality scores");

    // Test quality score calculation
    float quality_score = ai_annotation_calculate_quality_score(report);
    TEST_ASSERT(quality_score > 0.0f && quality_score <= 1.0f,
...
static bool test_learning_data_generation(void) {
    ...
    // Process the mock program
    AICompilationReport *report =
        ai_annotation_process_program(processor, (ASTNode *)&mock_program1);
    TEST_ASSERT_NOT_NULL(report, "Compilation report should be created");

    // Check if learning data was generated (even if empty)
    // Note: Learning data might be NULL if no annotations are found, which is expected for mock
    // data
    if (report->learning_data) {
        ...
        TEST_ASSERT(report->learning_data->impact_score >= 0.0f &&
                        report->learning_data->impact_score <= 1.0f,
    ...
    AICompilationReport *report2 =
        ai_annotation_process_program(processor, (ASTNode *)&mock_program2);
...
static bool test_memory_management(void) {
    ...
    if (learning_data->category)
        free(learning_data->category);
    if (learning_data->priority)
        free(learning_data->priority);
    if (learning_data->reasoning)
        free(learning_data->reasoning);
    for (size_t i = 0; i < learning_data->tag_count; i++) {
        if (learning_data->tags[i])
            free(learning_data->tags[i]);
    }
    if (learning_data->tags)
        free(learning_data->tags);
    free(learning_data);
...
static void print_test_summary(void) {
    ...
    printf("Assertions:      %d checked, %d failed\n", test_stats.assertions_checked,
           test_stats.assertions_failed);
    printf("Pass rate:       %.1f%%\n",
           test_stats.tests_run > 0 ? (100.0 * test_stats.tests_passed / test_stats.tests_run)
                                    : 0.0);
    printf("========================\n");
}
