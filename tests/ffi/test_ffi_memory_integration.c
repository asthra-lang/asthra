...
void test_pqc_key_management_scenario(void) {
    ...
    for (int i = 0; i < 4; i++) {
        key_slices[i] = Asthra_slice_new(sizeof(uint8_t), key_sizes[i], key_sizes[i], 
...
void test_ffi_string_processing_scenario(void) {
    ...
    base_messages[0] = Asthra_string_from_cstr("Processing operation: ", ASTHRA_OWNERSHIP_TRANSFER_FULL);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        AsthraFFIString op_str = Asthra_string_from_cstr(operations[i], ASTHRA_OWNERSHIP_TRANSFER_FULL);
        AsthraFFIString status_str = Asthra_string_from_cstr(statuses[i], ASTHRA_OWNERSHIP_TRANSFER_FULL);
        ...
        AsthraFFIResult interp_result = Asthra_string_interpolate(
            "Operation[{}]: {} - Status: {} - Time: {:.2f}s", args);
...
void test_error_handling_chain_scenario(void) {
    ...
    if (Asthra_result_is_ok(subslice_result)) {
        AsthraFFISliceHeader *subslice = (AsthraFFISliceHeader*)Asthra_result_unwrap_ok(subslice_result);
...
void test_mixed_allocation_scenario(void) {
    ...
    strings[0] = Asthra_string_from_cstr("Mixed allocation test string 1", ASTHRA_OWNERSHIP_TRANSFER_FULL);
    strings[1] = Asthra_string_from_cstr("Mixed allocation test string 2", ASTHRA_OWNERSHIP_TRANSFER_FULL);
