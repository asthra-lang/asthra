...
static bool test_task_creation(void) {
    ...
    bool result =
        ffi_generate_task_creation(g_generator, "test_function", arg_regs, 2, ASTHRA_REG_RAX);
...
static bool test_atomic_operations(void) {
    ...
    bool result = ffi_generate_atomic_cas(g_generator, ASTHRA_REG_RDI, ASTHRA_REG_RSI,
...
const size_t ffi_security_concurrency_test_count =
    sizeof(ffi_security_concurrency_tests) / sizeof(ffi_security_concurrency_tests[0]);
