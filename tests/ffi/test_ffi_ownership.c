...
typedef enum { FFI_OWNERSHIP_BORROW, FFI_OWNERSHIP_MOVE, FFI_OWNERSHIP_COPY } FFIOwnershipMode;
...
AsthraTestResult test_ffi_ownership_borrow(AsthraV12TestContext *ctx) {
    ...
    FFICallContext borrow_ctx = {.ownership_mode = FFI_OWNERSHIP_BORROW,
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, length == strlen(test_string),
                            "Borrowed string length should be %zu, got %zu", strlen(test_string),
                            length)) {
...
AsthraTestResult test_ffi_ownership_move(AsthraV12TestContext *ctx) {
    ...
    FFICallContext move_ctx = {.ownership_mode = FFI_OWNERSHIP_MOVE,
                               .memory_managed = true,
                               .data_size = 0,
...
AsthraTestResult test_ffi_ownership_copy(AsthraV12TestContext *ctx) {
    ...
    FFICallContext copy_ctx = {.ownership_mode = FFI_OWNERSHIP_COPY,
                               .memory_managed = true,
                               .data_size = 0,
                               .cleanup_func = test_c_cleanup};
