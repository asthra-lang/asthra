...
void test_memory_statistics(void) {
    ...
    printf("Initial stats: %zu allocations, %zu bytes\n", 
           initial_stats.total_allocations, initial_stats.current_bytes);
    ...
    printf("Updated stats: %zu total allocations, %zu current allocations, %zu bytes, %zu slices\n", 
           stats.total_allocations, stats.current_allocations, stats.current_bytes, stats.slice_count);
    ...
    TEST_ASSERT(final_stats.current_bytes < stats.current_bytes, 
                "Statistics track freed bytes");
    
    printf("Final stats: %zu current allocations, %zu bytes\n", 
           final_stats.current_allocations, final_stats.current_bytes);
}
...
void test_pointer_validation(void) {
    ...
    AsthraFFISliceHeader valid_slice = Asthra_slice_new(sizeof(double), 50, 100, ASTHRA_ZONE_HINT_MANUAL);
...
void test_memory_state_dump(void) {
    ...
    AsthraFFISliceHeader dump_slice = Asthra_slice_new(sizeof(float), 25, 50, ASTHRA_ZONE_HINT_SECURE);
...
// Test registration
static test_case_t diagnostic_tests[] = {
