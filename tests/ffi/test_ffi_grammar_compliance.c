...
static AsthraTestResult test_valid_single_ffi_annotations(AsthraTestContext *context) {
    const char *valid_annotations[] = {"#[transfer_full]", "#[transfer_none]", "#[borrowed]"};
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, parsed, "Valid SafeFFIAnnotation should parse: %s",
                                valid_annotations[i])) {
...
static AsthraTestResult test_invalid_multiple_ffi_annotations(AsthraTestContext *context) {
    ...
    const char *dangerous_patterns[] = {
        "#[transfer_full] #[transfer_none]", "#[transfer_full] #[borrowed]",
        "#[transfer_none] #[borrowed]",      "#[transfer_full] #[transfer_none] #[borrowed]",
        "#[borrowed] #[transfer_full]",      "#[transfer_none] #[transfer_full]"};
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, !parsed,
                                "Dangerous multiple FFI annotations MUST be rejected: %s",
 ...
 * Tests the grammar rule: FunctionDecl <- 'fn' SimpleIdent '(' ParamList? ')' '->'
 * SafeFFIAnnotation? Type Block
 */
static AsthraTestResult test_valid_function_declarations(AsthraTestContext *context) {
    const char *valid_functions[] = {
        ...
        "fn complex_function(#[transfer_full] ptr: *mut u8, #[borrowed] data: *const u8) -> "
        "#[transfer_none] *const i32 { }"};
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, parsed,
                                "Valid function with SafeFFIAnnotation should parse: %s",
                                valid_functions[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
static AsthraTestResult test_invalid_function_declarations(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(
                context, !parsed,
                "Dangerous function with multiple FFI annotations MUST be rejected: %s",
 ...
 * Tests the grammar rule: ExternDecl <- 'extern' STRING? 'fn' SimpleIdent '(' ExternParamList? ')'
 * '->' SafeFFIAnnotation? Type ';'
 */
static AsthraTestResult test_valid_extern_declarations(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, parsed,
                                "Valid extern with SafeFFIAnnotation should parse: %s",
                                valid_externs[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
static AsthraTestResult test_invalid_extern_declarations(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(
                context, !parsed,
                "Dangerous extern with multiple FFI annotations MUST be rejected: %s",
                dangerous_externs[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
static AsthraTestResult test_parameter_grammar_compliance(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < valid_count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, parsed, "Valid parameter pattern should parse: %s",
                                valid_params[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
    ...
    for (size_t i = 0; i < invalid_count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, !parsed, "Invalid parameter pattern MUST be rejected: %s",
                                invalid_params[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
static AsthraTestResult test_backward_compatibility(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(context, parsed,
                                "Existing valid FFI code should continue to work: %s",
                                existing_valid_code[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
...
void register_ffi_grammar_compliance_tests(void) {
    asthra_test_register_suite("FFI Grammar Compliance", setup_grammar_tests,
                               teardown_grammar_tests);

    // Core grammar compliance tests
    asthra_test_register_test("FFI Grammar Compliance", "Valid Single FFI Annotations",
                              test_valid_single_ffi_annotations);
    asthra_test_register_test("FFI Grammar Compliance", "Invalid Multiple FFI Annotations",
                              test_invalid_multiple_ffi_annotations);

    // Function and extern declaration tests
    asthra_test_register_test("FFI Grammar Compliance", "Valid Function Declarations",
                              test_valid_function_declarations);
    asthra_test_register_test("FFI Grammar Compliance", "Invalid Function Declarations",
                              test_invalid_function_declarations);
    asthra_test_register_test("FFI Grammar Compliance", "Valid Extern Declarations",
                              test_valid_extern_declarations);
    asthra_test_register_test("FFI Grammar Compliance", "Invalid Extern Declarations",
                              test_invalid_extern_declarations);

    // Parameter and AI safety tests
    asthra_test_register_test("FFI Grammar Compliance", "Parameter Grammar Compliance",
                              test_parameter_grammar_compliance);
    asthra_test_register_test("FFI Grammar Compliance", "AI Generation Safety",
                              test_ai_generation_safety);

    // Compatibility test
    asthra_test_register_test("FFI Grammar Compliance", "Backward Compatibility",
