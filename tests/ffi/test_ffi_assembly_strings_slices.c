...
static bool test_deterministic_string_operations(void) {
    ...
    bool result = ffi_generate_deterministic_string_op(g_generator, STRING_OP_COMPARE, operand_regs,
                                                       2, result_reg);
    TEST_ASSERT(result, "Deterministic string comparison generation failed");

    result = ffi_generate_deterministic_string_op(g_generator, STRING_OP_EQUALS, operand_regs, 2,
                                                  result_reg);
...
static bool test_slice_creation(void) {
    ...
    bool result =
        ffi_generate_slice_creation(g_generator, data_reg, len_reg, result_reg, is_mutable);
