...
AsthraTestResult test_ffi_enhanced_function_calls(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, result == 42,
                           "C function call should return 42, got %d", result)) {
...
AsthraTestResult test_ffi_complex_data_structures(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, array_sum == 15,
                           "Array sum should be 15, got %d", array_sum)) {
        return ASTHRA_TEST_FAIL;
    }
