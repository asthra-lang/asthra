...
void test_ownership_registration(void) {
    ...
    AsthraFFIResult reg_result =
        Asthra_ownership_register(test_ptr, 1024, ASTHRA_OWNERSHIP_TRANSFER_FULL, free);
...
void test_ownership_transfer(void) {
    ...
    AsthraFFIResult reg_result =
        Asthra_ownership_register(ptr1, 512, ASTHRA_OWNERSHIP_TRANSFER_FULL, free);
    TEST_ASSERT(Asthra_result_is_ok(reg_result), "Full ownership registration succeeds");

    // Transfer to no ownership
    AsthraFFIResult transfer_result =
        Asthra_ownership_transfer(ptr1, ASTHRA_OWNERSHIP_TRANSFER_NONE);
...
void test_ownership_modes(void) {
    ...
    AsthraFFIResult borrowed_reg =
        Asthra_ownership_register(static_buffer, 256, ASTHRA_OWNERSHIP_TRANSFER_NONE, NULL);
    ...
    AsthraFFIResult owned_reg =
        Asthra_ownership_register(owned_ptr, 128, ASTHRA_OWNERSHIP_TRANSFER_FULL, free);
    ...
    AsthraFFIResult copied_reg =
        Asthra_ownership_register(copied_ptr, 64, ASTHRA_OWNERSHIP_TRANSFER_COPY, free);
...
void test_ownership_edge_cases(void) {
    ...
    AsthraFFIResult null_reg =
        Asthra_ownership_register(NULL, 0, ASTHRA_OWNERSHIP_TRANSFER_NONE, NULL);
    ...
    AsthraFFIResult first_reg =
        Asthra_ownership_register(double_ptr, 100, ASTHRA_OWNERSHIP_TRANSFER_FULL, free);
    TEST_ASSERT(Asthra_result_is_ok(first_reg), "First registration succeeds");

    AsthraFFIResult second_reg =
        Asthra_ownership_register(double_ptr, 100, ASTHRA_OWNERSHIP_TRANSFER_FULL, free);
...
void test_ownership_cleanup(void) {
    ...
    AsthraFFIResult cleanup_reg = Asthra_ownership_register(
        auto_cleanup_ptr, 200, ASTHRA_OWNERSHIP_TRANSFER_FULL, custom_cleanup);
...
void test_ownership_statistics(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        AsthraFFIResult reg_result =
            Asthra_ownership_register(ptrs[i], 128 * (i + 1), ASTHRA_OWNERSHIP_TRANSFER_FULL, free);
...
// Test registration
static test_case_t ownership_tests[] = {{"Ownership Registration", test_ownership_registration},
