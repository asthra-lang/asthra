...
static bool test_generation_statistics(void) {
    ...
    // Get statistics
    ffi_get_generation_statistics(g_generator, &ffi_calls, &pattern_matches,
    ...
    TEST_ASSERT(ffi_calls > 0 || pattern_matches > 0 || string_ops > 0 || 
                slice_ops > 0 || security_ops > 0 || spawn_stmts > 0,
...
static bool test_performance_profiling(void) {
    ...
    printf("  Profiling data: %zu instructions, %zu register spills\n",
           data.instruction_count, data.register_spills);
...
const size_t ffi_optimization_test_count = sizeof(ffi_optimization_tests) / sizeof(ffi_optimization_tests[0]);
