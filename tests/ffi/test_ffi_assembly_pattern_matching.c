...
static bool test_result_construction(void) {
    ...
    bool result =
        ffi_generate_result_construction(g_generator, true, ASTHRA_REG_RDI, NULL, ASTHRA_REG_RAX);
    TEST_ASSERT(result, "Result.Ok construction failed");

    // Test Err construction
    result =
        ffi_generate_result_construction(g_generator, false, ASTHRA_REG_RDI, NULL, ASTHRA_REG_RAX);
...
test_function_t ffi_pattern_matching_tests[] = {
    test_pattern_match_generation,    test_result_pattern_match,  test_result_construction,
    test_exhaustive_pattern_matching, test_pattern_destructuring,
};

const size_t ffi_pattern_matching_test_count =
    sizeof(ffi_pattern_matching_tests) / sizeof(ffi_pattern_matching_tests[0]);
