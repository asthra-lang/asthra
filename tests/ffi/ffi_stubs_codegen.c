...
bool code_generator_emit_function_call(CodeGenerator *generator, const char *func_name, int arg_count, void *args) {
...
EnhancedCodegenContext* enhanced_codegen_create_context(void) {
    ...
    ctx->ffi_generator = ffi_assembly_generator_create(TARGET_ARCH_X86_64, CALLING_CONV_SYSTEM_V_AMD64);
...
bool enhanced_codegen_generate_function_call(EnhancedCodegenContext *ctx, const char *func_name) {
    if (!ctx || !ctx->initialized || !func_name) {
        if (ctx) atomic_fetch_add(&ctx->failed_generations, 1);
        return false;
    }
    ...
    if (success) {
        ...
        int written = snprintf(ctx->output_buffer + ctx->output_buffer_pos,
                              sizeof(ctx->output_buffer) - ctx->output_buffer_pos,
                              "call %s\n", func_name);
        if (written > 0) {
...
CodegenStats enhanced_codegen_get_stats(EnhancedCodegenContext *ctx) {
    ...
    if (ctx && ctx->initialized) {
        ...
        if (total_operations > 0) {
            stats.success_rate = (double)(total_operations - stats.failed_operations) / total_operations * 100.0;
        }
    }
