...
static bool compile_and_generate_ffi_code(const char *source, char *assembly_output,
                                          size_t output_size) {
    // Create parser
...
static AsthraTestResult test_option_parameter_ffi(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool_eq(context,
                                    strstr(assembly, "option_none") != NULL ||
                                        strstr(assembly, "xor") != NULL || strlen(assembly) > 10,
                                    true, "Assembly should contain Option None handling")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestSuite *create_option_ffi_test_suite(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create("Option FFI Tests", "Option<T> FFI marshaling testing");
    ...
    asthra_test_suite_add_test(suite, "test_option_parameter_ffi",
                               "Option parameter FFI marshaling", test_option_parameter_ffi);

    asthra_test_suite_add_test(suite, "test_option_return_ffi",
                               "Option return value FFI marshaling", test_option_return_ffi);

    asthra_test_suite_add_test(suite, "test_nested_option_ffi", "Nested Option FFI marshaling",
                               test_nested_option_ffi);

    asthra_test_suite_add_test(suite, "test_option_copy_ffi", "Option with copy semantics FFI",
