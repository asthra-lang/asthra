...
AsthraTestResult test_ffi_c17_compliance_types(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(i8) == 1,
                           "int8_t should be 1 byte")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(u8) == 1,
                           "uint8_t should be 1 byte")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(i16) == 2,
                           "int16_t should be 2 bytes")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(u16) == 2,
                           "uint16_t should be 2 bytes")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(i32) == 4,
                           "int32_t should be 4 bytes")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(u32) == 4,
                           "uint32_t should be 4 bytes")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(i64) == 8,
                           "int64_t should be 8 bytes")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, sizeof(u64) == 8,
                           "uint64_t should be 8 bytes")) {
        return ASTHRA_TEST_FAIL;
    }
    
    // Test value ranges
    if (!ASTHRA_TEST_ASSERT(&ctx->base, i8 == -128,
                           "int8_t minimum value should be -128")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(&ctx->base, u8 == 255,
                           "uint8_t maximum value should be 255")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_ffi_c17_compliance_functions(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < vla_size; i++) {
        if (!ASTHRA_TEST_ASSERT(&ctx->base, vla[i] == i * i,
                               "VLA element %d should be %d, got %d", i, i * i, vla[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, compound_array[i] == expected,
                               "Compound literal element %d should be %d, got %d", 
                               i, expected, compound_array[i])) {
            return ASTHRA_TEST_FAIL;
        }
    }
