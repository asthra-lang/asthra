...
static AsthraTestResult test_ffi_generator_creation(AsthraTestContext *context) {
    ...
    FFIAssemblyGenerator *generator = ffi_assembly_generator_create(
        TARGET_ARCH_X86_64, 
        CALLING_CONV_SYSTEM_V_AMD64
    );
    
    if (!ASTHRA_TEST_ASSERT_POINTER(context, generator, "Failed to create FFI assembly generator")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT_POINTER(context, generator->base_generator, "Base generator not initialized")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    // Test configuration defaults
    if (!ASTHRA_TEST_ASSERT_BOOL(context, generator->config.enable_bounds_checking, "Bounds checking should be enabled by default")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT_BOOL(context, generator->config.enable_security_features, "Security features should be enabled by default")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT_BOOL(context, generator->config.enable_concurrency, "Concurrency should be enabled by default")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    // Test runtime function names
    if (!ASTHRA_TEST_ASSERT_POINTER(context, generator->runtime_functions.gc_alloc, "GC alloc function name not set")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT_POINTER(context, generator->runtime_functions.string_concat, "String concat function name not set")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT_POINTER(context, generator->runtime_functions.spawn_task, "Spawn task function name not set")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_ffi_generator_configuration(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT_BOOL(context, !generator->config.enable_bounds_checking, "Bounds checking configuration failed")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT_BOOL(context, generator->config.pic_mode, "PIC mode configuration failed")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_int_eq(context, generator->config.max_variadic_args, 16, "Variadic args configuration failed")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_ffi_generator_statistics(AsthraTestContext *context) {
    ...
    ffi_get_generation_statistics(generator, &ffi_calls, &pattern_matches,
    ...
    if (!asthra_test_assert_int_eq(context, pattern_matches, 0, "Initial pattern matches should be 0")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_int_eq(context, string_ops, 0, "Initial string operations should be 0")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_int_eq(context, security_ops, 0, "Initial security operations should be 0")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_int_eq(context, spawn_stmts, 0, "Initial spawn statements should be 0")) {
        ffi_assembly_generator_destroy(generator);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    asthra_test_suite_add_test(suite, "ffi_generator_creation", "Test FFI generator creation", test_ffi_generator_creation);
    asthra_test_suite_add_test(suite, "ffi_generator_configuration", "Test FFI generator configuration", test_ffi_generator_configuration);
    asthra_test_suite_add_test(suite, "ffi_generator_statistics", "Test FFI generator statistics", test_ffi_generator_statistics);
    asthra_test_suite_add_test(suite, "string_operations", "Test string operations", test_string_operations);
    asthra_test_suite_add_test(suite, "slice_operations", "Test slice operations", test_slice_operations);
    asthra_test_suite_add_test(suite, "security_operations", "Test security operations", test_security_operations);
    asthra_test_suite_add_test(suite, "concurrency_operations", "Test concurrency operations", test_concurrency_operations);
    asthra_test_suite_add_test(suite, "assembly_validation", "Test assembly validation", test_assembly_validation);
    asthra_test_suite_add_test(suite, "nasm_output", "Test NASM output generation", test_nasm_output);
