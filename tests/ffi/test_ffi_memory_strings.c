...
void test_string_concatenation(void) {
    ...
    TEST_ASSERT(strncmp(concat_str->data, "Hello World!", 12) == 0,
                "Concatenated string has correct content");
...
void test_string_edge_cases(void) {
    ...
    AsthraFFIString special_str =
        Asthra_string_from_cstr("Special: \n\t\r\0embedded", ASTHRA_OWNERSHIP_TRANSFER_FULL);
    TEST_ASSERT(special_str.data != NULL, "Special character string creation succeeds");

    // Test UTF-8 string (if supported)
    AsthraFFIString utf8_str =
        Asthra_string_from_cstr("UTF-8: ñáéíóú", ASTHRA_OWNERSHIP_TRANSFER_FULL);
...
void test_string_ownership(void) {
    ...
    AsthraFFIString borrowed_str =
        Asthra_string_from_cstr(static_str, ASTHRA_OWNERSHIP_TRANSFER_NONE);
    ...
    AsthraFFIString owned_str =
        Asthra_string_from_cstr(dynamic_str, ASTHRA_OWNERSHIP_TRANSFER_FULL);
    ...
    AsthraFFIString copied_str =
        Asthra_string_from_cstr("Copy Test", ASTHRA_OWNERSHIP_TRANSFER_FULL);
