    ...
    for (size_t i = 0; i < sizeof(variants) / sizeof(variants[0]); i++) {
        ...
        switch (variants[i].type) {
            case ASTHRA_VARIANT_BOOL:
                TEST_ASSERT(retrieved->value.bool_val == variants[i].value.bool_val, "Bool value preserved");
                break;
            case ASTHRA_VARIANT_I32:
                TEST_ASSERT(retrieved->value.i32_val == variants[i].value.i32_val, "I32 value preserved");
                break;
            case ASTHRA_VARIANT_F64:
                TEST_ASSERT(retrieved->value.f64_val == variants[i].value.f64_val, "F64 value preserved");
                break;
...
// Test registration
static test_case_t security_tests[] = {
    {"Secure Allocation", test_secure_allocation},
