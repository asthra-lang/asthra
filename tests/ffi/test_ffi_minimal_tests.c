...
bool test_ffi_generator_creation(void) {
    ...
    TEST_ASSERT(generator->target_arch == TARGET_ARCH_X86_64,
                "Target architecture not set correctly");
    TEST_ASSERT(generator->calling_conv == CALLING_CONV_SYSTEM_V_AMD64,
                "Calling convention not set correctly");

    // Test configuration defaults
    TEST_ASSERT(generator->config.enable_bounds_checking == true,
                "Bounds checking should be enabled by default");
    TEST_ASSERT(generator->config.enable_security_features == true,
                "Security features should be enabled by default");
    TEST_ASSERT(generator->config.enable_concurrency == true,
    ...
    TEST_ASSERT(generator->runtime_functions.string_concat != NULL,
                "String concat function name not set");
    TEST_ASSERT(generator->runtime_functions.spawn_task != NULL,
...
bool test_ffi_generator_configuration(void) {
    ...
    TEST_ASSERT(generator->config.enable_bounds_checking == false,
...
bool test_slice_operations(void) {
    ...
    result = minimal_generate_slice_bounds_check(generator, ASTHRA_REG_RDI, ASTHRA_REG_RSI,
                                                 "bounds_error");
    TEST_ASSERT(result == true, "Slice bounds check generation failed");

    // Test slice to FFI conversion
    result =
        minimal_generate_slice_to_ffi(generator, ASTHRA_REG_RDI, ASTHRA_REG_RSI, ASTHRA_REG_RDX);
...
bool test_concurrency_operations(void) {
    ...
    bool result =
        minimal_generate_task_creation(generator, "test_function", arg_regs, 2, ASTHRA_REG_RAX);
...
bool test_assembly_validation(void) {
    ...
    minimal_generate_string_concatenation(generator, ASTHRA_REG_RDI, ASTHRA_REG_RSI,
                                          ASTHRA_REG_RAX);
...
bool test_generation_statistics(void) {
    ...
    minimal_get_generation_statistics(generator, &ffi_calls, &pattern_matches, &string_ops,
