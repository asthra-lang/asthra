...
void run_enhanced_ffi_tests(void) {
    ...
    if (parser) {
        EnhancedASTNode *ast =
            enhanced_parser_parse_program(parser, "fn main(void) { return 42; }");
        if (ast) {
    ...
    printf(
        "   FFI Memory: Allocated %zu bytes, Freed %zu bytes, Current %zu bytes, Peak %zu bytes\n",
    ...
    printf("   Runtime Memory: Total Allocations: %llu, Current Usage: %zu, Peak Usage: %zu\n",
           (unsigned long long)runtime_stats.total_allocations, runtime_stats.current_memory_usage,
           runtime_stats.peak_memory_usage);
