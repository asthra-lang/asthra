...
static test_suite_info_t test_suites[] = {
    {
        "Allocation", "Basic memory allocation, reallocation, and zone management",
        NULL // Would be set to actual function if linking individual test files
    },
    {"Slices", "Slice creation, bounds checking, element access, and subslicing", NULL},
    {"Strings", "String creation, concatenation, interpolation, and conversions", NULL},
    {"Patterns", "Pattern matching, error handling, and result types", NULL},
    {"Ownership", "Ownership tracking, transfer, and cleanup", NULL},
    {"Security", "Secure memory operations and variant arrays", NULL},
    {"Diagnostics", "Memory statistics, validation, and diagnostic functionality", NULL},
    {"Integration", "Real-world usage patterns and complex scenarios", NULL}};
...
void print_summary(int total_suites, int passed_suites, int failed_suites, time_t start_time,
                   time_t end_time) {
    printf("\n");
    printf("╔════════════════════════════════════════════════════════════╗\n");
...
void run_basic_functionality_tests(void) {
    ...
    // Test basic result
    int value = 42;
    AsthraFFIResult test_result =
        Asthra_result_ok(&value, sizeof(int), 0, ASTHRA_OWNERSHIP_TRANSFER_NONE);
...
void run_error_condition_tests(void) {
    ...
    AsthraFFISliceHeader slice = Asthra_slice_from_raw_parts(dummy_array, 3, sizeof(int), false,
...
static test_case_t master_tests[] = {{"Basic Functionality", run_basic_functionality_tests},
...
int main(int argc, char *argv[]) {
    ...
    if (run_master_only || run_all) {
        ...
        int master_failed =
            run_test_suite("Master", master_tests, sizeof(master_tests) / sizeof(master_tests[0]));
        total_suites++;
        if (master_failed == 0) {
            passed_suites++;
