...
static void test_result_creation(void) {
    ...
    AsthraFFIResult ok_result =
        Asthra_result_ok(&success_value, sizeof(int), 0, ASTHRA_OWNERSHIP_TRANSFER_NONE);
...
static void test_result_pattern_matching(void) {
    ...
    int test_value = 123;
    AsthraFFIResult ok_result =
        Asthra_result_ok(&test_value, sizeof(int), 0, ASTHRA_OWNERSHIP_TRANSFER_NONE);
    ...
    if (Asthra_result_is_ok(err_result)) {
        ...
        TEST_ASSERT(strcmp(msg, "Internal error") == 0,
                    "Pattern matched error has correct message");
        handled_err = true;
    }
...
static void test_error_contexts(void) {
    ...
    AsthraFFIResult mem_error =
        Asthra_result_err(ENOMEM, "Out of memory", "allocation_function", NULL);
    AsthraFFIResult bounds_error =
        Asthra_result_err(ERANGE, "Index out of bounds", "slice_access", NULL);
    ...
    TEST_ASSERT(Asthra_result_get_error_code(bounds_error) == ERANGE,
                "Bounds error has correct code");
...
static void test_result_chaining(void) {
    ...
    TEST_ASSERT(Asthra_result_is_err(invalid_get),
                "Invalid operation in chain fails appropriately");
...
static void test_error_recovery(void) {
    ...
    AsthraFFISliceHeader slice = Asthra_slice_from_raw_parts(test_array, 3, sizeof(int), false,
                                                             ASTHRA_OWNERSHIP_TRANSFER_NONE);
...
// Test registration
static test_case_t pattern_tests[] = {{"Result Creation", test_result_creation},
