...
bool assert_token_equals(AsthraTestContext* context, const Token* token, const TokenExpectation* expected) {
    ...
    if (expected->line > 0 && token->location.line != expected->line) {
        asthra_test_assert_int_eq(context, token->location.line, expected->line,
                                 "Token line mismatch: expected %zu, got %zu",
                                 expected->line, token->location.line);
        return false;
    }
    ...
    if (expected->column > 0 && token->location.column != expected->column) {
        asthra_test_assert_int_eq(context, token->location.column, expected->column,
                                 "Token column mismatch: expected %zu, got %zu",
                                 expected->column, token->location.column);
        return false;
    }
...
bool assert_ast_structure(AsthraTestContext* context, const ASTNode* node, const AstExpectation* expected) {
    ...
    if (node->type != expected->type) {
        asthra_test_assert_int_eq(context, node->type, expected->type,
                                 ...
                                 expected->type, node->type);
        return false;
    }

    // Check child count if specified (this would need to be implemented based on actual AST structure)
