...
ASTHRA_TEST_DEFINE(test_generic_ranges, ASTHRA_TEST_SEVERITY_MEDIUM) {
    // Test integer range
    int test_value = 50;
    if (!ASTHRA_TEST_ASSERT_RANGE(context, test_value, 1, 100,
                                  "Integer should be in range [1, 100]")) {
        ASTHRA_TEST_FAIL(context, "Integer range assertion failed");
    }

    // Test size_t range
    size_t test_size = 512;
    if (!ASTHRA_TEST_ASSERT_RANGE(context, test_size, (size_t)256, (size_t)1024,
                                  "Size should be in range [256, 1024]")) {
        ASTHRA_TEST_FAIL(context, "Size range assertion failed");
    }

    // Test double range
    double test_double = 3.14159;
    if (!ASTHRA_TEST_ASSERT_RANGE(context, test_double, 3.0, 4.0,
                                  "Double should be in range [3.0, 4.0]")) {
...
ASTHRA_TEST_DEFINE(test_atomic_statistics, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    bool cas_result =
        asthra_test_compare_and_swap_stat(&stats->max_duration_ns, &expected, new_value);
...
ASTHRA_TEST_DEFINE(test_designated_initializers, ASTHRA_TEST_SEVERITY_MEDIUM) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, (bool)default_config.parallel_execution, (bool)false,
                               "Default should not be parallel")) {
        ASTHRA_TEST_FAIL(context, "Default parallel execution setting incorrect");
    }

    if (!ASTHRA_TEST_ASSERT_EQ(context, (int)default_config.default_timeout_ns, (int)30000000000ULL,
    ...
    AsthraTestSuiteConfig custom_config = {.name = "Custom Test Suite",
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, (bool)custom_config.parallel_execution, (bool)true,
                               "Custom config should be parallel")) {
        ASTHRA_TEST_FAIL(context, "Custom parallel execution setting incorrect");
    }

    if (!ASTHRA_TEST_ASSERT_EQ(context, (bool)custom_config.stop_on_failure, (bool)true,
                               "Custom config should stop on failure")) {
        ASTHRA_TEST_FAIL(context, "Custom stop on failure setting incorrect");
    }

    if (!asthra_test_assert_int_eq(context, (int)custom_config.max_parallel_tests, 8,
                                   "Custom config should allow 8 parallel tests")) {
...
ASTHRA_TEST_DEFINE(test_runtime_integration, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    AsthraGCConfig gc_config = {.initial_heap_size = 1024 * 1024,
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, actual_len, expected_len,
                               "Concatenated string length should be correct")) {
        asthra_string_free(str1);
        asthra_string_free(str2);
...
ASTHRA_TEST_DEFINE(test_skip_demonstration, ASTHRA_TEST_SEVERITY_LOW) {
    ...
    ASTHRA_TEST_SKIP(context,
                     "This test is intentionally skipped to demonstrate skip functionality");
}
...
int main(void) {
    ...
    AsthraTestSuiteConfig config = {.name = "C17 Framework Demo Suite",
                                    .description =
                                        "Demonstration of C17 modernized test framework features",
    ...
    AsthraTestFunction tests[] = {test_generic_assertions,      test_generic_equality,
                                  test_generic_ranges,          test_atomic_statistics,
                                  test_designated_initializers, test_runtime_integration,
                                  test_skip_demonstration,      test_failure_demonstration};

    AsthraTestMetadata metadata[] = {
        test_generic_assertions_metadata,      test_generic_equality_metadata,
        test_generic_ranges_metadata,          test_atomic_statistics_metadata,
        test_designated_initializers_metadata, test_runtime_integration_metadata,
        test_skip_demonstration_metadata,      test_failure_demonstration_metadata};
