...
AsthraTestResult test_benchmark_timer_accuracy(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(context, timer.is_valid,
                                "Timer should be valid after start")) {
...
AsthraTestResult test_benchmark_configuration(AsthraTestContext *context) {
    ...
    AsthraBenchmarkConfig custom_config = asthra_benchmark_config_create(
        "custom_test", 5000, ASTHRA_BENCHMARK_MODE_SINGLE_THREADED);
...
AsthraTestResult test_simple_benchmark_execution(AsthraTestContext *context) {
    ...
    AsthraBenchmarkConfig config = asthra_benchmark_config_create(
        "cpu_test", 100, ASTHRA_BENCHMARK_MODE_SINGLE_THREADED);

    AsthraBenchmarkDefinition benchmark = {
        .config = config,
...
AsthraTestResult test_benchmark_suite_execution(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_pointer(context, suite,
                                   "Suite creation should succeed")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    AsthraBenchmarkDefinition benchmarks[] = {
        {
            .config = asthra_benchmark_config_create("string_ops", 200, ASTHRA_BENCHMARK_MODE_SINGLE_THREADED),
        ...
        {
            .config = asthra_benchmark_config_create("buffer_ops", 200, ASTHRA_BENCHMARK_MODE_SINGLE_THREADED),
        ...
        {
            .config = asthra_benchmark_config_create("memory_ops", 200, ASTHRA_BENCHMARK_MODE_SINGLE_THREADED),
        ...
        {
            .config = asthra_benchmark_config_create("atomic_ops", 1000, ASTHRA_BENCHMARK_MODE_SINGLE_THREADED),
    ...
    AsthraBenchmarkStatus suite_status = asthra_benchmark_execute_suite(
        suite, results, &result_count);
    ...
    if (!asthra_test_assert_size_eq(context, result_count, 4,
                                   "Should execute all 4 benchmarks")) {
        asthra_benchmark_suite_destroy(suite);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_statistical_analysis(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_size_eq(context, stats.min_ns, 1000,
                                   "Minimum should be correct")) {
        return ASTHRA_TEST_FAIL;
    }

    if (!asthra_test_assert_size_eq(context, stats.max_ns, 1300,
                                   "Maximum should be correct")) {
        return ASTHRA_TEST_FAIL;
    }

    // Mean should be 1123 (sum = 11230, count = 10)
    if (!asthra_test_assert_size_eq(context, stats.mean_ns, 1123,
