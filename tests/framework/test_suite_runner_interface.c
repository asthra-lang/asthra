...
int asthra_test_runner_add_suite(AsthraTestRunner *runner, const char *suite_name,
    ...
    if (runner->suite_count >= runner->capacity) {
        ...
        AsthraTestSuiteEntry *new_suites =
            realloc(runner->suites, new_capacity * sizeof(AsthraTestSuiteEntry));
        if (!new_suites)
            return -1;
...
int asthra_test_runner_execute(AsthraTestRunner *runner) {
    ...
    // Configure formatter
    asthra_test_formatter_set_config(runner->config.output_format, runner->config.colored_output,
