...
void filter_tests(AsthraTestRunner *runner) {
    for (size_t s = 0; s < runner->suite_count; s++) {
        ...
        if (!matches_pattern(suite->suite_name, runner->config.suite_patterns,
        ...
        for (size_t t = 0; t < suite->test_count; t++) {
            ...
            bool matches = matches_pattern(test->metadata.name, runner->config.test_patterns,
                                           runner->config.pattern_count);

            // Check exclude patterns
            bool excluded = is_excluded(test->metadata.name, runner->config.exclude_patterns,
