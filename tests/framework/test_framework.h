...
_Static_assert(sizeof(atomic_uint_fast64_t) >= sizeof(uint64_t),
               "Atomic types must support 64-bit counters");
...
#define ASTHRA_TEST_ASSERT_EQ(context, actual, expected, ...)                                      \
    _Generic((actual),                                                                             \
        ...
        default: asthra_test_assert_generic_eq)(context, actual, expected, __VA_ARGS__)
...
#define ASTHRA_TEST_ASSERT_RANGE(context, value, min, max, ...)                                    \
    _Generic((value),                                                                              \
        ...
        default: asthra_test_assert_generic_range)(context, value, min, max, __VA_ARGS__)
