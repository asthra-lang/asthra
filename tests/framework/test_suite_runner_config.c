...
AsthraTestOutputFormat parse_output_format(const char *format_str) {
    if (strcmp(format_str, "console") == 0)
        return ASTHRA_FORMAT_CONSOLE;
    if (strcmp(format_str, "plain") == 0)
        return ASTHRA_FORMAT_PLAIN;
    if (strcmp(format_str, "json") == 0)
        return ASTHRA_FORMAT_JSON;
    if (strcmp(format_str, "xml") == 0)
        return ASTHRA_FORMAT_XML;
    if (strcmp(format_str, "tap") == 0)
        return ASTHRA_FORMAT_TAP;
    if (strcmp(format_str, "junit") == 0)
        return ASTHRA_FORMAT_JUNIT;
    if (strcmp(format_str, "markdown") == 0)
        return ASTHRA_FORMAT_MARKDOWN;
    ...
    while ((c = getopt_long(argc, argv, "t:s:e:d:j:frvqo:h", long_options, NULL)) != -1) {
        ...
        case 't':
            ...
            config->test_patterns =
                realloc(config->test_patterns, (config->pattern_count + 1) * sizeof(char *));
            config->test_patterns[config->pattern_count++] = strdup(optarg);
            break;
        case 's':
            // Add suite pattern
            config->suite_patterns =
                realloc(config->suite_patterns, (config->suite_pattern_count + 1) * sizeof(char *));
            config->suite_patterns[config->suite_pattern_count++] = strdup(optarg);
            break;
        case 'e':
            // Add exclude pattern
            config->exclude_patterns = realloc(
                config->exclude_patterns, (config->exclude_pattern_count + 1) * sizeof(char *));
            config->exclude_patterns[config->exclude_pattern_count++] = strdup(optarg);
            break;
        case 'd':
            // Add test directory
            config->test_directories =
                realloc(config->test_directories, (config->directory_count + 1) * sizeof(char *));
