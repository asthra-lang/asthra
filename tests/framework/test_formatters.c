...
static AsthraTestFormatterConfig g_formatter_config = {.format = ASTHRA_FORMAT_CONSOLE,
                                                       .use_colors = true,
                                                       .show_timing = true,
                                                       .show_memory = false,
...
static const char *get_result_color(AsthraTestResult result) {
    if (!g_formatter_config.use_colors)
        return "";
...
static void format_console_suite_summary(const AsthraTestStatistics *stats) {
    ...
    fprintf(output, "\n%s=== Test Summary ===%s\n", g_formatter_config.use_colors ? ANSI_BOLD : "",
            reset);
    ...
    fprintf(output, "Tests run: %zu, Passed: %s%zu%s, Failed: %s%zu%s, Skipped: %s%zu%s\n", total,
            passed > 0 ? (g_formatter_config.use_colors ? ANSI_GREEN : "") : "", passed, reset,
            failed > 0 ? (g_formatter_config.use_colors ? ANSI_RED : "") : "", failed, reset,
            skipped > 0 ? (g_formatter_config.use_colors ? ANSI_YELLOW : "") : "", skipped, reset);
...
static void format_json_test_result(FILE *output, const AsthraTestContext *context, bool is_first) {
    ...
    fprintf(output, "      \"result\": \"%s\",\n",
            ...
            : context->result == ASTHRA_TEST_SKIP ? "skip"
                                                  : "error");
