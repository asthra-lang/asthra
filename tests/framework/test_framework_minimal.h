...
#define DEFINE_TEST(test_name)                                                                     \
    ...
    static const AsthraTestMetadata test_name##_metadata = {.name = #test_name,                    \
                                                            .file = __FILE__,                      \
                                                            .line = __LINE__,                      \
                                                            .skip = false,                         \
                                                            .skip_reason = NULL};                  \
...
#define ASTHRA_TEST_ASSERT(context, condition, ...)                                                \
    _Generic((condition),                                                                          \
        ...
        default: asthra_test_assert_generic)(context, condition, __VA_ARGS__)

// Additional compatibility functions for codegen tests
static inline bool asthra_test_assert_pointer(AsthraTestContext *context, void *ptr,
                                              const char *message) {
    if (ptr == NULL) {
        printf("ASSERTION FAILED: %s - Pointer is NULL at %s:%d\n", message,
               context ? context->metadata.file : "unknown", context ? context->metadata.line : 0);
        return false;
    }
    return true;
}

static inline bool asthra_test_assert_bool(AsthraTestContext *context, bool condition,
                                           const char *message) {
    if (!condition) {
        printf("ASSERTION FAILED: %s at %s:%d\n", message,
               context ? context->metadata.file : "unknown", context ? context->metadata.line : 0);
...
static inline bool asthra_test_assert_bool_eq(AsthraTestContext *context, bool actual,
...
static inline bool asthra_test_assert_int(AsthraTestContext *context, int condition,
                                          const char *message) {
    if (!condition) {
...
static inline bool asthra_test_assert_long(AsthraTestContext *context, long condition,
                                           const char *message) {
    if (!condition) {
...
static inline bool asthra_test_assert_size(AsthraTestContext *context, size_t condition,
                                           const char *message) {
    if (!condition) {
...
static inline bool asthra_test_assert_string(AsthraTestContext *context, const char *str,
                                             const char *message) {
    if (!str || strlen(str) == 0) {
...
static inline bool asthra_test_assert_generic(AsthraTestContext *context, int condition,
                                              const char *message) {
    if (!condition) {
...
static inline AsthraTestResult
asthra_test_run_suite(AsthraTestResult (*tests[])(AsthraTestContext *),
...
static inline AsthraTestSuiteConfig
create_statement_test_suite_config(const char *suite_name, const char *description,
                                   AsthraTestStatistics *stats) {
    AsthraTestSuiteConfig config = {.suite_name = suite_name,
                                    .verbose = false,
...
static inline AsthraTestContext *asthra_test_context_create(AsthraTestMetadata *metadata,
                                                            AsthraTestStatistics *stats) {
