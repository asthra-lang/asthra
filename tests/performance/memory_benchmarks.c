...
static MemoryBenchmarkResult benchmark_zone_allocation(BenchmarkZoneType zone_type) {
    ...
    result.allocations_per_second = allocation_time > 0 ? 
        (size_t)((double)BENCHMARK_ALLOCATION_COUNT / allocation_time * 1000.0) : 0;
    result.deallocations_per_second = deallocation_time > 0 ? 
        (size_t)((double)BENCHMARK_ALLOCATION_COUNT / deallocation_time * 1000.0) : 0;
    ...
    result.fragmentation_percentage = result.peak_memory_bytes > theoretical_usage ?
        ((result.peak_memory_bytes - theoretical_usage) * 100) / theoretical_usage : 0;
...
static MemoryBenchmarkResult benchmark_large_allocations(void) {
    ...
    result.deallocation_time_ms = deallocation_time;
    result.allocations_per_second = allocation_time > 0 ? 
        (size_t)((double)large_alloc_count / allocation_time * 1000.0) : 0;
    result.deallocations_per_second = deallocation_time > 0 ? 
        (size_t)((double)large_alloc_count / deallocation_time * 1000.0) : 0;
    result.peak_memory_bytes = peak_memory - start_memory;
    result.current_memory_bytes = end_memory - start_memory;
    result.memory_leaks = (end_memory > start_memory) ? (end_memory - start_memory) : 0;
...
ComprehensiveMemoryBenchmark memory_benchmarks_run_comprehensive(void) {
    ...
    overall->allocation_time_ms += large_alloc_result.allocation_time_ms + access_pattern_result.allocation_time_ms;
    overall->deallocation_time_ms += large_alloc_result.deallocation_time_ms + access_pattern_result.deallocation_time_ms;
    ...
    overall->allocations_per_second = overall->allocation_time_ms > 0 ?
        (size_t)((double)total_allocs / overall->allocation_time_ms * 1000.0) : 0;
    overall->deallocations_per_second = overall->deallocation_time_ms > 0 ?
        (size_t)((double)total_deallocs / overall->deallocation_time_ms * 1000.0) : 0;
    
    overall->current_memory_bytes = atomic_load(&current_memory_usage);
    overall->peak_memory_bytes = atomic_load(&peak_memory_usage);
    
    // Memory safety validation
    benchmark.memory_safety_validated = (overall->memory_leaks == 0) && 
                                       (total_allocs == total_deallocs);
...
void memory_benchmarks_print_results(ComprehensiveMemoryBenchmark benchmark) {
    ...
    for (int i = 0; i < BENCHMARK_ZONE_COUNT; i++) {
        ...
        printf("  Zone %d: %zu allocs/sec, %zu deallocs/sec, %.2f MB peak, %zu%% fragmentation\n",
               ...
               (double)zone->peak_memory_bytes / (1024.0 * 1024.0),
               zone->fragmentation_percentage);
    }
...
bool memory_benchmarks_validate_performance(ComprehensiveMemoryBenchmark benchmark) {
    ...
    if (peak_memory_mb <= target_peak_memory_mb * 0.9) { // 10% reduction
        printf("✅ Memory usage target met: %.2f MB (target: %.2f MB)\n",
               peak_memory_mb, target_peak_memory_mb * 0.9);
    } else {
