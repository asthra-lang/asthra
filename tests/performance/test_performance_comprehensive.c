...
static void test_channel_throughput_performance(void) {
    ...
    AsthraConcurrencyChannel *channel =
        Asthra_channel_create(sizeof(int), BUFFER_SIZE, "perf_test_channel");
    if (!channel) {
    ...
    double throughput =
        (sent_count + received_count) / (elapsed_ms / 1000.0); // operations per second
...
static void test_channel_latency_performance(void) {
    ...
    AsthraConcurrencyChannel *channel =
        Asthra_channel_create(sizeof(int), 1, "latency_test_channel");
    if (!channel) {
        printf("    ❌ Failed to create channel\n");
        return;
    }
