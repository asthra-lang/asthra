...
static AsthraTestResult test_memory_allocation_performance(AsthraTestContext *context) {
    ...
    printf("Memory allocation performance: %.2f allocs/sec (%.6f ms avg)\n", allocs_per_second,
           (time_spent * 1000.0) / ALLOC_ITERATIONS);
...
static AsthraTestResult test_string_operation_performance(AsthraTestContext *context) {
    ...
    for (int i = 0; i < STRING_ITERATIONS; i++) {
        ...
        if (!asthra_test_assert_pointer(context, copy,
                                        "String copy memory allocation should succeed")) {
            return ASTHRA_TEST_FAIL;
        }
    ...
    printf("String operation performance: %.2f copies/sec (%.6f ms avg)\n", copies_per_second,
           (time_spent * 1000.0) / STRING_ITERATIONS);
...
static AsthraTestResult test_arithmetic_performance(AsthraTestContext *context) {
    ...
    printf("Arithmetic performance: %.2f ops/sec (%.6f ms avg)\n", ops_per_second,
           (time_spent * 1000.0) / ARITHMETIC_ITERATIONS);
...
int main(void) {
    ...
    asthra_test_suite_add_test(suite, "memory_allocation_performance",
                               "Test basic memory allocation performance",
                               test_memory_allocation_performance);
    asthra_test_suite_add_test(suite, "string_operation_performance",
                               "Test string operation performance",
                               test_string_operation_performance);
    asthra_test_suite_add_test(suite, "arithmetic_performance",
                               "Test arithmetic operation performance",
