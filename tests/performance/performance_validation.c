...
static double measure_keyword_lookup_performance(void) {
    const char *keywords[] = {"let",    "fn",   "if",    "else", "while", "for",    "match", "enum",
    ...
    double total_ns =
        (double)(end.tv_sec - start.tv_sec) * 1000000000.0 + (double)(end.tv_nsec - start.tv_nsec);
...
void performance_validation_init_baseline(void) {
    ...
    global_baseline.memory_usage_bytes =
        (size_t)(VALIDATION_BASELINE_MEMORY_USAGE_MB * 1024 * 1024);
...
TestPerformanceComparison performance_validation_compare(PerformanceMeasurement current) {
    ...
    if (global_baseline.keyword_lookup_time_ns > 0) {
        double lookup_speedup =
            ((global_baseline.keyword_lookup_time_ns - current.keyword_lookup_time_ns) /
             global_baseline.keyword_lookup_time_ns) *
            100.0;
    ...
    if (global_baseline.memory_usage_bytes > 0) {
        double memory_reduction =
            ((double)global_baseline.memory_usage_bytes - (double)current.memory_usage_bytes) /
    ...
    if (current.keyword_lookup_time_ns >
        global_baseline.keyword_lookup_time_ns * (1.0 + REGRESSION_THRESHOLD_PERCENT / 100.0)) {
    ...
    if (current.reference_counting_time_ns >
        global_baseline.reference_counting_time_ns * (1.0 + REGRESSION_THRESHOLD_PERCENT / 100.0)) {
        comparison.has_regression = true;
        snprintf(comparison.regression_details + strlen(comparison.regression_details),
                 sizeof(comparison.regression_details) - strlen(comparison.regression_details),
    ...
    if (current.memory_usage_bytes >
        global_baseline.memory_usage_bytes * (1.0 + REGRESSION_THRESHOLD_PERCENT / 100.0)) {
        comparison.has_regression = true;
        snprintf(comparison.regression_details + strlen(comparison.regression_details),
                 sizeof(comparison.regression_details) - strlen(comparison.regression_details),
