...
static AsthraTestResult test_never_function_detection_performance(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < OPTIMIZATION_ITERATIONS; i++) {
        MockControlFlowGraph cfg = {.block_count = CFG_BLOCK_COUNT + (i % 10),
    ...
    if (!ASTHRA_TEST_ASSERT(context, elapsed < 1.0,
                            "Never detection should complete in under 1 second")) {
...
static AsthraTestResult test_branch_hint_generation_performance(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < BRANCH_HINT_ITERATIONS; i++) {
        MockControlFlowGraph cfg = {.block_count = 10,
                                    .has_never_calls =
                                        true, // Ensure we have Never calls to generate hints for
    ...
    if (!ASTHRA_TEST_ASSERT(context, elapsed < 0.5,
                            "Branch hint generation should complete in under 0.5 seconds")) {
        asthra_test_context_end(context, ASTHRA_TEST_FAIL);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_never_optimization_pipeline_performance(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < OPTIMIZATION_ITERATIONS / 10;
         i++) { // Fewer iterations for complete pipeline
        MockControlFlowGraph cfg = {.block_count = CFG_BLOCK_COUNT + (i % 20),
                                    .has_never_calls = false,
                                    .successor_count = 25};
    ...
    if (!ASTHRA_TEST_ASSERT(context, elapsed < 2.0,
                            "Optimization pipeline should complete in under 2 seconds")) {
        asthra_test_context_end(context, ASTHRA_TEST_FAIL);
        return ASTHRA_TEST_FAIL;
    }
    if (!ASTHRA_TEST_ASSERT(context, stats.never_optimizations > 0,
...
int main(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create_lightweight("Never Type Optimization Performance Tests");

    asthra_test_suite_add_test(suite, "never_function_detection_performance",
                               "Test Never function detection performance",
                               test_never_function_detection_performance);
    asthra_test_suite_add_test(suite, "branch_hint_generation_performance",
                               "Test branch hint generation performance",
                               test_branch_hint_generation_performance);
    asthra_test_suite_add_test(suite, "never_optimization_pipeline_performance",
                               "Test complete Never optimization pipeline performance",
                               test_never_optimization_pipeline_performance);
