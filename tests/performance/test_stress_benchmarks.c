...
void test_stress_many_channels(void) {
        ...
        *channel_id = i;
        ...
        pthread_create(&threads[i], NULL, ^void* (void* arg) {
            ...
            for (int j = 0; j < MESSAGES_PER_CHANNEL; j++) {
                ...
                AsthraConcurrencyResult result = asthra_channel_send(channels[id], &message, sizeof(int));
    ...
    for (int i = 0; i < NUM_CHANNELS; i++) {
        ...
        while (1) {
            int message;
            AsthraConcurrencyResult result = asthra_channel_recv(channels[i], &message, sizeof(int));
    ...
    printf("  Stress test: %d channels, %d total messages in %.2f ms\n",
           NUM_CHANNELS, total_consumed, elapsed_ms);
            ...
            for (int op = 0; op < NUM_OPERATIONS / NUM_WORKER_THREADS; op++) {
                ...
                switch (operation_type) {
                    case 0: {
                        // Channel send/recv
                        int data = id * 1000 + op;
                        if (asthra_channel_send(ch, &data, sizeof(int)) == ASTHRA_CONCURRENCY_SUCCESS) {
                            int received;
                            if (asthra_channel_recv(ch, &received, sizeof(int)) == ASTHRA_CONCURRENCY_SUCCESS) {
                    ...
                    case 2: {
                        ...
                        if (op % 10 == 0) {
                            bool is_leader;
                            if (asthra_barrier_wait(barrier, &is_leader) == ASTHRA_CONCURRENCY_SUCCESS) {
                                operations_completed++;
                            }
                        } else {
...
void test_stress_high_contention(void) {
    ...
    AsthraConcurrencySemaphoreHandle* shared_semaphore = asthra_semaphore_create(1); // Single permit
        ...
        pthread_create(&threads[i], NULL, ^void* (void* arg) {
            ...
            for (int op = 0; op < CONTENTION_OPERATIONS; op++) {
                ...
                if (op % 2 == 0) {
                    ...
                    if (asthra_semaphore_acquire(shared_semaphore) == ASTHRA_CONCURRENCY_SUCCESS) {
                        // Very brief critical section
                        volatile int dummy = id + op;
                        (void)dummy;
                        
                        if (asthra_semaphore_release(shared_semaphore) == ASTHRA_CONCURRENCY_SUCCESS) {
                    ...
                    if (asthra_channel_send(shared_channel, &data, sizeof(int)) == ASTHRA_CONCURRENCY_SUCCESS) {
                        int received;
                        if (asthra_channel_recv(shared_channel, &received, sizeof(int)) == ASTHRA_CONCURRENCY_SUCCESS) {
                            operation_succeeded = true;
                        }
                    }
                }
