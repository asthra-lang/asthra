...
void run_stress_tests(void) {
    ...
    *consumer_count = 0;
    ...
    ASSERT_TRUE(total_operations >=
                NUM_OPERATIONS *
                    0.5); // At least 50% success (since only half are channel ops that complete)
...
void print_performance_summary(void) {
    ...
    if (asthra_concurrency_result_is_success(result)) {
        ...
        if (final_stats.callbacks_enqueued > 0) {
            printf("\nPerformance Analysis:\n");
            printf("  Callback processing rate: %.2f%%\n", (double)final_stats.callbacks_processed /
                                                               final_stats.callbacks_enqueued *
                                                               100.0);
    ...
    if (asthra_concurrency_result_is_success(result) &&
        ASTHRA_STATS_MEMORY_USAGE(final_stats) > 5 * 1024 * 1024) {
