...
void test_memory_efficiency(void) {
    ...
    printf("  Memory usage: %llu bytes for %d objects (%.2f KB total, %.2f KB creation time)\n",
           memory_increase, NUM_CHANNELS + NUM_BARRIERS + NUM_SEMAPHORES,
           memory_increase / 1024.0, creation_time);
    
    // Memory per object should be reasonable
    double bytes_per_object = (double)memory_increase / (NUM_CHANNELS + NUM_BARRIERS + NUM_SEMAPHORES);
    ...
    printf("  Final memory usage: %llu bytes (leaked: %lld bytes)\n",
           final_memory, (int64_t)(final_memory - initial_memory));
...
void test_tier1_vs_tier2_performance(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        *thread_data = NUM_TASKS / 4;
        ...
        pthread_create(&tier1_threads[i], NULL, ^void* (void* arg) {
    ...
    // Producer thread
    pthread_t producer;
    pthread_create(&producer, NULL, ^void* (void* arg) {
    ...
    for (int i = 0; i < 4; i++) {
        pthread_create(&tier2_threads[i], NULL, ^void* (void* arg) {
            while (1) {
                int task;
                AsthraConcurrencyResult result = asthra_channel_recv(work_channel, &task, sizeof(int));
    ...
    while (collected < NUM_TASKS) {
        int result;
        if (asthra_channel_recv(result_channel, &result, sizeof(int)) == ASTHRA_CONCURRENCY_SUCCESS) {
            collected++;
        } else {
            break;
        }
