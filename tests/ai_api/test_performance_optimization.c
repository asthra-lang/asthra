...
AsthraTestResult test_semantic_cache_basic_operations(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT_STR_EQ(context, (char *)retrieved_data, test_data,
...
AsthraTestResult test_cache_lru_eviction(AsthraTestContext *context) {
    ...
    asthra_test_assert_size_eq(context, stats.current_entries, 3,
...
AsthraTestResult test_cache_configuration(AsthraTestContext *context) {
    ...
    CacheConfig custom_config = {
        .max_entries = 50, .max_memory_mb = 5, .ttl_seconds = 60, .enable_statistics = true};
...
int main(void) {
    ...
    AsthraTestSuite *suite = asthra_test_suite_create(
        "AI API Performance Tests", "Tests for AI API performance optimization and caching");

    // Semantic cache tests
    asthra_test_suite_add_test(suite, "test_semantic_cache_basic_operations",
                               "Test basic cache operations", test_semantic_cache_basic_operations);
    asthra_test_suite_add_test(suite, "test_cache_lru_eviction", "Test LRU cache eviction",
                               test_cache_lru_eviction);
    asthra_test_suite_add_test(suite, "test_memory_efficiency", "Test memory efficiency",
                               test_memory_efficiency);
    asthra_test_suite_add_test(suite, "test_cache_thread_safety", "Test cache thread safety",
                               test_cache_thread_safety);

    // Performance monitoring tests
    asthra_test_suite_add_test(suite, "test_performance_monitoring", "Test performance monitoring",
                               test_performance_monitoring);
    asthra_test_suite_add_test(suite, "test_cache_performance_improvement",
                               "Test cache performance improvement",
                               test_cache_performance_improvement);
    asthra_test_suite_add_test(suite, "test_cache_configuration", "Test cache configuration",
