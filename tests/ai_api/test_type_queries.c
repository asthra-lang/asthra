...
AsthraTestResult test_compatible_types_query(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (strcmp(types[i], "i32") == 0)
            found_i32 = true;
        if (strcmp(types[i], "string") == 0)
            found_string = true;
        if (strcmp(types[i], "bool") == 0)
            found_bool = true;
    }
...
AsthraTestResult test_type_info_queries(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (strcmp(available_types[i], "i32") == 0)
            found_i32 = true;
        if (strcmp(available_types[i], "void") == 0)
            found_void = true;
    }
...
int main(void) {
    AsthraTestSuite *suite = asthra_test_suite_create(
        "AI API Type Queries Tests", "Tests for type inference and type information queries");

    asthra_test_suite_add_test(suite, "test_type_inference", "Test type inference functionality",
                               test_type_inference);
    asthra_test_suite_add_test(suite, "test_type_compatibility", "Test type compatibility checking",
                               test_type_compatibility);
    asthra_test_suite_add_test(suite, "test_compatible_types_query", "Test compatible types query",
                               test_compatible_types_query);
    asthra_test_suite_add_test(suite, "test_type_info_queries", "Test type information queries",
                               test_type_info_queries);
    asthra_test_suite_add_test(suite, "test_detailed_type_info", "Test detailed type information",
                               test_detailed_type_info);
    asthra_test_suite_add_test(suite, "test_type_queries_error_handling",
                               "Test type queries error handling",
                               test_type_queries_error_handling);
