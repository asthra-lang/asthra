...
AsthraTestResult test_symbol_info_query(AsthraTestContext *context) {
    ...
    if (func_info) {
        asthra_test_assert_str_eq(context, func_info->name, "create_user",
...
AsthraTestResult test_struct_info_query(AsthraTestContext *context) {
    ...
    if (info) {
        ...
        // Test field extraction infrastructure (fields may be 0 if semantic analyzer doesn't
        // populate them yet) The field extraction infrastructure is implemented and ready for when
        // semantic analysis is enhanced
...
AsthraTestResult test_api_thread_safety(AsthraTestContext *context) {
    ...
    if (info1 && info2) {
        asthra_test_assert_str_eq(context, info1->name, info2->name,
                                  "Concurrent queries returned different results");
...
int main(void) {
    AsthraTestSuite *suite = asthra_test_suite_create(
        "AI Semantic API Tests", "Tests for the AI Semantic API functionality");

    asthra_test_suite_add_test(suite, "test_api_initialization", "Test API initialization",
                               test_api_initialization);
    asthra_test_suite_add_test(suite, "test_symbol_info_query", "Test symbol information queries",
                               test_symbol_info_query);
    asthra_test_suite_add_test(suite, "test_struct_info_query", "Test struct information queries",
                               test_struct_info_query);
    asthra_test_suite_add_test(suite, "test_api_thread_safety", "Test API thread safety",
                               test_api_thread_safety);
