...
AsthraTestResult test_find_declaration(AsthraTestContext *context) {
    ...
    if (decl) {
        asthra_test_assert_int_eq(context, decl->type, AST_STRUCT_DECL, "Should find struct declaration");
        printf("DEBUG: Found Point declaration at line %d, column %d\n", 
               decl->location.line, decl->location.column);
    } else {
        printf("INFO: Declaration finding not available yet - semantic analyzer needs enhancement\n");
    }
    
    // Test finding function declaration
    decl = asthra_ai_find_declaration(api, "add");
    if (decl) {
        asthra_test_assert_int_eq(context, decl->type, AST_FUNCTION_DECL, "Should find function declaration");
...
AsthraTestResult test_find_usages(AsthraTestContext *context) {
    ...
    if (usages && count > 0) {
        ...
        printf("DEBUG: First usage - type: %s, scope: %s, line: %zu\n",
               usage->usage_type, usage->scope_name, usage->location.line);
...
AsthraTestResult test_symbol_locations(AsthraTestContext *context) {
    ...
    if (locations && count > 0) {
        ...
        ASTHRA_TEST_ASSERT_TRUE(context, location->line > 0, "Location should have valid line number");
        ASTHRA_TEST_ASSERT_TRUE(context, location->column > 0, "Location should have valid column number");
...
AsthraTestResult test_code_context(AsthraTestContext *context) {
    ...
    accessible = asthra_ai_is_symbol_accessible(api, "NonExistentSymbol", "test_file.asthra", 10, 1);
...
// Main test runner
int main(void) {
    AsthraTestSuite *suite = asthra_test_suite_create("AI API AST Navigation Tests", "Tests for AST navigation and code location functionality");
    
    asthra_test_suite_add_test(suite, "test_find_declaration", "Test finding symbol declarations", test_find_declaration);
    asthra_test_suite_add_test(suite, "test_find_usages", "Test finding symbol usages", test_find_usages);
    asthra_test_suite_add_test(suite, "test_symbol_documentation", "Test symbol documentation extraction", test_symbol_documentation);
    asthra_test_suite_add_test(suite, "test_symbol_locations", "Test getting symbol locations", test_symbol_locations);
    asthra_test_suite_add_test(suite, "test_ast_traversal", "Test AST traversal functionality", test_ast_traversal);
    asthra_test_suite_add_test(suite, "test_code_context", "Test code context and analysis", test_code_context);
    asthra_test_suite_add_test(suite, "test_ast_navigation_error_handling", "Test AST navigation error handling", test_ast_navigation_error_handling);
