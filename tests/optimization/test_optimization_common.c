...
CommandResult execute_command(const char *command, int timeout_seconds) {
    ...
    if (which_result != -1 && WIFEXITED(which_result) && WEXITSTATUS(which_result) == 0) {
        snprintf(full_command, sizeof(full_command), "gtimeout %d %s > %s 2> %s", timeout_seconds,
                 command, temp_stdout, temp_stderr);
    } else {
        // Fallback: run without timeout on macOS
        snprintf(full_command, sizeof(full_command), "%s > %s 2> %s", command, temp_stdout,
                 temp_stderr);
    }
#else
    // Linux and other Unix systems - use standard timeout command
    snprintf(full_command, sizeof(full_command), "timeout %d %s > %s 2> %s", timeout_seconds,
             command, temp_stdout, temp_stderr);
#endif
...
PerformanceBenchmark benchmark_performance(const char *baseline_binary,
                                           const char *optimized_binary, const char *test_input) {
    ...
    snprintf(optimized_command, sizeof(optimized_command), "time %s %s", optimized_binary,
             test_input);
...
bool validate_binary_optimization(const char *binary_path) {
    ...
    return (result.exit_code == 0 && (strstr(result.output, "not stripped") != NULL ||
