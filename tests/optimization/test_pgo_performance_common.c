...
PGOBenchmarkComparison pgo_compare_performance(PGOPerformanceStats baseline, PGOPerformanceStats optimized) {
    ...
    if (baseline.avg_time_ms > 0) {
        ...
        double pooled_std = sqrt((baseline.std_dev_ms * baseline.std_dev_ms + 
                                 optimized.std_dev_ms * optimized.std_dev_ms) / 2.0);
        ...
        if (pooled_std > 0) {
            ...
            comparison.confidence_level = (t_stat > 2.0) ? 0.95 : 
                                         (t_stat > 1.5) ? 0.80 : 0.50;
            comparison.statistically_significant = (t_stat > 1.5);
        }
        
        comparison.meets_threshold = comparison.improvement_percent >= PGO_PERFORMANCE_THRESHOLD_PERCENT;
    }
...
bool pgo_run_binary_benchmark(const char *binary_path, const char *args, PGOPerformanceStats *stats) {
    ...
    for (int i = 0; i < PGO_BENCHMARK_ITERATIONS && successful_runs < PGO_BENCHMARK_ITERATIONS; i++) {
        char command[PGO_MAX_COMMAND_LENGTH];
        snprintf(command, sizeof(command), "%s %s > /dev/null 2>&1", 
                 binary_path, args ? args : "");
