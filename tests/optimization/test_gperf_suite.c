...
static AsthraTestResult test_gperf_makefile_targets(AsthraTestContext *context) {
    ...
    const char *expected_gperf_files[] = {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, 
                               "gperf-test target should succeed: %s", result.error)) {
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_gperf_keyword_extraction(AsthraTestContext *context) {
    ...
    CommandResult result = execute_command("bash optimization/extract_keywords_main.sh", TEST_TIMEOUT_SECONDS);
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, 
...
static AsthraTestResult test_gperf_hash_function_quality(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, 
                               "Gperf hash function compilation should succeed: %s", result.error)) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestFunction gperf_test_functions[] = {
    test_gperf_makefile_targets,
    test_gperf_keyword_extraction,
    test_gperf_hash_function_quality
};

AsthraTestMetadata gperf_test_metadata[] = {
    {"Gperf Makefile Targets", __FILE__, __LINE__, "test_gperf_makefile_targets", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL},
    {"Gperf Keyword Extraction", __FILE__, __LINE__, "test_gperf_keyword_extraction", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL},
    {"Gperf Hash Function Quality", __FILE__, __LINE__, "test_gperf_hash_function_quality", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL}
};
