...
static AsthraTestResult test_pgo_makefile_targets(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, 
                               "pgo-run target should succeed: %s", result.error)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, 
                               "pgo-merge target should succeed: %s", result.error)) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, 
                               "pgo-use target should succeed: %s", result.error)) {
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_pgo_performance_improvement(AsthraTestContext *context) {
    ...
    PerformanceBenchmark benchmark = benchmark_performance("bin/asthra", "bin/asthra-pgo-optimized", test_input);
...
AsthraTestFunction pgo_test_functions[] = {
    test_pgo_makefile_targets,
    test_pgo_complete_workflow,
    test_pgo_performance_improvement
};

AsthraTestMetadata pgo_test_metadata[] = {
    {"PGO Makefile Targets", __FILE__, __LINE__, "test_pgo_makefile_targets", ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
    {"PGO Complete Workflow", __FILE__, __LINE__, "test_pgo_complete_workflow", ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
    {"PGO Performance Improvement", __FILE__, __LINE__, "test_pgo_performance_improvement", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL}
};
