...
static AsthraTestResult test_makefile_help_updated(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result.exit_code, 0, "Make help should succeed: %s",
                               result.error)) {
        return ASTHRA_TEST_FAIL;
    }

    // Check for new optimization targets in help output
    const char *expected_targets[] = {"pgo-optimize", "sanitizer-comprehensive", "gperf-keywords",
                                      "optimize-analyze"};
...
static AsthraTestResult test_makefile_optimize_analyze_target(AsthraTestContext *context) {
    ...
    const char *expected_artifacts[] = {"bin/asthra-pgo-optimized",
...
AsthraTestFunction makefile_test_functions[] = {test_makefile_help_updated,
                                                test_makefile_optimize_analyze_target,
                                                test_makefile_legacy_deprecation};

AsthraTestMetadata makefile_test_metadata[] = {
    {"Makefile Help Updated", __FILE__, __LINE__, "test_makefile_help_updated",
     ASTHRA_TEST_SEVERITY_LOW, 0, false, NULL},
    {"Makefile Optimize-Analyze Target", __FILE__, __LINE__,
     "test_makefile_optimize_analyze_target", ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
    {"Makefile Legacy Deprecation", __FILE__, __LINE__, "test_makefile_legacy_deprecation",
     ASTHRA_TEST_SEVERITY_LOW, 0, false, NULL}};

const size_t makefile_test_count =
    sizeof(makefile_test_functions) / sizeof(makefile_test_functions[0]);
