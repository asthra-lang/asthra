...
AsthraTestResult test_ubsan_integer_overflow_detection(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, max_int, INT_MAX, 
                               "INT_MAX should be correctly set")) {
...
AsthraTestResult test_ubsan_null_pointer_dereference_detection(AsthraTestContext *context) {
    ...
    *valid_ptr = 42;
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, *valid_ptr, 42, 
                               "Valid pointer dereference should work")) {
        free(valid_ptr);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_ubsan_array_bounds_detection(AsthraTestContext *context) {
    ...
    // Valid access
    if (!ASTHRA_TEST_ASSERT_EQ(context, array[5], 25, 
                               "Valid array access should work")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_ubsan_division_by_zero_detection(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, result, 20, 
...
int main(void) {
    ...
    AsthraTestResult results[] = {
        test_ubsan_integer_overflow_detection(context),
