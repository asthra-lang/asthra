...
static AsthraTestResult test_gperf_build_system_integration(AsthraTestContext *context) {
    ...
    char command[MAX_COMMAND_LENGTH];
    snprintf(command, sizeof(command), "make clean >/dev/null 2>&1 && make gperf-keywords >/dev/null 2>&1 && make >/dev/null 2>&1");
...
static bool run_test_suite(const char *suite_name, 
    ...
    printf("%s Tests: %s\n", suite_name, 
           ...
           (suite_result == ASTHRA_TEST_SKIP) ? "SKIPPED" : "FAILED");
...
int main(void) {
    ...
    // Run all test suites
    all_passed &= run_test_suite("File Generation", 
                                 gperf_file_generation_tests, 
                                 gperf_file_generation_metadata, 
                                 gperf_file_generation_test_count, 
                                 &config);
    
    all_passed &= run_test_suite("Keyword Extraction", 
                                 gperf_keyword_extraction_tests, 
                                 gperf_keyword_extraction_metadata, 
                                 gperf_keyword_extraction_test_count, 
                                 &config);
    
    all_passed &= run_test_suite("Hash Correctness", 
                                 gperf_hash_correctness_tests, 
                                 gperf_hash_correctness_metadata, 
                                 gperf_hash_correctness_test_count, 
                                 &config);
    
    all_passed &= run_test_suite("Performance", 
                                 gperf_performance_tests, 
                                 gperf_performance_metadata, 
                                 gperf_performance_test_count, 
                                 &config);
    
    // Run integration tests
    AsthraTestFunction integration_tests[] = {
        test_gperf_makefile_integration,
        test_gperf_dependency_tracking,
        test_gperf_build_system_integration
    };
    
    AsthraTestMetadata integration_metadata[] = {
        {"Gperf Makefile Integration", __FILE__, __LINE__, "test_gperf_makefile_integration", ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
        {"Gperf Dependency Tracking", __FILE__, __LINE__, "test_gperf_dependency_tracking", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL},
        {"Gperf Build System Integration", __FILE__, __LINE__, "test_gperf_build_system_integration", ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL}
    };
    
    size_t integration_test_count = sizeof(integration_tests) / sizeof(integration_tests[0]);
    
    all_passed &= run_test_suite("Integration", 
                                 integration_tests, 
