...
static AsthraTestResult test_extracted_keywords_completeness(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, file != NULL, 
                           "Should be able to open gperf file")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(context, found_count >= required_count, 
                           "Should find at least %zu keywords, found %zu", 
...
static AsthraTestResult test_keyword_extraction_accuracy(AsthraTestContext *context) {
    ...
    double validity_ratio = total_keyword_lines > 0 ? 
        (double)valid_keyword_lines / total_keyword_lines : 0.0;
    ...
    printf("Keyword extraction accuracy: %d/%d valid lines (%.1f%%)\n", 
           valid_keyword_lines, total_keyword_lines, validity_ratio * 100.0);
...
AsthraTestFunction gperf_keyword_extraction_tests[] = {
    test_keyword_extraction_script,
    test_extracted_keywords_completeness,
    test_keyword_extraction_accuracy
};

AsthraTestMetadata gperf_keyword_extraction_metadata[] = {
    {"Keyword Extraction Script", __FILE__, __LINE__, "test_keyword_extraction_script", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL},
    {"Extracted Keywords Completeness", __FILE__, __LINE__, "test_extracted_keywords_completeness", ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
    {"Keyword Extraction Accuracy", __FILE__, __LINE__, "test_keyword_extraction_accuracy", ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL}
};

const size_t gperf_keyword_extraction_test_count = sizeof(gperf_keyword_extraction_tests) / sizeof(gperf_keyword_extraction_tests[0]); 
