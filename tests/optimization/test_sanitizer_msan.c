...
AsthraTestResult test_msan_uninitialized_memory_detection(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, buffer != NULL, "Memory allocation should succeed")) {
    ...
    if (!ASTHRA_TEST_ASSERT_EQ(context, buffer[50], 'A', "Initialized memory access should work")) {
        free(buffer);
        return ASTHRA_TEST_FAIL;
    }
