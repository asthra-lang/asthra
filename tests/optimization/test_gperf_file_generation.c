...
static AsthraTestResult test_gperf_file_structure(AsthraTestContext *context) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (found_keywords_section && !found_delimiter && line[0] != '%' && line[0] != '#' &&
            strstr(line, ",")) {
            keyword_count++;
        }
    }
    ...
    if (!ASTHRA_TEST_ASSERT(context, keyword_count >= 20,
                            "Gperf file should contain at least 20 keywords, found %d",
                            keyword_count)) {
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_gperf_generated_files(AsthraTestContext *context) {
    const char *generated_files[] = {"optimization/asthra_keywords_hash.c",
...
static AsthraTestResult test_gperf_header_file_content(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, file != NULL, "Should be able to open header file")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestFunction gperf_file_generation_tests[] = {
    test_gperf_file_structure, test_gperf_generated_files, test_gperf_header_file_content};

AsthraTestMetadata gperf_file_generation_metadata[] = {
    {"Gperf File Structure", __FILE__, __LINE__, "test_gperf_file_structure",
     ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
    {"Gperf Generated Files", __FILE__, __LINE__, "test_gperf_generated_files",
     ASTHRA_TEST_SEVERITY_HIGH, 0, false, NULL},
    {"Gperf Header File Content", __FILE__, __LINE__, "test_gperf_header_file_content",
     ASTHRA_TEST_SEVERITY_MEDIUM, 0, false, NULL}};

const size_t gperf_file_generation_test_count =
    sizeof(gperf_file_generation_tests) / sizeof(gperf_file_generation_tests[0]);
