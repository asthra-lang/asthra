...
static AsthraTestResult run_test_suite(const char *suite_name, 
                                       AsthraTestFunction *functions,
    ...
    printf("Suite '%s' result: %s\n", suite_name, 
           ...
           (suite_result == ASTHRA_TEST_FAIL) ? "FAIL" : "SKIP");
...
int main(void) {
    ...
    // Run PGO test suite
    total_suites++;
    AsthraTestResult pgo_result = run_test_suite("PGO Test Suite", 
                                                 pgo_test_functions, 
                                                 pgo_test_metadata, 
                                                 pgo_test_count, 
                                                 &config);
    if (pgo_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (pgo_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
    
    // Run Sanitizer test suite
    total_suites++;
    AsthraTestResult sanitizer_result = run_test_suite("Sanitizer Test Suite", 
                                                       sanitizer_test_functions, 
                                                       sanitizer_test_metadata, 
                                                       sanitizer_test_count, 
                                                       &config);
    if (sanitizer_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (sanitizer_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
    
    // Run Gperf test suite
    total_suites++;
    AsthraTestResult gperf_result = run_test_suite("Gperf Integration Test Suite", 
                                                   gperf_test_functions, 
                                                   gperf_test_metadata, 
                                                   gperf_test_count, 
                                                   &config);
    if (gperf_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (gperf_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
    
    // Run Automation Scripts test suite
    total_suites++;
    AsthraTestResult automation_result = run_test_suite("Automation Scripts Test Suite", 
                                                        automation_test_functions, 
                                                        automation_test_metadata, 
                                                        automation_test_count, 
                                                        &config);
    if (automation_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (automation_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
    
    // Run Makefile Integration test suite
    total_suites++;
    AsthraTestResult makefile_result = run_test_suite("Makefile Integration Test Suite", 
                                                      makefile_test_functions, 
                                                      makefile_test_metadata, 
                                                      makefile_test_count, 
                                                      &config);
    if (makefile_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (makefile_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
    
    // Run CI/CD Compatibility test suite
    total_suites++;
    AsthraTestResult cicd_result = run_test_suite("CI/CD Compatibility Test Suite", 
                                                  cicd_test_functions, 
                                                  cicd_test_metadata, 
                                                  cicd_test_count, 
                                                  &config);
    if (cicd_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (cicd_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
    
    // New: Run Advanced Optimization Pass test suite
    total_suites++;
    AsthraTestResult advanced_opt_result = run_test_suite("Advanced Optimization Pass Test Suite", 
                                                          advanced_optimization_pass_test_functions, 
                                                          advanced_optimization_pass_test_metadata, 
                                                          advanced_optimization_pass_test_count, 
                                                          &config);
    if (advanced_opt_result == ASTHRA_TEST_PASS) passed_suites++;
    else if (advanced_opt_result == ASTHRA_TEST_FAIL) failed_suites++;
    else skipped_suites++;
