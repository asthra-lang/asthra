 ...
 */
static FileCheckResult *fast_check_single_file(const char *file_path,
...
FastCheckCommandResult *fast_check_run_command_with_engine(FastCheckEngine *engine, char **files,
    ...
    if (!engine || !files || file_count == 0 || !config)
        return NULL;
    ...
    for (size_t i = 0; i < file_count; i++) {
        ...
        if (file_result) {
            ...
            if (file_result->status == FILE_STATUS_OK ||
    ...
    cmd_result->avg_time_per_file =
        cmd_result->files_checked > 0 ? cmd_result->total_time_ms / cmd_result->files_checked : 0.0;
...
bool fast_check_syntax_only(const char *file_path) {
    ...
    FastCheckCommandConfig config = {.syntax_only = true,
                                     .watch_mode = false,
                                     .enable_cache = false,
                                     .verbose = false,
                                     .max_errors = 1,
...
bool fast_check_incremental_validate(char **changed_files, size_t file_count) {
    ...
    FastCheckCommandResult *result =
        fast_check_run_command_with_engine(engine, changed_files, file_count, config);
