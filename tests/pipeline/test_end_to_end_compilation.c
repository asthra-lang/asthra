...
static bool test_single_file_compilation(void) {
    ...
    const char *simple_program = "package test;\n"
                                 "\n"
    ...
    bool compilation_success =
        pipeline_orchestrator_compile_file(test_orchestrator, input_path, TEST_EXECUTABLE_PATH);
...
static bool test_multiple_functions_compilation(void) {
    ...
    const char *multi_function_program = "package test;\n"
                                         "\n"
...
static bool test_compilation_error_handling(void) {
    ...
    const char *invalid_program = "package test;\n"
                                  "\n"
                                  "pub fn main(none) -> int {\n"
...
static bool test_compilation_performance(void) {
    ...
    const char *performance_program = "package test;\n"
                                      "\n"
...
static bool test_pipeline_phase_transitions(void) {
    ...
    double total_time;
    pipeline_orchestrator_get_statistics(test_orchestrator, &total_files, &total_lines,
...
int main(int argc, char **argv) {
    ...
    // Core compilation tests
    printf("\n--- Core Compilation Tests ---\n");
    if (test_single_file_compilation())
        passed++;
    total++;
    if (test_multiple_functions_compilation())
        passed++;
    total++;
    ...
    printf("\n--- Performance Tests ---\n");
    if (test_compilation_performance())
        passed++;
    total++;

    // Pipeline validation tests
    printf("\n--- Pipeline Validation Tests ---\n");
    if (test_pipeline_phase_transitions())
        passed++;
    total++;
