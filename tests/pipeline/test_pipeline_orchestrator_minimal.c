...
static bool test_basic_data_structures(void) {
    ...
    TEST_ASSERT(lexer_data.execution_time_ms == 123.45, "Lexer phase data execution time field works");
...
int main(void) {
    ...
    // Run minimal tests
    if (test_phase_names()) passed++;
    total++;
    if (test_time_function()) passed++;
    total++;
    if (test_null_safety()) passed++;
    total++;
    if (test_basic_data_structures()) passed++;
    total++;
    if (test_orchestrator_creation_minimal()) passed++;
    total++;
