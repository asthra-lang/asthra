...
static bool test_orchestrator_creation_and_destruction(void) {
    ...
    if (orchestrator) {
        ...
        TEST_ASSERT(orchestrator->current_phase == ASTHRA_PHASE_LEXING, 
                   "Initial phase is lexing");
        TEST_ASSERT(!orchestrator->pipeline_active, 
                   "Pipeline starts in inactive state");
        TEST_ASSERT(!orchestrator->error_occurred, 
                   "No errors initially");
        TEST_ASSERT(orchestrator->source_file_count == 0, 
                   "No source files initially");
...
static bool test_orchestrator_configuration(void) {
    ...
    if (orchestrator) {
        ...
        TEST_ASSERT(orchestrator->config.max_errors == 100, 
                   "Default max errors is 100");
        TEST_ASSERT(orchestrator->config.timeout_seconds == 300.0, 
        ...
        TEST_ASSERT(orchestrator->config.verbose_output, 
                   "Configuration modification works");
        TEST_ASSERT(orchestrator->config.parallel_compilation, 
                   "Parallel compilation can be enabled");
        TEST_ASSERT(orchestrator->config.max_errors == 50, 
                   "Max errors can be modified");
...
static bool test_state_management(void) {
    ...
    if (orchestrator) {
        ...
        TEST_ASSERT(!orchestrator->error_occurred, 
                   "Error state cleared");
        TEST_ASSERT(!orchestrator->pipeline_active, 
                   "Pipeline active state cleared");
        TEST_ASSERT(orchestrator->source_file_count == 0, 
                   "Source file count reset");
        TEST_ASSERT(orchestrator->current_phase == ASTHRA_PHASE_LEXING, 
                   "Phase reset to lexing");
...
static bool test_utility_functions(void) {
    ...
    TEST_ASSERT(strcmp(pipeline_get_phase_name(ASTHRA_PHASE_SEMANTIC_ANALYSIS), "Semantic Analysis") == 0,
               "Semantic analysis phase name correct");
    TEST_ASSERT(strcmp(pipeline_get_phase_name(ASTHRA_PHASE_CODE_GENERATION), "Code Generation") == 0,
...
static bool test_error_handling(void) {
    ...
    if (orchestrator) {
        ...
        bool result = pipeline_orchestrator_compile_file(orchestrator, 
                                                        "/nonexistent/file.asthra", 
                                                        "output");
...
static bool test_statistics_api(void) {
    ...
    if (orchestrator) {
        ...
        pipeline_orchestrator_get_statistics(orchestrator, 
...
int main(void) {
    ...
    // Run tests
    if (test_orchestrator_creation_and_destruction()) passed++;
    total++;
    if (test_orchestrator_configuration()) passed++;
    total++;
    if (test_state_management()) passed++;
    total++;
    if (test_utility_functions()) passed++;
    total++;
    if (test_error_handling()) passed++;
    total++;
    if (test_statistics_api()) passed++;
    total++;
