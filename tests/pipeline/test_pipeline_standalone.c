...
static bool test_compiler_phase_enum(void) {
    ...
    printf("  Phase enumeration values: LEXING=%d, PARSING=%d, SEMANTIC=%d, OPTIMIZATION=%d, CODEGEN=%d, LINKING=%d\n",
...
int main(void) {
    ...
    total++;
    if (test_compiler_phase_enum()) passed++;
    total++;
    if (test_basic_memory_operations()) passed++;
    total++;
    if (test_performance_measurement()) passed++;
    total++;
