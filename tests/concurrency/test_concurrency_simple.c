...
static ConcurrencyTestResult test_basic_thread_creation(AsthraTestContext *context);
static ConcurrencyTestResult test_mutex_synchronization(AsthraTestContext *context);
static ConcurrencyTestResult test_concurrent_counter(AsthraTestContext *context);
...
static ConcurrencyTestResult test_basic_thread_creation(AsthraTestContext *context) {
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        ...
        if (create_result != 0) {
            ...
            asthra_test_context_end(context, CONCURRENCY_TEST_FAIL);
    ...
    asthra_test_context_end(context, CONCURRENCY_TEST_PASS);
...
static ConcurrencyTestResult test_concurrent_counter(AsthraTestContext *context) {
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        int create_result =
            pthread_create(&threads[i], NULL, counter_thread_function, &shared_counter);
        if (create_result != 0) {
            printf("Error: Failed to create thread %d (error: %d)\n", i, create_result);
            asthra_test_context_end(context, CONCURRENCY_TEST_FAIL);
            return CONCURRENCY_TEST_FAIL;
...
int main(void) {
    ...
    AsthraTestMetadata metadata1 = {.name = "test_basic_thread_creation",
                                    .file = __FILE__,
                                    .line = __LINE__,
    ...
    AsthraTestMetadata metadata2 = {.name = "test_mutex_synchronization",
                                    .file = __FILE__,
                                    .line = __LINE__,
    ...
    AsthraTestMetadata metadata3 = {.name = "test_concurrent_counter",
                                    .file = __FILE__,
                                    .line = __LINE__,
    ...
    // Run the tests
    ConcurrencyTestResult result1 = test_basic_thread_creation(context1);
    ConcurrencyTestResult result2 = test_mutex_synchronization(context2);
    ConcurrencyTestResult result3 = test_concurrent_counter(context3);

    // Print results
    printf("\nBasic thread creation: %s\n", result1 == CONCURRENCY_TEST_PASS ? "PASS" : "FAIL");
    printf("Mutex synchronization: %s\n", result2 == CONCURRENCY_TEST_PASS ? "PASS" : "FAIL");
    printf("Concurrent counter: %s\n", result3 == CONCURRENCY_TEST_PASS ? "PASS" : "FAIL");
    ...
    if (result1 == CONCURRENCY_TEST_PASS && result2 == CONCURRENCY_TEST_PASS &&
        result3 == CONCURRENCY_TEST_PASS) {
