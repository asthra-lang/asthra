...
// Global test context
ConcurrencyTestContext* g_concurrency_context = NULL;
...
ConcurrencyTestContext* concurrency_test_context_create(size_t max_threads) {
    ConcurrencyTestContext* context = calloc(1, sizeof(ConcurrencyTestContext));
...
void concurrency_test_get_statistics(size_t* total_ops, size_t* successful_ops, 
    ...
    if (!g_concurrency_context) return;
    
    if (total_ops) *total_ops = atomic_load(&g_concurrency_context->total_operations);
    if (successful_ops) *successful_ops = atomic_load(&g_concurrency_context->successful_operations);
    if (failed_ops) *failed_ops = atomic_load(&g_concurrency_context->failed_operations);
    if (duration_ms) *duration_ms = g_concurrency_context->test_duration_ms;
    if (race_conditions) *race_conditions = atomic_load(&g_concurrency_context->race_conditions_detected);
    if (deadlocks) *deadlocks = atomic_load(&g_concurrency_context->deadlocks_detected);
}
