...
ConcurrencyTestResult test_concurrency_spawn_with_handle_operations(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, tasks[i] != NULL, "Failed to spawn task %d with handle",
                                     i)) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, result != NULL, "Task %d result should not be NULL", i)) {
            cleanup_tasks(tasks, num_tasks);
            return CONCURRENCY_TEST_FAIL;
        }

        int expected = inputs[i].data * inputs[i].task_id;
        if (!CONCURRENCY_TEST_ASSERT(ctx, result->data == expected,
...
ConcurrencyTestResult test_concurrency_await_expressions(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, tasks[i] != NULL, "Failed to spawn timeout task %d", i)) {
            cleanup_tasks(tasks, i);
            return CONCURRENCY_TEST_FAIL;
        }
    }
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, result != NULL && *result == delays[task_idx],
                                     "Await task %d should return %d, got %d", task_idx,
                                     delays[task_idx], result ? *result : -1)) {
            cleanup_tasks(tasks, num_tasks);
            return CONCURRENCY_TEST_FAIL;
        }
