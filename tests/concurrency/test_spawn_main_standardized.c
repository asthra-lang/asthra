...
extern AsthraTestResult test_concurrency_spawn_basic(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_spawn_deterministic(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_spawn_multiple(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_spawn_with_arguments(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_spawn_multiple_statements(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_spawn_with_handle_operations(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_await_expressions(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_deterministic_behavior(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_performance_scalability(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_error_cases(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_spawn_with_c_functions(AsthraV12TestContext *ctx);
extern AsthraTestResult test_concurrency_c_threads_calling_asthra(AsthraV12TestContext *ctx);
...
AsthraTestResult adapter_spawn_deterministic(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_spawn_deterministic(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.basic_tests_passed++;
    return result;
}

AsthraTestResult adapter_spawn_multiple(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_spawn_multiple(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.basic_tests_passed++;
    return result;
}

AsthraTestResult adapter_spawn_with_arguments(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_spawn_with_arguments(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.basic_tests_passed++;
    return result;
}

AsthraTestResult adapter_spawn_multiple_statements(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_spawn_multiple_statements(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.basic_tests_passed++;
    return result;
}

// Adapter functions for handle/await tests
AsthraTestResult adapter_spawn_with_handle_operations(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_spawn_with_handle_operations(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.handle_tests_passed++;
    return result;
}

AsthraTestResult adapter_await_expressions(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_await_expressions(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.handle_tests_passed++;
    return result;
}

// Adapter functions for performance tests
AsthraTestResult adapter_deterministic_behavior(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_deterministic_behavior(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.performance_tests_passed++;
    return result;
}

AsthraTestResult adapter_performance_scalability(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_performance_scalability(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.performance_tests_passed++;
    return result;
}

// Adapter functions for error handling tests
AsthraTestResult adapter_error_cases(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_error_cases(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.error_tests_passed++;
    return result;
}

// Adapter functions for C integration tests
AsthraTestResult adapter_spawn_with_c_functions(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_spawn_with_c_functions(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.integration_tests_passed++;
    return result;
}

AsthraTestResult adapter_c_threads_calling_asthra(AsthraTestContext *context) {
    ConcurrencyTestAdapter *adapter = (ConcurrencyTestAdapter*)context->user_data;
    AsthraTestResult result = test_concurrency_c_threads_calling_asthra(&adapter->legacy_ctx);
    if (result == ASTHRA_TEST_PASS) adapter->stats.integration_tests_passed++;
    return result;
}
...
int main(void) {
    ...
    asthra_test_suite_set_analysis_callback(g_adapter.standard_suite, concurrency_analysis_callback);
    
    // Register basic spawn functionality tests (critical)
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_basic", "Basic spawn functionality",
        adapter_spawn_basic, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_deterministic", "Deterministic spawn behavior",
        adapter_spawn_deterministic, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_multiple", "Multiple spawn operations",
        adapter_spawn_multiple, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_with_arguments", "Spawn with arguments",
        adapter_spawn_with_arguments, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_multiple_statements", "Spawn with multiple statements",
        adapter_spawn_multiple_statements, &g_adapter);
    
    // Register handle/await tests (critical)
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_with_handle_operations", "Handle operations and await",
        adapter_spawn_with_handle_operations, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "await_expressions", "Await expression handling",
        adapter_await_expressions, &g_adapter);
    
    // Register performance tests (medium priority)
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "deterministic_behavior", "Deterministic behavior validation",
        adapter_deterministic_behavior, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "performance_scalability", "Performance scalability",
        adapter_performance_scalability, &g_adapter);
    
    // Register error handling tests (medium priority)
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "error_cases", "Error handling cases",
        adapter_error_cases, &g_adapter);
    
    // Register C integration tests (medium priority)
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "spawn_with_c_functions", "Spawn with C functions",
        adapter_spawn_with_c_functions, &g_adapter);
    
    asthra_test_suite_add_test_with_data(g_adapter.standard_suite,
        "c_threads_calling_asthra", "C threads calling Asthra",
        adapter_c_threads_calling_asthra, &g_adapter);
    
    // Run the test suite
    AsthraTestResult result = asthra_test_suite_run(g_adapter.standard_suite);
