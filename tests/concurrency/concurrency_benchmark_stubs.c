...
void asthra_benchmark_end(AsthraV12TestContext *ctx) {
    ...
    if (global_timer.duration_ms > 0.0) {
        ctx->benchmark.throughput_ops_per_sec =
            ((double)total_ops / global_timer.duration_ms) * 1000.0;
    }
    ...
    if (ctx->benchmark.throughput_ops_per_sec <
        BASELINE_TASK_THROUGHPUT_OPS_PER_SEC * 0.8) { // 20% tolerance
...
bool asthra_benchmark_check_regression(AsthraV12TestContext *ctx) {
    if (!ctx)
        return true; // Assume regression if context is invalid
