...
bool test_task_spawn_and_completion(void) {
    ...
    AsthraConcurrencyTaskHandle *handle =
        Asthra_spawn_task(simple_task_function, &test_value, sizeof(int), NULL);
    ...
    printf("[TEST DEBUG] Got task result, tag: %d, is_ok: %s\n", task_result.tag,
           asthra_result_is_ok(task_result) ? "true" : "false");
    fflush(stdout);
...
bool test_task_timeout(void) {
    ...
    AsthraConcurrencyTaskHandle *handle =
        Asthra_spawn_task(long_running_task_function, NULL, 0, NULL);
    ASSERT(handle != NULL, "Task spawn should succeed");
...
bool test_task_spawn_options(void) {
    ...
    AsthraConcurrencyTaskSpawnOptions options = {.stack_size = 1024 * 1024, // 1MB stack
                                                 .priority = 0,
                                                 .detached = false,
                                                 .name = "test_task",
                                                 .affinity_mask = NULL};
    ...
    int test_value = 10;
    AsthraConcurrencyTaskHandle *handle =
        Asthra_spawn_task(simple_task_function, &test_value, sizeof(int), &options);
