...
AsthraTestResult test_concurrency_spawn_basic(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, *result == 84,
                           "Task result should be 84, got %d", *result)) {
        free(result);
        test_task_destroy(task);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_concurrency_spawn_deterministic(AsthraV12TestContext *ctx) {
    ...
    for (int run = 0; run < num_runs; run++) {
        ...
        int *result = (int*)task->result;
        if (!ASTHRA_TEST_ASSERT(ctx, result != NULL,
                               "Deterministic test result should not be NULL on run %d", run)) {
            test_task_destroy(task);
            return ASTHRA_TEST_FAIL;
        }
        ...
        if (run == 0) {
            ...
            if (!ASTHRA_TEST_ASSERT(ctx, *result == first_result,
...
AsthraTestResult test_concurrency_spawn_multiple(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, result != NULL && *result == expected,
                               "Task %d result should be %d, got %d",
                               i, expected, result ? *result : -1)) {
            // Cleanup all tasks
...
AsthraTestResult test_concurrency_spawn_with_arguments(AsthraV12TestContext *ctx) {
    struct TaskArgs args = {15, 7, '+'};
    
    TestTask *task = test_spawn(task_with_arguments, &args);
    
    if (!ASTHRA_TEST_ASSERT(ctx, task != NULL,
                           "Failed to spawn task with arguments", "")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    int *result = (int*)task->result;
    if (!ASTHRA_TEST_ASSERT(ctx, result != NULL && *result == 22,
...
AsthraTestResult test_concurrency_spawn_multiple_statements(AsthraV12TestContext *ctx) {
    int input_value = 21;
    TestTask *task = test_spawn(multi_statement_task, &input_value);
    
    if (!ASTHRA_TEST_ASSERT(ctx, task != NULL,
                           "Failed to spawn multi-statement task", "")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, *result >= 17,
