...
TokenType keyword_to_token_type(const char* keyword) {
    if (!keyword) return TOKEN_IDENTIFIER;
    
    // Tier 1 keywords (still supported)
    if (strcmp(keyword, "spawn") == 0) return TOKEN_SPAWN;
    if (strcmp(keyword, "spawn_with_handle") == 0) return TOKEN_SPAWN_WITH_HANDLE;
    if (strcmp(keyword, "await") == 0) return TOKEN_AWAIT;
...
bool contains_spawn_with_handle_statement(ASTNode* node) {
    ...
    if (node->type == AST_SPAWN_WITH_HANDLE_STMT) return true;
    ...
    switch (node->type) {
        case AST_PROGRAM:
            if (node->data.program.declarations) {
                for (size_t i = 0; i < node->data.program.declarations->count; i++) {
                     if (contains_spawn_with_handle_statement(node->data.program.declarations->nodes[i])) {
                        return true;
                    }
                }
            }
            break;
        case AST_FUNCTION_DECL:
            if (node->data.function_decl.body && contains_spawn_with_handle_statement(node->data.function_decl.body)) {
                return true;
            }
            break;
        case AST_BLOCK:
        ...
        case AST_IF_STMT:
            if (contains_spawn_with_handle_statement(node->data.if_stmt.then_block) ||
                (node->data.if_stmt.else_block && contains_spawn_with_handle_statement(node->data.if_stmt.else_block))) {
                return true;
            }
            break;
        default:
...
int count_await_expressions(ASTNode* node) {
    ...
    int count = 0;
    if (node->type == AST_AWAIT_EXPR) count = 1;
...
bool has_annotation(ASTNode* function_node, const char* annotation_name) {
    if (!function_node || !annotation_name) return false;
    ...
    if (function_node->type == AST_FUNCTION_DECL && function_node->data.function_decl.annotations) {
        for (size_t i = 0; i < function_node->data.function_decl.annotations->count; i++) {
            ...
            if (annotation->type == AST_SEMANTIC_TAG && 
                annotation->data.semantic_tag.name &&
...
ASTNode* find_function_declaration(ASTNode* root, const char* function_name) {
    if (!root || !function_name) return NULL;
    
    if (root->type == AST_FUNCTION_DECL && 
        root->data.function_decl.name &&
        ...
        case AST_PROGRAM:
            if (root->data.program.declarations) {
                for (size_t i = 0; i < root->data.program.declarations->count; i++) {
                    ASTNode* result = find_function_declaration(root->data.program.declarations->nodes[i], function_name);
        ...
        case AST_BLOCK:
            if (root->data.block.statements) {
                for (size_t i = 0; i < root->data.block.statements->count; i++) {
                     ASTNode* result = find_function_declaration(root->data.block.statements->nodes[i], function_name);
                    if (result) return result;
                }
            }
            break;
