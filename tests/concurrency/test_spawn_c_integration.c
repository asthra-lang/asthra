...
AsthraTestResult test_concurrency_spawn_with_c_functions(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, task != NULL,
                           "Failed to spawn task with C function", "")) {
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, result != NULL,
                           "C function result should not be NULL", "")) {
        test_task_destroy(task);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(ctx, strstr(result, "Processed: Hello from C") != NULL,
...
AsthraTestResult test_concurrency_c_threads_calling_asthra(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, task != NULL,
                           "Failed to spawn C thread calling Asthra", "")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, result != NULL,
                           "Asthra function result should not be NULL", "")) {
        test_task_destroy(task);
        return ASTHRA_TEST_FAIL;
    }
    
    if (!ASTHRA_TEST_ASSERT(ctx, *result == 49,
                           "Asthra function result should be 49, got %d", *result)) {
        free(result);
        test_task_destroy(task);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, error_task != NULL,
                           "Failed to spawn error test task", "")) {
        free(result);
        test_task_destroy(task);
        return ASTHRA_TEST_FAIL;
    }
