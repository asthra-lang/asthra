...
ConcurrencyTestResult test_concurrency_error_cases(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, null_task != NULL, "Failed to spawn null return task", "")) {
        return CONCURRENCY_TEST_FAIL;
    }

    test_task_join(null_task);

    if (!CONCURRENCY_TEST_ASSERT(ctx, null_task->result == NULL,
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, error_task != NULL, "Failed to spawn error condition task",
                                 "")) {
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, success_task != NULL, "Failed to spawn success case task",
                                 "")) {
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, result != NULL && *result == 20,
                                 "Success case should return 20, got %d", result ? *result : -1)) {
        if (result)
            free(result);
