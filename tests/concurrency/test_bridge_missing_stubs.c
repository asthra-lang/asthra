...
AsthraResult simple_task_function(void *args, size_t args_size) {
    ...
    AsthraResult final_result =
        asthra_result_ok(result, sizeof(int), ASTHRA_TYPE_I32, ASTHRA_OWNERSHIP_C);
    printf("[STUB DEBUG] Created final result, tag: %d, value ptr: %p\n", final_result.tag,
           final_result.data.ok.value);
    fflush(stdout);
...
AsthraResult failing_task_function(void *args, size_t args_size) {
    ...
    *error_code = 42;
    ...
    AsthraResult result =
        asthra_result_err(error_code, sizeof(int), ASTHRA_TYPE_I32, ASTHRA_OWNERSHIP_C);
...
AsthraResult mutex_increment_task(void *args, size_t args_size) {
    if (!args || args_size != sizeof(MutexTestData *)) {
        AsthraResult result = {.tag = ASTHRA_RESULT_ERR,
                               .data.err = {.error = (void *)"Invalid arguments",
                                            .error_size = 17,
                                            .error_type_id = 0}};
        return result;
    }
