...
AsthraTestResult test_concurrency_deterministic_behavior(AsthraV12TestContext *ctx) {
    ...
    for (int run = 0; run < num_runs; run++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, task != NULL,
                               "Failed to spawn deterministic task run %d", run)) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, result != NULL && *result == expected_result,
                               "Deterministic task run %d should return %d, got %d",
                               run, expected_result, result ? *result : -1)) {
...
AsthraTestResult test_concurrency_performance_scalability(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, tasks[i] != NULL,
                               "Failed to spawn performance task %d", i)) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(ctx, tasks[i]->result != NULL,
    ...
    if (!ASTHRA_TEST_ASSERT(ctx, time_ms < 1000.0,
                           "Performance test took too long: %.2f ms", time_ms)) {
        return ASTHRA_TEST_FAIL;
    }
