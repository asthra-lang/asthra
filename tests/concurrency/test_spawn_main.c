...
static void update_spawn_test_stats(SpawnTestStats *stats, AsthraTestResult result, const char *category) {
    stats->total_tests++;
    if (result == ASTHRA_TEST_PASS) {
        stats->passed_tests++;
        if (strcmp(category, "basic") == 0) stats->basic_tests_passed++;
        else if (strcmp(category, "handle") == 0) stats->handle_tests_passed++;
        else if (strcmp(category, "performance") == 0) stats->performance_tests_passed++;
        else if (strcmp(category, "error") == 0) stats->error_tests_passed++;
        else if (strcmp(category, "integration") == 0) stats->integration_tests_passed++;
    } else {
        stats->failed_tests++;
    }
}
