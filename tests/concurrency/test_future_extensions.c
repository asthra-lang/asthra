...
ConcurrencyTestResult test_concurrency_await_foundation(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, future != NULL, "Future creation should succeed", "")) {
        return CONCURRENCY_TEST_FAIL;
    }

    // Test initial state
    if (!CONCURRENCY_TEST_ASSERT(ctx, !atomic_load(&future->is_ready),
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, task != NULL, "Future completing task should be created",
                                 "")) {
        test_future_destroy(future);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, result != NULL, "Future await should return result", "")) {
        test_task_join(task);
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, *int_result == 246, "Future result should be 246, got %d",
                                 *int_result)) {
        free(result);
        test_task_join(task);
        test_task_destroy(task);
...
ConcurrencyTestResult test_concurrency_task_handles(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, task != NULL, "Task creation should succeed", "")) {
        return CONCURRENCY_TEST_FAIL;
    }

    TestTaskHandle *handle = test_task_handle_create(task);

    if (!CONCURRENCY_TEST_ASSERT(ctx, handle != NULL, "Task handle creation should succeed", "")) {
        test_task_join(task);
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, handle->task == task, "Handle should reference correct task",
                                 "")) {
        test_task_join(task);
        test_task_destroy(task);
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, handle->handle_id > 0, "Handle should have valid ID", "")) {
        test_task_join(task);
        test_task_destroy(task);
        test_task_handle_destroy(handle);
        return CONCURRENCY_TEST_FAIL;
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, handle->future != NULL,
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, result != NULL, "Handle await should return result", "")) {
        test_task_destroy(task);
        test_task_handle_destroy(handle);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    int *int_result = (int *)result;
    if (!CONCURRENCY_TEST_ASSERT(ctx, *int_result == 110, "Handle result should be 110, got %d",
                                 *int_result)) {
        free(result);
        test_task_destroy(task);
        test_task_handle_destroy(handle);
...
ConcurrencyTestResult test_concurrency_future_compatibility(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_futures; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, futures[i] != NULL, "Future %d creation should succeed",
                                     i)) {
            // Cleanup
            for (int j = 0; j < i; j++) {
    ...
    for (int i = 0; i < num_futures; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, result != NULL, "Future %d await should return result",
                                     i)) {
            // Cleanup
            for (int j = 0; j < num_futures; j++) {
                if (j != i && tasks[j]->result)
                    free(tasks[j]->result);
                test_task_destroy(tasks[j]);
                test_future_destroy(futures[j]);
            }
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, *int_result == expected,
                                     "Future %d result should be %d, got %d", i, expected,
                                     *int_result)) {
            // Cleanup
            for (int j = 0; j < num_futures; j++) {
                if (tasks[j]->result)
