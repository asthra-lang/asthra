...
ConcurrencyTestResult test_concurrency_task_creation(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, task->task_id > 0, "Task should have valid ID", "")) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }

    if (!CONCURRENCY_TEST_ASSERT(ctx, task->function == lifecycle_test_function,
                                 "Task should have correct function pointer", "")) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }

    if (!CONCURRENCY_TEST_ASSERT(ctx, task->arg == phases, "Task should have correct argument",
                                 "")) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, status >= TASK_STATUS_RUNNING,
...
ConcurrencyTestResult test_concurrency_task_execution(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, phases[0] == 1, "Phase 1 should have completed", "")) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }

    if (!CONCURRENCY_TEST_ASSERT(ctx, phases[1] == 1, "Phase 2 should have completed", "")) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }

    if (!CONCURRENCY_TEST_ASSERT(ctx, phases[2] == 1, "Phase 3 should have completed", "")) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, duration_ms >= 3 && duration_ms <= 50,
...
ConcurrencyTestResult test_concurrency_task_cleanup(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, cleanup_flag == 42, "Cleanup should have occurred, flag: %d",
                                 cleanup_flag)) {
        test_task_destroy(task);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, cleanup_flags[i] == 42,
