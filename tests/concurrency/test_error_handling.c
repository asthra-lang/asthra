...
ConcurrencyTestResult test_concurrency_result_propagation(AsthraV12TestContext *ctx) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, success_task != NULL, "Success task creation should succeed",
                                 "")) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, success_result != NULL, "Success result should not be NULL",
                                 "")) {
        test_task_destroy(success_task);
        return CONCURRENCY_TEST_FAIL;
    }

    if (!CONCURRENCY_TEST_ASSERT(ctx, success_result->is_ok, "Success result should be Ok", "")) {
        free(success_result);
        test_task_destroy(success_task);
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, *success_value == 30, "Success value should be 30, got %d",
                                 *success_value)) {
        free(success_value);
        free(success_result);
        test_task_destroy(success_task);
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, error_task != NULL, "Error task creation should succeed",
                                 "")) {
        free(success_value);
        free(success_result);
        test_task_destroy(success_task);
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, error_result != NULL, "Error result should not be NULL",
                                 "")) {
        free(success_value);
        free(success_result);
        test_task_destroy(success_task);
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, !error_result->is_ok, "Error result should be Err", "")) {
        free(success_value);
        free(success_result);
        free(error_result);
        test_task_destroy(success_task);
    ...
    if (!CONCURRENCY_TEST_ASSERT(
            ctx, strcmp(error_result->data.error_message, "Negative input not allowed") == 0,
...
ConcurrencyTestResult test_concurrency_error_across_tasks(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, tasks[i] != NULL, "Task %d creation should succeed", i)) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, final_task_count == num_tasks,
                                 "All tasks should have executed: %d/%d", final_task_count,
                                 num_tasks)) {
        // Cleanup
        for (int i = 0; i < num_tasks; i++) {
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, final_error_count == 3, "Should have 3 errors, got %d",
                                 final_error_count)) {
        // Cleanup
        for (int i = 0; i < num_tasks; i++) {
            test_task_destroy(tasks[i]);
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, final_success_count == 6, "Should have 6 successes, got %d",
                                 final_success_count)) {
        // Cleanup
        for (int i = 0; i < num_tasks; i++) {
            test_task_destroy(tasks[i]);
    ...
    for (int i = 0; i < num_tasks; i++) {
        if (i % 3 == 0) {
            // Should be error
            if (!CONCURRENCY_TEST_ASSERT(ctx, !shared_data.results[i].is_ok,
...
ConcurrencyTestResult test_concurrency_error_handling_stress(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < stress_task_count; i++) {
        ...
        if (!CONCURRENCY_TEST_ASSERT(ctx, stress_tasks[i] != NULL,
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, success_count + error_count == stress_task_count,
                                 "All tasks should be accounted for: %d + %d = %d", success_count,
                                 error_count, stress_task_count)) {
        return CONCURRENCY_TEST_FAIL;
    }
    ...
    if (!CONCURRENCY_TEST_ASSERT(
            ctx, error_count >= expected_errors - 2 && error_count <= expected_errors + 2,
    ...
    if (!CONCURRENCY_TEST_ASSERT(ctx, duration_seconds < 5.0,
                                 "Stress test should complete in reasonable time: %.2f seconds",
                                 duration_seconds)) {
        return CONCURRENCY_TEST_FAIL;
    }
