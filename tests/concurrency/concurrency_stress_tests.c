...
void *concurrency_sync_stress_thread(void *arg) {
    ...
    if (success && !sync_primitives_test_rwlock_read(g_concurrency_context->sync_primitives,
                                                     data->iterations / 2)) {
        success = false;
        atomic_fetch_add(&g_concurrency_context->failed_operations, 1);
    }
...
bool run_concurrency_stress_test(int num_threads, int iterations_per_thread) {
    ...
    // Create thread data
    ConcurrencyStressTestData *producer_data =
        calloc((size_t)num_threads, sizeof(ConcurrencyStressTestData));
    ConcurrencyStressTestData *consumer_data =
        calloc((size_t)num_threads, sizeof(ConcurrencyStressTestData));
    ...
    for (int i = 0; i < num_threads; i++) {
        if (pthread_create(&producer_threads[i], NULL, concurrency_producer_thread,
                           &producer_data[i]) != 0) {
    ...
    for (int i = 0; i < num_threads; i++) {
        if (pthread_create(&consumer_threads[i], NULL, concurrency_consumer_thread,
                           &consumer_data[i]) != 0) {
    ...
    double duration_ms;
    concurrency_test_get_statistics(&total_ops, &successful_ops, &failed_ops, &duration_ms,
                                    &race_conditions, &deadlocks);
...
bool test_concurrency_performance_integration(void) {
    ...
    bool performance_acceptable =
        performance_validation_run_complete("concurrency_integration_performance");
