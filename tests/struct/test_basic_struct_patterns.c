...
static bool test_basic_struct_pattern(void) {
    ...
    TEST_ASSERT(ast_node_list_size(program->data.program.declarations) == 1, "Expected 1 declaration");
    ...
    TEST_ASSERT(ast_node_list_size(body->data.block.statements) == 1, "Expected 1 statement (match)");
    ...
    TEST_ASSERT(match_stmt->data.match_stmt.expression->type == AST_IDENTIFIER, "Expected identifier expression");
    ...
    TEST_ASSERT(arm->data.match_arm.pattern->type == AST_STRUCT_PATTERN, "Expected AST_STRUCT_PATTERN");
    
    ASTNode *struct_pattern = arm->data.match_arm.pattern;
    TEST_ASSERT(strcmp(struct_pattern->data.struct_pattern.struct_name, "Point") == 0, "Expected 'Point' struct name");
    TEST_ASSERT(struct_pattern->data.struct_pattern.field_patterns != NULL, "Field patterns are NULL");
    ...
    TEST_ASSERT(field1->data.field_pattern.binding_name != NULL, "Expected explicit binding for 'x'");
    ...
    TEST_ASSERT(field2->data.field_pattern.binding_name != NULL, "Expected explicit binding for 'y'");
...
static bool test_empty_struct_pattern(void) {
    ...
    TEST_ASSERT(strcmp(struct_pattern->data.struct_pattern.struct_name, "Unit") == 0, "Expected 'Unit' struct name");
    
    // Check that field patterns is empty or NULL
    if (struct_pattern->data.struct_pattern.field_patterns != NULL) {
        TEST_ASSERT(ast_node_list_size(struct_pattern->data.struct_pattern.field_patterns) == 0, "Expected 0 field patterns");
    }
