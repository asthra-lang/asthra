...
static bool test_multiple_struct_patterns(void) {
    ...
    TEST_ASSERT(strcmp(pattern1->data.struct_pattern.struct_name, "Point") == 0,
    ...
    TEST_ASSERT(strcmp(pattern2->data.struct_pattern.struct_name, "Rectangle") == 0,
                "Expected 'Rectangle' struct name");
    ...
    TEST_ASSERT(strcmp(pattern3->data.struct_pattern.struct_name, "Circle") == 0,
                "Expected 'Circle' struct name");
...
static bool test_mixed_enum_and_struct_patterns(void) {
    ...
    TEST_ASSERT(strcmp(pattern1->data.enum_pattern.enum_name, "Result") == 0,
                "Expected 'Result' enum name");
    TEST_ASSERT(strcmp(pattern1->data.enum_pattern.variant_name, "Ok") == 0,
                "Expected 'Ok' variant name");

    // Check Err pattern
    ASTNode *arm2 = ast_node_list_get(match_stmt->data.match_stmt.arms, 1);
    ASTNode *pattern2 = arm2->data.match_arm.pattern;
    TEST_ASSERT(pattern2->type == AST_ENUM_PATTERN, "Expected AST_ENUM_PATTERN for Result.Err");
    TEST_ASSERT(strcmp(pattern2->data.enum_pattern.enum_name, "Result") == 0,
                "Expected 'Result' enum name");
    TEST_ASSERT(strcmp(pattern2->data.enum_pattern.variant_name, "Err") == 0,
                "Expected 'Err' variant name");
...
static bool test_nested_struct_patterns(void) {
    ...
    ASTNode *pattern1 = arm1->data.match_arm.pattern;
    TEST_ASSERT(pattern1->type == AST_STRUCT_PATTERN, "Expected AST_STRUCT_PATTERN");
    TEST_ASSERT(strcmp(pattern1->data.struct_pattern.struct_name, "Container") == 0,
                "Expected 'Container' struct name");

    // Check that this test properly validates nested patterns
    // (Note: This test assumes nested patterns are supported; if not, this validates the outer
    // pattern only)
    TEST_ASSERT(ast_node_list_size(pattern1->data.struct_pattern.field_patterns) >= 1,
                "Expected at least 1 field pattern");
...
static bool test_varying_field_counts(void) {
    ...
    ASTNode *pattern1 = arm1->data.match_arm.pattern;
    TEST_ASSERT(pattern1->type == AST_STRUCT_PATTERN, "Expected AST_STRUCT_PATTERN");
    TEST_ASSERT(strcmp(pattern1->data.struct_pattern.struct_name, "Unit") == 0,
    ...
    ASTNode *pattern2 = arm2->data.match_arm.pattern;
    TEST_ASSERT(pattern2->type == AST_STRUCT_PATTERN, "Expected AST_STRUCT_PATTERN");
    TEST_ASSERT(strcmp(pattern2->data.struct_pattern.struct_name, "Single") == 0,
                "Expected 'Single' struct name");
    TEST_ASSERT(ast_node_list_size(pattern2->data.struct_pattern.field_patterns) == 1,
                "Expected 1 field pattern");

    // Check Pair pattern (2 fields)
    ASTNode *arm3 = ast_node_list_get(match_stmt->data.match_stmt.arms, 2);
    ASTNode *pattern3 = arm3->data.match_arm.pattern;
    TEST_ASSERT(pattern3->type == AST_STRUCT_PATTERN, "Expected AST_STRUCT_PATTERN");
    TEST_ASSERT(strcmp(pattern3->data.struct_pattern.struct_name, "Pair") == 0,
                "Expected 'Pair' struct name");
    TEST_ASSERT(ast_node_list_size(pattern3->data.struct_pattern.field_patterns) == 2,
    ...
    TEST_ASSERT(strcmp(pattern4->data.struct_pattern.struct_name, "Triple") == 0,
                "Expected 'Triple' struct name");
    TEST_ASSERT(ast_node_list_size(pattern4->data.struct_pattern.field_patterns) == 3,
                "Expected 3 field patterns");
    ...
    TEST_ASSERT(strcmp(pattern5->data.struct_pattern.struct_name, "Quad") == 0,
                "Expected 'Quad' struct name");
    TEST_ASSERT(ast_node_list_size(pattern5->data.struct_pattern.field_patterns) == 4,
