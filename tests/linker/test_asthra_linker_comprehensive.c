...
static bool test_symbol_resolution(void) {
    ...
    // Test symbol lookup
    Asthra_SymbolEntry *found_symbol =
        asthra_symbol_resolver_find_symbol(resolver, "test_function");
    if (!found_symbol) {
...
static bool test_object_file_manager(void) {
    ...
    asthra_object_manager_get_statistics(manager, &loaded_count, &cache_hit_rate, &total_time,
                                         &symbols_processed);
...
static bool test_linker_statistics(void) {
    ...
    asthra_linker_get_statistics(linker, &objects_linked, &executables_generated, &total_time_ms,
                                 &cache_hit_rate);
...
int main(void) {
    ...
    struct {
        const char *name;
        bool (*test_func)(void);
    } tests[] = {{"Linker Creation", test_linker_creation},
