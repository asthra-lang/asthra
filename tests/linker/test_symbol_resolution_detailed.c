...
static bool test_symbol_conflict_resolution(void) {
    ...
    // Add a global symbol
    Asthra_ObjectSymbol global_symbol = {
        .name = (char *)"conflicted_symbol",
        .address = 0x1000,
        .size = 128,
        .binding = ASTHRA_SYMBOL_GLOBAL,
        .type = ASTHRA_SYMBOL_FUNCTION
    ...
    Asthra_ObjectSymbol conflicting_symbol = {
        .name = (char *)"conflicted_symbol",
        .address = 0x2000,
        .size = 64,
        .binding = ASTHRA_SYMBOL_GLOBAL,
    ...
    Asthra_ObjectSymbol weak_symbol = {
        .name = (char *)"conflicted_symbol",
        .address = 0x3000,
        .size = 32,
        .binding = ASTHRA_SYMBOL_WEAK,
...
static bool test_symbol_references(void) {
    ...
    // Add a symbol
    Asthra_ObjectSymbol symbol = {
        .name = (char *)"referenced_function",
        .address = 0x1000,
        .size = 128,
        .binding = ASTHRA_SYMBOL_GLOBAL,
        .type = ASTHRA_SYMBOL_FUNCTION
    ...
    for (int i = 0; i < 3; i++) {
        bool ref_result = asthra_symbol_resolver_add_reference(
            resolver, 
            "referenced_function",
            "test.o",
            ".text",
    ...
    for (size_t i = 0; i < NUM_SYMBOLS; i++) {
        ...
        if (!add_result) {
            ...
            for (size_t j = 0; j <= i; j++) {
                ...
                Asthra_SymbolEntry *entry = asthra_symbol_resolver_find_symbol(resolver, cleanup_name);
                if (entry && entry->name) {
...
static bool test_symbol_statistics(void) {
    ...
    // Add some references
    asthra_symbol_resolver_add_reference(resolver, "func1", "test.o", ".text", 0x3000, ASTHRA_REF_ABSOLUTE, 0);
    asthra_symbol_resolver_add_reference(resolver, "func2", "test.o", ".text", 0x3100, ASTHRA_REF_RELATIVE, 0);
    asthra_symbol_resolver_add_reference(resolver, "undefined", "test.o", ".text", 0x3200, ASTHRA_REF_ABSOLUTE, 0);
...
int main(void) {
    ...
    struct {
        ...
        {"Symbol Addition and Lookup", test_symbol_addition_and_lookup},
