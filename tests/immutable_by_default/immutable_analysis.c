...
static void add_mutability_violation(ImmutableSemanticAnalyzer* analyzer, 
                                    const char* variable_name,
                                    const char* reason,
                                    int line, int column) {
    if (!analyzer || analyzer->violation_count >= analyzer->max_violations) return;
...
bool immutable_analyze_variable_declaration(ImmutableSemanticAnalyzer* analyzer,
                                          EnhancedASTNode* var_decl,
                                          const char* variable_name) {
    if (!analyzer || !var_decl || !variable_name) return false;
    ...
    if (has_mut_keyword) {
        ...
        if (analyzer->symbol_mutability.mutable_count < 1000) { // Arbitrary limit
            analyzer->symbol_mutability.mutable_symbols[analyzer->symbol_mutability.mutable_count++] = strdup(variable_name);
        }
    } else {
        ...
        if (analyzer->symbol_mutability.immutable_count < 1000) { // Arbitrary limit
            analyzer->symbol_mutability.immutable_symbols[analyzer->symbol_mutability.immutable_count++] = strdup(variable_name);
        }
    }
 ...
 */
bool immutable_analyze_mutation(ImmutableSemanticAnalyzer* analyzer,
                               EnhancedASTNode* mutation_site,
                               const char* variable_name) {
    if (!analyzer || !mutation_site || !variable_name) return false;
        ...
        for (size_t i = 0; i < analyzer->symbol_mutability.immutable_count; i++) {
            if (strcmp(analyzer->symbol_mutability.immutable_symbols[i], variable_name) == 0) {
                ...
                add_mutability_violation(analyzer, variable_name,
                                       "Attempted mutation of immutable variable",
                                       0, 0); // Line/column would come from AST
                return false;
            }
        }
    }
 ...
 */
bool immutable_analyze_borrow(ImmutableSemanticAnalyzer* analyzer,
                             EnhancedASTNode* borrow_site,
                             const char* variable_name,
                             bool is_mutable_borrow) {
    if (!analyzer || !borrow_site || !variable_name) return false;
