 ...
 *
 * @file test_fixed_array_types.c
 * @brief Tests for fixed-size array type system
 *
 * Tests the Phase 1 implementation of fixed-size array types including:
 * - Array type creation and properties
...
ASTHRA_TEST_DEFINE(test_fixed_array_creation, ASTHRA_TEST_SEVERITY_CRITICAL) {
...
int main(void) {
    printf("Starting Fixed Array Types Test\n");

    AsthraTestMetadata metadata = {.name = "test_fixed_array_creation",
                                   .file = __FILE__,
                                   .line = __LINE__,
                                   .function = "test_fixed_array_creation",
                                   .severity = ASTHRA_TEST_SEVERITY_CRITICAL,
                                   .timeout_ns = 0,
                                   .skip = false};

    AsthraTestFunction tests[] = {test_fixed_array_creation};
    AsthraTestMetadata metadatas[] = {metadata};

    AsthraTestSuiteConfig config = asthra_test_suite_config_create(
        "Fixed Array Types Test Suite", "Testing fixed array type creation and operations");
