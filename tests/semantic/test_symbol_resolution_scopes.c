...
ASTHRA_TEST_DEFINE(test_scope_shadowing, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    bool success1 = semantic_declare_symbol(ctx->analyzer, "x", SYMBOL_VARIABLE,
    ...
    bool success2 = semantic_declare_symbol(ctx->analyzer, "x", SYMBOL_VARIABLE,
    ...
    ASTHRA_TEST_ASSERT_NOT_NULL(context, entry,
                                "Failed to resolve outer scope variable after exit");
...
ASTHRA_TEST_DEFINE(test_scope_variable_out_of_scope, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    bool success = semantic_declare_symbol(ctx->analyzer, "inner_var", SYMBOL_VARIABLE,
                                           semantic_get_builtin_type(ctx->analyzer, "int"),
