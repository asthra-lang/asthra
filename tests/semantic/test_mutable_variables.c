...
ASTHRA_TEST_DEFINE(test_mutable_variable_assignment, ASTHRA_TEST_SEVERITY_CRITICAL) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, test_mutability_success(source, "mutable_variable_assignment"),
...
ASTHRA_TEST_DEFINE(test_mutable_variable_compound_assignment, ASTHRA_TEST_SEVERITY_CRITICAL) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, test_mutability_success(source, "mutable_compound_assignment"),
...
int main(void) {
    ...
    AsthraTestMetadata test_metadata[] = {test_mutable_variable_assignment_metadata,
                                          test_mutable_variable_compound_assignment_metadata};

    AsthraTestFunction test_functions[] = {test_mutable_variable_assignment,
                                           test_mutable_variable_compound_assignment};

    size_t test_count = sizeof(test_functions) / sizeof(test_functions[0]);

    AsthraTestSuiteConfig config = {.name = "Mutable Variable Tests",
                                    .description =
                                        "Tests for mutable variable semantics with mut keyword",
