 ...
 * Composite Type Tests for Semantic Analyzer
 *
 * Copyright (c) 2024 Asthra Project
 * Licensed under the terms specified in LICENSE
 *
 * Tests for composite types (slices, structs)
 */
...
ASTHRA_TEST_DEFINE(test_slice_type_checking, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, test_type_check_success(source, "slice_type_checking"),
                            "Should handle slice type declarations");

    return ASTHRA_TEST_PASS;
}

ASTHRA_TEST_DEFINE(test_slice_type_mismatch, ASTHRA_TEST_SEVERITY_HIGH) {
    const char *source = "package test;\n"
                         "pub fn main(none) -> void {\n"
                         "    let nums: []int = [\"a\", \"b\", \"c\"];\n"
                         "}\n";
    ...
    ASTHRA_TEST_ASSERT_TRUE(
        context, test_type_check_error(source, SEMANTIC_ERROR_TYPE_MISMATCH, "slice_type_mismatch"),
        "Should detect type mismatch in slice elements");

    return ASTHRA_TEST_PASS;
}

ASTHRA_TEST_DEFINE(test_struct_type_checking, ASTHRA_TEST_SEVERITY_HIGH) {
    const char *source = "package test;\n"
                         "pub struct Point {\n"
                         "    pub x: int,\n"
                         "    pub y: int\n"
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, test_type_check_success(source, "struct_type_checking"),
                            "Should handle struct type declarations and instantiation");

    return ASTHRA_TEST_PASS;
}

ASTHRA_TEST_DEFINE(test_struct_field_type_mismatch, ASTHRA_TEST_SEVERITY_HIGH) {
    const char *source = "package test;\n"
                         "pub struct Point {\n"
                         "    pub x: int,\n"
                         "    pub y: int\n"
    ...
    ASTHRA_TEST_ASSERT_TRUE(
        context,
        test_type_check_error(source, SEMANTIC_ERROR_TYPE_MISMATCH, "struct_field_type_mismatch"),
        "Should detect type mismatch in struct field initialization");
...
int main(void) {
    ...
    AsthraTestMetadata test_metadata[] = {
        test_slice_type_checking_metadata, test_slice_type_mismatch_metadata,
        test_struct_type_checking_metadata, test_struct_field_type_mismatch_metadata};

    AsthraTestFunction test_functions[] = {test_slice_type_checking, test_slice_type_mismatch,
                                           test_struct_type_checking,
                                           test_struct_field_type_mismatch};

    size_t test_count = sizeof(test_functions) / sizeof(test_functions[0]);

    AsthraTestSuiteConfig config = {.name = "Composite Type Tests",
                                    .description =
                                        "Tests for semantic analyzer composite type checking",
