...
ASTHRA_TEST_DEFINE(test_module_alias_registration, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    bool success =
        symbol_table_add_alias(ctx->analyzer->current_scope, "io", "std/io", module_table);
...
ASTHRA_TEST_DEFINE(test_import_resolution_undefined_module, ASTHRA_TEST_SEVERITY_MEDIUM) {
    ...
    SymbolTable *resolved =
        symbol_table_resolve_alias(ctx->analyzer->current_scope, "undefined_module");
