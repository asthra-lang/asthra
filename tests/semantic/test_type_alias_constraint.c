...
static AsthraTestResult test_type_alias_as_constraint_should_fail_gracefully(AsthraTestContext* context) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, has_errors, "Parser should report errors for unsupported type alias syntax");
...
static AsthraTestResult test_generic_with_constraint_syntax_should_fail(AsthraTestContext* context) {
    ...
    bool has_errors = parser_had_error(parser);
    ASTHRA_TEST_ASSERT_TRUE(context, has_errors, "Parser should report errors for constraint syntax");
...
static AsthraTestResult test_valid_generic_syntax_should_work(AsthraTestContext* context) {
    ...
    if (ast) {
        ...
        if (analyzer) {
            ...
            ASTHRA_TEST_ASSERT_TRUE(context, analysis_success, "Semantic analysis should succeed for valid generics");
