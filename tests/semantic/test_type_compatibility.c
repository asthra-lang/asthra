...
ASTHRA_TEST_DEFINE(test_assignment_compatibility, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, test_type_check_success(source, "assignment_compatibility"),
...
ASTHRA_TEST_DEFINE(test_assignment_incompatibility, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context,
                            test_type_check_error(source, SEMANTIC_ERROR_INCOMPATIBLE_TYPES,
                                                  "assignment_incompatibility"),
...
ASTHRA_TEST_DEFINE(test_binary_operator_type_mismatch, ASTHRA_TEST_SEVERITY_HIGH) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context,
                            test_type_check_error(source, SEMANTIC_ERROR_MISMATCHED_TYPES,
...
int main(void) {
    ...
    AsthraTestMetadata test_metadata[] = {
        test_assignment_compatibility_metadata, test_assignment_incompatibility_metadata,
        test_binary_operator_type_checking_metadata, test_binary_operator_type_mismatch_metadata};

    AsthraTestFunction test_functions[] = {
        test_assignment_compatibility, test_assignment_incompatibility,
        test_binary_operator_type_checking, test_binary_operator_type_mismatch};

    size_t test_count = sizeof(test_functions) / sizeof(test_functions[0]);

    AsthraTestSuiteConfig config = {.name = "Type Compatibility Tests",
                                    .description =
                                        "Tests for semantic analyzer type compatibility checking",
