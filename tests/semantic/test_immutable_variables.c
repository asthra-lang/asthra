...
ASTHRA_TEST_DEFINE(test_immutable_variable_read, ASTHRA_TEST_SEVERITY_CRITICAL) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        test_mutability_success(source, "immutable_variable_read"),
...
ASTHRA_TEST_DEFINE(test_immutable_variable_assignment_error, ASTHRA_TEST_SEVERITY_CRITICAL) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        test_mutability_error(source, SEMANTIC_ERROR_IMMUTABLE_MODIFICATION, "immutable_variable_assignment"),
        "Should error on assignment to immutable variable");
    
    return ASTHRA_TEST_PASS;
}

ASTHRA_TEST_DEFINE(test_immutable_variable_compound_assignment_error, ASTHRA_TEST_SEVERITY_CRITICAL) {
    const char* source = 
        "package test;\n"
        "\n"
        "pub fn main(none) -> void {\n"
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        test_mutability_error(source, SEMANTIC_ERROR_IMMUTABLE_MODIFICATION, "immutable_compound_assignment"),
...
int main(void) {
    ...
    AsthraTestMetadata test_metadata[] = {
        test_immutable_variable_read_metadata,
        test_immutable_variable_assignment_error_metadata,
        test_immutable_variable_compound_assignment_error_metadata
    };
    
    AsthraTestFunction test_functions[] = {
        test_immutable_variable_read,
