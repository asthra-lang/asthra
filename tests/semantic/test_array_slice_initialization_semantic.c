...
static AsthraTestResult test_repeated_array_syntax_not_yet_implemented(AsthraTestContext *context) {
    ...
    const char *non_const_code = "package test;\n"
                                 "pub fn test(none) -> void {\n"
                                 "    let count: i32 = 10;\n"
    ...
    ASTHRA_TEST_ASSERT(context,
                       test_semantic_error(non_const_code, SEMANTIC_ERROR_INVALID_EXPRESSION,
                                           "repeated_array_non_const"),
                       "Should fail with non-constant count");

    // Test with negative count (should fail)
    const char *negative_code = "package test;\n"
                                "pub fn test(none) -> void {\n"
                                "    let arr: []i32 = [0; -5];\n" // negative count
                                "    return ();\n"
                                "}\n";
    ...
    ASTHRA_TEST_ASSERT(context,
                       test_semantic_error(negative_code, SEMANTIC_ERROR_INVALID_EXPRESSION,
                                           "repeated_array_negative"),
...
static AsthraTestResult test_invalid_array_index_type(AsthraTestContext *context) {
    ...
    ASTHRA_TEST_ASSERT(
        context, test_semantic_error(code, SEMANTIC_ERROR_INVALID_OPERATION, "invalid_index_type"),
...
AsthraTestSuite *create_array_slice_initialization_semantic_test_suite(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create("Array/Slice Semantic Analysis",
    ...
    asthra_test_suite_add_test(
        suite, "test_repeated_array_syntax_not_yet_implemented",
    ...
    asthra_test_suite_add_test(suite, "test_empty_array_with_none",
                               "Test empty array syntax with 'none'", test_empty_array_with_none);

    asthra_test_suite_add_test(suite, "test_array_indexing_works", "Test array indexing operations",
    ...
    asthra_test_suite_add_test(suite, "test_array_length_operation", "Test array length operation",
