...
static AsthraTestResult test_parse_if_statements(AsthraTestContext* context) {
    const char* if_statements[] = {
        ...
        "if x > 0 { return x; } else if x < 0 { return -x; } else { return 0; }" // Grammar: if Expr Block else IfStmt
    };
...
static AsthraTestResult test_parse_match_statements(AsthraTestContext* context) {
    const char* match_statements[] = {
        "match value { 1 => { return \"one\"; } 2 => { return \"two\"; } _ => { return \"other\"; } }",  // Grammar: match Expr { MatchArm* }
        "match option { Option.Some(value) => { process(value); } Option.None => { handle_none(); } }"  // Grammar: MatchArm <- Pattern => Block
    };
...
static AsthraTestResult test_parse_return_statements(AsthraTestContext* context) {
    ...
    for (size_t i = 0; i < valid_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse valid return statement")) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
    ...
    for (size_t i = 0; i < invalid_count; i++) {
        ...
        if (result != NULL) {
            asthra_test_assert_bool(context, false, "Bare return should fail in v1.14+ but parsing succeeded");
            ast_free_node(result);
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_parse_loop_control_in_context(AsthraTestContext* context) {
    ...
    for (size_t i = 0; i < example_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse loop with control statements")) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestSuite* create_grammar_statements_test_suite(void) {
    AsthraTestSuite* suite = asthra_test_suite_create("Grammar Statement Tests", 
                                                     "Statement parsing testing");
    ...
    asthra_test_suite_add_test(suite, "test_parse_assignment_statements", 
                              ...
                              test_parse_assignment_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_if_statements", 
                              "Parse if statements", 
                              test_parse_if_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_for_statements", 
                              "Parse for statements", 
                              test_parse_for_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_match_statements", 
                              "Parse match statements", 
                              test_parse_match_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_return_statements", 
                              "Parse return statements", 
                              test_parse_return_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_block_statements", 
                              "Parse block statements", 
                              test_parse_block_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_unsafe_blocks", 
                              "Parse unsafe blocks", 
                              test_parse_unsafe_blocks);
    
    asthra_test_suite_add_test(suite, "test_parse_break_statements", 
                              "Parse break statements", 
                              test_parse_break_statements);
    
    asthra_test_suite_add_test(suite, "test_parse_continue_statements", 
