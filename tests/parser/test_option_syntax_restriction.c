...
static bool parse_fails(const char *source) {
    ...
    if (!parser)
        return true; // Failed to create parser counts as failure
...
AsthraTestSuite *create_option_syntax_restriction_test_suite(void) {
    AsthraTestSuite *suite = asthra_test_suite_create(
        "Option Syntax Restriction Tests", "Ensure Option values use explicit enum syntax");
    ...
    asthra_test_suite_add_test(suite, "test_option_some_correct_syntax",
                               "Option.Some(value) syntax works", test_option_some_correct_syntax);

    asthra_test_suite_add_test(suite, "test_option_none_correct_syntax", "Option.None syntax works",
