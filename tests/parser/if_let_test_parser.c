...
bool test_basic_if_let_parsing(void) {
    ...
    bool success = (if_let_stmt != NULL && 
                   if_let_stmt->type == AST_IF_LET_STMT &&
                   validate_if_let_ast(if_let_stmt));
    
    if (if_let_stmt) ast_free_node(if_let_stmt);
...
bool test_if_let_with_else(void) {
    ...
    bool success = (if_let_stmt != NULL && 
                   if_let_stmt->type == AST_IF_LET_STMT &&
                   validate_if_let_ast(if_let_stmt) &&
                   if_let_stmt->data.if_let_stmt.else_block != NULL);
...
bool test_nested_if_let(void) {
    const char *source = 
        "if let Option.Some(result) = get_result() {\n"
    ...
    bool success = (outer_stmt != NULL && 
                   outer_stmt->type == AST_IF_LET_STMT &&
                   validate_if_let_ast(outer_stmt));
    
    if (outer_stmt) ast_free_node(outer_stmt);
    destroy_test_parser(parser);
    return success;
}
