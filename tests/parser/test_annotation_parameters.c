...
static AsthraTestResult test_parse_annotation_parameters(AsthraTestContext* context) {
    const char* test_source = "#[validate(min=0, max=100)] pub fn set_percentage(value: i32) -> void {}";
...
static AsthraTestResult test_parse_nested_annotations(AsthraTestContext* context) {
    const char* test_source = "#[doc(description=\"Function\", version=\"1.0\")] pub fn documented_function(none) -> void {}";
    Parser* parser = create_test_parser(test_source);
...
static AsthraTestResult test_parse_complex_parameter_types(AsthraTestContext* context) {
    const char* test_cases[] = {
        ...
        "#[array(size=10, type=\"i32\")] pub fn process_array(none) -> void {}",            // String and integer parameters
        "#[config(enabled=true, debug=false)] pub fn configure(none) -> void {}",           // Boolean parameters
        "#[timeout(seconds=30, retries=3)] pub fn network_call(none) -> void {}"            // Multiple integer parameters
    };
    ...
    for (size_t i = 0; i < case_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse complex parameter types for case %zu", i)) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_parse_array_parameters(AsthraTestContext* context) {
    const char* test_source = "#[allowed_values(values=\"1,2,3,5,8\")] pub fn fibonacci_check(n: i32) -> bool {}";
    Parser* parser = create_test_parser(test_source);
    ...
    if (!asthra_test_assert_not_null(context, result, "Failed to parse array parameter annotation")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_parse_object_parameters(AsthraTestContext* context) {
    const char* test_source = "#[cache(type=\"LRU\", size=1000, ttl=3600)] pub fn cached_lookup(none) -> void {}";
    Parser* parser = create_test_parser(test_source);
    ...
    if (!asthra_test_assert_not_null(context, result, "Failed to parse object parameter annotation")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_parse_parameter_expressions(AsthraTestContext* context) {
    const char* test_source = "#[bounds(min=MIN_VALUE, max=MAX_VALUE)] pub fn bounded_function(none) -> void {}";
    Parser* parser = create_test_parser(test_source);
    ...
    if (!asthra_test_assert_not_null(context, result, "Failed to parse expression parameter annotation")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_parse_parameter_defaults(AsthraTestContext* context) {
    const char* test_source = "#[retry(attempts=3, delay=1000, exponential_backoff=true)] pub fn resilient_operation(none) -> void {}";
    Parser* parser = create_test_parser(test_source);
    ...
    if (!asthra_test_assert_not_null(context, result, "Failed to parse default parameter annotation")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestSuite* create_annotation_parameters_test_suite(void) {
    ...
    asthra_test_suite_add_test(suite, "test_parse_annotation_parameters", 
                              ...
                              test_parse_annotation_parameters);
    
    asthra_test_suite_add_test(suite, "test_parse_nested_annotations", 
                              "Parse nested annotations", 
                              test_parse_nested_annotations);
    
    asthra_test_suite_add_test(suite, "test_parse_annotation_validation", 
                              "Parse annotation validation", 
                              test_parse_annotation_validation);
    
    asthra_test_suite_add_test(suite, "test_parse_complex_parameter_types", 
                              "Parse complex parameter types", 
                              test_parse_complex_parameter_types);
    
    asthra_test_suite_add_test(suite, "test_parse_array_parameters", 
                              "Parse array parameters", 
                              test_parse_array_parameters);
    
    asthra_test_suite_add_test(suite, "test_parse_object_parameters", 
    ...
    asthra_test_suite_add_test(suite, "test_parse_parameter_expressions", 
                              ...
                              test_parse_parameter_expressions);
    
    asthra_test_suite_add_test(suite, "test_parse_parameter_defaults", 
