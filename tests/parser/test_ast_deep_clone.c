...
static AsthraTestResult test_struct_literal_deep_clone(AsthraTestContext *context) {
    ...
    field_init1->data.assignment.value =
        ast_create_node(AST_INTEGER_LITERAL, (SourceLocation){0, 0});
    ...
    field_init2->data.assignment.value =
        ast_create_node(AST_INTEGER_LITERAL, (SourceLocation){0, 0});
    ...
    asthra_test_assert_string_eq(context, clone->data.struct_literal.struct_name, "Point",
                                 "Struct name mismatch");

    // Verify type args are deep cloned
    ASTHRA_TEST_ASSERT(context, clone->data.struct_literal.type_args != NULL,
                       "Type args not cloned");
    ASTHRA_TEST_ASSERT(
        context, clone->data.struct_literal.type_args != original->data.struct_literal.type_args,
        "Type args not deep cloned");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.struct_literal.type_args),
                               (size_t)1, "Type args count mismatch");
    ...
    asthra_test_assert_string_eq(context, cloned_type_arg->data.base_type.name, "i32",
                                 "Type arg name mismatch");

    // Verify field inits are deep cloned
    ASTHRA_TEST_ASSERT(context, clone->data.struct_literal.field_inits != NULL,
                       "Field inits not cloned");
    ASTHRA_TEST_ASSERT(context,
                       clone->data.struct_literal.field_inits !=
                           original->data.struct_literal.field_inits,
                       "Field inits not deep cloned");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.struct_literal.field_inits),
                               (size_t)2, "Field inits count mismatch");
    ...
    asthra_test_assert_int_eq(context, cloned_field1->type, AST_ASSIGNMENT,
                              "First field type mismatch");
    asthra_test_assert_string_eq(context,
                                 cloned_field1->data.assignment.target->data.identifier.name, "x",
                                 "First field name mismatch");
    asthra_test_assert_long_eq(context,
                               cloned_field1->data.assignment.value->data.integer_literal.value,
                               (int64_t)10, "First field value mismatch");
    ...
    asthra_test_assert_int_eq(context, cloned_field2->type, AST_ASSIGNMENT,
                              "Second field type mismatch");
    asthra_test_assert_string_eq(context,
                                 cloned_field2->data.assignment.target->data.identifier.name, "y",
                                 "Second field name mismatch");
    asthra_test_assert_long_eq(context,
                               cloned_field2->data.assignment.value->data.integer_literal.value,
                               (int64_t)20, "Second field value mismatch");
...
static AsthraTestResult test_enum_decl_deep_clone(AsthraTestContext *context) {
    ...
    variant1->data.enum_variant_decl.associated_type =
        ast_create_node(AST_IDENTIFIER, (SourceLocation){0, 0});
    ...
    asthra_test_assert_string_eq(context, clone->data.enum_decl.name, "Option",
                                 "Enum name mismatch");
    asthra_test_assert_int_eq(context, (int)clone->data.enum_decl.visibility,
                              (int)VISIBILITY_PUBLIC, "Visibility mismatch");

    // Verify type params are deep cloned
    ASTHRA_TEST_ASSERT(context, clone->data.enum_decl.type_params != NULL,
                       "Type params not cloned");
    ASTHRA_TEST_ASSERT(context,
                       clone->data.enum_decl.type_params != original->data.enum_decl.type_params,
                       "Type params not deep cloned");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.enum_decl.type_params),
                               (size_t)1, "Type params count mismatch");

    // Verify variants are deep cloned
    ASTHRA_TEST_ASSERT(context, clone->data.enum_decl.variants != NULL, "Variants not cloned");
    ASTHRA_TEST_ASSERT(context, clone->data.enum_decl.variants != original->data.enum_decl.variants,
                       "Variants not deep cloned");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.enum_decl.variants),
                               (size_t)2, "Variants count mismatch");
    ...
    asthra_test_assert_string_eq(context, cloned_variant1->data.enum_variant_decl.name, "Some",
                                 "Variant name mismatch");
    ASTHRA_TEST_ASSERT(context, cloned_variant1->data.enum_variant_decl.associated_type != NULL,
                       "Associated type not cloned");
    asthra_test_assert_string_eq(
        context, cloned_variant1->data.enum_variant_decl.associated_type->data.identifier.name, "T",
        "Associated type name mismatch");

    // Verify annotations are deep cloned
    ASTHRA_TEST_ASSERT(context, clone->data.enum_decl.annotations != NULL,
                       "Annotations not cloned");
    ASTHRA_TEST_ASSERT(context,
                       clone->data.enum_decl.annotations != original->data.enum_decl.annotations,
                       "Annotations not deep cloned");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.enum_decl.annotations),
                               (size_t)1, "Annotations count mismatch");
...
static AsthraTestResult test_function_decl_deep_clone(AsthraTestContext *context) {
    ...
    // Set return type
    original->data.function_decl.return_type =
        ast_create_node(AST_BASE_TYPE, (SourceLocation){0, 0});
    ...
    asthra_test_assert_string_eq(context, clone->data.function_decl.name, "calculate",
                                 "Function name mismatch");
    asthra_test_assert_int_eq(context, (int)clone->data.function_decl.visibility,
                              (int)VISIBILITY_PUBLIC, "Visibility mismatch");

    // Verify params are deep cloned
    ASTHRA_TEST_ASSERT(context, clone->data.function_decl.params != NULL, "Params not cloned");
    ASTHRA_TEST_ASSERT(context,
                       clone->data.function_decl.params != original->data.function_decl.params,
                       "Params not deep cloned");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.function_decl.params),
                               (size_t)2, "Params count mismatch");

    // Verify return type is cloned
    ASTHRA_TEST_ASSERT(context, clone->data.function_decl.return_type != NULL,
                       "Return type not cloned");
    ASTHRA_TEST_ASSERT(
        context, clone->data.function_decl.return_type != original->data.function_decl.return_type,
        "Return type not deep cloned");
    asthra_test_assert_string_eq(context,
                                 clone->data.function_decl.return_type->data.base_type.name, "i32",
    ...
    ASTHRA_TEST_ASSERT(context, clone->data.function_decl.body != original->data.function_decl.body,
                       "Body not deep cloned");
    ASTHRA_TEST_ASSERT(context, clone->data.function_decl.body->data.block.statements != NULL,
                       "Body statements not cloned");
    ASTHRA_TEST_ASSERT(context,
                       clone->data.function_decl.body->data.block.statements !=
                           original->data.function_decl.body->data.block.statements,
                       "Body statements not deep cloned");
...
static AsthraTestResult test_clone_independence(AsthraTestContext *context) {
    ...
    asthra_test_assert_size_eq(context, ast_node_list_size(original->data.array_literal.elements),
                               (size_t)2, "Original was modified");
    asthra_test_assert_size_eq(context, ast_node_list_size(clone->data.array_literal.elements),
                               (size_t)3, "Clone was not modified");
    ...
    asthra_test_assert_long_eq(context, orig_elem1->data.integer_literal.value, (int64_t)1,
                               "Original element was modified");
    asthra_test_assert_long_eq(context, cloned_elem1->data.integer_literal.value, (int64_t)100,
                               "Clone element was not modified");
...
int main(int argc, char *argv[]) {
    ...
    AsthraTestMetadata metadata = {.name = "AST Deep Clone Tests",
                                   .file = __FILE__,
                                   .line = __LINE__,
                                   .function = "main",
