...
static AsthraTestResult test_single_annotation_in_function_context(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT_NOT_NULL(context, func,
                                         "Function with SafeFFIAnnotation should parse: %s",
                                         function_sources[i])) {
            parser_destroy(parser);
            lexer_destroy(lexer);
            return ASTHRA_TEST_FAIL;
        ...
        if (!ASTHRA_TEST_ASSERT_INT_EQ(context, func->type, AST_FUNCTION_DECL,
                                       "Should be AST_FUNCTION_DECL for: %s",
                                       function_sources[i])) {
            ast_free_node(func);
            parser_destroy(parser);
            lexer_destroy(lexer);
...
void register_safe_ffi_annotation_parser_tests(void) {
    asthra_test_register_suite("SafeFFIAnnotation Parser", setup_parser_tests,
                               teardown_parser_tests);

    // Core annotation parsing tests
    asthra_test_register_test("SafeFFIAnnotation Parser", "Parse Transfer Full Annotation",
                              test_parse_transfer_full_annotation);
    asthra_test_register_test("SafeFFIAnnotation Parser", "Parse Transfer None Annotation",
                              test_parse_transfer_none_annotation);
    asthra_test_register_test("SafeFFIAnnotation Parser", "Parse Borrowed Annotation",
                              test_parse_borrowed_annotation);

    // Error handling tests
    asthra_test_register_test("SafeFFIAnnotation Parser", "Reject Invalid Annotation Names",
                              test_reject_invalid_annotation_names);
    asthra_test_register_test("SafeFFIAnnotation Parser", "Reject Malformed Annotation Syntax",
                              test_reject_malformed_annotation_syntax);
    asthra_test_register_test("SafeFFIAnnotation Parser", "Parser Error Recovery",
                              test_parser_error_recovery);

    // Context integration tests
    asthra_test_register_test("SafeFFIAnnotation Parser", "Single Annotation in Function Context",
                              test_single_annotation_in_function_context);
    asthra_test_register_test("SafeFFIAnnotation Parser", "Single Annotation in Extern Context",
                              test_single_annotation_in_extern_context);
    asthra_test_register_test("SafeFFIAnnotation Parser", "Annotation in Complete Program Context",
                              test_annotation_in_complete_program_context);

    // Safety validation test
    asthra_test_register_test("SafeFFIAnnotation Parser", "Mutual Exclusivity at Parser Level",
...
int main(void) {
    ...
    if (result == 0) {
        ...
        printf("parse_safe_ffi_annotation() correctly handles single annotations and rejects "
               "conflicts.\n");
    } else {
