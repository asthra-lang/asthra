...
static bool verify_sizeof_expr(ASTNode *expr) {
    if (!expr || expr->type != AST_UNARY_EXPR)
        return false;
    ...
    return (operand->type == AST_BASE_TYPE || operand->type == AST_STRUCT_TYPE ||
            operand->type == AST_ENUM_TYPE || operand->type == AST_SLICE_TYPE ||
            operand->type == AST_ARRAY_TYPE || operand->type == AST_PTR_TYPE ||
            operand->type == AST_RESULT_TYPE || operand->type == AST_OPTION_TYPE ||
