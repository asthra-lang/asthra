...
static AsthraTestResult test_if_let_simple_identifier(AsthraTestContext* context) {
    const char* test_cases[] = {
        "if let x = 42 { }",
        "if let value = compute() { }",
        "if let _ignored = 42 { }",
        "if let mut data = get_data() { }",
        NULL
    };
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_not_null(context, parser, "Failed to create parser for: %s", test_cases[i])) {
            return ASTHRA_TEST_FAIL;
        }
        
        ASTNode* result = parse_if_stmt(parser);
        
        if (!asthra_test_assert_not_null(context, result, "Failed to parse if-let: %s", test_cases[i])) {
...
static AsthraTestResult test_if_let_enum_patterns(AsthraTestContext* context) {
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse if-let enum: %s", test_cases[i])) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_if_let_struct_patterns(AsthraTestContext* context) {
    const char* test_cases[] = {
        "if let Point { x: a, y: b } = get_point() { }",
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse if-let struct: %s", test_cases[i])) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_if_let_nested_patterns(AsthraTestContext* context) {
    const char* test_cases[] = {
        "if let Option.Some(Result.Ok(value)) = nested { }",
...
static AsthraTestResult test_unary_complex_combinations(AsthraTestContext* context) {
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse complex unary: %s", test_cases[i])) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_unary_with_primaries(AsthraTestContext* context) {
    ...
    for (int i = 0; test_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse unary with primary: %s", test_cases[i])) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_unary_invalid_combinations(AsthraTestContext* context) {
    const char* invalid_cases[] = {
        "&&var",          // Double address-of (not allowed by grammar)
    ...
    for (int i = 0; invalid_cases[i] != NULL; i++) {
        ...
        if (!asthra_test_assert_not_null(context, parser, "Failed to create parser for: %s", invalid_cases[i])) {
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_pattern_reserved_keywords(AsthraTestContext* context) {
    const char* test_cases[] = {
        "match value { true => { } }",           // Boolean literal pattern
