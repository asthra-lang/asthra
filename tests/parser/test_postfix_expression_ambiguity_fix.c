...
static int test_valid_associated_function_tokenization(void) {
    ...
    if (token1.type == TOKEN_IDENTIFIER &&
        ...
        token3.type == TOKEN_IDENTIFIER && token4.type == TOKEN_IDENTIFIER &&
...
static int test_generic_type_tokenization(void) {
    ...
    if (token1.type == TOKEN_IDENTIFIER && token2.type == TOKEN_LESS_THAN &&
        token3.type == TOKEN_I32 && // i32 is a type keyword
        token4.type == TOKEN_GREATER_THAN && token5.type == TOKEN_DOUBLE_COLON &&
        token6.type == TOKEN_IDENTIFIER) {
        ...
        printf("  Expected: IDENTIFIER(%d), LESS_THAN(%d), I32(%d), GREATER_THAN(%d), "
               "DOUBLE_COLON(%d), IDENTIFIER(%d)\n",
               TOKEN_IDENTIFIER, TOKEN_LESS_THAN, TOKEN_I32, TOKEN_GREATER_THAN, TOKEN_DOUBLE_COLON,
               TOKEN_IDENTIFIER);
        printf("  Got: %d, %d, %d, %d, %d, %d\n", token1.type, token2.type, token3.type,
               token4.type, token5.type, token6.type);
        return 0;
    }
}
...
int main(void) {
    ...
    total++;
    if (test_double_colon_token_recognition())
        passed++;
    total++;
    if (test_valid_associated_function_tokenization())
        passed++;
    total++;
    if (test_generic_type_tokenization())
        passed++;
