...
static AsthraTestResult test_parse_function_declaration(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_int_eq(context, result->type, AST_PROGRAM, "Should be AST_PROGRAM")) {
        ast_free_node(result);
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestSuite *create_grammar_toplevel_test_suite(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create("Grammar Top-Level Tests", "Top-level grammar production testing");
    ...
    asthra_test_suite_add_test(suite, "test_parse_package_declaration",
                               "Parse package declarations", test_parse_package_declaration);

    asthra_test_suite_add_test(suite, "test_parse_import_declaration", "Parse import declarations",
                               test_parse_import_declaration);

    asthra_test_suite_add_test(suite, "test_parse_function_declaration",
                               "Parse function declarations", test_parse_function_declaration);

    asthra_test_suite_add_test(suite, "test_parse_struct_declaration", "Parse struct declarations",
                               test_parse_struct_declaration);

    asthra_test_suite_add_test(suite, "test_parse_extern_declaration", "Parse extern declarations",
