 ...
 */
static bool verify_binary_expr(ASTNode* node, BinaryOperator expected_op, 
                              ASTNode** left_out, ASTNode** right_out) {
    ...
    if (left_out) *left_out = node->data.binary_expr.left;
    if (right_out) *right_out = node->data.binary_expr.right;
...
static AsthraTestResult test_complex_bitwise_precedence(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_not_null(context, result, "Failed to parse complex bitwise expression")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestSuite* create_operator_precedence_test_suite(void) {
    ...
    asthra_test_suite_add_test(suite, "test_shift_operator_precedence", 
                              "Fix shift operator precedence", 
                              test_shift_operator_precedence);
    ...
    asthra_test_suite_add_test(suite, "test_memory_address_calculation", 
                              "Memory address calculation fix", 
                              test_memory_address_calculation);
