...
static AsthraTestResult test_nested_structure_validation(AsthraTestContext* context) {
    const char* test_source = 
        "fn complex_function(data: Point) -> Result<i32, String> {\n"
        "    if (data.x > 0) {\n"
...
static AsthraTestResult test_ast_memory_management(AsthraTestContext* context) {
    ...
    if (final_memory > initial_memory + 1024) { // Allow 1KB variation
        asthra_test_log(context, "Warning: Potential memory leak detected. "
                               "Initial: %zu, Final: %zu", initial_memory, final_memory);
    }
...
static AsthraTestResult test_multi_statement_block_handling(AsthraTestContext* context) {
    const char* test_source = 
        "fn multi_statement_function(none) {\n"
        "    let a: i32 = 1;\n"
        "    let b: i32 = 2;\n"
        "    let c: i32 = 3;\n"
        "    let d: i32 = 4;\n"
...
AsthraTestSuite* create_ast_complex_structures_test_suite(void) {
    AsthraTestSuite* suite = asthra_test_suite_create("AST Complex Structures Tests", 
                                                     "Complex nested AST structures and memory management");
    ...
    asthra_test_suite_add_test(suite, "test_nested_structure_validation", 
                              ...
                              test_nested_structure_validation);
    
    asthra_test_suite_add_test(suite, "test_ast_memory_management", 
                              "AST memory management", 
                              test_ast_memory_management);
    
    asthra_test_suite_add_test(suite, "test_deep_nesting_handling", 
                              "Deep nesting handling", 
                              test_deep_nesting_handling);
    
    asthra_test_suite_add_test(suite, "test_complex_expression_trees", 
    ...
    asthra_test_suite_add_test(suite, "test_ast_size_complexity_limits", 
                              "AST size and complexity limits", 
                              test_ast_size_complexity_limits);
    ...
    asthra_test_suite_add_test(suite, "test_large_ast_stress_test", 
                              "Large AST stress test", 
