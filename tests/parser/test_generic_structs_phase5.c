 ...
 */
static bool check_type_params(ASTNodeList *type_params, const char **expected_names, size_t expected_count) {
    if (!type_params && expected_count == 0) return true;
    if (!type_params || !expected_names) return false;
    ...
    for (size_t i = 0; i < expected_count; i++) {
        ...
        if (!param || param->type != AST_IDENTIFIER) return false;
        if (strcmp(param->data.identifier.name, expected_names[i]) != 0) return false;
    }
 ...
 */
static bool check_type_args(ASTNodeList *type_args, const char **expected_types, size_t expected_count) {
    if (!type_args && expected_count == 0) return true;
    if (!type_args || !expected_types) return false;
    ...
    for (size_t i = 0; i < expected_count; i++) {
        ...
        if (arg->type == AST_IDENTIFIER) {
            if (strcmp(arg->data.identifier.name, expected_types[i]) != 0) return false;
        } else if (arg->type == AST_STRUCT_TYPE) {
            if (strcmp(arg->data.struct_type.name, expected_types[i]) != 0) return false;
        } else {
...
static bool test_multiple_type_parameters(void) {
    ...
    TEST_ASSERT(check_type_params(type_params, expected_params, 2), "Type parameters are 'A' and 'B'");
...
static bool test_complex_generic_type_arguments(void) {
    ...
    TEST_ASSERT(check_type_args(type_args, expected_args, 2), "Type arguments are 'string' and 'User'");
...
static bool test_generic_struct_pattern_parsing(void) {
    ...
    TEST_ASSERT(check_type_args(type_args, expected_args, 2), "Type arguments are 'i32' and 'string'");
...
static bool test_nested_generic_types(void) {
    ...
    TEST_ASSERT(strcmp(inner_type->data.struct_type.name, "Vec") == 0, "Inner struct name is 'Vec'");
    ...
    TEST_ASSERT(check_type_args(inner_type_args, expected_inner_args, 1), "Inner type argument is 'i32'");
