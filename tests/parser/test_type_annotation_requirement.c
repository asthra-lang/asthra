...
static AsthraTestResult test_valid_type_annotations(AsthraTestContext *context) {
    const char *valid_declarations[] = {
        "let x: i32 = 42;",           "let name: string = \"hello\";",
        "let is_valid: bool = true;", "let pi: f64 = 3.14159;",
        "let count: usize = 100;",    "let result: Result<i32, string> = Result.Ok(42);"};
    ...
    for (size_t i = 0; i < decl_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result,
                                         "Failed to parse valid type annotation")) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }

        // Verify that type is not NULL
        if (!asthra_test_assert_not_null(context, result->data.let_stmt.type,
                                         "Type should not be NULL in v1.15+")) {
...
static AsthraTestResult test_missing_type_annotations_rejected(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < decl_count; i++) {
        ...
        if (!asthra_test_assert_null(
                context, result, "Should reject variable declaration without type annotation")) {
            if (result)
                ast_free_node(result);
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        ...
        if (!asthra_test_assert_true(
                context, parser_has_error(parser),
                "Parser should have error state after rejecting invalid syntax")) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_version_compatibility(AsthraTestContext *context) {
    ...
    const char *v1_14_code[] = {"let x = 42;", "let name = \"hello\";", "let items = [];"};
    ...
    for (size_t i = 0; i < code_count; i++) {
        ...
        if (!asthra_test_assert_null(context, result,
                                     "v1.14 syntax should be rejected in v1.15+")) {
            if (result)
                ast_free_node(result);
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
...
static AsthraTestResult test_error_message_quality(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_true(context, parser_has_error(parser),
                                 "Parser should have error state with helpful message")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestSuite *create_type_annotation_requirement_test_suite(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create("Type Annotation Requirement Tests (v1.15+)",
