...
static AsthraTestResult test_parse_pub_struct(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, struct_decl->type == AST_STRUCT_DECL,
                            "Node should be AST_STRUCT_DECL")) {
...
static AsthraTestResult test_parse_impl_block(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, impl_block->type == AST_IMPL_BLOCK,
                            "Node should be AST_IMPL_BLOCK")) {
...
static AsthraTestResult test_parse_self_parameter(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, method->type == AST_METHOD_DECL,
                            "Node should be AST_METHOD_DECL")) {
...
static AsthraTestResult test_parse_associated_function_call(AsthraTestContext *context) {
    ...
    if (!ASTHRA_TEST_ASSERT(context, call_expr != NULL,
                            "Associated function call should be parsed")) {
        cleanup_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(context,
                            strcmp(call_expr->data.associated_func_call.struct_name, "Point") == 0,
                            "Struct name should be 'Point'")) {
        ast_free_node(call_expr);
        cleanup_parser(parser);
        return ASTHRA_TEST_FAIL;
    ...
    if (!ASTHRA_TEST_ASSERT(context,
                            strcmp(call_expr->data.associated_func_call.function_name, "new") == 0,
