...
static AsthraTestResult test_ast_visitor_pattern(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_int_gt(context, visit_count, 0, "Should visit at least one node")) {
        ast_free_node(ast);
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_ast_traversal_performance(AsthraTestContext *context) {
    ...
    const char *test_source = "fn performance_test(none) {\n"
                              "    let a: i32 = 1;\n"
                              "    let b: i32 = 2;\n"
                              "    let c: i32 = 3;\n"
                              "    let d: i32 = 4;\n"
...
AsthraTestSuite *create_ast_traversal_test_suite(void) {
    AsthraTestSuite *suite = asthra_test_suite_create(
        "AST Traversal Tests", "AST traversal algorithms and visitor patterns");
    ...
    // AST traversal tests
    asthra_test_suite_add_test(suite, "test_ast_traversal_preorder", "AST preorder traversal",
                               test_ast_traversal_preorder);

    asthra_test_suite_add_test(suite, "test_ast_traversal_postorder", "AST postorder traversal",
                               test_ast_traversal_postorder);

    asthra_test_suite_add_test(suite, "test_ast_visitor_pattern", "AST visitor pattern",
                               test_ast_visitor_pattern);

    asthra_test_suite_add_test(suite, "test_ast_visitor_data_collection",
                               "AST visitor data collection", test_ast_visitor_data_collection);
    ...
    asthra_test_suite_add_test(suite, "test_ast_traversal_performance", "AST traversal performance",
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {.name = "AST Traversal Tests",
                                   .file = __FILE__,
                                   .line = __LINE__,
                                   .function = "main",
