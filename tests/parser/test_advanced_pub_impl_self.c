...
static AsthraTestResult test_parse_nested_impl_blocks(AsthraTestContext *context) {
    const char *source = "priv struct Point { x: f64, y: f64 }\n"
                         "impl Point {\n"
    ...
    if (!ASTHRA_TEST_ASSERT(context, decl_count == 2,
                            "Program should have 2 declarations (struct + impl)")) {
        ast_free_node(program);
        cleanup_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_parse_visibility_modifiers(AsthraTestContext *context) {
    const char *source = "pub struct Point {\n"
                         "    pub x: f64,\n"
                         "    priv y: f64\n" // explicit private
                         "}\n"
                         "impl Point {\n"
