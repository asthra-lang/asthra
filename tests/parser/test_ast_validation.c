...
static AsthraTestResult test_ast_structure_consistency(AsthraTestContext* context) {
    const char* test_source = "fn test(x: i32) -> i32 { if (x > 0) { return x; } else { return -x; } }";
...
static AsthraTestResult test_ast_parent_child_relationships(AsthraTestContext* context) {
    ...
    // Note: Parent field checking removed as current AST implementation doesn't include parent pointers
    // This is a design choice for simplicity and performance
    // Parent relationships can be tracked during traversal if needed
...
static AsthraTestResult test_ast_node_integrity_validation(AsthraTestContext* context) {
    const char* test_source = "fn integrity_test(none) { let a: i32 = 1; let b: i32 = a + 2; return b; }";
    Parser* parser = create_test_parser(test_source);
...
static AsthraTestResult test_ast_semantic_consistency(AsthraTestContext* context) {
    const char* test_source = "fn semantic_test(x: i32) -> i32 { if (x > 0) { return x; } return 0; }";
    Parser* parser = create_test_parser(test_source);
...
AsthraTestSuite* create_ast_validation_test_suite(void) {
    AsthraTestSuite* suite = asthra_test_suite_create("AST Validation Tests", 
                                                     "AST structure validation and consistency checks");
    ...
    // AST validation tests
    asthra_test_suite_add_test(suite, "test_ast_structure_consistency", 
    ...
    asthra_test_suite_add_test(suite, "test_ast_type_checking_integration", 
                              "AST type checking integration", 
                              test_ast_type_checking_integration);
    
    asthra_test_suite_add_test(suite, "test_ast_node_integrity_validation", 
                              "AST node integrity validation", 
                              test_ast_node_integrity_validation);
    
    asthra_test_suite_add_test(suite, "test_ast_semantic_consistency", 
                              "AST semantic consistency", 
                              test_ast_semantic_consistency);
    
    asthra_test_suite_add_test(suite, "test_ast_structural_validity", 
                              "AST structural validity", 
                              test_ast_structural_validity);
    
    asthra_test_suite_add_test(suite, "test_ast_error_handling_validation", 
                              "AST error handling validation", 
                              test_ast_error_handling_validation);
