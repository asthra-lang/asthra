...
static AsthraTestResult test_ast_node_list_operations(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_not_null(context, node1, "Failed to create node1") ||
        ...
        if (node1) ast_free_node(node1);
        if (node2) ast_free_node(node2);
        if (node3) ast_free_node(node3);
        ast_node_list_destroy(list);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_ast_node_memory_lifecycle(AsthraTestContext* context) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (!asthra_test_assert_not_null(context, node, "Failed to create node in lifecycle test")) {
            return ASTHRA_TEST_FAIL;
        }
    ...
    if (final_memory > initial_memory + 512) {
        asthra_test_log(context, "Warning: Potential memory leak in node lifecycle. "
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {
        .name = "AST Node Management Tests",
        .file = __FILE__,
        .line = __LINE__,
        .function = "main",
