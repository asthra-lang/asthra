...
static AsthraTestResult test_valid_single_operators(AsthraTestContext* context) {
    ...
    for (size_t i = 0; i < expr_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, parser, "Failed to create parser for: %s", valid_expressions[i])) {
            return ASTHRA_TEST_FAIL;
        }
        
        ASTNode* result = parse_expr(parser);
        
        if (!asthra_test_assert_not_null(context, result, "Failed to parse valid expression: %s", valid_expressions[i])) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
        
        // Verify it's a unary expression
        if (!asthra_test_assert_int_eq(context, result->type, AST_UNARY_EXPR, "Expected unary expression for: %s", valid_expressions[i])) {
...
static AsthraTestResult test_valid_logical_pointer_combinations(AsthraTestContext* context) {
    ...
    for (size_t i = 0; i < expr_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, result, "Failed to parse valid combination: %s", valid_expressions[i])) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_invalid_multiple_logical(AsthraTestContext* context) {
    ...
    for (size_t i = 0; i < expr_count; i++) {
        ...
        if (!asthra_test_assert_not_null(context, parser, "Failed to create parser for: %s", invalid_expressions[i])) {
            return ASTHRA_TEST_FAIL;
        }
...
AsthraTestSuite* create_unary_operator_restrictions_test_suite(void) {
    ...
    asthra_test_suite_add_test(suite, "test_valid_single_operators", 
                              "Test valid single unary operators", 
                              test_valid_single_operators);
