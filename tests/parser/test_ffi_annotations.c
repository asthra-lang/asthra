...
static AsthraTestResult test_parse_ffi_param_annotations(AsthraTestContext *context) {
    const char *test_source =
        "extern fn ffi_function(#[borrowed] data: *u8, #[transfer_full] result: *i32) -> void;";
    ...
    if (!asthra_test_assert_not_null(context, result,
                                     "Failed to parse FFI parameter annotations")) {
...
static AsthraTestResult test_parse_library_link_annotations(AsthraTestContext *context) {
    ...
    for (size_t i = 0; i < case_count; i++) {
        ...
        if (!asthra_test_assert_not_null(
                context, result, "Failed to parse library link annotation for case %zu", i)) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_parse_calling_convention_annotations(AsthraTestContext *context) {
    const char *test_cases[] = {"extern fn cdecl_function(none) -> void;",
    ...
    for (size_t i = 0; i < case_count; i++) {
        ...
        if (!asthra_test_assert_not_null(
                context, result, "Failed to parse calling convention annotation for case %zu", i)) {
            destroy_test_parser(parser);
            return ASTHRA_TEST_FAIL;
        }
...
static AsthraTestResult test_parse_memory_safety_annotations(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_not_null(context, result,
                                     "Failed to parse memory safety annotations")) {
        destroy_test_parser(parser);
        return ASTHRA_TEST_FAIL;
    }
...
static AsthraTestResult test_parse_callback_annotations(AsthraTestContext *context) {
    const char *test_source = "extern fn register_callback(#[transfer_none] cb: *mut u8, "
                              "#[transfer_none] user_data: *mut u8) -> void;";
...
AsthraTestSuite *create_ffi_annotations_test_suite(void) {
    AsthraTestSuite *suite =
        asthra_test_suite_create("FFI Annotation Tests", "FFI annotation parsing testing");
    ...
    asthra_test_suite_add_test(suite, "test_parse_ffi_param_annotations",
                               "Parse FFI parameter annotations", test_parse_ffi_param_annotations);

    asthra_test_suite_add_test(suite, "test_parse_extern_annotations", "Parse extern annotations",
                               test_parse_extern_annotations);

    asthra_test_suite_add_test(suite, "test_parse_transfer_semantics", "Parse transfer semantics",
                               test_parse_transfer_semantics);

    asthra_test_suite_add_test(suite, "test_parse_c_abi_annotations", "Parse C ABI annotations",
    ...
    asthra_test_suite_add_test(suite, "test_parse_buffer_size_annotations",
                               "Parse buffer size annotations", test_parse_buffer_size_annotations);

    asthra_test_suite_add_test(suite, "test_parse_callback_annotations",
                               "Parse callback annotations", test_parse_callback_annotations);
