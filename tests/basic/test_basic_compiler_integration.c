...
static void add_compiler_error(EnhancedAsthraCompilerContext* ctx, const char* message, 
                              int line, int column, const char* phase, int severity) {
...
CompilationResult* asthra_compiler_compile_source(AsthraCompilerContext* context, 
                                                 ...
                                                 const char* output_path) {
    if (!context || !source_code) return NULL;
...
CompilationResult* asthra_compiler_compile_file(AsthraCompilerContext* context, 
                                               ...
                                               const char* output_path) {
    if (!context || !input_path) return NULL;
 ...
 */
void asthra_compiler_get_statistics(AsthraCompilerContext* context, 
                                   size_t* files_compiled,
    ...
    if (files_compiled) *files_compiled = atomic_load(&ctx->files_compiled);
    if (lines_processed) *lines_processed = atomic_load(&ctx->lines_processed);
    if (total_time_ms) *total_time_ms = ctx->compilation_time_ms;
    if (memory_usage) *memory_usage = ctx->memory_usage_bytes;
}
