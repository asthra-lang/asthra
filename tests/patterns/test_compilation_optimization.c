...
static bool test_decision_tree_optimization(void) {
    ...
    // Test various points
    StructValue origin = create_struct_value((StructValue[]){
        {"x", create_i32_value(0)},
        {"y", create_i32_value(0)}
    }, 2);
    String result_str = execute_pattern_match_with_value(result, &origin);
    ASSERT_STRING_EQ(result_str.data, "origin");
    
    StructValue first_quad = create_struct_value((StructValue[]){
        {"x", create_i32_value(5)},
        {"y", create_i32_value(3)}
    }, 2);
...
static bool test_compilation_time_performance(void) {
    ...
    const char* complex_source = 
        ...
        "    match s {\n"
        "        Shape::Circle { center: Point { x: 0, y: 0 }, radius: r } if r > 0.0 => \"origin circle\",\n"
        "        Shape::Circle { center, radius } if radius > 10.0 => \"large circle\",\n"
        "        Shape::Circle { .. } => \"small circle\",\n"
        "        Shape::Rectangle { top_left: Point { x: x1, y: y1 }, bottom_right: Point { x: x2, y: y2 } } if (x2-x1) == (y2-y1) => \"square\",\n"
...
static bool test_optimization_levels(void) {
    ...
    const char* source = 
        "pub enum Color { Red, Green, Blue, Yellow, Orange }\n"
...
int main(void) {
    ...
    // Pattern compilation optimization tests
    if (!test_basic_pattern_optimization()) all_tests_passed = false;
    if (!test_decision_tree_optimization()) all_tests_passed = false;
    if (!test_compilation_time_performance()) all_tests_passed = false;
    if (!test_optimization_levels()) all_tests_passed = false;
    ...
    if (all_tests_passed) {
        ...
        printf("📊 Pattern Matching Expansion Plan: Compilation Optimization Features Demonstrated\n");
        return 0;
    } else {
