...
static bool test_complex_boolean_guards(void) {
    ...
    TupleValue test_tuple = create_tuple_value((Value[]){
        create_i32_value(25),
        create_f64_value(92.5)
    }, 2);
...
static bool test_mathematical_guards(void) {
    ...
    // Test first quadrant
    test_tuple = create_tuple_value((Value[]){
        create_f64_value(3.0),
        create_f64_value(2.0)
    }, 2);
    result_str = execute_pattern_match_with_value(pattern_result, &test_tuple);
...
static bool test_function_call_guards(void) {
    ...
    const char* source = 
        "fn analyze_list(items: List<i32>) -> String {\n"
        "    match items {\n"
...
int main(void) {
    ...
    // Complex guard condition tests
    if (!test_complex_boolean_guards()) all_tests_passed = false;
    if (!test_mathematical_guards()) all_tests_passed = false;
    if (!test_multiple_guard_conditions()) all_tests_passed = false;
    if (!test_function_call_guards()) all_tests_passed = false;
