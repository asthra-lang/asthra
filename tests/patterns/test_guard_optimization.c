...
static bool test_guard_optimization(void) {
    ...
    const char* source = 
        "fn optimized_guards(x: i32) -> String {\n"
        "    match x {\n"
...
static bool test_range_optimization(void) {
    ...
    const char* source = 
        "fn optimized_ranges(x: i32) -> String {\n"
        "    match x {\n"
...
static bool test_guard_error_handling(void) {
    ...
    const char* type_mismatch_source = 
        "fn type_mismatch_guard(x: i32) -> String {\n"
        "    match x {\n"
...
static bool test_guard_compilation_errors(void) {
    ...
    const char* undefined_var_source = 
        "fn undefined_var_guard(x: i32) -> String {\n"
        "    match x {\n"
    ...
    const char* wrong_return_source = 
        "fn wrong_return_guard(x: i32) -> String {\n"
        "    match x {\n"
...
static bool test_guard_performance(void) {
    ...
    TupleValue test_data = create_tuple_value((Value[]){
        create_i32_value(3),
        create_i32_value(4),
        create_i32_value(5),
        create_i32_value(6)
...
static bool test_guard_memory_efficiency(void) {
    ...
    const char* source = 
        "fn memory_efficient_guards(x: i32) -> String {\n"
        "    match x {\n"
        "        n if n % 2 == 0 => \"even\",\n"
...
int main(void) {
    ...
    // Optimization tests
    if (!test_guard_optimization()) all_tests_passed = false;
    if (!test_range_optimization()) all_tests_passed = false;
    if (!test_short_circuit_optimization()) all_tests_passed = false;
    
    // Error handling tests
    if (!test_guard_error_handling()) all_tests_passed = false;
    if (!test_guard_compilation_errors()) all_tests_passed = false;
    
    // Performance tests
    if (!test_guard_performance()) all_tests_passed = false;
    if (!test_guard_memory_efficiency()) all_tests_passed = false;
