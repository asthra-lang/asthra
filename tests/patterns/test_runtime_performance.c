...
static bool test_pattern_matching_throughput(void) {
    ...
    const char* source = 
        ...
        "        MessageType::Image { width, height, .. } => \"image: \" + width.to_string() + \"x\" + height.to_string(),\n"
        "        MessageType::Video { duration, .. } => \"video: \" + duration.to_string() + \"s\",\n"
    ...
    TaggedUnionValue messages[] = {
        ...
        create_tagged_union_value("MessageType", "Image", 
                                 create_struct_value((StructValue[]){
                                     {"url", create_string_value("image.jpg")},
                                     {"width", create_i32_value(1920)},
                                     {"height", create_i32_value(1080)}
                                 }, 3)),
        create_tagged_union_value("MessageType", "Video",
                                 create_struct_value((StructValue[]){
                                     {"url", create_string_value("video.mp4")},
                                     {"duration", create_f64_value(120.5)}
                                 }, 2)),
        create_tagged_union_value("MessageType", "Audio",
                                 create_struct_value((StructValue[]){
                                     {"url", create_string_value("audio.mp3")},
                                     {"length", create_f64_value(180.0)}
                                 }, 2)),
        create_tagged_union_value("MessageType", "File",
                                 create_struct_value((StructValue[]){
                                     {"name", create_string_value("document.pdf")},
...
static bool test_latency_characteristics(void) {
    ...
    EnumValue priorities[] = {
        create_enum_value("Priority", "Low", 1),
        create_enum_value("Priority", "Medium", 2),
        create_enum_value("Priority", "High", 3),
        create_enum_value("Priority", "Critical", 4)
    };
...
static bool test_performance_scalability(void) {
    ...
    EnumValue simple_values[] = {
        create_enum_value("Simple", "A", 0),
    ...
    TaggedUnionValue complex_values[] = {
        create_tagged_union_value("Complex", "A",
                                 create_tuple_value((Value[]){create_i32_value(42), create_string_value("test")}, 2)),
        create_tagged_union_value("Complex", "B",
                                 create_struct_value((StructValue[]){
                                     {"x", create_i32_value(10)},
...
static bool test_execution_memory_usage(void) {
    ...
    TaggedUnionValue small_val = create_tagged_union_value("MemoryTest", "Small", create_i32_value(42));
 ...
 */
static TaggedUnionValue create_tagged_union_value(const char* type_name, const char* variant, Value value) {
...
int main(void) {
    ...
    // Runtime performance analysis tests
    if (!test_runtime_performance_analysis()) all_tests_passed = false;
    if (!test_pattern_matching_throughput()) all_tests_passed = false;
    if (!test_latency_characteristics()) all_tests_passed = false;
    if (!test_performance_scalability()) all_tests_passed = false;
    if (!test_execution_memory_usage()) all_tests_passed = false;
