...
static bool test_enum_guards(void) {
    ...
    // Test large value
    TaggedUnionValue some_large = create_tagged_union_value("Option", "Some", 
                                                           create_i32_value(150));
    ...
    // Test positive value
    TaggedUnionValue some_positive = create_tagged_union_value("Option", "Some", 
                                                              create_i32_value(42));
    ...
    // Test zero
    TaggedUnionValue some_zero = create_tagged_union_value("Option", "Some", 
                                                          create_i32_value(0));
...
static bool test_complex_enum_guards(void) {
    ...
    TaggedUnionValue ok_valid = create_tagged_union_value("Result", "Ok", 
                                                         create_i32_value(75));
    ...
    // Test timeout error
    TaggedUnionValue err_timeout = create_tagged_union_value("Result", "Err", 
                                                            create_string_value("connection timeout"));
    result_str = execute_pattern_match_with_value(pattern_result, &err_timeout);
    ASSERT_STRING_EQ(result_str.data, "timeout error");
    
    // Test network error
    TaggedUnionValue err_network = create_tagged_union_value("Result", "Err", 
                                                            create_string_value("network unreachable"));
...
static bool test_struct_guards(void) {
    ...
    const char* source = 
        ...
        "    match rect {\n"
        "        Rectangle { top_left: Point { x, y }, width: w, height: h } if w == h => \"square\",\n"
        "        Rectangle { top_left: Point { x, y }, width: w, height: h } if w > h => \"wide rectangle\",\n"
        "        Rectangle { top_left: Point { x, y }, width: w, height: h } if h > w => \"tall rectangle\",\n"
        "        Rectangle { top_left: Point { x, y }, width: w, height: h } if x < 0.0 || y < 0.0 => \"negative position\",\n"
    ...
    // Test square
    StructValue point_fields = create_struct_value((StructValue[]){
        {"x", create_f64_value(10.0)},
        {"y", create_f64_value(20.0)}
    }, 2);
    
    StructValue rect_fields = create_struct_value((StructValue[]){
        {"top_left", &point_fields},
    ...
    // Test wide rectangle
    rect_fields = create_struct_value((StructValue[]){
        {"top_left", &point_fields},
...
static bool test_mixed_enum_struct_guards(void) {
    ...
    const char* source = 
        ...
        "    match result {\n"
        "        UserResult::Valid(User { name, age, score }) if age >= 18 && score >= 80.0 => \"adult high performer\",\n"
        "        UserResult::Valid(User { name, age, score }) if age >= 18 => \"adult user\",\n"
        "        UserResult::Valid(User { name, age, score }) if age < 18 && score >= 90.0 => \"young achiever\",\n"
        "        UserResult::Valid(User { name, age, score }) if name.length() == 0 => \"anonymous user\",\n"
        "        UserResult::Valid(_) => \"regular user\",\n"
        "        UserResult::Invalid { reason, attempted_user: User { age, .. } } if age < 0 => \"invalid age\",\n"
        "        UserResult::Invalid { reason, .. } if reason.contains(\"name\") => \"name validation error\",\n"
...
int main(void) {
    ...
    // Guard expressions with enums
    if (!test_enum_guards()) all_tests_passed = false;
    if (!test_complex_enum_guards()) all_tests_passed = false;
    
    // Guard expressions with structs
    if (!test_struct_guards()) all_tests_passed = false;
    if (!test_mixed_enum_struct_guards()) all_tests_passed = false;
