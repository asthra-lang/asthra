...
static bool test_memory_layout_optimization(void) {
    ...
    const char *source = "struct OptimizedStruct {\n"
                         "    a: i8,\n"
                         "    b: i64,\n"
                         "    c: i8,\n"
                         "    d: i32,\n"
    ...
    StructValue test_struct = create_struct_value((StructValue[]){{"a", create_i8_value(0)},
...
static bool test_cache_efficiency(void) {
    ...
    StructValue test_data[] = {
        create_struct_value((StructValue[]){{"id", create_i32_value(1)},
                                            {"category", create_i32_value(1)},
                                            {"priority", create_i32_value(8)},
                                            {"status", create_i32_value(1)}},
                            4),
        create_struct_value((StructValue[]){{"id", create_i32_value(2)},
                                            {"category", create_i32_value(2)},
                                            {"priority", create_i32_value(3)},
                                            {"status", create_i32_value(1)}},
                            4),
        create_struct_value((StructValue[]){{"id", create_i32_value(3)},
...
static bool test_cache_line_optimization(void) {
    ...
    const char *source = "struct CacheLineStruct {\n"
                         "    field1: i32,\n"
                         "    field2: i32,\n"
                         "    field3: i32,\n"
                         "    field4: i32,\n"
    ...
    StructValue cache_struct = create_struct_value((StructValue[]){{"field1", create_i32_value(1)},
...
static bool test_memory_prefetching_optimization(void) {
    ...
    StructValue large_data = create_struct_value((StructValue[]){{"metadata", create_i32_value(1)},
...
int main(void) {
    ...
    // Memory usage optimization tests
    if (!test_memory_usage_optimization())
        all_tests_passed = false;
    if (!test_memory_layout_optimization())
        all_tests_passed = false;
    if (!test_memory_allocation_optimization())
        all_tests_passed = false;

    // Cache efficiency tests
    if (!test_cache_efficiency())
        all_tests_passed = false;
    if (!test_cache_line_optimization())
        all_tests_passed = false;
    if (!test_memory_prefetching_optimization())
        all_tests_passed = false;
    ...
    if (all_tests_passed) {
        ...
        printf("📊 Pattern Matching Expansion Plan: Memory/Cache Optimization Features "
               "Demonstrated\n");
        return 0;
    } else {
