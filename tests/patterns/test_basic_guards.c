...
static bool test_simple_numeric_guards(void) {
    ...
    const char *source = "fn classify_number(x: i32) -> String {\n"
                         "    match x {\n"
                         "        n if n < 0 => \"negative\",\n"
                         "        n if n == 0 => \"zero\",\n"
...
static bool test_string_guards(void) {
    ...
    const char *source = "fn analyze_string(s: String) -> String {\n"
                         "    match s {\n"
...
static bool test_basic_guard_compilation(void) {
    ...
    const char *source = "fn simple_guards(x: i32, y: i32) -> String {\n"
                         "    match (x, y) {\n"
    ...
    // Test first greater
    Value test_tuple = create_tuple_value((Value[]){create_i32_value(10), create_i32_value(5)}, 2);
    ...
    // Test equal
    test_tuple = create_tuple_value((Value[]){create_i32_value(7), create_i32_value(7)}, 2);
...
int main(void) {
    ...
    // Basic guard expression tests
    if (!test_simple_numeric_guards())
        all_tests_passed = false;
    if (!test_string_guards())
        all_tests_passed = false;
    if (!test_basic_guard_compilation())
        all_tests_passed = false;
