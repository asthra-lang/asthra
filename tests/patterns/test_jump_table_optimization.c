...
static bool test_enum_jump_table_optimization(void) {
    ...
    EnumValue test_colors[] = {
        create_enum_value("Color", "RED", 0), create_enum_value("Color", "GREEN", 1),
        create_enum_value("Color", "BLUE", 2), create_enum_value("Color", "YELLOW", 3),
...
static bool test_sparse_jump_table_optimization(void) {
    ...
    const char *source = "enum HttpStatus {\n"
                         "    OK = 200,\n"
                         "    NOT_FOUND = 404,\n"
                         "    SERVER_ERROR = 500,\n"
                         "    BAD_REQUEST = 400,\n"
...
static bool test_jump_table_density_analysis(void) {
    ...
    const char *dense_source = "enum DenseEnum { A = 1, B = 2, C = 3, D = 4, E = 5 }\n"
    ...
    const char *sparse_source = "enum SparseEnum { X = 1, Y = 100, Z = 1000 }\n"
...
static bool test_jump_table_vs_linear_performance(void) {
    ...
    // Test operations
    EnumValue test_ops[] = {
        create_enum_value("Operation", "ADD", 0), create_enum_value("Operation", "MUL", 2),
        create_enum_value("Operation", "AND", 5), create_enum_value("Operation", "SHR", 9)};
...
static bool test_jump_table_boundary_conditions(void) {
    ...
    const char *source = "enum BoundaryEnum {\n"
                         "    MIN = 0,\n"
                         "    MID = 5,\n"
                         "    MAX = 10\n"
                         "}\n"
...
int main(void) {
    ...
    // Jump table optimization tests
    if (!test_enum_jump_table_optimization())
        all_tests_passed = false;
    if (!test_sparse_jump_table_optimization())
        all_tests_passed = false;
    if (!test_jump_table_density_analysis())
        all_tests_passed = false;
    if (!test_jump_table_vs_linear_performance())
        all_tests_passed = false;
    if (!test_jump_table_boundary_conditions())
        all_tests_passed = false;
    ...
    if (all_tests_passed) {
        ...
        printf(
            "📊 Pattern Matching Expansion Plan: Jump Table Optimization Features Demonstrated\n");
        return 0;
    } else {
