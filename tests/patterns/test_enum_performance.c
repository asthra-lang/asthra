...
static bool test_runtime_execution_performance(void) {
    ...
    const char *source = "enum TestEnum {\n"
                         "    A, B, C, D, E, F, G, H, I, J\n"
                         "}\n"
                         "\n"
...
static bool test_memory_usage_optimization(void) {
    ...
    const char *source = "enum MemoryTestEnum {\n"
                         "    SMALL,\n"
                         "    MEDIUM,\n"
                         "    LARGE\n"
                         "}\n"
...
static bool test_compilation_scalability(void) {
    ...
    const char *source = "enum ScalabilityEnum {\n"
                         "    V1, V2, V3, V4, V5, V6, V7, V8, V9, V10,\n"
                         "    V11, V12, V13, V14, V15, V16, V17, V18, V19, V20,\n"
...
static bool test_complex_pattern_performance(void) {
    ...
    const char *source =
        ...
        "        ComplexEnum::Triple(x, y, z) => \"Triple: \" + x.to_string() + \", \" + "
        "y.to_string() + \", \" + z.to_string(),\n"
        "        ComplexEnum::Struct { a, b, c } => \"Struct: \" + a.to_string() + \", \" + b + "
        "\", \" + c.to_string(),\n"
...
int main(void) {
    ...
    // Performance tests
    if (!test_large_enum_pattern_performance())
        all_tests_passed = false;
    if (!test_runtime_execution_performance())
        all_tests_passed = false;
    if (!test_memory_usage_optimization())
        all_tests_passed = false;
    if (!test_compilation_scalability())
        all_tests_passed = false;
    if (!test_complex_pattern_performance())
        all_tests_passed = false;
