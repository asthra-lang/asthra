    ...
    if (parser->error_count > 0) {
        ...
        for (size_t i = 0; i < parser->error_count; i++) {
            fprintf(stderr, "  Error at %s:%d:%d: %s\n", 
                    parser->errors[i].location.filename ? parser->errors[i].location.filename : test_name,
                    parser->errors[i].location.line,
                    parser->errors[i].location.column,
...
static AsthraTestResult test_nested_loops_with_break(AsthraTestContext* context) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        compile_and_analyze(source, "nested_loops_with_break"),
...
static AsthraTestResult test_continue_with_side_effects(AsthraTestContext* context) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        compile_and_analyze(source, "continue_with_side_effects"),
...
static AsthraTestResult test_multiple_nested_loops(AsthraTestContext* context) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        compile_and_analyze(source, "multiple_nested_loops"),
...
static AsthraTestResult test_loop_control_in_unsafe_block(AsthraTestContext* context) {
    ...
    ASTHRA_TEST_ASSERT_TRUE(context, 
        compile_and_analyze(source, "loop_control_in_unsafe_block"),
...
AsthraTestSuite* create_loop_control_flow_integration_test_suite(void) {
    AsthraTestSuite* suite = asthra_test_suite_create("Loop Control Flow Integration Tests", 
    ...
    asthra_test_suite_add_test(suite, "test_multiple_nested_loops", 
