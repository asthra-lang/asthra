...
typedef enum { DATA_TYPE_INTEGER, DATA_TYPE_STRING, DATA_TYPE_ARRAY, DATA_TYPE_INVALID } DataType;
...
AsthraTestResult test_integration_pattern_matching_with_ffi(AsthraV12TestContext *ctx) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, int_result.is_ok, "Integer processing should succeed")) {
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, string_result.is_ok, "String processing should succeed")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base,
                            strstr(string_result.result.ok_value.data.string_value,
                                   "Processed: Hello World") != NULL,
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, array_result.is_ok, "Array processing should succeed")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base,
                            array_result.result.ok_value.data.array_value.array_size == 5,
