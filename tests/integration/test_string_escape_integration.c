...
AsthraTestResult test_string_escapes_in_expressions(AsthraTestContext *context) {
    const char *source = "let msg = \"Hello\\nWorld\";\n"
    ...
    if (!asthra_test_assert_int_eq(context, (int)token1.type, (int)TOKEN_LET,
                                   "Expected LET token")) {
        lexer_destroy(lexer);
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_int_eq(context, (int)token2.type, (int)TOKEN_IDENTIFIER,
    ...
    if (!asthra_test_assert_int_eq(context, (int)token3.type, (int)TOKEN_ASSIGN,
                                   "Expected ASSIGN token")) {
        lexer_destroy(lexer);
        return ASTHRA_TEST_FAIL;
    }
    token_free(&token3);
    ...
    if (!asthra_test_assert_int_eq(context, (int)token4.type, (int)TOKEN_STRING,
