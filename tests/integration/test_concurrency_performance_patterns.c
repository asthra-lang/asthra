...
void test_cpu_intensive_workload(void) {
    ...
    const char* cpu_code = 
        ...
        "fn parallel_computation_simple(tasks: Vec<ComputeTask>) -> Result<Vec<ComputeResult>, string> {\n"
        ...
        "#[non_deterministic]\n"
        "fn parallel_computation_advanced(tasks: Vec<ComputeTask>) -> Result<Vec<ComputeResult>, string> {\n"
...
void test_io_intensive_workload(void) {
    ...
    // Assert that the duration is roughly as expected (e.g., total_tasks * delay_per_task / num_threads)
    // This is a simplified check, as actual performance varies.
    // Expect duration to be closer to io_delay_ms if tasks are truly parallel, or num_io_tasks * io_delay_ms if sequential.
    // Since we're using separate threads, it should be closer to io_delay_ms * (num_io_tasks / num_cores) roughly.
    // For this simple test, we just check it's not excessively long.
    ASSERT_TRUE(duration > 0);
    ASSERT_TRUE(duration < (num_io_tasks * io_delay_ms * 2)); // Should be less than (total expected sequential time * 2)
...
void test_race_conditions_and_deadlocks(void) {
    ...
    for (int i = 0; i < num_unsafe_threads; ++i) {
        pthread_create(&unsafe_threads[i], NULL, unsafe_increment_task, &increments_per_unsafe_thread);
    }
    ...
    // We expect the unsafe counter to be *less than or equal to* the expected value due to race conditions
    // It's non-deterministic, so we can't assert strict equality, but we can assert it's not greater.
    ...
    // Test 3: Simulated Deadlock (conceptual - complex to simulate directly without language features)
