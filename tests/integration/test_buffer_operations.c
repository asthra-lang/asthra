...
AsthraTestResult test_buffer_operations(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_size_eq(context, copied, src_size, 
                                   "Should copy all source bytes")) {
    ...
    // Then append part2
    size_t appended = asthra_buffer_append(buffer, sizeof(buffer), copied1, 
                                          part2, part2_size);
    ...
    if (!asthra_test_assert_bool(context, equal1, 
                                "Identical buffers should be equal")) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, !equal2, 
