...
ASTHRA_PURE ASTHRA_NONNULL(1) static size_t test_pure_function(const char *restrict str) {
...
ASTHRA_MALLOC ASTHRA_WARN_UNUSED_RESULT static void *test_malloc_function(size_t size) {
...
AsthraTestResult test_static_analysis_attributes(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_size_eq(context, length1, 13, "String length should be 13")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_int_eq(context, value1, 42, "Const function should return 42")) {
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {.name = "static_analysis_attributes",
                                   .file = __FILE__,
                                   .line = __LINE__,
