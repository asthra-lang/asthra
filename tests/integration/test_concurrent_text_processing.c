...
AsthraTestResult test_integration_concurrent_text_processing(AsthraV12TestContext *ctx) {
    ...
    // Initialize tasks
    const char *test_inputs[] = {"hello world",     "asthra language", "concurrent processing",
                                 "ffi integration", "error handling",  "string operations"};
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, result == 0, "Thread %d creation should succeed", i)) {
            // Cleanup
            for (int j = 0; j < i; j++) {
    ...
    int success_count = 0;
    for (int i = 0; i < num_tasks; i++) {
        if (!ASTHRA_TEST_ASSERT(&ctx->base, tasks[i].success, "Task %d should succeed", i)) {
            // Cleanup
            for (int j = 0; j < num_tasks; j++) {
                free(tasks[j].input_text);
                if (tasks[j].processed_text)
                    free(tasks[j].processed_text);
            }
            return ASTHRA_TEST_FAIL;
        }
        ...
        case 1: // Uppercase
            if (!ASTHRA_TEST_ASSERT(&ctx->base,
                                    strstr(tasks[i].processed_text, "HELLO") != NULL ||
        ...
        case 2: // Prefix
            if (!ASTHRA_TEST_ASSERT(&ctx->base,
                                    strstr(tasks[i].processed_text, "PROCESSED:") != NULL,
        ...
        case 3: // Reverse - just check it's different from input
            if (!ASTHRA_TEST_ASSERT(&ctx->base,
                                    strcmp(tasks[i].processed_text, tasks[i].input_text) != 0,
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, success_count == num_tasks, "All %d tasks should succeed",
                            num_tasks)) {
        // Cleanup
        for (int i = 0; i < num_tasks; i++) {
            free(tasks[i].input_text);
            if (tasks[i].processed_text)
                free(tasks[i].processed_text);
        }
        return ASTHRA_TEST_FAIL;
    }
