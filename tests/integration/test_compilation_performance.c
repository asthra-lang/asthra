...
static PerformanceTestFixture* setup_performance_fixture(const char* source, const char* filename) {
    ...
    fixture->ffi_generator = ffi_assembly_generator_create(TARGET_ARCH_X86_64, CALLING_CONV_SYSTEM_V_AMD64);
...
static char* generate_large_source_code(size_t num_functions) {
    size_t estimated_size = num_functions * 200 + 100; // Rough estimate per function + package header
...
AsthraTestResult test_compilation_performance(AsthraTestContext* context) {
    ...
    const char* source = "package performance_test; pub fn fibonacci(n: i32) -> i32 { return n; } pub fn main(void) -> i32 { return 0; }";
    ...
    printf("Performance test: tokens=%zu, structure=%s, instructions=%zu\n",
           token_count, valid_structure ? "valid" : "invalid", estimated_instructions);
    ...
    if (!asthra_test_assert_bool(context, estimated_instructions < 1000, "Too many instructions: %zu", estimated_instructions)) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_memory_usage_during_compilation(AsthraTestContext* context) {
    ...
    const char* source = "package memory_test; pub struct Point { x: i32, y: i32, z: i32 } pub fn main(void) -> i32 { return 0; }";
    ...
    printf("Memory test: source=%zu, ast=%zu, symbols=%zu, total=%zu bytes\n",
           source_size, estimated_ast_size, estimated_symbol_table_size, total_estimated_memory);
...
AsthraTestResult test_large_file_compilation(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_bool(context, total_tokens < 2000, 
                                "Too many tokens: %zu", total_tokens)) {
        return ASTHRA_TEST_FAIL;
    }
    
    if (!asthra_test_assert_bool(context, total_symbols < 500, 
                                "Too many symbols: %zu", total_symbols)) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_compilation_speed_scaling(AsthraTestContext* context) {
    ...
    for (size_t i = 0; i < num_tests; i++) {
        ...
        printf("Scaling test %zu: functions=%zu, time=%zu units\n", 
               i, function_counts[i], processing_times[i]);
    }
    ...
    if (!asthra_test_assert_bool(context, scaling_factor <= expected_scaling * 1.5, 
                                "Scaling too poor: %.2fx (max: %.2fx)", 
                                scaling_factor, expected_scaling * 1.5)) {
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {
        .name = "compilation_performance_integration",
        .file = __FILE__,
        .line = __LINE__,
        .function = "main",
    ...
    AsthraTestResult results[] = {
        test_compilation_performance(context),
        test_memory_usage_during_compilation(context),
        test_large_file_compilation(context),
        test_compilation_speed_scaling(context)
    };
