...
static AsthraTestResult test_minimal_function_compilation(AsthraTestContext* context) {
    ...
    while ((token = lexer_next_token(lexer)).type != TOKEN_EOF) {
        token_count++;
        if (token_count > 50) break; // Safety limit
    }
...
static AsthraTestResult test_minimal_expression_compilation(AsthraTestContext* context) {
    ...
    const char* source = 
        "package test_minimal_expression;\n"
        "\n"
    ...
    if (!asthra_test_assert_bool(context, has_arithmetic, "Should contain arithmetic expressions")) {
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {
        .name = "minimal_integration_tests",
        .file = __FILE__,
        .line = __LINE__,
        .function = "main",
