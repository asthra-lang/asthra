 ...
 */
static ErrorHandlingTestFixture* setup_error_handling_fixture(const char* source, const char* filename) {
...
AsthraTestResult test_lexer_error_propagation(AsthraTestContext* context) {
    ...
    if (!asthra_test_assert_pointer_eq(context, fixture->ast, NULL, "Expected parsing to fail due to lexer error")) {
...
AsthraTestResult test_parser_error_propagation(AsthraTestContext* context) {
    ...
    fixture->ast = parser_parse_program(fixture->parser);
    if (!asthra_test_assert_pointer_eq(context, fixture->ast, NULL, "Expected parsing to fail due to missing type annotation")) {
        cleanup_error_handling_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_semantic_error_propagation(AsthraTestContext* context) {
    ...
    ErrorHandlingTestFixture* fixture = setup_error_handling_fixture(source, "semantic_error_test.o");
    if (!asthra_test_assert_pointer(context, fixture, "Failed to setup error handling fixture")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    if (!asthra_test_assert_bool_eq(context, success, false, "Expected semantic analysis to fail")) {
        cleanup_error_handling_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_type_mismatch_error(AsthraTestContext* context) {
    ...
    bool success = analyze_test_ast(fixture->analyzer, fixture->ast);
    if (!asthra_test_assert_bool_eq(context, success, false, "Expected semantic analysis to fail due to type mismatch")) {
        cleanup_error_handling_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_codegen_error_propagation(AsthraTestContext* context) {
    ...
    ErrorHandlingTestFixture* fixture = setup_error_handling_fixture(source, "codegen_error_test.o");
    if (!asthra_test_assert_pointer(context, fixture, "Failed to setup error handling fixture")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_multiple_error_conditions(AsthraTestContext* context) {
    ...
    ErrorHandlingTestFixture* fixture = setup_error_handling_fixture(source, "multiple_errors_test.o");
    if (!asthra_test_assert_pointer(context, fixture, "Failed to setup error handling fixture")) {
        return ASTHRA_TEST_FAIL;
    }
    ...
    fixture->ast = parser_parse_program(fixture->parser);
    if (!asthra_test_assert_pointer_eq(context, fixture->ast, NULL, "Expected parsing to fail due to syntax errors")) {
        cleanup_error_handling_fixture(fixture);
        return ASTHRA_TEST_FAIL;
    }
...
int main(void) {
    AsthraTestSuite suite = {
        ...
        .tests = {
            { "Lexer Error Propagation", test_lexer_error_propagation },
