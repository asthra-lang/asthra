...
QualityGateResult validate_quality_gates(const char* feature_name, 
    ...
    const char* error_programs[] = {
        ...
        "package main; pub fn main(none) -> void { fn foo(void) { return 42; } }", // Invalid function syntax
    ...
    for (size_t i = 0; i < program_count; i++) {
        RealProgramTestResult test_result = validate_complete_program(
            test_programs[i], feature_name, &config);
    ...
    for (size_t i = 0; i < error_program_count; i++) {
        RealProgramTestResult test_result = validate_complete_program(
            error_programs[i], "error_detection", &config);
    ...
    result.error_handling_correct = (error_detection_rate >= 0.75); // 75% of error programs should be caught
    result.feature_coverage_complete = (success_rate >= 0.9); // 90% for complete coverage
    
    // Calculate overall score
    int gates_passed = 0;
    if (result.grammar_compliant) gates_passed++;
    if (result.end_to_end_functional) gates_passed++;
    if (result.performance_acceptable) gates_passed++;
    if (result.error_handling_correct) gates_passed++;
    if (result.feature_coverage_complete) gates_passed++;
...
bool validate_real_functionality(const char* feature_name,
                               const char** real_world_programs,
                               size_t program_count) {
    if (!feature_name || !real_world_programs || program_count == 0) {
        return false;
    }
    
    QualityGateResult quality = validate_quality_gates(feature_name, real_world_programs, program_count);
    
    // Real functionality requires all quality gates to pass
    return quality.grammar_compliant && 
           quality.end_to_end_functional && 
           quality.performance_acceptable && 
           quality.feature_coverage_complete;
}
