...
bool detect_false_positive(const char* feature_name, 
    ...
    if (individual_result.success && integration_result.success) {
        double time_ratio = integration_result.execution_time_ms / individual_result.execution_time_ms;
...
bool validate_feature_functionality(const char* feature_name, const char** test_programs, 
    ...
    for (size_t i = 0; i < program_count; i++) {
        ...
        RealProgramTestResult result = validate_complete_program(
            test_programs[i], test_name, config);
    ...
    if (config && config->verbose_output) {
        fprintf(config->output_stream, "Feature %s: %zu/%zu tests passed (%.1f%%)\n",
                feature_name, passed, program_count, success_rate * 100.0);
    }
