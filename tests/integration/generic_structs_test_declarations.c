...
bool test_generic_struct_declaration_edge_cases(void) {
    ...
    TEST_ASSERT(
        test_parse_success("pub struct X<A> { value: A }", "Single character type parameter"),
        "Single character type parameter parsing");

    // Long type parameter names
    TEST_ASSERT(test_parse_success("pub struct Container<TypeParameter> { value: TypeParameter }",
    ...
    TEST_ASSERT(test_parse_success("pub struct Empty<T> { }", "Empty generic struct"),
...
bool test_invalid_generic_struct_syntax(void) {
    ...
    TEST_ASSERT(test_parse_failure("pub struct Vec<T { data: T }", "Missing closing angle bracket"),
                "Missing closing angle bracket correctly fails");

    // Missing opening angle bracket
    TEST_ASSERT(test_parse_failure("pub struct VecT> { data: T }", "Missing opening angle bracket"),
                "Missing opening angle bracket correctly fails");

    // Empty type parameter list
    TEST_ASSERT(test_parse_failure("pub struct Vec<> { data: i32 }", "Empty type parameter list"),
    ...
    TEST_ASSERT(!test_semantic_success("pub struct Dup<T, T> { first: T, second: T }",
                                       "Duplicate type parameters"),
                "Duplicate type parameters correctly fails semantic analysis");

    // Invalid type parameter names
    TEST_ASSERT(
        test_parse_failure("pub struct Bad<123> { value: 123 }", "Numeric type parameter name"),
        "Numeric type parameter name correctly fails");

    // Missing comma between type parameters
    TEST_ASSERT(test_parse_failure("pub struct Pair<A B> { first: A, second: B }",
                                   "Missing comma between type parameters"),
