...
RealProgramTestResult validate_complete_program(const char* source, const char* test_name, const TestSuiteConfig* config) {
    ...
    if (result.metrics.parse_time_ms > 0) {
        result.metrics.parse_speed_loc_per_ms = (double)result.metrics.lines_of_code / result.metrics.parse_time_ms;
    }
...
bool run_test_suite(RealProgramTestSuite* suite, const TestSuiteConfig* config) {
    ...
    for (size_t i = 0; i < suite->test_count; i++) {
        ...
        RealProgramTestResult result = validate_complete_program(
            test->source_code, test->name, config);
            ...
            if (test->expected_error_pattern && result.error_message) {
                ...
                if (strstr(result.error_message, test->expected_error_pattern) == NULL) {
                    ...
                    if (config->verbose_output) {
                        fprintf(config->output_stream, "  Expected error pattern '%s' not found in '%s'\n",
        ...
        if (test_passed && test->max_parse_time_ms > 0) {
            if (result.execution_time_ms > test->max_parse_time_ms) {
                test_passed = false;
                if (config->verbose_output) {
                    fprintf(config->output_stream, "  Performance requirement failed: %.2fms > %zums\n",
