...
AsthraTestResult test_struct_program_compilation(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_pointer(context, runtime_memory,
                                    "Runtime memory allocation should succeed")) {
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_function_parameters_compilation(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(context, contains_asthra && contains_integration,
                                 "String should contain both parts")) {
        free(result);
        return ASTHRA_TEST_FAIL;
    }
...
AsthraTestResult test_loop_compilation(AsthraTestContext *context) {
    ...
    if (!asthra_test_assert_bool(context, written == strlen(test_content),
...
int main(void) {
    ...
    AsthraTestMetadata metadata = {.name = "basic_compilation_integration",
                                   .file = __FILE__,
                                   .line = __LINE__,
                                   .function = "main",
    ...
    AsthraTestResult results[] = {
        test_simple_program_compilation(context), test_struct_program_compilation(context),
        test_function_parameters_compilation(context), test_control_flow_compilation(context),
        test_loop_compilation(context)};

    const char *test_names[] = {"Simple program compilation", "Runtime integration",
                                "Memory and string operations", "Pipeline coordination",
