...
bool test_deduplication_integration(void) {
    ...
    if (result->generator && result->generator->generic_registry) {
        ...
        TEST_ASSERT(vec_info->instantiation_count == 1,
                    "Only one Vec<i32> instantiation despite multiple uses");
    }
    ...
    if (result->c_code_output) {
        ...
        TEST_ASSERT(second_vec_i32 == NULL || strstr(second_vec_i32, "typedef struct") == NULL,
...
bool test_performance_many_instantiations(void) {
    ...
    if (result->generator && result->generator->generic_registry) {
        GenericStructInfo *pair_info =
            generic_registry_lookup_struct(result->generator->generic_registry, "Pair");
        TEST_ASSERT(pair_info != NULL, "Pair generic struct registered");
        TEST_ASSERT(pair_info->instantiation_count >= 6,
                    "Multiple different Pair instantiations created");
...
bool test_type_system_consistency(void) {
    ...
    if (result->generator && result->generator->generic_registry) {
        GenericStructInfo *container_info =
            generic_registry_lookup_struct(result->generator->generic_registry, "Container");
