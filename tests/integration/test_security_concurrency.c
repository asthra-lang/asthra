...
static void *security_test_worker(void *arg) {
    ...
    for (int i = 0; i < 100; i++) {
        args->comparison_result =
            secure_buffer_constant_time_compare(args->buffer, args->test_data);
...
AsthraTestResult test_integration_security_with_concurrency(AsthraV12TestContext *ctx) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        if (!ASTHRA_TEST_ASSERT(&ctx->base, thread_args[i].comparison_result == expected,
                                "Thread %d comparison result should be %s", i,
                                expected ? "true" : "false")) {
            secure_buffer_destroy(secure_buf);
            return ASTHRA_TEST_FAIL;
        }
    ...
    if (!ASTHRA_TEST_ASSERT(&ctx->base, is_zeroed, "Secure data should be zeroed")) {
        secure_buffer_destroy(secure_buf);
        return ASTHRA_TEST_FAIL;
    }
