    ...
    case AST_FUNCTION_DECL:
        ...
        if (node->data.function_decl.params) {
            for (size_t i = 0; i < ast_node_list_size(node->data.function_decl.params); i++) {
                collect_symbol_usages_recursive(
                    ast_node_list_get(node->data.function_decl.params, i), collector);
            }
        }
    ...
    case AST_CALL_EXPR:
        ...
        if (node->data.call_expr.args) {
            for (size_t i = 0; i < ast_node_list_size(node->data.call_expr.args); i++) {
                collect_symbol_usages_recursive(ast_node_list_get(node->data.call_expr.args, i),
                                                collector);
            }
        }
        break;
    ...
    case AST_BLOCK:
        if (node->data.block.statements) {
            for (size_t i = 0; i < ast_node_list_size(node->data.block.statements); i++) {
                collect_symbol_usages_recursive(ast_node_list_get(node->data.block.statements, i),
                                                collector);
            }
        }
        break;
    ...
    case AST_PROGRAM:
        if (node->data.program.declarations) {
            for (size_t i = 0; i < ast_node_list_size(node->data.program.declarations); i++) {
                collect_symbol_usages_recursive(
                    ast_node_list_get(node->data.program.declarations, i), collector);
            }
        }
        break;
...
AISymbolUsage **asthra_ai_find_symbol_usages(AsthraSemanticsAPI *api, const char *symbol_name,
    ...
    *count = 0;
    ...
    SymbolUsageCollector collector = {.target_symbol = symbol_name,
                                      .usages = NULL,
                                      .usage_count = 0,
                                      .usage_capacity = 0,
...
char *asthra_ai_get_symbol_documentation(AsthraSemanticsAPI *api, const char *symbol_name) {
    ...
    char *doc = malloc(256);
    if (doc) {
        snprintf(doc, 256, "Documentation for %s (kind: %s)", symbol_name,
                 symbol->kind == SYMBOL_FUNCTION   ? "function"
                 : symbol->kind == SYMBOL_TYPE     ? "type"
                 : symbol->kind == SYMBOL_VARIABLE ? "variable"
                                                   : "symbol");
    }
