...
static ASTNode *find_parent_by_traversal(ASTNode *root, ASTNode *target, ASTNode *current_parent) {
    ...
    switch (root->type) {
        case AST_PROGRAM:
            if (root->data.program.package_decl == target) return root;
        ...
        case AST_FUNCTION_DECL:
            if (root->data.function_decl.return_type == target) return root;
            if (root->data.function_decl.body == target) return root;
        ...
        case AST_BINARY_EXPR:
            if (root->data.binary_expr.left == target) return root;
            if (root->data.binary_expr.right == target) return root;
        ...
        case AST_CALL_EXPR:
            if (root->data.call_expr.function == target) return root;
