...
void add_usage_to_collector(SymbolUsageCollector *collector, ASTNode *node,
    ...
    if (collector->usage_count >= collector->usage_capacity) {
        ...
        AISymbolUsage **new_usages =
            realloc(collector->usages, sizeof(AISymbolUsage *) * new_capacity);
        if (!new_usages)
            return;
    ...
    usage->location.file_path =
        strdup(collector->source_file_path ? collector->source_file_path : "unknown");
...
void collect_symbol_names(SymbolTable *table, char ***symbols, size_t *count, size_t *capacity) {
    ...
    // This is a simplified implementation - in reality we'd need to iterate through the symbol
    // table For now, just add some common symbols
