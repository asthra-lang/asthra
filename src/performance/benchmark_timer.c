...
uint64_t asthra_benchmark_get_time_ns(void) {
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
        return (uint64_t)ts.tv_sec * ASTHRA_BENCHMARK_NANOSECONDS_PER_SECOND + 
               (uint64_t)ts.tv_nsec;
    }
...
uint64_t asthra_benchmark_get_cpu_time_ns(void) {
    ...
    if (task_info(mach_task_self(), TASK_THREAD_TIMES_INFO,
                  (task_info_t)&thread_info, &thread_info_count) == KERN_SUCCESS) {
        return (uint64_t)(thread_info.user_time.seconds + thread_info.system_time.seconds) * 
               ASTHRA_BENCHMARK_NANOSECONDS_PER_SECOND +
               (uint64_t)(thread_info.user_time.microseconds + thread_info.system_time.microseconds) * 1000;
    }
#elif defined(__linux__)
    struct rusage usage;
...
AsthraBenchmarkTimer asthra_benchmark_timer_start(void) {
    AsthraBenchmarkTimer timer = {
        .start_ns = asthra_benchmark_get_time_ns(),
        .end_ns = 0,
        .duration_ns = 0,
        .is_valid = true
    };
...
double asthra_benchmark_ns_to_milliseconds(uint64_t nanoseconds) {
    return (double)nanoseconds / (double)(ASTHRA_BENCHMARK_NANOSECONDS_PER_SECOND / ASTHRA_BENCHMARK_MILLISECONDS_PER_SECOND);
}

double asthra_benchmark_ns_to_microseconds(uint64_t nanoseconds) {
    return (double)nanoseconds / (double)(ASTHRA_BENCHMARK_NANOSECONDS_PER_SECOND / ASTHRA_BENCHMARK_MICROSECONDS_PER_SECOND);
} 
