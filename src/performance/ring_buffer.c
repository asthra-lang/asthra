...
AsthraRingBuffer* asthra_ring_buffer_create(
    ...
    if (!name || buffer_size == 0) return NULL;
    ...
    AsthraRingBuffer *buffer = aligned_alloc(ASTHRA_MEMORY_POOL_CACHE_LINE_SIZE,
                                             sizeof(AsthraRingBuffer));
...
size_t asthra_ring_buffer_write(AsthraRingBuffer *buffer, const void *data, size_t size) {
    ...
    if (bytes_to_write <= bytes_to_end) {
        ...
        memcpy(buffer->memory_region, (const char*)data + bytes_to_end, bytes_to_write - bytes_to_end);
    }
