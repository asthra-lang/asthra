...
static AsthraBenchmarkStatus
execute_single_threaded_benchmark(const AsthraBenchmarkDefinition *benchmark,
    ...
    for (uint64_t i = 0; i < config->iterations; i++) {
        ...
        // Check for timeout
        uint64_t elapsed_ms =
            (asthra_benchmark_get_time_ns() - result->execution_start_time) / 1000000;
