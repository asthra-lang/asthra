...
char *format_source_location(const DiagnosticSpan *location) {
    ...
    snprintf(result, 256,
             ...
             "\"end_column\":%zu"
             "}",
             file_path, location->start_line, location->start_column, location->end_line,
             location->end_column);
...
char *ai_linter_generate_guidance_json(AILintResult **results, size_t count) {
    if (!results)
        return strdup("{\"error\":\"No results provided\"}");
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        written = snprintf(ptr, remaining,
                           ...
                           "\"auto_fixable\":%s"
                           "}",
                           (i > 0) ? "," : "", rule_id, category, severity, title, description,
                           ai_guidance, location, result->confidence, result->impact_score,
    ...
    written = snprintf(ptr, remaining,
                       ...
                       "}"
                       "}",
                       stats.total_issues, stats.auto_fixable_count, stats.high_impact_count,
                       stats.average_confidence, stats.average_impact, stats.by_category[0],
                       stats.by_category[1], stats.by_category[2], stats.by_category[3],
                       stats.by_category[4], stats.by_severity[0], stats.by_severity[1],
                       stats.by_severity[2]);
...
char *ai_linter_generate_rule_catalog_json(AILinter *linter) {
    if (!linter)
        return strdup("{\"error\":\"No linter provided\"}");
    ...
    for (size_t i = 0; i < linter->rule_count; i++) {
        ...
        written = snprintf(ptr, remaining,
                           ...
                           "}",
                           (i > 0) ? "," : "", rule_id, category, severity, title, description,
                           ai_guidance, rule->impact_score, rule->auto_fixable ? "true" : "false");
...
char *ai_linter_generate_fix_suggestions_json(AILintResult **results, size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        written = snprintf(ptr, remaining,
                           ...
                           "\"confidence\":%.2f"
                           "}",
                           first ? "" : ",", rule_id, ai_guidance, location, result->confidence);
