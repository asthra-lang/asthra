    ...
    if (node->data.function_decl.visibility == VISIBILITY_PUBLIC) {
        if (!has_documentation_comment(node)) {
            ...
            *result = ai_lint_result_create(
                ...
                "Public functions should have documentation comments explaining their purpose, parameters, and return values",
...
bool check_too_many_parameters(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    if (param_count > 5) { // Threshold for too many parameters
        ...
        *result = ai_lint_result_create(
            ...
            "Functions with many parameters are hard to understand and maintain. Consider using a struct or builder pattern",
...
bool check_magic_numbers(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    if (node->type != AST_INTEGER_LITERAL && node->type != AST_FLOAT_LITERAL) return false;
    
    // Skip common non-magic numbers (0, 1, 2, etc.)
    if (node->type == AST_INTEGER_LITERAL) {
        long value = node->data.integer_literal.value;
        if (value >= 0 && value <= 2) return false; // 0, 1, 2 are usually not magic
    }
...
bool check_deep_nesting(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    if (is_deeply_nested(node->data.function_decl.body, 4)) { // Max 4 levels
        ...
        *result = ai_lint_result_create(
            ...
            "Deeply nested code is hard to read and maintain. Consider extracting functions or using early returns",
            "Refactor deeply nested code using early returns, guard clauses, or extracted functions",
            location,
...
static int count_function_parameters(ASTNode *func_node) {
    if (!func_node || func_node->type != AST_FUNCTION_DECL) return 0;
...
static bool is_deeply_nested(ASTNode *node, int max_depth) {
    if (!node || max_depth <= 0) return max_depth < 0;
...
static bool has_descriptive_name(const char *name, ASTNodeType context) {
    ...
    if (strcmp(name, "temp") == 0 || strcmp(name, "tmp") == 0 ||
        strcmp(name, "data") == 0 || strcmp(name, "val") == 0 ||
        strcmp(name, "var") == 0 || strcmp(name, "obj") == 0) {
        return false;
    }
...
static bool should_have_error_handling(ASTNode *node) {
    if (!node || node->type != AST_FUNCTION_DECL) return false;
...
void register_maintainability_rules(AILinter *linter) {
    ...
    AILintRule error_handling_rule = {
        ...
        .description = "Functions that can fail should return Result<T, E> and handle errors properly",
        .ai_guidance = "Add proper error handling with Result<T, E> return type and error propagation",
    ...
    AILintRule documentation_rule = {
        ...
        .description = "Public functions should have documentation comments explaining their purpose, parameters, and return values",
        .ai_guidance = "Add documentation comment above function: /// Description of function purpose",
    ...
    AILintRule parameters_rule = {
        ...
        .description = "Functions with many parameters are hard to understand and maintain. Consider using a struct or builder pattern",
        .ai_guidance = "Refactor to use a parameter struct or builder pattern to reduce parameter count",
    ...
    AILintRule magic_numbers_rule = {
        ...
        .description = "Numeric literals should be replaced with named constants for better maintainability",
        .ai_guidance = "Replace magic number with a named constant: const BUFFER_SIZE: usize = 1024;",
    ...
    AILintRule nesting_rule = {
        .rule_id = "AI_MAINT_005",
        .category = AI_LINT_MAINTAINABILITY,
        .default_severity = AI_LINT_WARNING,
        .title = "Deeply nested code detected",
        .description = "Deeply nested code is hard to read and maintain. Consider extracting functions or using early returns",
        .ai_guidance = "Refactor deeply nested code using early returns, guard clauses, or extracted functions",
    ...
    AILintRule naming_rule = {
        ...
        .ai_guidance = "Use descriptive names that clearly indicate purpose: 'user_count' instead of 'n'",
