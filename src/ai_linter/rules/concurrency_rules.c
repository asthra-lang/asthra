...
static bool is_shared_variable(const char *var_name, AsthraSemanticsAPI *api) {
    ...
    return (strstr(var_name, "shared") != NULL || strstr(var_name, "global") != NULL ||
            strstr(var_name, "counter") != NULL || strstr(var_name, "state") != NULL ||
...
static bool is_atomic_operation(ASTNode *node) {
    ...
    if (node->type == AST_CALL_EXPR) {
        ...
        if (func && func->type == AST_IDENTIFIER) {
            ...
            return (strstr(func_name, "atomic") != NULL || strcmp(func_name, "lock") == 0 ||
                    strcmp(func_name, "unlock") == 0 || strcmp(func_name, "acquire") == 0 ||
...
static bool has_data_race_pattern(ASTNode *node) {
    ...
    if (node->type == AST_ASSIGNMENT) {
        ...
        if (lvalue && lvalue->type == AST_IDENTIFIER) {
            ...
            if (is_shared_variable(var_name, NULL) && !is_in_synchronized_block(node) &&
...
bool check_data_race_risk(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    if (has_data_race_pattern(node)) {
        ASTNode *lvalue = node->data.assignment.target;
        const char *var_name =
            (lvalue && lvalue->type == AST_IDENTIFIER) ? lvalue->data.identifier.name : "unknown";
        ...
        *result = ai_lint_result_create(
            "AI_CONC_001", AI_LINT_CONCURRENCY, AI_LINT_ERROR, "Potential data race detected",
            "Unsynchronized access to shared variables can cause data races", guidance, location,
...
bool check_deadlock_potential(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    if (strcmp(func_name, "lock") == 0 || strcmp(func_name, "acquire") == 0) {
        ...
        *result = ai_lint_result_create(
            "AI_CONC_002", AI_LINT_CONCURRENCY, AI_LINT_WARNING, "Potential deadlock risk detected",
            "Multiple lock acquisitions can lead to deadlocks if not ordered consistently",
            "Ensure consistent lock ordering across all threads to prevent deadlocks", location,
...
bool check_improper_spawn_usage(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    if (!node || node->type != AST_SPAWN_STMT)
        return false;
    ...
    *result = ai_lint_result_create(
        "AI_CONC_003", AI_LINT_CONCURRENCY, AI_LINT_SUGGESTION, "Spawn without error handling",
        "Spawned tasks should have proper error handling and resource cleanup",
        "Consider using spawn_with_handle for better error handling and task management", location,
...
bool check_channel_misuse(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    if (strcmp(func_name, "send") == 0 || strcmp(func_name, "recv") == 0) {
        ...
        *result = ai_lint_result_create(
            "AI_CONC_004", AI_LINT_CONCURRENCY, AI_LINT_WARNING, "Potential channel deadlock",
            "Channel operations without timeouts can cause deadlocks",
            "Use select statements with timeouts for robust channel operations", location,
            0.6f, // Medium confidence
            0.7f, // Good impact
            false // Requires manual review
        );
        ...
        if (target && target->type == AST_IDENTIFIER) {
            ...
            if (is_shared_variable(var_name, api) && !is_in_synchronized_block(node)) {
                DiagnosticSpan location = {.start_line = node->location.line,
                                           ...
                                           .end_line = node->location.line,
                                           .end_column =
                                               node->location.column + (int)strlen(var_name),
                                           .file_path = strdup("current_file")};

                *result = ai_lint_result_create(
                    "AI_CONC_005", AI_LINT_CONCURRENCY, AI_LINT_WARNING,
                    "Missing synchronization detected",
                    "Shared variable access should be synchronized to prevent race conditions",
                    "Use mutexes, atomic operations, or channels to synchronize access", location,
                    0.7f, // Good confidence
                    0.8f, // High impact
                    false // Requires manual synchronization
                );
...
bool check_blocking_in_async(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    bool is_blocking = (strcmp(func_name, "sleep") == 0 || strcmp(func_name, "wait") == 0 ||
                        strcmp(func_name, "join") == 0 || strstr(func_name, "blocking") != NULL);
    ...
    if (is_blocking) {
        ...
        *result = ai_lint_result_create(
            "AI_CONC_006", AI_LINT_CONCURRENCY, AI_LINT_WARNING,
            "Blocking operation in async context",
            "Blocking operations can prevent other tasks from running",
            "Use async alternatives or move blocking operations to separate threads", location,
            0.6f, // Medium confidence
            0.7f, // Good impact
...
void register_concurrency_rules(AILinter *linter) {
    ...
    AILintRule rule1 = {.rule_id = strdup("AI_CONC_001"),
                        ...
                        .ai_guidance =
                            strdup("Use synchronization primitives for shared variable access"),
    ...
    AILintRule rule2 = {.rule_id = strdup("AI_CONC_002"),
                        .category = AI_LINT_CONCURRENCY,
                        ...
                        .ai_guidance =
                            strdup("Ensure consistent lock ordering to prevent deadlocks"),
    ...
    AILintRule rule3 = {.rule_id = strdup("AI_CONC_003"),
                        .category = AI_LINT_CONCURRENCY,
    ...
    AILintRule rule4 = {.rule_id = strdup("AI_CONC_004"),
                        .category = AI_LINT_CONCURRENCY,
                        .default_severity = AI_LINT_WARNING,
    ...
    AILintRule rule5 = {.rule_id = strdup("AI_CONC_005"),
                        .category = AI_LINT_CONCURRENCY,
                        .default_severity = AI_LINT_WARNING,
                        .title = strdup("Missing synchronization"),
                        .description = strdup("Shared variables need synchronization"),
                        .ai_guidance =
                            strdup("Use mutexes, atomics, or channels for synchronization"),
    ...
    AILintRule rule6 = {.rule_id = strdup("AI_CONC_006"),
                        .category = AI_LINT_CONCURRENCY,
                        .default_severity = AI_LINT_WARNING,
