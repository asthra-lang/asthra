...
static bool contains_user_input_pattern(ASTNode *node) {
    ...
    if (node->type == AST_IDENTIFIER) {
        ...
        return (strstr(name, "input") != NULL ||
                ...
                strstr(name, "request") != NULL ||
                strstr(name, "param") != NULL ||
...
static bool is_buffer_operation(ASTNode *node) {
    ...
    return (strcmp(func_name, "memcpy") == 0 ||
...
bool check_buffer_overflow_risk(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    if (!node || !is_buffer_operation(node)) return false;
    ...
    if (!has_bounds_checking(node)) {
        ...
        *result = ai_lint_result_create(
            ...
            "Add explicit bounds checking or use safer string/memory functions",
            location,
...
bool check_unvalidated_user_input(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    const char *func_name = func->data.identifier.name;
    bool is_dangerous = (strcmp(func_name, "system") == 0 ||
                        strcmp(func_name, "exec") == 0 ||
                        strcmp(func_name, "eval") == 0 ||
                        strcmp(func_name, "sql_query") == 0);
            ...
            for (size_t i = 0; i < args->count; i++) {
                ...
                if (contains_user_input_pattern(arg)) {
                    ...
                    *result = ai_lint_result_create(
                        ...
                        "Passing unvalidated user input to dangerous functions can lead to injection attacks",
                        "Validate and sanitize all user input before using in system calls or queries",
                        location,
...
bool check_hardcoded_secrets(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    if (!node || node->type != AST_STRING_LITERAL) return false;
    ...
    if (!looks_like_secret && strlen(str_value) > 20) {
        ...
        for (const char *p = str_value; *p; p++) {
            if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z')) alpha_count++;
    ...
    if (looks_like_secret) {
        DiagnosticSpan location = {
            ...
            .start_column = node->location.column,
            .end_line = node->location.line,
            .end_column = node->location.column + (int)strlen(str_value) + 2, // Include quotes
            .file_path = strdup("current_file")
        };
    ...
    if (node->type == AST_UNARY_EXPR && 
        ...
        if (operand && operand->type == AST_IDENTIFIER) {
            ...
            *result = ai_lint_result_create(
                ...
                "Wrap pointer operations in unsafe blocks and add null pointer checks",
                location,
                0.7f, // Good confidence
                0.7f, // Good impact
...
bool check_missing_error_handling(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    ...
    bool returns_result = (strstr(func_name, "parse") != NULL ||
                          strstr(func_name, "read") != NULL ||
                          strstr(func_name, "write") != NULL ||
                          strstr(func_name, "open") != NULL ||
                          strstr(func_name, "connect") != NULL ||
                          strstr(func_name, "send") != NULL ||
    ...
    if (returns_result) {
        ...
        *result = ai_lint_result_create(
            ...
            "Use match expressions or error propagation to handle Result types properly",
            location,
...
void register_security_rules(AILinter *linter) {
    ...
    AILintRule rule1 = {
        .rule_id = strdup("AI_SEC_001"),
        .category = AI_LINT_SECURITY,
        .default_severity = AI_LINT_ERROR,
        .title = strdup("Unsafe function usage"),
        .description = strdup("Using unsafe functions can lead to security vulnerabilities"),
