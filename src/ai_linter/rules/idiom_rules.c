        ...
        if (func_name && func_name->type == AST_IDENTIFIER &&
            ...
            if (contains_index_access_pattern(body, loop_var)) {
                DiagnosticSpan location = {
                    ...
                    .file_path = strdup("current_file") // Would be set properly in real implementation
                };
                ...
                *result = ai_lint_result_create(
                    ...
                    "Use iterator patterns instead of manual indexing for better readability and safety",
...
static bool check_explicit_return(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    if (!node || node->type != AST_RETURN_STMT) return false;
    ...
    if (parent && is_last_statement_in_function(node, parent)) {
        ...
        *result = ai_lint_result_create(
            ...
            "Remove 'return' keyword from the last expression in function body",
            location,
...
static bool check_nested_if_else(ASTNode *node, AsthraSemanticsAPI *api, AILintResult **result) {
    if (!node || node->type != AST_IF_STMT) return false;
    ...
    if (depth >= 3) {
        ...
        if (could_be_match_expression(node, api)) {
            ...
            *result = ai_lint_result_create(
                ...
                AI_LINT_IDIOM,
                AI_LINT_SUGGESTION,
                "Deep if-else nesting detected",
                "Consider using match expressions for complex conditional logic",
                "Replace nested if-else with match expression for better readability",
                location,
        ...
        case AST_BLOCK:
            if (body->data.block.statements) {
                for (size_t i = 0; i < body->data.block.statements->count; i++) {
                    if (contains_index_access_pattern(&body->data.block.statements->nodes[i], loop_var)) {
                        return true;
                    }
                }
            }
...
static int count_if_else_depth(ASTNode *if_stmt) {
    if (!if_stmt || if_stmt->type != AST_IF_STMT) return 0;
...
void register_idiom_rules(AILinter *linter) {
    ...
    AILintRule rule1 = {
        ...
        .ai_guidance = strdup("Replace 'for i in range(0, vec.len()) { vec[i] }' with 'for item in vec { item }'"),
    ...
    AILintRule rule3 = {
        ...
        .ai_guidance = strdup("Replace nested if-else with match expression for better readability"),
