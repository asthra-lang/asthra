...
static bool is_string_type(const char *var_name, AsthraSemanticsAPI *api) {
    ...
    return (strstr(var_name, "str") != NULL ||
            strstr(var_name, "message") != NULL ||
            strstr(var_name, "text") != NULL ||
            strstr(var_name, "content") != NULL);
}
...
static bool is_vec_new_call(ASTNode *node) {
    ...
    if (!func || func->type != AST_ASSOCIATED_FUNC_CALL) return false;
    ...
    if (struct_name && strcmp(struct_name, "Vec") == 0 &&
        func_name && strcmp(func_name, "new") == 0) {
        return true;
    }
                ...
                if (lvalue && lvalue->type == AST_IDENTIFIER) {
                    ...
                    if (contains_string_concatenation(stmt, var_name) &&
                        ...
                        *result = ai_lint_result_create(
                            ...
                            "Repeated string concatenation has O(n²) complexity. Use StringBuilder for better performance",
                    ...
                    if (func && func->type == AST_FIELD_ACCESS) {
                        ...
                        if (method_name && strcmp(method_name, "clone") == 0) {
                            ...
                            *result = ai_lint_result_create(
                                ...
                                "Use references (&item) or move semantics instead of .clone() in performance-critical loops",
                                location,
                    ...
                    if (stmt->type == AST_EXPR_STMT) {
                        ...
                        if (expr && expr->type == AST_INDEX_ACCESS) {
                            ...
                            *result = ai_lint_result_create(
                                ...
                                "Iterator patterns are often more efficient and safer than manual indexing",
...
void register_performance_rules(AILinter *linter) {
    ...
    AILintRule string_concat_rule = {
        ...
        .description = "Repeated string concatenation has O(n²) complexity. Use StringBuilder for better performance",
        .ai_guidance = "Replace 'str = str + other' with StringBuilder.append() for efficient concatenation",
    ...
    AILintRule preallocation_rule = {
        ...
        .description = "Pre-allocate collections when size is predictable to avoid repeated reallocations",
        .ai_guidance = "Use Vec::with_capacity(size) instead of Vec::new() when iteration count is known",
    ...
    AILintRule cloning_rule = {
        ...
        .description = "Cloning objects in loops can be expensive. Consider borrowing or moving instead",
        .ai_guidance = "Use references (&item) or move semantics instead of .clone() in performance-critical loops",
    ...
    AILintRule indexing_rule = {
        ...
        .ai_guidance = "Replace 'for i in range(0, vec.len()) { vec[i] }' with 'for item in vec { item }'",
