...
const char *ai_lint_category_to_string(AILintCategory category) {
    ...
    case AI_LINT_IDIOM:
        return "idiom";
    case AI_LINT_PERFORMANCE:
        return "performance";
    case AI_LINT_SECURITY:
        return "security";
    case AI_LINT_CONCURRENCY:
        return "concurrency";
    case AI_LINT_MAINTAINABILITY:
...
const char *ai_lint_severity_to_string(AILintSeverity severity) {
    switch (severity) {
    case AI_LINT_SUGGESTION:
        return "suggestion";
    case AI_LINT_WARNING:
        return "warning";
    case AI_LINT_ERROR:
        return "error";
    default:
...
void ai_linter_enable_category(AILinter *linter, AILintCategory category, bool enabled) {
    if (!linter || category < 0 || category >= 5)
        return;
...
static void analyze_ast_node(AILinter *linter, ASTNode *node, AILintResult ***results,
    ...
    for (size_t i = 0; i < linter->rule_count; i++) {
        ...
        if (!linter->enabled_categories[rule->category])
            continue;
        ...
        if (rule->check_function(node, linter->semantic_api, &result)) {
            ...
            if (*result_count >= *result_capacity) {
                ...
                AILintResult **new_results =
                    realloc(*results, sizeof(AILintResult *) * new_capacity);
                if (!new_results) {
    ...
    case AST_PROGRAM:
        if (node->data.program.declarations) {
            for (size_t i = 0; i < node->data.program.declarations->count; i++) {
                analyze_ast_node(linter, node->data.program.declarations->nodes[i], results,
    ...
    case AST_FUNCTION_DECL:
        if (node->data.function_decl.body) {
            analyze_ast_node(linter, node->data.function_decl.body, results, result_count,
                             result_capacity);
        }
        break;
    case AST_BLOCK:
        if (node->data.block.statements) {
            for (size_t i = 0; i < node->data.block.statements->count; i++) {
                analyze_ast_node(linter, node->data.block.statements->nodes[i], results,
                                 result_count, result_capacity);
            }
        }
        break;
...
AILintResult **ai_linter_analyze_file(AILinter *linter, const char *file_path,
                                      size_t *result_count) {
    if (!linter || !file_path || !result_count)
        return NULL;
