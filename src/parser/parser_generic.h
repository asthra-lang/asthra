...
#define parse_node(parser, type)                                                                   \
    _Generic((type), ASTNodeType: parse_node_by_type_impl, const char *: parse_node_by_name_impl)( \
        parser, type)

#define expect_token_safe(parser, expected)                                                        \
    _Generic((expected), TokenType: expect_token_impl, const char *: expect_token_by_name_impl)(   \
        parser, expected)

#define consume_token_safe(parser, expected)                                                       \
    _Generic((expected), TokenType: consume_token_impl, const char *: consume_token_by_name_impl)( \
        parser, expected)

// Type-safe error reporting with _Generic
#define report_parse_error(parser, ...)                                                            \
    _Generic((parser), Parser *: report_parse_error_impl)(parser, __VA_ARGS__)

#define report_parse_warning(parser, ...)                                                          \
    _Generic((parser), Parser *: report_parse_warning_impl)(parser, __VA_ARGS__)

// Type-safe AST creation with compound literals
#define CREATE_BINARY_EXPR_SAFE(parser, op, left, right)                                           \
    create_ast_node_safe(                                                                          \
        parser, AST_BINARY_EXPR,                                                                   \
...
#define CREATE_IDENTIFIER_SAFE(parser, name)                                                       \
    create_ast_node_safe(parser, AST_IDENTIFIER, (ASTNodeData){.identifier = {.name = (name)}})
