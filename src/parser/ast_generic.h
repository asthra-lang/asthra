...
#define ast_get_child_count(node) _Generic((node), \
    ASTNode*: ast_get_child_count_impl, \
    const ASTNode*: ast_get_child_count_impl \
)(node)

#define ast_get_child(node, index) _Generic((node), \
    ASTNode*: ast_get_child_impl, \
    const ASTNode*: ast_get_child_const_impl \
)(node, index)

#define ast_set_child(node, index, child) _Generic((node), \
    ASTNode*: ast_set_child_impl \
)(node, index, child)

#define ast_add_child(node, child) _Generic((node), \
    ASTNode*: ast_add_child_impl \
)(node, child)

#define ast_remove_child(node, index) _Generic((node), \
    ASTNode*: ast_remove_child_impl \
)(node, index)

// Phase 2.1: Enhanced node creation with compound literals
typedef union {
    struct { BinaryOperator operator; ASTNode *left; ASTNode *right; } binary_expr;
    struct { UnaryOperator operator; ASTNode *operand; } unary_expr;
    ...
    struct { uint32_t value; } char_literal;  // Unicode code point
...
#define AST_CREATE_BINARY_EXPR(op, left, right, loc) \
    ast_create_node_with_data(AST_BINARY_EXPR, loc, \
...
#define AST_CREATE_IDENTIFIER(name, loc) \
    ast_create_node_with_data(AST_IDENTIFIER, loc, \
...
#define ast_validate_node(node) _Generic((node), \
    ASTNode*: ast_validate_node_impl, \
    const ASTNode*: ast_validate_node_const_impl \
)(node)

#define ast_is_expression(node) _Generic((node), \
    ASTNode*: ast_is_expression_impl, \
    const ASTNode*: ast_is_expression_impl \
)(node)

#define ast_is_statement(node) _Generic((node), \
    ASTNode*: ast_is_statement_impl, \
    const ASTNode*: ast_is_statement_impl \
)(node)

#define ast_is_declaration(node) _Generic((node), \
    ASTNode*: ast_is_declaration_impl, \
    const ASTNode*: ast_is_declaration_impl \
)(node)
