...
ASTNode *parse_extern_decl(Parser *parser) {
    ...
    // Parse extern parameter list according to v1.25 grammar: ExternParamList <- ExternParam (',' ExternParam)* / 'none'
    ...
    if (match_token(parser, TOKEN_NONE)) {
        ...
        report_error(parser, "Unexpected 'void' in extern parameter list. Use 'none' for empty parameters");
        return NULL;
    } else {
...
ASTNode *parse_extern_param(Parser *parser) {
    ...
    if (match_token(parser, TOKEN_MUT)) {
        report_error(parser, 
            "Extern function parameters cannot have 'mut' keyword. "
            "Parameters are always immutable in Asthra. "
            "Mutable access to extern function parameters should be handled through pointer types.");
