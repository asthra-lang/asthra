...
ASTNode *parse_enum_decl(Parser *parser) {
    ...
    if (match_token(parser, TOKEN_IDENTIFIER)) {
        ...
        char error_msg[256];
        snprintf(error_msg, sizeof(error_msg), "Expected enum name, got %s",
    ...
    if (match_token(parser, TOKEN_LESS_THAN)) {
        ...
        do {
            ...
            if (match_token(parser, TOKEN_COLON)) {
                report_error(parser, "Type parameter constraints are not supported. Use 'enum "
                                     "Name<T>' without constraints.");
    ...
    if (match_token(parser, TOKEN_NONE)) {
        ...
        do {
            ...
            if (match_token(parser, TOKEN_COMMA)) {
                ...
                report_error(parser,
                             "Expected ',' between enum variants or '}' after last variant");
                for (size_t i = 0; i < variant_count; i++) {
                    ast_free_node(variants[i]);
                }
                free(variants);
