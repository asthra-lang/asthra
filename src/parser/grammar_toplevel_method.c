...
ASTNode *parse_impl_block(Parser *parser) {
    ...
    node->data.impl_block.annotations = NULL; // impl blocks don't have annotations in current design
...
ASTNode *parse_method_decl(Parser *parser) {
    ...
    // Parse method parameter list according to v1.25 grammar: MethodParamList <- SelfParam (',' Param)* / ParamList
    ...
    if (match_token(parser, TOKEN_NONE)) {
        ...
        report_error(parser, "Unexpected 'void' in method parameter list. Use 'none' for empty parameters in v1.19");
        free(method_name);
        return NULL;
