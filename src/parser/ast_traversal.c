    ...
    switch (node->type) {
        case AST_PROGRAM:
            return (node->data.program.imports ? ast_node_list_size(node->data.program.imports) : 0) +
                   (node->data.program.declarations ? ast_node_list_size(node->data.program.declarations) : 0) +
                   (node->data.program.package_decl ? 1 : 0);
                   
        case AST_FUNCTION_DECL:
            return (node->data.function_decl.params ? ast_node_list_size(node->data.function_decl.params) : 0) +
                   (node->data.function_decl.return_type ? 1 : 0) +
                   (node->data.function_decl.body ? 1 : 0) +
                   (node->data.function_decl.annotations ? ast_node_list_size(node->data.function_decl.annotations) : 0);
                   
        case AST_STRUCT_DECL:
            return (node->data.struct_decl.fields ? ast_node_list_size(node->data.struct_decl.fields) : 0) +
                   (node->data.struct_decl.type_params ? ast_node_list_size(node->data.struct_decl.type_params) : 0) +
                   (node->data.struct_decl.annotations ? ast_node_list_size(node->data.struct_decl.annotations) : 0);
                   
        case AST_ENUM_DECL:
            return (node->data.enum_decl.variants ? ast_node_list_size(node->data.enum_decl.variants) : 0) +
                   (node->data.enum_decl.type_params ? ast_node_list_size(node->data.enum_decl.type_params) : 0) +
                   (node->data.enum_decl.annotations ? ast_node_list_size(node->data.enum_decl.annotations) : 0);
        ...
        case AST_STRUCT_LITERAL:
            return (node->data.struct_literal.type_args ? ast_node_list_size(node->data.struct_literal.type_args) : 0) +
                   (node->data.struct_literal.field_inits ? ast_node_list_size(node->data.struct_literal.field_inits) : 0);
                   
        case AST_IMPL_BLOCK:
            return (node->data.impl_block.methods ? ast_node_list_size(node->data.impl_block.methods) : 0) +
                   (node->data.impl_block.annotations ? ast_node_list_size(node->data.impl_block.annotations) : 0);
                   
        case AST_METHOD_DECL:
            return (node->data.method_decl.params ? ast_node_list_size(node->data.method_decl.params) : 0) +
                   (node->data.method_decl.return_type ? 1 : 0) +
                   (node->data.method_decl.body ? 1 : 0) +
                   (node->data.method_decl.annotations ? ast_node_list_size(node->data.method_decl.annotations) : 0);
            
        case AST_SLICE_TYPE:
            return node->data.slice_type.element_type ? 1 : 0;
            
        case AST_ARRAY_TYPE:
            return (node->data.array_type.element_type ? 1 : 0) +
                   (node->data.array_type.size ? 1 : 0);
            
        case AST_STRUCT_TYPE:
            return node->data.struct_type.type_args ? ast_node_list_size(node->data.struct_type.type_args) : 0;
            
        case AST_ENUM_TYPE:
            return node->data.enum_type.type_args ? ast_node_list_size(node->data.enum_type.type_args) : 0;
            
        case AST_PTR_TYPE:
            return node->data.ptr_type.pointee_type ? 1 : 0;
            
        case AST_CONST_DECL:
            return (node->data.const_decl.type ? 1 : 0) +
                   (node->data.const_decl.value ? 1 : 0) +
                   (node->data.const_decl.annotations ? ast_node_list_size(node->data.const_decl.annotations) : 0);
...
ASTNode *ast_get_child_impl(ASTNode *node, size_t index) {
    ...
    switch (node->type) {
        case AST_BLOCK:
            if (node->data.block.statements && index < ast_node_list_size(node->data.block.statements)) {
                return ast_node_list_get(node->data.block.statements, index);
            }
            break;
            
        case AST_BINARY_EXPR:
            if (index == 0) return node->data.binary_expr.left;
            if (index == 1) return node->data.binary_expr.right;
            break;
            
        case AST_UNARY_EXPR:
            if (index == 0) return node->data.unary_expr.operand;
            break;
            
        case AST_ENUM_VARIANT:
            if (index == 0) return node->data.enum_variant.value;
            break;
            
        case AST_ENUM_TYPE:
            if (node->data.enum_type.type_args && index < ast_node_list_size(node->data.enum_type.type_args)) {
                return ast_node_list_get(node->data.enum_type.type_args, index);
            }
            break;
            
        case AST_SLICE_TYPE:
            if (index == 0) return node->data.slice_type.element_type;
            break;
            
        case AST_ARRAY_TYPE:
            if (index == 0) return node->data.array_type.element_type;
            if (index == 1) return node->data.array_type.size;
            break;
            
        case AST_PTR_TYPE:
            if (index == 0) return node->data.ptr_type.pointee_type;
            break;
...
bool ast_set_child_impl(ASTNode *node, size_t index, ASTNode *child) {
    ...
    switch (node->type) {
        case AST_BINARY_EXPR:
            if (index == 0) {
                if (node->data.binary_expr.left) ast_release_node(node->data.binary_expr.left);
                node->data.binary_expr.left = ast_retain_node(child);
                return true;
            }
            if (index == 1) {
                if (node->data.binary_expr.right) ast_release_node(node->data.binary_expr.right);
        ...
        case AST_UNARY_EXPR:
            if (index == 0) {
                if (node->data.unary_expr.operand) ast_release_node(node->data.unary_expr.operand);
...
bool ast_is_declaration_impl(const ASTNode *node) {
    ...
    return node->type == AST_FUNCTION_DECL ||
           node->type == AST_STRUCT_DECL ||
           node->type == AST_STRUCT_FIELD ||
           node->type == AST_ENUM_DECL ||
           node->type == AST_ENUM_VARIANT_DECL ||
           node->type == AST_EXTERN_DECL ||
           node->type == AST_PARAM_DECL ||
           node->type == AST_IMPL_BLOCK ||
