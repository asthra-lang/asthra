...
ASTNode *parse_top_level_decl(Parser *parser) {
    ...
    if (parser->current_token.type == TOKEN_IDENTIFIER) {
        ...
        if (id && strcmp(id, "type") == 0) {
            report_error(parser, "Type aliases are not yet implemented. Type aliases and generic constraints are planned future features.");
    ...
    if (match_token(parser, TOKEN_PUB)) {
        ...
        report_error(parser, "Expected explicit visibility modifier 'pub' or 'priv' (grammar requirement)");
