...
ASTNode *parse_var_decl(Parser *parser) {
    ...
    if (match_token(parser, TOKEN_LEFT_PAREN)) {
        ...
        if (!expect_token(parser, TOKEN_COLON)) {
            report_error(parser, "Expected ':' after pattern in declaration. "
...
ASTNode *parse_lvalue(Parser *parser) {
    ...
    while (true) {
        if (match_token(parser, TOKEN_DOT)) {
            ...
            ASTNode *field_access = ast_create_node(AST_FIELD_ACCESS, parser->current_token.location);
            if (!field_access) {
                free(field_name);
                ast_free_node(lvalue);
                return NULL;
            ...
            ASTNode *array_access = ast_create_node(AST_INDEX_ACCESS, parser->current_token.location);
            if (!array_access) {
                ast_free_node(index);
                ast_free_node(lvalue);
                return NULL;
