...
ASTNode *parse_match_stmt(Parser *parser) {
    ...
    while (!match_token(parser, TOKEN_RIGHT_BRACE) && !at_end(parser)) {
        ...
        if (match_token(parser, TOKEN_COMMA)) {
            ...
            // If no comma and not at end, we still continue (match arms can be separated by
            // nothing) This maintains compatibility with grammar while supporting comma separators
        }
    }
