...
void pop_parse_context(Parser *parser) {
    ...
    ExtendedParser *ext_parser = (ExtendedParser*)parser;
    if (!ext_parser->context_stack) return;
...
ParseContext current_parse_context(Parser *parser) {
    if (!parser) return PARSE_CONTEXT_EXPRESSION;
    
    ExtendedParser *ext_parser = (ExtendedParser*)parser;
    if (!ext_parser->context_stack) return PARSE_CONTEXT_EXPRESSION;
...
void register_symbol(Parser *parser, const char *name, ASTNode *declaration) {
    ...
    ExtendedParser *ext_parser = (ExtendedParser*)parser;
    if (!ext_parser->current_scope) return;
...
bool is_postfix_suffix_start(Parser *parser) {
    ...
    return current == TOKEN_LEFT_PAREN || current == TOKEN_DOT || 
           current == TOKEN_LEFT_BRACKET;
}
...
bool is_lvalue_context(Parser *parser) {
    ...
    while (!at_end(&parser_copy)) {
        ...
        if (type == TOKEN_LEFT_PAREN) paren_level++;
        else if (type == TOKEN_RIGHT_PAREN) paren_level--;
        else if (type == TOKEN_LEFT_BRACKET) bracket_level++;
        else if (type == TOKEN_RIGHT_BRACKET) bracket_level--;
        else if (type == TOKEN_LEFT_BRACE) brace_level++;
        else if (type == TOKEN_RIGHT_BRACE) brace_level--;
        
        // Stop at expression boundaries when delimiters are balanced
        else if ((type == TOKEN_SEMICOLON || type == TOKEN_COMMA) && 
                 paren_level == 0 && bracket_level == 0 && brace_level == 0) {
            break;
        }
