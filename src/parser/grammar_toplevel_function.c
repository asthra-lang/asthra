...
ASTNode *parse_function_decl(Parser *parser) {
    ...
    // Parse parameter list according to current grammar: ParamList <- Param (',' Param)* / 'none'
    ASTNodeList *params = NULL;
    
    if (match_token(parser, TOKEN_NONE)) {
        // Explicit none for empty parameter list (semantic clarity)
        advance_token(parser);
        params = NULL; // No parameters
    } else if (match_token(parser, TOKEN_VOID)) {
        // Legacy compatibility error with helpful message
        report_error(parser, "Unexpected 'void' in parameter list. Use 'none' for empty parameters");
        free(function_name);
        return NULL;
    } else {
    ...
    // Expect closing parenthesis for all parameter list cases
    if (!expect_token(parser, TOKEN_RIGHT_PAREN)) {
        if (params) ast_node_list_destroy(params);
        free(function_name);
        return NULL;
    }
    ...
    ASTNode *return_annotation = NULL;  // NEW: SafeFFIAnnotation for return type
    
    if (match_token(parser, TOKEN_ARROW)) {
        advance_token(parser);
        
        // NEW: Check for SafeFFIAnnotation before type (Grammar: '->' SafeFFIAnnotation? Type)
        ...
        if (!return_type) {
            free(function_name);
            if (return_annotation) ast_free_node(return_annotation);
            return NULL;
        }
    }
    ...
    if (!body) {
        free(function_name);
        if (return_type) ast_free_node(return_type);
        if (return_annotation) ast_free_node(return_annotation);
        return NULL;
    }
    
    ASTNode *node = ast_create_node(AST_FUNCTION_DECL, start_loc);
    if (!node) {
        free(function_name);
        if (return_type) ast_free_node(return_type);
        if (return_annotation) ast_free_node(return_annotation);
    ...
    // NEW: Store return type annotation in annotations list
    if (return_annotation) {
        node->data.function_decl.annotations = ast_node_list_create(1);
        if (node->data.function_decl.annotations) {
            ast_node_list_add(&node->data.function_decl.annotations, return_annotation);
        } else {
...
ASTNode *parse_param(Parser *parser) {
    ...
    // Parse optional SafeFFIAnnotation
    ...
    // v1.25: Check for invalid mut in parameter (function parameters are always immutable)
    if (match_token(parser, TOKEN_MUT)) {
        report_error(parser, 
            "Function parameters cannot have 'mut' keyword. "
            "Parameters are always immutable in Asthra. "
            "If you need to modify the value, create a local mutable copy: 'let mut local_copy = param_name;'");
    ...
    // Create parameter declaration node
