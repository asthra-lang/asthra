...
ASTNode *parse_function_decl(Parser *parser) {
    ...
    if (match_token(parser, TOKEN_NONE)) {
        ...
        report_error(parser,
                     "Unexpected 'void' in parameter list. Use 'none' for empty parameters");
        free(function_name);
        return NULL;
    } else {
...
ASTNode *parse_param(Parser *parser) {
    ...
    if (match_token(parser, TOKEN_MUT)) {
        report_error(parser, "Function parameters cannot have 'mut' keyword. "
                             "Parameters are always immutable in Asthra. "
                             "If you need to modify the value, create a local mutable copy: 'let "
                             "mut local_copy = param_name;'");
        if (annotation)
