...
bool is_multiline_string_start(Lexer *lexer) {
    ...
    if (lexer->position + 3 < lexer->source_length && peek_char(lexer, 0) == 'r' &&
        peek_char(lexer, 1) == '"' && peek_char(lexer, 2) == '"' && peek_char(lexer, 3) == '"') {
        return true;
    }

    // Check for processed multi-line string: """
    if (lexer->position + 2 < lexer->source_length && peek_char(lexer, 0) == '"' &&
