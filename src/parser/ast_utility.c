...
void ast_print(ASTNode *node, int indent) {
    ...
    switch (node->type) {
        case AST_IMPORT_DECL:
            printf(" (path: \"%s\"", node->data.import_decl.path ? node->data.import_decl.path : "null");
        ...
        case AST_STRING_LITERAL:
            printf(" (\"%s\")", node->data.string_literal.value ? node->data.string_literal.value : "null");
            break;
        ...
        case AST_STRUCT_LITERAL:
            printf(" (%s with %zu fields)", 
                   node->data.struct_literal.struct_name ? node->data.struct_literal.struct_name : "null",
        ...
        case AST_ENUM_VARIANT:
            printf(" (%s.%s)", 
                   ...
                   node->data.enum_variant.variant_name ? node->data.enum_variant.variant_name : "null");
            break;
        case AST_STRUCT_PATTERN:
            printf(" (%s with %zu field patterns)", 
                   node->data.struct_pattern.struct_name ? node->data.struct_pattern.struct_name : "null",
                   node->data.struct_pattern.field_patterns ? ast_node_list_size(node->data.struct_pattern.field_patterns) : 0);
            break;
        ...
        case AST_ASSOCIATED_FUNC_CALL:
            printf(" (struct: %s, func: %s, type_args: %s)", 
                   node->data.associated_func_call.struct_name ? node->data.associated_func_call.struct_name : "null",
                   node->data.associated_func_call.function_name ? node->data.associated_func_call.function_name : "null",
                   node->data.associated_func_call.type_args ? "present" : "none");
            break;
        case AST_FIELD_ACCESS:
            printf(" (field: %s)", node->data.field_access.field_name ? node->data.field_access.field_name : "null");
            break;
        default:
            break;
    }
    ...
    switch (node->type) {
        ...
        case AST_STRUCT_PATTERN:
            if (node->data.struct_pattern.field_patterns) {
                for (size_t i = 0; i < ast_node_list_size(node->data.struct_pattern.field_patterns); i++) {
                    ASTNode *field_pattern = ast_node_list_get(node->data.struct_pattern.field_patterns, i);
                    if (field_pattern) {
...
bool ast_validate(ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case AST_BINARY_EXPR:
            return node->data.binary_expr.left != NULL && 
        ...
        case AST_UNARY_EXPR:
            return node->data.unary_expr.operand != NULL &&
                   ast_validate(node->data.unary_expr.operand);
...
ASTNode *ast_find_node(ASTNode *root, ASTNodeType type, const char *name) {
    ...
    // Check current node
    if (root->type == type) {
        if (!name) return root;  // Type match is enough
        ...
        switch (type) {
            ...
            case AST_ENUM_DECL:
                ...
                break;
            case AST_ENUM_VARIANT_DECL:
                if (root->data.enum_variant_decl.name && strcmp(root->data.enum_variant_decl.name, name) == 0) {
                    return root;
                }
                break;
            default:
...
const char *ast_node_type_name(ASTNodeType type) {
    switch (type) {
        ...
        case AST_VISIBILITY_MODIFIER: return "VISIBILITY_MODIFIER";
        ...
        case AST_ENUM_VARIANT_DECL: return "ENUM_VARIANT_DECL";
        ...
        case AST_IF_LET_STMT: return "IF_LET_STMT";
        case AST_SPAWN_STMT: return "SPAWN_STMT";
        case AST_SPAWN_WITH_HANDLE_STMT: return "SPAWN_WITH_HANDLE_STMT";
        case AST_AWAIT_EXPR: return "AWAIT_EXPR";
        ...
        case AST_UNSAFE_BLOCK: return "UNSAFE_BLOCK";
        case AST_BINARY_EXPR: return "BINARY_EXPR";
        case AST_UNARY_EXPR: return "UNARY_EXPR";
        case AST_POSTFIX_EXPR: return "POSTFIX_EXPR";
        case AST_CALL_EXPR: return "CALL_EXPR";
        case AST_ASSOCIATED_FUNC_CALL: return "ASSOCIATED_FUNC_CALL";
        case AST_FIELD_ACCESS: return "FIELD_ACCESS";

        case AST_INDEX_ACCESS: return "INDEX_ACCESS";
        case AST_SLICE_LENGTH_ACCESS: return "SLICE_LENGTH_ACCESS";
        ...
        case AST_UNIT_LITERAL: return "UNIT_LITERAL";
        case AST_STRUCT_LITERAL: return "STRUCT_LITERAL";
        case AST_IDENTIFIER: return "IDENTIFIER";
        case AST_BASE_TYPE: return "BASE_TYPE";
        case AST_SLICE_TYPE: return "SLICE_TYPE";
        case AST_ARRAY_TYPE: return "ARRAY_TYPE";
        case AST_STRUCT_TYPE: return "STRUCT_TYPE";
        case AST_ENUM_TYPE: return "ENUM_TYPE";
        case AST_PTR_TYPE: return "PTR_TYPE";
        case AST_RESULT_TYPE: return "RESULT_TYPE";
        case AST_MATCH_ARM: return "MATCH_ARM";
        case AST_PATTERN: return "PATTERN";
        case AST_ENUM_PATTERN: return "ENUM_PATTERN";
        case AST_STRUCT_PATTERN: return "STRUCT_PATTERN";
        case AST_FIELD_PATTERN: return "FIELD_PATTERN";
        case AST_ENUM_VARIANT: return "ENUM_VARIANT";
        case AST_SEMANTIC_TAG: return "SEMANTIC_TAG";
        case AST_OWNERSHIP_TAG: return "OWNERSHIP_TAG";
        case AST_FFI_ANNOTATION: return "FFI_ANNOTATION";
        case AST_SECURITY_TAG: return "SECURITY_TAG";
        case AST_HUMAN_REVIEW_TAG: return "HUMAN_REVIEW_TAG";
...
const char *binary_operator_name(BinaryOperator op) {
    switch (op) {
        ...
        case BINOP_NE: return "NE";
        case BINOP_LT: return "LT";
        case BINOP_LE: return "LE";
        case BINOP_GT: return "GT";
        case BINOP_GE: return "GE";
        case BINOP_AND: return "AND";
        case BINOP_OR: return "OR";
        case BINOP_BITWISE_AND: return "BITWISE_AND";
        case BINOP_BITWISE_OR: return "BITWISE_OR";
        case BINOP_BITWISE_XOR: return "BITWISE_XOR";
        case BINOP_LSHIFT: return "LSHIFT";
        case BINOP_RSHIFT: return "RSHIFT";
        default: return "UNKNOWN";
    }
}
...
const char *unary_operator_name(UnaryOperator op) {
    switch (op) {
        case UNOP_MINUS: return "MINUS";
        case UNOP_NOT: return "NOT";
        case UNOP_BITWISE_NOT: return "BITWISE_NOT";
        case UNOP_DEREF: return "DEREF";
        case UNOP_ADDRESS_OF: return "ADDRESS_OF";
