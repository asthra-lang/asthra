/**
 * Asthra Programming Language Compiler - Struct Pattern Grammar Implementation
 * Struct pattern parsing functions
 *
...
// =============================================================================
// STRUCT PATTERN PARSING
// =============================================================================

ASTNode *parse_struct_pattern_impl(Parser *parser, char *name, SourceLocation start_loc) {
...
ASTNode *parse_struct_pattern(Parser *parser) {
    ...
    while (!match_token(parser, TOKEN_RIGHT_BRACE) && !at_end(parser)) {
        ...
        if (match_token(parser, TOKEN_COMMA)) {
            ...
            report_error(parser,
                         "Expected ',' between field patterns or '}' after last field pattern");
            for (size_t i = 0; i < field_count; i++) {
                ast_free_node(field_patterns[i]);
            }
            free(field_patterns);
...
ASTNode *parse_field_pattern(Parser *parser) {
    ...
    if (!expect_token(parser, TOKEN_COLON)) {
        report_error(parser, "Expected ':' after field name in pattern. Use explicit syntax like "
    ...
    if (match_token(parser, TOKEN_IDENTIFIER) &&
        ...
        if (match_token(parser, TOKEN_DOT) || match_token(parser, TOKEN_LEFT_BRACE) ||
            ...
            if (match_token(parser, TOKEN_DOT)) {
                ...
                if (match_token(parser, TOKEN_LEFT_PAREN)) {
                    ...
                    if (match_token(parser, TOKEN_IDENTIFIER)) {
                        nested_pattern->data.enum_pattern.binding =
                            strdup(parser->current_token.data.identifier.name);
                        advance_token(parser);
                    }
        ...
        report_error(parser, "Expected struct name before '{' in nested pattern. Use syntax like "
                             "'field: StructName { ... }'");
        free(field_name);
        return NULL;
    } else {
        ...
        if (!nested_pattern) {
            report_error(
                parser, "Expected binding name, '_', or nested pattern after ':' in field pattern");
            free(field_name);
            return NULL;
        }
    }
