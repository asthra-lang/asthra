...
static void add_ai_annotation_to_result(AIAnnotationAnalysisResult *result,
                                        ASTNode *annotation_node, ASTNode *target);
static void add_validation_error(AIAnnotationAnalysisResult *result, AIAnnotation *annotation,
...
bool ai_annotation_check_conflicts(AIAnnotation *annotation, AIAnnotation *existing[],
    ...
    for (size_t i = 0; i < existing_count; i++) {
        ...
        if ((annotation->type == AI_ANNOTATION_CONFIDENCE &&
             existing[i]->type == AI_ANNOTATION_CONFIDENCE) ||
            (annotation->type == AI_ANNOTATION_HYPOTHESIS &&
             existing[i]->type == AI_ANNOTATION_HYPOTHESIS)) {
...
static void add_ai_annotation_to_result(AIAnnotationAnalysisResult *result,
                                        ASTNode *annotation_node, ASTNode *target) {
    if (!result || !annotation_node || !target)
        return;

    // Expand capacity if needed
    if (result->annotation_count >= result->capacity) {
        size_t new_capacity = result->capacity * 2;
        AIAnnotation *new_annotations =
            realloc(result->annotations, sizeof(AIAnnotation) * new_capacity);
        if (!new_annotations)
    ...
    if (annotation_node->data.semantic_tag.params &&
        ...
        case AI_ANNOTATION_CONFIDENCE:
            if (first_param->type == AST_STRING_LITERAL) {
                annotation->value.confidence_score =
                    (float)atof(first_param->data.string_literal.value);
...
static void add_validation_error(AIAnnotationAnalysisResult *result, AIAnnotation *annotation,
    ...
    char **new_errors =
        realloc(result->validation_errors, sizeof(char *) * (result->error_count + 1));
    if (!new_errors)
        return;
