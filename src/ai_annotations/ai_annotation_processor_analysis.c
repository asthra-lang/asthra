...
void ai_annotation_calculate_overall_confidence(AICompilationReport *report,
    ...
    report->overall_confidence =
        confidence_count > 0 ? total_confidence / (float)confidence_count : 0.0f;
}
...
void ai_annotation_extract_improvement_suggestions(AICompilationReport *report,
    ...
    for (size_t i = 0; i < analysis->annotation_count; i++) {
        AIAnnotation *annotation = &analysis->annotations[i];
        if (annotation->type == AI_ANNOTATION_OPTIMIZE || annotation->type == AI_ANNOTATION_TODO) {
            suggestion_count++;
        }
    }
...
void ai_annotation_extract_todo_items(AICompilationReport *report,
    ...
    for (size_t i = 0; i < analysis->annotation_count; i++) {
        ...
        if (annotation->type == AI_ANNOTATION_TODO) {
            if (annotation->value.text_value) {
                report->todo_items[report->todo_count] = strdup(annotation->value.text_value);
                report->todo_count++;
            }
        }
    }
...
void ai_annotation_calculate_quality_scores(AICompilationReport *report,
    ...
    if (report->test_coverage_score > 1.0f)
        report->test_coverage_score = 1.0f;
    if (report->security_score > 1.0f)
        report->security_score = 1.0f;
    if (report->maintainability_score > 1.0f)
        report->maintainability_score = 1.0f;
}
...
AIAnnotationMetadata *ai_annotation_generate_learning_data(AIAnnotationAnalysisResult *analysis) {
    ...
    if (analysis->valid_annotations > analysis->invalid_annotations) {
        data->reasoning =
            strdup("High annotation validity rate indicates good AI annotation patterns");
