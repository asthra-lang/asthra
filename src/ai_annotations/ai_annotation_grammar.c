...
} ai_annotation_map[] = {{"ai_confidence", AI_ANNOTATION_CONFIDENCE},
    ...
    *param_count = 0;
    ...
    while (!match_token(parser, TOKEN_RIGHT_PAREN) && !at_end(parser)) {
        ...
        if (match_token(parser, TOKEN_STRING)) {
            ...
            char float_str[32];
            snprintf(float_str, sizeof(float_str), "%.2f",
                     parser->current_token.data.float_val.value);
            ...
            report_error(
                parser,
                "Expected annotation value (string, identifier, integer, float, or boolean)");
            free_ai_annotation_params(params, *param_count);
            return NULL;
        }
...
bool validate_ai_hypothesis_annotation(AIAnnotationParam *params, size_t param_count) {
    ...
    return params[0].value.string_value && strlen(params[0].value.string_value) > 0;
}
...
bool validate_ai_review_annotation(AIAnnotationParam *params, size_t param_count) {
    ...
    return params[0].type == ANNOTATION_PARAM_STRING && params[0].value.string_value &&
...
bool validate_ai_refinement_annotation(AIAnnotationParam *params, size_t param_count) {
    ...
    return params[0].type == ANNOTATION_PARAM_INT && params[0].value.int_value > 0;
}
        ...
        if (node->data.semantic_tag.params) {
            for (size_t i = 0; i < param_count; i++) {
                ...
                case ANNOTATION_PARAM_STRING:
                    ...
                    if (param_node) {
                        param_node->data.string_literal.value =
                            strdup(params[i].value.string_value);
                    }
                    break;
