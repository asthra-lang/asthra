...
char *ai_annotation_generate_json_report(AICompilationReport *report) {
    ...
    // Improvement suggestions
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  \"improvement_suggestions\": [\n");
    if (report->suggestion_count > 0) {
        for (size_t i = 0; i < report->suggestion_count; i++) {
            offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, 
                              "    \"%s\"%s\n", report->improvement_suggestions[i], 
    ...
    // Security concerns
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  \"security_concerns\": [\n");
    if (report->concern_count > 0) {
        for (size_t i = 0; i < report->concern_count; i++) {
            offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, 
                              "    \"%s\"%s\n", report->security_concerns[i], 
                              i < report->concern_count - 1 ? "," : "");
        }
    }
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  ],\n");
    
    // TODO items
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  \"todo_items\": [\n");
    if (report->todo_count > 0) {
        for (size_t i = 0; i < report->todo_count; i++) {
            offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, 
                              "    \"%s\"%s\n", report->todo_items[i], 
                              i < report->todo_count - 1 ? "," : "");
        }
    }
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  ],\n");
    
    // Quality scores
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  \"quality_scores\": {\n");
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, 
                      "    \"test_coverage\": %.2f,\n", report->test_coverage_score);
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, 
                      "    \"security\": %.2f,\n", report->security_score);
    ...
    // Learning data
    offset += snprintf(buffer + (size_t)offset, buffer_size - (size_t)offset, "  \"learning_data\": ");
...
bool ai_annotation_emit_metadata(AIAnnotationProcessor *processor, 
    ...
    if (!processor || !output_path || !report) return false;
    
    // Generate JSON report
    char *json_report = ai_annotation_generate_json_report(report);
    if (!json_report) return false;
...
char *ai_annotation_generate_learning_report(AIAnnotationProcessor *processor, 
                                            AICompilationReport *report) {
    if (!processor || !report) return NULL;
