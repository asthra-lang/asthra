...
bool analyze_return_statement(SemanticAnalyzer *analyzer, ASTNode *stmt) {
    ...
    if (current_function && current_function->type && current_function->type->category == TYPE_FUNCTION) {
        ...
        if (expected_return_type && actual_return_type) {
            ...
            bool is_unit_return = (actual_return_type->category == TYPE_PRIMITIVE && 
            ...
            } else if (!semantic_check_type_compatibility(analyzer, actual_return_type, expected_return_type)) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_RETURN,
                                     stmt->location,
...
bool analyze_match_statement(SemanticAnalyzer *analyzer, ASTNode *stmt) {
    ...
    if (!has_catch_all) {
        ...
        semantic_report_warning(analyzer, stmt->location,
        ...
        switch (statement->type) {
            ...
            case AST_IF_STMT: {
                ...
                if (then_block && else_block &&
                    block_returns_never(analyzer, then_block) &&
