...
bool analyze_const_declaration(SemanticAnalyzer *analyzer, ASTNode *const_decl) {
    ...
    if (!validate_const_type_compatibility(analyzer, const_type, const_value, const_decl->location)) {
    ...
    SymbolEntry *const_symbol = symbol_entry_create(const_name, SYMBOL_CONST, const_type, const_decl);
    if (!const_symbol) {
        const_value_destroy(const_value);
        type_descriptor_release(const_type);
 ...
 */
static bool validate_integer_range(SemanticAnalyzer *analyzer, 
                                 PrimitiveKind int_type, 
                                 int64_t value, 
    ...
    switch (int_type) {
        ...
        case PRIMITIVE_I16:
            if (value < -32768 || value > 32767) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                                     location,
                                     "Value %lld out of range for i16 (-32768 to 32767)", (long long)value);
                return false;
            }
            break;
        ...
        case PRIMITIVE_U64:
            if (value < 0) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                                     location,
                                     "Value %lld cannot be negative for unsigned type", (long long)value);
                return false;
            }
            break;
...
bool validate_const_type_compatibility(SemanticAnalyzer *analyzer, 
                                      ...
                                      SourceLocation location) {
    if (!analyzer || !declared_type || !const_value) return false;
    ...
    if (declared_type->category == TYPE_PRIMITIVE) {
        switch (declared_type->data.primitive.primitive_kind) {
            ...
            case PRIMITIVE_U64:
                ...
                return validate_integer_range(analyzer, (PrimitiveKind)declared_type->data.primitive.primitive_kind, 
                                            const_value->data.integer_value, location);
                
            case PRIMITIVE_F32:
            case PRIMITIVE_F64:
                if (const_value->type != CONST_VALUE_FLOAT && const_value->type != CONST_VALUE_INTEGER) {
                    semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                                         location,
