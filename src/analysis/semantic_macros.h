...
#define semantic_analyze_node(analyzer, node) _Generic((node), \
    ASTNode*: semantic_analyze_node_impl, \
    const ASTNode*: semantic_analyze_node_const_impl \
)(analyzer, node)

#define semantic_check_type(analyzer, node, expected_type) _Generic((node), \
    ASTNode*: semantic_check_type_impl, \
    const ASTNode*: semantic_check_type_const_impl \
)(analyzer, node, expected_type)

#define semantic_resolve_symbol(analyzer, name) _Generic((name), \
    char*: semantic_resolve_symbol_impl, \
    const char*: semantic_resolve_symbol_impl \
)(analyzer, name)

// Type descriptor operations with _Generic
#define type_descriptor_create(category, ...) _Generic((category), \
    TypeCategory: type_descriptor_create_impl \
)(category, __VA_ARGS__)

#define type_descriptor_compare(type1, type2) _Generic((type1), \
    TypeDescriptor*: type_descriptor_compare_impl, \
    const TypeDescriptor*: type_descriptor_compare_const_impl \
)(type1, type2)

// Symbol table operations with thread safety
#define symbol_table_insert(table, name, symbol) _Generic((table), \
    SymbolTable*: symbol_table_insert_impl \
)(table, name, symbol)

#define symbol_table_lookup(table, name) _Generic((table), \
    SymbolTable*: symbol_table_lookup_impl, \
    const SymbolTable*: symbol_table_lookup_const_impl \
)(table, name)
...
bool semantic_check_type_const_impl(SemanticAnalyzer *analyzer, const ASTNode *node, TypeDescriptor *expected);
