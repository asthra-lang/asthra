...
static void add_allocation(MemoryManager *manager, MemoryAllocation *allocation) {
    ...
    while (current_bytes > peak_bytes) {
        if (atomic_compare_exchange_weak(&manager->zone_stats[zone].peak_bytes, &peak_bytes,
                                         current_bytes)) {
            break;
        }
    }
...
bool memory_manager_free(MemoryManager *manager, void *ptr, const OwnershipContext *ownership) {
    ...
    if (ownership && allocation->ownership.ownership_type != ownership->ownership_type) {
        atomic_fetch_add(
            &manager->zone_stats[allocation->ownership.memory_zone].ownership_violations, 1);
        if (manager->strict_mode) {
