    ...
    if (expr->type == AST_ENUM_VARIANT) {
            ...
            if (var_symbol && var_symbol->kind == SYMBOL_VARIABLE) {
                ...
                if (!var_symbol->type) {
                    semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH, expr->location,
                                          "Cannot access field on variable '%s' with unknown type",
                                          enum_name);
                    return false;
                }
                ...
                SymbolEntry *field_symbol = symbol_table_lookup_safe(
                    var_symbol->type->data.struct_type.fields, variant_name);
                if (!field_symbol) {
        ...
        if (!variant_symbol || variant_symbol->kind != SYMBOL_ENUM_VARIANT) {
            ...
            if (qualified_name) {
                ...
                SymbolEntry *qualified_variant =
                    semantic_resolve_identifier(analyzer, qualified_name);
                free(qualified_name);
        ...
        if (enum_symbol->is_generic) {
            ...
            if (!used_context && expr->data.enum_variant.value) {
                TypeDescriptor *value_type =
                    semantic_get_expression_type(analyzer, expr->data.enum_variant.value);
