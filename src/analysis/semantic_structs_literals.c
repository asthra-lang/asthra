...
bool analyze_struct_literal_expression(SemanticAnalyzer *analyzer, ASTNode *struct_literal) {
    ...
    if (struct_symbol->kind != SYMBOL_TYPE) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                             struct_literal->location,
                             "Symbol '%s' is not a type (kind: %d)", struct_name, struct_symbol->kind);
        return false;
    }
    ...
    if (type_args) {
        ...
        if (!struct_symbol->is_generic) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_TYPE,
                                 struct_literal->location,
                                 "Struct '%s' is not generic but type arguments provided", struct_name);
            return false;
        }
        ...
        if (arg_count != struct_symbol->type_param_count) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_TYPE,
                                 struct_literal->location,
                                 "Type argument count mismatch for struct '%s': expected %zu, got %zu",
                                 struct_name, struct_symbol->type_param_count, arg_count);
            return false;
        }
        ...
        for (size_t i = 0; i < arg_count; i++) {
            ...
            if (type_arg) {
                ...
                if (!arg_type) {
                    ...
                    semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_TYPE,
                                         type_arg->location,
                                         "Invalid type argument %zu for struct '%s'", i + 1, struct_name);
                    return false;
                }
        ...
        TypeDescriptor *instance_type = type_descriptor_create_generic_instance(
            struct_type, arg_types, arg_count);
    ...
    if (field_inits) {
        ...
        for (size_t i = 0; i < field_init_count; i++) {
            ...
            SymbolEntry *field_symbol = symbol_table_lookup_local(field_lookup_type->data.struct_type.fields, field_name);
            if (!field_symbol) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_UNDEFINED_SYMBOL,
                                     field_init->location,
            ...
            TypeDescriptor *actual_field_type = get_instance_field_type(actual_struct_type, field_symbol);
            ...
            if (field_value && actual_field_type) {
                ...
                if (value_type) {
                    bool compatible = semantic_check_type_compatibility(analyzer, value_type, actual_field_type);
                    if (!compatible) {
                        semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                                             field_init->location,
                                             "Type mismatch in field '%s': expected %s, got %s",
                                             field_name,
