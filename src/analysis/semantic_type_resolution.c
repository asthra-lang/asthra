...
TypeDescriptor *analyze_type_node(SemanticAnalyzer *analyzer, ASTNode *type_node) {
    ...
    case AST_BASE_TYPE: {
        ...
        // Type not found
        semantic_report_error(analyzer, SEMANTIC_ERROR_UNDEFINED_TYPE, type_node->location,
    ...
    case AST_STRUCT_TYPE: {
        ...
        SymbolEntry *type_symbol = symbol_table_lookup_safe(analyzer->current_scope, type_name);
        if (!type_symbol ||
            (type_symbol->kind != SYMBOL_TYPE && type_symbol->kind != SYMBOL_TYPE_PARAMETER)) {
        ...
        if (type_node->data.struct_type.type_args) {
            ...
            if (!type_symbol->is_generic) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_TYPE, type_node->location,
                                      "Type '%s' is not generic but type arguments provided",
                                      type_name);
            ...
            if (arg_count != type_symbol->type_param_count) {
                semantic_report_error(
                    analyzer, SEMANTIC_ERROR_INVALID_TYPE, type_node->location,
                    "Type argument count mismatch for '%s': expected %zu, got %zu", type_name,
                    type_symbol->type_param_count, arg_count);
                type_descriptor_release(resolved_type);
                return NULL;
            }
            ...
            TypeDescriptor *instance_type =
                type_descriptor_create_generic_instance(resolved_type, arg_types, arg_count);
            ...
            if (!instance_type) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_INTERNAL, type_node->location,
    ...
    case AST_ENUM_TYPE: {
        ...
        SymbolEntry *type_symbol = symbol_table_lookup_safe(analyzer->current_scope, type_name);
        if (!type_symbol || type_symbol->kind != SYMBOL_TYPE || !type_symbol->type) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_UNDEFINED_SYMBOL, type_node->location,
                                  "Unknown type: %s", type_name);
            return NULL;
        }
        ...
        if (type_node->data.enum_type.type_args) {
            ...
            if (arg_count != type_symbol->type_param_count) {
                semantic_report_error(
                    analyzer, SEMANTIC_ERROR_INVALID_TYPE, type_node->location,
                    "Type argument count mismatch for type '%s': expected %zu, got %zu", type_name,
                    type_symbol->type_param_count, arg_count);
                type_descriptor_release(base_type);
                return NULL;
            }
    ...
    case AST_ARRAY_TYPE: {
        ...
        if (size_value->type != CONST_VALUE_INTEGER) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH, size_node->location,
    ...
    case AST_OPTION_TYPE: {
        ...
        if (!option_symbol || option_symbol->kind != SYMBOL_TYPE || !option_symbol->type ||
            option_symbol->type->category != TYPE_ENUM) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_UNDEFINED_TYPE, type_node->location,
        ...
        TypeDescriptor *option_type =
            type_descriptor_create_generic_instance(option_symbol->type, type_args, 1);
