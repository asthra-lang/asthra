            ...
            if (expr->data.array_literal.elements) {
                ...
                for (size_t i = 0; i < element_count; i++) {
                    ...
                    if (!semantic_analyze_expression(analyzer, element)) {
                        if (expected_element_type) type_descriptor_release(expected_element_type);
                        result = false;
                        break;
                    }
                    ...
                    if (element_type) {
                        if (expected_element_type == NULL) {
                            ...
                            if (!semantic_check_type_compatibility(analyzer, element_type, expected_element_type)) {
                                semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                                                     element->location,
                                                     "Array element type mismatch: expected %s, got %s",
                                                     expected_element_type->name ? expected_element_type->name : "unknown",
            ...
            if (expr->data.tuple_literal.elements) {
                ...
                if (result) {
                    ...
                    if (element_types) {
                        ...
                        if (all_types_valid) {
                            TypeDescriptor *tuple_type = type_descriptor_create_tuple(element_types, element_count);
                            if (tuple_type) {
