...
bool validate_return_type_ffi_annotation(SemanticAnalyzer *analyzer, 
    ...
    if (!analyzer || !annotation_name || !function_node) return false;
    
    // Only transfer_full and transfer_none are allowed on return types
    if (strcmp(annotation_name, "borrowed") == 0) {
        report_invalid_ffi_annotation_context_error(analyzer, function_node, 
                                                   annotation_name, CONTEXT_RETURN_TYPE);
        return false;
    }
...
bool validate_parameter_ffi_annotation(SemanticAnalyzer *analyzer, 
                                      const char *annotation_name,
                                      ASTNode *param_node) {
    if (!analyzer || !annotation_name || !param_node) return false;
...
bool validate_ffi_transfer_annotation_context(SemanticAnalyzer *analyzer, 
    ...
    if (!analyzer || !annotation_name || !target_node) return false;
    ...
    switch (context) {
        ...
        default:
            ...
            report_invalid_ffi_annotation_context_error(analyzer, target_node, 
...
bool validate_ffi_annotation_mutual_exclusivity(SemanticAnalyzer *analyzer, 
    ...
    if (!analyzer || !node || !annotations) return true;
...
bool validate_function_ffi_annotations(SemanticAnalyzer *analyzer, ASTNode *func_node) {
    if (!analyzer || !func_node) return true;
    
    if (func_node->type != AST_FUNCTION_DECL) return true;
    ...
    if (params) {
        for (size_t i = 0; i < params->count; i++) {
            ...
            if (!param || param->type != AST_PARAM_DECL) continue;
            
            ASTNodeList *param_annotations = param->data.param_decl.annotations;
            if (!param_annotations) continue;
...
bool validate_extern_ffi_annotations(SemanticAnalyzer *analyzer, ASTNode *extern_node) {
    if (!analyzer || !extern_node) return true;
    
    if (extern_node->type != AST_EXTERN_DECL) return true;
    ...
    if (return_annotations) {
        // Check mutual exclusivity for return type annotations
        if (!validate_ffi_annotation_mutual_exclusivity(analyzer, extern_node, return_annotations)) {
            valid = false;
        }
...
void report_invalid_ffi_annotation_context_error(SemanticAnalyzer *analyzer,
                                                ...
                                                const char *annotation_name,
                                                AnnotationContext context) {
    if (!analyzer || !node || !annotation_name) return;
    ...
    switch (context) {
        case CONTEXT_RETURN_TYPE:
            if (strcmp(annotation_name, "borrowed") == 0) {
                ...
                snprintf(error_message, sizeof(error_message),
                        "FFI annotation '#[%s]' is not valid for return types.",
                        annotation_name);
            }
            break;
        ...
        case CONTEXT_PARAMETER:
            snprintf(error_message, sizeof(error_message),
                    "FFI annotation '#[%s]' is not valid for parameters.",
                    annotation_name);
            break;
...
void report_ffi_annotation_conflict_error(SemanticAnalyzer *analyzer,
    ...
    if (!analyzer || !node || !first_annotation || !second_annotation) return;
