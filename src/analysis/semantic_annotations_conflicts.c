...
bool check_annotation_conflicts(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    for (size_t i = 0; i < annotation_count; i++) {
        ...
        if (!annotation1 || annotation1->type != AST_SEMANTIC_TAG)
            continue;
        ...
        for (size_t j = i + 1; j < annotation_count; j++) {
            ...
            if (!annotation2 || annotation2->type != AST_SEMANTIC_TAG)
                continue;
            ...
            for (size_t k = 0; conflicts[k].annotation1 != NULL; k++) {
                ...
                if ((strcmp(conflict->annotation1, name1) == 0 &&
                     strcmp(conflict->annotation2, name2) == 0) ||
                    (strcmp(conflict->annotation1, name2) == 0 &&
                     strcmp(conflict->annotation2, name1) == 0)) {
                    ...
                    case CONFLICT_REDUNDANT:
                        issue_annotation_warning(analyzer, node, WARNING_MEDIUM,
                                                 conflict->resolution_hint);
                        break;

                    case CONFLICT_DEPRECATED_COMBINATION:
                        issue_annotation_warning(analyzer, node, WARNING_HIGH,
                                                 conflict->resolution_hint);
                        break;
                    }
                }
