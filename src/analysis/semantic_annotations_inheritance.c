...
static bool validate_function_annotation_inheritance(SemanticAnalyzer *analyzer, ASTNode *func_decl);
static bool validate_method_annotation_inheritance(SemanticAnalyzer *analyzer, ASTNode *method_decl);
static bool validate_struct_annotation_inheritance(SemanticAnalyzer *analyzer, ASTNode *struct_decl);
static bool validate_single_annotation_dependencies(SemanticAnalyzer *analyzer, ASTNode *node, 
                                                   const char *annotation_name, ASTNodeList *annotations);
...
bool validate_annotation_dependencies(SemanticAnalyzer *analyzer, ASTNode *node, ASTNodeList *annotations) {
    ...
    for (size_t i = 0; i < ast_node_list_size(annotations); i++) {
        ...
        if (!validate_single_annotation_dependencies(analyzer, node, annotation_name, annotations)) {
            return false;
        }
    }
...
static bool validate_single_annotation_dependencies(SemanticAnalyzer *analyzer, ASTNode *node, 
    ...
    static const struct {
        ...
        {"cache_friendly", "performance_critical", "Cache optimization requires performance critical marking"},
        {"constant_time", "security_critical", "Constant time operations should be security critical"},
        {NULL, NULL, NULL}
    };
    ...
    for (size_t i = 0; ANNOTATION_DEPENDENCIES[i].annotation != NULL; i++) {
        if (strcmp(annotation_name, ANNOTATION_DEPENDENCIES[i].annotation) == 0) {
            ...
            if (!found_required) {
                report_missing_dependency_error(analyzer, node, annotation_name, 
                                               required, ANNOTATION_DEPENDENCIES[i].reason);
                return false;
            }
        }
    }
