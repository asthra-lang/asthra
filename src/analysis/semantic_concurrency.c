    ...
    if (function_type && function_type->category == TYPE_FUNCTION) {
        ...
        if (expected_param_count != actual_arg_count) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH,
                                 ...
                                 function_name, expected_param_count, actual_arg_count);
            return false;
        }
...
bool analyze_spawn_with_handle_statement(SemanticAnalyzer *analyzer, ASTNode *stmt) {
    ...
    SymbolEntry *existing_handle = symbol_table_lookup_local(analyzer->current_scope, handle_var_name);
    if (existing_handle) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_DUPLICATE_SYMBOL,
                             stmt->location,
                             "Handle variable '%s' already defined in current scope", handle_var_name);
        return false;
    }
    
    // Create a symbol entry for the handle variable
    SymbolEntry *handle_symbol = symbol_entry_create(handle_var_name, 
                                                     SYMBOL_VARIABLE,
