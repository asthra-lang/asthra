...
void semantic_report_type_mismatch_detailed(
    ...
    if (!analyzer || !expected_type || !actual_type) return;
    
    char message[512];
    snprintf(message, sizeof(message), "type mismatch%s%s",
             context ? " in " : "",
             context ? context : "");
    ...
    if (expected_type->category == TYPE_PRIMITIVE && actual_type->category == TYPE_PRIMITIVE) {
        if (strcmp(expected_type->name, "int") == 0 && strcmp(actual_type->name, "float") == 0) {
            fprintf(stderr, "  " COLOR_GREEN "help" COLOR_RESET ": use 'as int' to convert float to int\n");
        } else if (strcmp(expected_type->name, "float") == 0 && strcmp(actual_type->name, "int") == 0) {
            fprintf(stderr, "  " COLOR_GREEN "help" COLOR_RESET ": use 'as float' to convert int to float\n");
        }
    }
}
...
void semantic_report_undefined_with_similar(
    ...
    if (!analyzer || !symbol_name) return;
    ...
    if (suggestion_count > 0) {
        semantic_report_error_with_suggestion(
            analyzer, SEMANTIC_ERROR_UNDEFINED_SYMBOL, location,
            message, symbol_name, (const char **)suggestions, suggestion_count
        );
    } else {
        semantic_report_error_enhanced(
            analyzer, SEMANTIC_ERROR_UNDEFINED_SYMBOL, location,
            message, NULL,
            "make sure the symbol is declared before use"
        );
    }
...
const char *get_diagnostic_color(DiagnosticLevel level) {
    switch (level) {
        case DIAG_LEVEL_ERROR:   return COLOR_RED;
        case DIAG_LEVEL_WARNING: return COLOR_YELLOW;
        case DIAG_LEVEL_NOTE:    return COLOR_BLUE;
        case DIAG_LEVEL_HELP:    return COLOR_GREEN;
...
void print_enhanced_diagnostic(
    ...
    switch (diagnostic->level) {
        ...
        case DIAG_LEVEL_WARNING: level_str = "warning"; break;
        case DIAG_LEVEL_NOTE:    level_str = "note"; break;
        case DIAG_LEVEL_HELP:    level_str = "help"; break;
    }
    
    if (use_color) {
        color = get_diagnostic_color(diagnostic->level);
        fprintf(stderr, "%s%s%s%s: %s\n", 
                COLOR_BOLD, color, level_str, COLOR_RESET, diagnostic->message);
    } else {
    ...
    if (diagnostic->context) {
        ...
        fprintf(stderr, "  %s→%s %s:%d:%d\n",
                ...
                ctx->primary_location.line,
                ctx->primary_location.column);
