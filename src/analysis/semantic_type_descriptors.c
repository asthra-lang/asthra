...
void type_descriptor_release(TypeDescriptor *type) {
    ...
    // // printf("DEBUG: type_descriptor_release called on type=%p, name='%s', category=%d,
    // ref_count=%u\n",
    ...
    if (type->name && type->category == TYPE_PRIMITIVE) {
        const char *primitive_names[] = {"void",  "bool", "i8",   "i16",    "i32",  "i64",  "i128",
                                         "isize", "u8",   "u16",  "u32",    "u64",  "u128", "usize",
                                         "f32",   "f64",  "char", "string", "never"};
