...
bool semantic_validate_ffi_type(SemanticAnalyzer *analyzer, TypeDescriptor *type) {
    ...
    case TYPE_SLICE:
        ...
        semantic_report_error(analyzer, SEMANTIC_ERROR_UNSUPPORTED_OPERATION,
                              ...
                              "Slices passed to FFI will decay to pointers. Ensure C function "
                              "expects pointer type. Length information is lost.");
    ...
    case TYPE_STRUCT:
        ...
        // For simplicity, for now, we only allow structs that are explicitly marked as
        // FFI-compatible or have a simple, flat structure (e.g., only primitive fields). This needs
        // significant enhancement for full FFI support.
        semantic_report_error(
            analyzer, SEMANTIC_ERROR_UNSUPPORTED_OPERATION,
            (SourceLocation){0}, // No specific location for type descriptor
    ...
    case TYPE_FUNCTION: // Function pointers (callbacks)
        semantic_report_error(
            analyzer, SEMANTIC_ERROR_UNSUPPORTED_OPERATION, (SourceLocation){0},
            "Function pointers (callbacks) in FFI require explicit type annotations and care.");
        return true; // Placeholder: assume compatible for now

    default:
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_TYPE, (SourceLocation){0},
...
bool semantic_check_ffi_annotations(SemanticAnalyzer *analyzer, ASTNodeList *annotations) {
    ...
    for (size_t i = 0; i < annotations->count; i++) {
        ...
        case FFI_BORROWED:
            // These are transfer semantics, validation depends on usage context (e.g., param/return
            // type) For now, their presence is syntactically valid.
            break;
        default:
            semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_ANNOTATION, annotation->location,
...
bool semantic_validate_extern_function(SemanticAnalyzer *analyzer, ASTNode *extern_decl) {
    ...
    if (return_type_node) {
        ...
        if (!semantic_validate_ffi_type(analyzer, return_type_desc)) {
            semantic_report_error(
                analyzer, SEMANTIC_ERROR_INVALID_TYPE, return_type_node->location,
    ...
    if (params) {
        for (size_t i = 0; i < params->count; i++) {
            ...
            if (param_decl->type != AST_PARAM_DECL) {
                semantic_report_error(
                    analyzer, SEMANTIC_ERROR_INTERNAL, param_decl->location,
                    "Expected parameter declaration in extern function parameter list");
                return false;
            }

            TypeDescriptor *param_type_desc =
                analyze_type_node(analyzer, param_decl->data.param_decl.type);
            if (!param_type_desc) {
            ...
            if (!semantic_validate_ffi_type(analyzer, param_type_desc)) {
                semantic_report_error(
                    analyzer, SEMANTIC_ERROR_INVALID_TYPE, param_decl->location,
            ...
            if (!semantic_check_ffi_annotations(analyzer,
                                                param_decl->data.param_decl.annotations)) {
                return false;
            }
        }
    }
