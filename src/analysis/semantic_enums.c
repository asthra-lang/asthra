...
bool analyze_enum_declaration(SemanticAnalyzer *analyzer, ASTNode *enum_decl) {
    ...
    if (!enum_name) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_DECLARATION,
                             enum_decl->location,
    ...
    if (enum_decl->data.enum_decl.type_params) {
        if (!validate_enum_type_parameters(analyzer, enum_decl->data.enum_decl.type_params, enum_decl->location)) {
            return false;
        }
    }
    ...
    if (is_generic) {
        ...
        if (!register_enum_type_parameters(analyzer, enum_decl->data.enum_decl.type_params, enum_decl->location)) {
    ...
    if (enum_decl->data.enum_decl.variants) {
        if (!check_duplicate_variants(analyzer, enum_decl->data.enum_decl.variants, enum_decl->location)) {
            return false;
        }
    }
    ...
    enum_type->data.enum_type.variant_count = enum_decl->data.enum_decl.variants ? 
        ast_node_list_size(enum_decl->data.enum_decl.variants) : 0;
    atomic_init(&enum_type->ref_count, 1);
    
    // Create a symbol table for enum variants
    enum_type->data.enum_type.variants = symbol_table_create(enum_type->data.enum_type.variant_count > 0 ? 
        enum_type->data.enum_type.variant_count : 4);
    ...
    if (!symbol_table_insert_safe(enum_scope, enum_name, enum_symbol)) {
        ...
        free((char*)enum_type->name);
        free(enum_type);
        semantic_report_error(analyzer, SEMANTIC_ERROR_SYMBOL_TABLE,
                             enum_decl->location,
        ...
        for (size_t i = 0; i < ast_node_list_size(variants); i++) {
            ...
            if (variant->type == AST_ENUM_VARIANT_DECL) {
                if (variant->data.enum_variant_decl.value) {
                    ...
                    if (!validate_enum_variant_value(analyzer, variant->data.enum_variant_decl.value, &explicit_value)) {
                        return false;
                    }
...
bool analyze_enum_variant_declaration(SemanticAnalyzer *analyzer, ASTNode *variant_decl, const char *enum_name, TypeDescriptor *enum_type) {
    ...
    SymbolEntry *existing = symbol_table_lookup_local(enum_type->data.enum_type.variants, variant_name);
    if (existing) {
        free(qualified_name);
        semantic_report_error(analyzer, SEMANTIC_ERROR_DUPLICATE_SYMBOL,
                             variant_decl->location,
                             "Variant '%s' already declared in enum '%s'", variant_name, enum_name);
        return false;
    }
    ...
    SymbolEntry *variant_symbol = symbol_entry_create(variant_name, SYMBOL_ENUM_VARIANT, variant_type, variant_decl);
    if (!variant_symbol) {
        free(qualified_name);
    ...
    if (variant_decl->data.enum_variant_decl.associated_type) {
        ...
        TypeDescriptor *associated_type = analyze_type_node(analyzer, variant_decl->data.enum_variant_decl.associated_type);
        if (!associated_type) {
    ...
    if (variant_decl->data.enum_variant_decl.value) {
        int64_t value;
        if (!validate_enum_variant_value(analyzer, variant_decl->data.enum_variant_decl.value, &value)) {
            symbol_entry_destroy(variant_symbol);
            free(qualified_name);
            return false;
        }
    ...
    if (!symbol_table_insert_safe(enum_type->data.enum_type.variants, variant_name, variant_symbol)) {
        symbol_entry_destroy(variant_symbol);
        free(qualified_name);
        semantic_report_error(analyzer, SEMANTIC_ERROR_SYMBOL_TABLE,
                             variant_decl->location,
                             "Failed to insert variant '%s' into enum variant table", variant_name);
        return false;
    }
    ...
    if (qualified_symbol) {
        ...
        if (!symbol_table_insert_safe(analyzer->current_scope, qualified_name, qualified_symbol)) {
            // Not a fatal error - variant is still accessible through unqualified name within enum context
            // This mainly affects cross-module qualified access
    ...
    for (size_t i = 0; i < variant_count; i++) {
        ...
        for (size_t j = i + 1; j < variant_count; j++) {
            ...
            if (strcmp(name_i, name_j) == 0) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_DUPLICATE_SYMBOL,
                                     variant_j->location,
                                     "Duplicate variant name '%s' in enum", name_i);
                return false;
            }
        }
    }
