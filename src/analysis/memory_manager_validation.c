    ...
    if (function_decl->type == AST_EXTERN_DECL) {
        ...
        if (params && args) {
            ...
            if (param_count != arg_count) {
                snprintf(validation->error_message, sizeof(validation->error_message),
                         "Argument count mismatch: expected %zu, got %zu", param_count, arg_count);
                ...
                if (param_annotations) {
                    for (size_t j = 0; j < ast_node_list_size(param_annotations); j++) {
                        ...
                        if (annotation && annotation->type == AST_FFI_ANNOTATION) {
                            FFITransferType transfer =
                                annotation->data.ffi_annotation.transfer_type;
...
void format_ownership_context(const OwnershipContext *context, char *buffer, size_t buffer_size) {
    ...
    snprintf(buffer, buffer_size, "Ownership: %s, Zone: %s, Mutable: %s, Borrowed: %s, Cleanup: %s",
             ownership_names[context->ownership_type], zone_names[context->memory_zone],
             context->is_mutable ? "yes" : "no", context->is_borrowed ? "yes" : "no",
