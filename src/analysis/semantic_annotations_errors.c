...
void report_unknown_annotation_error(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    semantic_report_error(analyzer, SEMANTIC_ERROR_UNKNOWN_ANNOTATION, node->location,
...
void report_parameter_validation_error(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_ANNOTATION_PARAM, node->location,
                          "Invalid parameter '%s' in annotation '#[%s]': %s", param_name,
                          annotation_name, issue);
}
...
void report_context_validation_error(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_ANNOTATION_CONTEXT, node->location,
...
void report_missing_non_deterministic_annotation(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    semantic_report_error(
        analyzer, SEMANTIC_ERROR_NON_DETERMINISTIC_WITHOUT_ANNOTATION, node->location,
...
void report_annotation_conflict_error(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    snprintf(error_message, sizeof(error_message), "Conflicting annotations '%s' and '%s': %s",
...
void report_duplicate_annotation_error(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    semantic_report_error(analyzer, SEMANTIC_ERROR_DUPLICATE_ANNOTATION, node->location,
                          error_message);
}
...
void report_missing_dependency_error(SemanticAnalyzer *analyzer, ASTNode *node,
    ...
    semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_ANNOTATION_PARAM, node->location,
                          "Annotation '#[%s]' requires '#[%s]' to be present. %s", annotation_name,
                          required_annotation, reason ? reason : "");
}
