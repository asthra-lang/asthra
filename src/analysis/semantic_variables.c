...
bool analyze_let_statement(SemanticAnalyzer *analyzer, ASTNode *stmt) {
    ...
    if (!type_node) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION,
                             ...
                             "Use 'let %s: Type = value;' syntax", var_name);
        return false;
    }
...
bool analyze_assignment_validation(SemanticAnalyzer *analyzer, ASTNode *assignment) {
    ...
    if (target_type && value_type) {
        if (!semantic_check_type_compatibility(analyzer, value_type, target_type)) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_INCOMPATIBLE_TYPES,
                                 assignment->location,
    ...
    switch (target->type) {
        case AST_IDENTIFIER: {
            ...
            if (!symbol->flags.is_mutable) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_IMMUTABLE_MODIFICATION,
                                     target->location,
