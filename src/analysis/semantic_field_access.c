...
bool analyze_field_access(SemanticAnalyzer *analyzer, ASTNode *node) {
    ...
    if (object_type->category == TYPE_TUPLE) {
        ...
        if (!node->type_info) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_INTERNAL,
                                 node->location,
                                 "Failed to create type info for tuple element at index %ld", index);
            type_descriptor_release(object_type);
            return false;
        }
    ...
    if (fields_table) {
        ...
        if (!field_symbol) {
            ...
            char **suggestions = find_similar_identifiers(field_name, 
                                                         fields_table, 
                                                         3, &suggestion_count);
            
            if (suggestion_count > 0) {
                char msg[256];
                snprintf(msg, sizeof(msg), "no field named '%s' in struct '%s'", 
                         field_name, object_type->name ? object_type->name : "unknown");
                ...
                semantic_report_error_enhanced(analyzer, SEMANTIC_ERROR_UNDEFINED_SYMBOL,
                                             node->location,
                                             "field not found",
        ...
        if (field_symbol->type) {
            ...
            if (object_type->category == TYPE_GENERIC_INSTANCE) {
                ...
                // This is a simplified version - a full implementation would need proper substitution
