...
bool semantic_check_type_compatibility(SemanticAnalyzer *analyzer, 
    ...
    if (type1->category == TYPE_PRIMITIVE && type2->category == TYPE_PRIMITIVE) {
        ...
        // Can type1 be used where type2 is expected?
        if ((type1_prim == PRIMITIVE_I8 && (type2_prim == PRIMITIVE_I16 || type2_prim == PRIMITIVE_I32 || type2_prim == PRIMITIVE_I64)) ||
            (type1_prim == PRIMITIVE_I16 && (type2_prim == PRIMITIVE_I32 || type2_prim == PRIMITIVE_I64)) ||
        ...
        if (type1_prim == PRIMITIVE_I32 && (type2_prim == PRIMITIVE_I8 || type2_prim == PRIMITIVE_I16 ||
    ...
    if (type1->category == TYPE_TUPLE && type2->category == TYPE_TUPLE) {
        ...
        for (size_t i = 0; i < type1->data.tuple.element_count; i++) {
            if (!semantic_check_type_compatibility(analyzer, 
                                                  type1->data.tuple.element_types[i],
    ...
    if (type1->category == TYPE_GENERIC_INSTANCE && type2->category == TYPE_GENERIC_INSTANCE) {
        ...
        if (type1->data.generic_instance.type_arg_count != type2->data.generic_instance.type_arg_count) {
            return false;
        }
...
bool semantic_can_cast(SemanticAnalyzer *analyzer, 
    ...
    if (from->category == TYPE_PRIMITIVE && to->category == TYPE_PRIMITIVE) {
        ...
        if ((from_prim == PRIMITIVE_I8 && (to_prim == PRIMITIVE_I16 || to_prim == PRIMITIVE_I32 || to_prim == PRIMITIVE_I64)) ||
            (from_prim == PRIMITIVE_I16 && (to_prim == PRIMITIVE_I32 || to_prim == PRIMITIVE_I64)) ||
        ...
        if ((from_prim == PRIMITIVE_I64 && (to_prim == PRIMITIVE_I32 || to_prim == PRIMITIVE_I16 || to_prim == PRIMITIVE_I8)) ||
        ...
        if ((from_prim == PRIMITIVE_I8 || from_prim == PRIMITIVE_I16 || from_prim == PRIMITIVE_I32 || from_prim == PRIMITIVE_I64) &&
        ...
        if ((from_prim == PRIMITIVE_F32 || from_prim == PRIMITIVE_F64) &&
            (to_prim == PRIMITIVE_I8 || to_prim == PRIMITIVE_I16 || to_prim == PRIMITIVE_I32 || to_prim == PRIMITIVE_I64)) {
            return false; // Requires explicit cast
        }
    }
