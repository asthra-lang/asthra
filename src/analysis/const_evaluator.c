...
static ConstValue *evaluate_const_literal(SemanticAnalyzer *analyzer, ASTNode *literal) {
    ...
    default:
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION, literal->location,
...
static ConstValue *evaluate_const_identifier(SemanticAnalyzer *analyzer, const char *identifier) {
    ...
    if (!symbol) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_UNDEFINED_SYMBOL,
                              (SourceLocation){NULL, 0, 0, 0}, "Undefined constant identifier '%s'",
                              identifier);
        return NULL;
    }
    ...
    if (symbol->kind != SYMBOL_CONST) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION,
                              (SourceLocation){NULL, 0, 0, 0}, "Identifier '%s' is not a constant",
                              identifier);
        return NULL;
    }
 ...
 */
static ConstValue *evaluate_const_binary_op(SemanticAnalyzer *analyzer, ConstValue *left,
                                            BinaryOperator op, ConstValue *right) {
    if (!analyzer || !left || !right)
        return NULL;
    ...
    if ((left->type == CONST_VALUE_FLOAT || left->type == CONST_VALUE_INTEGER) &&
        ...
        double l = (left->type == CONST_VALUE_FLOAT) ? left->data.float_value
                                                     : (double)left->data.integer_value;
        double r = (right->type == CONST_VALUE_FLOAT) ? right->data.float_value
                                                      : (double)right->data.integer_value;
    ...
    semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_MISMATCH, (SourceLocation){NULL, 0, 0, 0},
 ...
 */
static ConstValue *evaluate_const_unary_op(SemanticAnalyzer *analyzer, UnaryOperator op,
                                           ConstValue *operand) {
    if (!analyzer || !operand)
        return NULL;
...
ConstValue *evaluate_const_expression(SemanticAnalyzer *analyzer, ASTNode *const_expr) {
    ...
    case CONST_EXPR_BINARY_OP: {
        ...
        ConstValue *left =
            evaluate_const_expression(analyzer, const_expr->data.const_expr.data.binary.left);
        if (!left)
            return NULL;

        ConstValue *right =
            evaluate_const_expression(analyzer, const_expr->data.const_expr.data.binary.right);
        if (!right) {
            const_value_destroy(left);
            return NULL;
        }

        ConstValue *result = evaluate_const_binary_op(
            analyzer, left, const_expr->data.const_expr.data.binary.op, right);
        const_value_destroy(left);
    ...
    case CONST_EXPR_UNARY_OP: {
        // Evaluate operand
        ConstValue *operand =
            evaluate_const_expression(analyzer, const_expr->data.const_expr.data.unary.operand);
        if (!operand)
            return NULL;

        ConstValue *result =
            evaluate_const_unary_op(analyzer, const_expr->data.const_expr.data.unary.op, operand);
    ...
    default:
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION, const_expr->location,
...
bool check_const_dependency_cycle(SemanticAnalyzer *analyzer, const char *const_name,
                                  ASTNode *const_expr) {
    if (!analyzer || !const_name || !const_expr)
        return false;
    ...
    if (const_expr->type == AST_CONST_EXPR) {
        switch (const_expr->data.const_expr.expr_type) {
        case CONST_EXPR_BINARY_OP:
            if (!check_const_dependency_cycle(analyzer, const_name,
                                              const_expr->data.const_expr.data.binary.left) ||
                !check_const_dependency_cycle(analyzer, const_name,
