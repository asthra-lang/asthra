...
static TypeDescriptor* get_binary_op_result_type(
    SemanticAnalyzer *analyzer, BinaryOperator op, TypeDescriptor *left_type, TypeDescriptor *right_type, const SourceLocation *location) {
    ...
    switch (op) {
        ...
        case BINOP_BITWISE_AND: case BINOP_BITWISE_OR: case BINOP_BITWISE_XOR: case BINOP_LSHIFT: case BINOP_RSHIFT: {
...
bool analyze_binary_expression(SemanticAnalyzer *analyzer, ASTNode *expr) {
    ...
    if (!left || !right) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_EXPRESSION,
                             expr->location, "Binary expression missing operand(s)");
        return false;
    }
    ...
    // Get operand types
    TypeDescriptor *left_type = left->type_info ? (TypeDescriptor*)left->type_info->type_descriptor : NULL;
    TypeDescriptor *right_type = right->type_info ? (TypeDescriptor*)right->type_info->type_descriptor : NULL;

    if (!left_type || !right_type) {
        // More specific error reporting
        if (!left->type_info) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_INFERENCE_FAILED,
                                 expr->location, "Left operand has no type information attached");
        } else if (!left->type_info->type_descriptor) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_INFERENCE_FAILED,
                                 expr->location, "Left operand type info has no type descriptor");
        } else if (!right->type_info) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_INFERENCE_FAILED,
                                 expr->location, "Right operand has no type information attached");
        } else if (!right->type_info->type_descriptor) {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_INFERENCE_FAILED,
                                 expr->location, "Right operand type info has no type descriptor");
        } else {
            semantic_report_error(analyzer, SEMANTIC_ERROR_TYPE_INFERENCE_FAILED,
                                 expr->location, "Could not infer types for binary expression operands");
        }
        return false;
    }
    ...
    if (left->flags.is_constant_expr && right->flags.is_constant_expr && is_integer_type(result_type_descriptor)) {
        ...
        if (op == BINOP_DIV) {
            ...
            if (is_zero) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_DIVISION_BY_ZERO,
                                     expr->location, "Division by zero in constant expression");
                return false;
            }
        }
    }
    ...
    if (!expr->type_info) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INTERNAL,
                             expr->location, "Failed to create type info for binary expression result");
        return false;
    }
...
bool analyze_unary_expression(SemanticAnalyzer *analyzer, ASTNode *expr) {
    ...
    if (!operand) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_EXPRESSION,
                             expr->location, "Unary expression missing operand");
        return false;
    }
