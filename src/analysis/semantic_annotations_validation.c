...
bool validate_annotation_context(SemanticAnalyzer *analyzer, const char *annotation_name,
    ...
    if (is_ffi_transfer_annotation(annotation_name)) {
        ...
        return validate_ffi_transfer_annotation_context(analyzer, annotation_name, target_node,
                                                        context);
    }
    ...
    if ((def->valid_contexts & node_context) == 0) {
        report_context_validation_error(analyzer, target_node, annotation_name,
                                        def->valid_contexts);
        return false;
    }
        ...
        if (def) {
            ...
            if (target_context == CONTEXT_STATEMENT && (def->valid_contexts & CONTEXT_FUNCTION)) {
                issue_annotation_warning(
                    analyzer, target_node, WARNING_LOW,
            ...
            if (target_context == CONTEXT_EXPRESSION && (def->valid_contexts & CONTEXT_STATEMENT)) {
                issue_annotation_warning(analyzer, target_node, WARNING_LOW,
                                         "Annotation applied to expression context, typically used "
...
bool analyze_declaration_annotations(SemanticAnalyzer *analyzer, ASTNode *decl) {
    ...
    for (size_t i = 0; i < ast_node_list_size(annotations); i++) {
        ...
        if (annotation->type == AST_SEMANTIC_TAG) {
            ...
            if (annotation_name && !validate_annotation_context_with_inheritance(
                                       analyzer, annotation_name, decl, true)) {
                return false;
            }
        }
    }
