...
bool analyze_struct_declaration(SemanticAnalyzer *analyzer, ASTNode *struct_decl) {
    ...
    ASTNodeList *type_params =
        struct_decl->data.struct_decl.type_params; // Phase 3: Generic Structs

    if (!struct_name) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION, struct_decl->location,
    ...
    if (type_params &&
        !validate_struct_type_parameters(analyzer, type_params, struct_decl->location)) {
        return false;
    }
    ...
    if (!struct_type) {
        semantic_report_error(analyzer, SEMANTIC_ERROR_NONE, struct_decl->location,
    ...
    if (fields && field_count > 0) {
        for (size_t i = 0; i < field_count; i++) {
            ...
            if (!field_name) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION, field->location,
            ...
            if (symbol_table_lookup_local(struct_type->data.struct_type.fields, field_name)) {
                semantic_report_error(analyzer, SEMANTIC_ERROR_DUPLICATE_SYMBOL, field->location,
                                      "Duplicate field '%s' in struct '%s'", field_name,
                                      struct_name);
                continue;
            }
            ...
            if (field_type_node) {
                ...
                if (!field_type) {
                    semantic_report_error(
                        analyzer, SEMANTIC_ERROR_INVALID_TYPE, field_type_node->location,
                        "Invalid type for field '%s' in struct '%s'", field_name, struct_name);
                    continue;
                }

                // Phase 3: For generic structs, validate that field types reference valid type
                // parameters
                if (is_generic &&
                    !validate_field_type_parameters(analyzer, field_type_node, type_params,
                ...
                semantic_report_error(analyzer, SEMANTIC_ERROR_INVALID_OPERATION, field->location,
                                      "Field '%s' in struct '%s' missing type annotation",
            ...
            SymbolEntry *field_symbol =
                symbol_entry_create(field_name, SYMBOL_FIELD, field_type, field);
            if (field_symbol) {
                if (!symbol_table_insert_safe(struct_type->data.struct_type.fields, field_name,
                                              field_symbol)) {
    ...
    SymbolEntry *struct_symbol =
        symbol_entry_create(struct_name, SYMBOL_TYPE, struct_type, struct_decl);
    if (!struct_symbol) {
        symbol_table_destroy(struct_type->data.struct_type.methods);
        symbol_table_destroy(struct_type->data.struct_type.fields);
        free((char *)struct_type->name);
