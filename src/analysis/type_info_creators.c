...
TypeInfo *type_info_create_result(TypeInfo *ok_type, TypeInfo *err_type) {
    ...
    snprintf(name_buffer, (size_t)sizeof(name_buffer), "Result<%s, %s>", ok_type->name,
             err_type->name);
    ...
    type_info->size =
        sizeof(bool) + (ok_type->size > err_type->size ? ok_type->size : err_type->size);
    type_info->alignment =
...
TypeInfo *type_info_create_option(TypeInfo *value_type) {
    ...
    type_info->alignment =
        value_type->alignment > sizeof(bool) ? value_type->alignment : sizeof(bool);
...
TypeInfo *type_info_create_pointer(TypeInfo *pointee_type, bool is_mutable) {
    ...
    char name_buffer[256];
    snprintf(name_buffer, (size_t)sizeof(name_buffer), "*%s%s", is_mutable ? "mut " : "const ",
             pointee_type->name);
...
TypeInfo *type_info_create_function(TypeInfo **param_types, size_t param_count,
    ...
    for (size_t i = 0; i < param_count; i++) {
        if (i > 0)
            strcat(name_buffer, ", ");
        if (param_types[i]) {
            strncat(name_buffer, param_types[i]->name,
