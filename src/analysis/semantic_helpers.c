...
TypeDescriptor *semantic_promote_arithmetic_types(SemanticAnalyzer *analyzer,
    ...
    if (left_type->category == TYPE_PRIMITIVE && right_type->category == TYPE_PRIMITIVE) {
        ...
        static const struct {
            ...
            const char *name;
            int rank;
        } integer_ranks[] = {{PRIMITIVE_I8, "i8", 1},   {PRIMITIVE_I16, "i16", 2},
                             {PRIMITIVE_I32, "i32", 3}, {PRIMITIVE_I64, "i64", 4},
                             {PRIMITIVE_U8, "u8", 1},   {PRIMITIVE_U16, "u16", 2},
                             {PRIMITIVE_U32, "u32", 3}, {PRIMITIVE_U64, "u64", 4}};
...
bool semantic_types_equal(TypeDescriptor *type1, TypeDescriptor *type2) {
    ...
    case TYPE_SLICE:
        return semantic_types_equal(type1->data.slice.element_type, type2->data.slice.element_type);
...
bool semantic_analyze_node_impl(SemanticAnalyzer *analyzer, ASTNode *node) {
    ...
    default:
        semantic_report_error(analyzer, SEMANTIC_ERROR_NONE, node->location,
