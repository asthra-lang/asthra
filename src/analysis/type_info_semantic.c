...
static int map_descriptor_to_info_primitive(int descriptor_kind) {
    ...
    case PRIMITIVE_BOOL:
        return PRIMITIVE_INFO_BOOL;
    case PRIMITIVE_I8:
        return PRIMITIVE_INFO_I8;
    case PRIMITIVE_I16:
        return PRIMITIVE_INFO_I16;
    case PRIMITIVE_I32:
        return PRIMITIVE_INFO_I32;
    case PRIMITIVE_I64:
        return PRIMITIVE_INFO_I64;
    case PRIMITIVE_I128:
        return PRIMITIVE_INFO_I128;
    case PRIMITIVE_U8:
        return PRIMITIVE_INFO_U8;
    case PRIMITIVE_U16:
        return PRIMITIVE_INFO_U16;
    case PRIMITIVE_U32:
        return PRIMITIVE_INFO_U32;
    case PRIMITIVE_U64:
        return PRIMITIVE_INFO_U64;
    case PRIMITIVE_U128:
        return PRIMITIVE_INFO_U128;
    case PRIMITIVE_F32:
        return PRIMITIVE_INFO_F32;
    case PRIMITIVE_F64:
        return PRIMITIVE_INFO_F64;
    case PRIMITIVE_CHAR:
        return PRIMITIVE_INFO_CHAR;
...
static int map_info_to_descriptor_primitive(enum PrimitiveInfoKind info_kind) {
    ...
    case PRIMITIVE_INFO_USIZE:
        return PRIMITIVE_U32; // Map to u32 for now
    case PRIMITIVE_INFO_ISIZE:
        return PRIMITIVE_I32; // Map to i32 for now
    default:
        return PRIMITIVE_VOID; // fallback
    }
}
...
TypeInfo *type_info_from_type_descriptor(const TypeDescriptor *descriptor) {
    ...
    case TYPE_SLICE: {
        TypeInfo *element_type_info =
            type_info_from_type_descriptor(descriptor->data.slice.element_type);
    ...
    case TYPE_RESULT: {
        ...
        if (ok_type_info && err_type_info) {
            ...
            if (ok_type_info)
                type_info_release(ok_type_info);
            if (err_type_info)
                type_info_release(err_type_info);
        }
        break;
    }
    ...
    case TYPE_OPTION: {
        TypeInfo *value_type_info =
            type_info_from_type_descriptor(descriptor->data.option.value_type);
        if (value_type_info) {
    ...
    case TYPE_POINTER: {
        TypeInfo *pointee_type_info =
            type_info_from_type_descriptor(descriptor->data.pointer.pointee_type);
    ...
    case TYPE_FUNCTION: {
        ...
        if (descriptor->data.function.param_count > 0) {
            ...
            for (size_t i = 0; i < descriptor->data.function.param_count; i++) {
                param_type_infos[i] =
                    type_info_from_type_descriptor(descriptor->data.function.param_types[i]);
        ...
        TypeInfo *return_type_info =
            type_info_from_type_descriptor(descriptor->data.function.return_type);
        ...
        type_info = type_info_create_function(
            param_type_infos, descriptor->data.function.param_count, return_type_info);
...
TypeDescriptor *type_descriptor_from_type_info(const TypeInfo *type_info) {
    ...
    case TYPE_INFO_SLICE:
        if (type_info->data.slice.element_type) {
            TypeDescriptor *element_descriptor =
                type_descriptor_from_type_info(type_info->data.slice.element_type);
    ...
    case TYPE_INFO_RESULT:
        if (type_info->data.result.ok_type && type_info->data.result.err_type) {
            TypeDescriptor *ok_descriptor =
                type_descriptor_from_type_info(type_info->data.result.ok_type);
            TypeDescriptor *err_descriptor =
                type_descriptor_from_type_info(type_info->data.result.err_type);
            if (ok_descriptor && err_descriptor) {
                ...
                if (ok_descriptor)
                    type_descriptor_release(ok_descriptor);
                if (err_descriptor)
                    type_descriptor_release(err_descriptor);
            }
        }
        break;
    ...
    case TYPE_INFO_OPTION:
        if (type_info->data.option.value_type) {
            TypeDescriptor *value_descriptor =
                type_descriptor_from_type_info(type_info->data.option.value_type);
            if (value_descriptor) {
    ...
    case TYPE_INFO_POINTER:
        if (type_info->data.pointer.pointee_type) {
            TypeDescriptor *pointee_descriptor =
                type_descriptor_from_type_info(type_info->data.pointer.pointee_type);
    ...
    case TYPE_INFO_STRUCT:
        descriptor =
            type_descriptor_create_struct(type_info->name, type_info->data.struct_info.field_count);
        break;
