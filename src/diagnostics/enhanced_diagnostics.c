...
static char *escape_json_string(const char *str) {
    if (!str) return duplicate_string("null");
...
static const char *suggestion_type_to_string(SuggestionType type) {
    switch (type) {
        case SUGGESTION_INSERT: return "insert";
        case SUGGESTION_DELETE: return "delete";
        case SUGGESTION_REPLACE: return "replace";
        default: return "unknown";
    }
}
...
static bool ensure_span_capacity(EnhancedDiagnostic *diagnostic) {
    ...
    if (diagnostic->span_count >= current_capacity) {
        ...
        DiagnosticSpan *new_spans = realloc(diagnostic->spans, 
                                           sizeof(DiagnosticSpan) * new_capacity);
...
static bool ensure_suggestion_capacity(EnhancedDiagnostic *diagnostic) {
    if (!diagnostic->suggestions) {
        // First allocation
        diagnostic->suggestions = malloc(sizeof(DiagnosticSuggestion) * INITIAL_SUGGESTION_CAPACITY);
    ...
    if (diagnostic->suggestion_count >= current_capacity) {
        size_t new_capacity = current_capacity * 2;
        DiagnosticSuggestion *new_suggestions = realloc(diagnostic->suggestions, 
                                                       sizeof(DiagnosticSuggestion) * new_capacity);
...
bool enhanced_diagnostic_add_span(EnhancedDiagnostic *diagnostic, const DiagnosticSpan *span) {
    ...
    if (!ensure_span_capacity(diagnostic)) return false;
...
bool enhanced_diagnostic_add_suggestion(EnhancedDiagnostic *diagnostic, const DiagnosticSuggestion *suggestion) {
    ...
    if (!ensure_suggestion_capacity(diagnostic)) return false;
...
bool enhanced_diagnostic_set_metadata(EnhancedDiagnostic *diagnostic, const DiagnosticMetadata *metadata) {
    if (!diagnostic || !metadata) return false;
    ...
    if (metadata->type_count > 0 && metadata->inferred_types) {
        ...
        if (!diagnostic->metadata->inferred_types) return false;
    ...
    if (metadata->method_count > 0 && metadata->available_methods) {
        ...
        if (!diagnostic->metadata->available_methods) return false;
        
        for (size_t i = 0; i < metadata->method_count; i++) {
            diagnostic->metadata->available_methods[i] = duplicate_string(metadata->available_methods[i]);
        }
    } else {
    ...
    if (metadata->symbol_count > 0 && metadata->similar_symbols) {
        ...
        if (!diagnostic->metadata->similar_symbols) return false;
        
        for (size_t i = 0; i < metadata->symbol_count; i++) {
            diagnostic->metadata->similar_symbols[i] = duplicate_string(metadata->similar_symbols[i]);
        }
    } else {
...
char *enhanced_diagnostic_to_json(EnhancedDiagnostic *diagnostic) {
    ...
    if (diagnostic->span_count > 0) {
        ...
        for (size_t i = 0; i < diagnostic->span_count; i++) {
            ...
            if (span->file_path) {
                ...
                snprintf(temp, sizeof(temp), ",\n      \"file_path\": %s", escaped_path ? escaped_path : "null");
                APPEND_STR(temp);
                free(escaped_path);
            }
            ...
            if (span->label) {
                ...
                snprintf(temp, sizeof(temp), ",\n      \"label\": %s", escaped_label ? escaped_label : "null");
                APPEND_STR(temp);
                free(escaped_label);
            }
    ...
    if (diagnostic->suggestion_count > 0) {
        ...
        for (size_t i = 0; i < diagnostic->suggestion_count; i++) {
            ...
            snprintf(temp, sizeof(temp), "      \"confidence\": \"%s\"", confidence_level_to_string(suggestion->confidence));
            APPEND_STR(temp);
            
            if (suggestion->rationale) {
                char *escaped_rationale = escape_json_string(suggestion->rationale);
                snprintf(temp, sizeof(temp), ",\n      \"rationale\": %s", escaped_rationale ? escaped_rationale : "null");
                APPEND_STR(temp);
    ...
    if (diagnostic->metadata) {
        ...
        if (diagnostic->metadata->error_category) {
            ...
            snprintf(temp, sizeof(temp), "    \"error_category\": %s", escaped_category ? escaped_category : "null");
            APPEND_STR(temp);
        ...
        if (diagnostic->metadata->symbol_count > 0) {
            ...
            for (size_t i = 0; i < diagnostic->metadata->symbol_count; i++) {
                ...
                snprintf(temp, sizeof(temp), "%s%s", i > 0 ? ", " : "", escaped_symbol ? escaped_symbol : "null");
                APPEND_STR(temp);
                free(escaped_symbol);
            }
            APPEND_STR("]");
