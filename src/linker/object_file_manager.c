...
bool asthra_object_manager_add_search_path(Asthra_ObjectFileManager *manager,
    ...
    if (!manager || !search_path)
        return false;

    // Reallocate search paths array
    char **new_paths =
        realloc(manager->search_paths, (manager->search_path_count + 1) * sizeof(char *));
    if (!new_paths) {
...
bool asthra_object_manager_load_files(Asthra_ObjectFileManager *manager, const char **file_paths,
                                      size_t file_count, size_t *loaded_count) {
    if (!manager || !file_paths || file_count == 0)
        return false;
    ...
    if (loaded_count)
        *loaded_count = successful_loads;

    return successful_loads == file_count;
}

bool asthra_object_manager_unload_file(Asthra_ObjectFileManager *manager, const char *file_path) {
    if (!manager || !file_path)
        return false;
...
size_t asthra_object_manager_get_all_files(Asthra_ObjectFileManager *manager,
                                           Asthra_ObjectFile **files, size_t max_files) {
    if (!manager || !files)
        return 0;
    ...
    while (current) {
        ...
        if (symbol) {
            if (file_containing_symbol)
                *file_containing_symbol = current;
            return symbol;
        }
        current = current->next;
    }
...
bool asthra_object_manager_check_compatibility(Asthra_ObjectFileManager *manager,
                                               Asthra_ObjectFile *file1, Asthra_ObjectFile *file2,
                                               char **error_message) {
    if (!manager || !file1 || !file2)
        return false;
...
void asthra_object_manager_get_statistics(Asthra_ObjectFileManager *manager, size_t *files_loaded,
                                          double *cache_hit_rate, double *total_load_time_ms,
                                          size_t *symbols_processed) {
    if (!manager)
        return;

    if (files_loaded)
        *files_loaded = manager->statistics.files_loaded;
    if (total_load_time_ms)
        *total_load_time_ms = manager->statistics.total_load_time_ms;
    if (symbols_processed)
        *symbols_processed = manager->statistics.total_symbols_processed;

    if (cache_hit_rate) {
        size_t total_ops = manager->statistics.cache_hits + manager->statistics.cache_misses;
        *cache_hit_rate =
            total_ops > 0 ? (double)manager->statistics.cache_hits / (double)total_ops : 0.0;
    }
}
...
const char *asthra_object_manager_get_last_error(Asthra_ObjectFileManager *manager) {
    if (!manager)
        return "Invalid object file manager";
