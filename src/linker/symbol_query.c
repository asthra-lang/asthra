...
void asthra_symbol_resolver_get_statistics(Asthra_SymbolResolver *resolver, size_t *total_symbols,
    ...
    if (hash_efficiency) {
        if (resolver->symbol_count > 0) {
            double collisions_per_symbol =
                (double)resolver->hash_stats.hash_collisions / (double)resolver->symbol_count;
    ...
    for (size_t i = 0; i < resolver->table_size; i++) {
        ...
        while (entry) {
            fprintf(output_file, "  %s: addr=0x%016llx size=%zu status=%d type=%d refs=%zu\n",
                    entry->name, entry->address, entry->size, entry->status, entry->type,
