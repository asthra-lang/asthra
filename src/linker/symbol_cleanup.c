...
void asthra_symbol_entry_free(Asthra_SymbolEntry *entry) {
    ...
    if (entry->defining_file) free(entry->defining_file);
    if (entry->section_name) free(entry->section_name);
    
    // Free all references
    Asthra_SymbolReference *ref = entry->references;
    while (ref) {
        Asthra_SymbolReference *next = ref->next;
        if (ref->referencing_file) free(ref->referencing_file);
        if (ref->section_name) free(ref->section_name);
        free(ref);
        ref = next;
    }
...
void asthra_symbol_conflict_free(Asthra_SymbolConflict *conflict) {
    ...
    if (conflict->symbol_name) free(conflict->symbol_name);
    if (conflict->description) free(conflict->description);
