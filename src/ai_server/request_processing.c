...
DevServerResponse *process_development_server_request(AsthraDevelopmentServer *server,
    ...
    ClientRateLimit *client_limit =
        asthra_dev_server_get_or_create_rate_limit(server, client_id_str, capacity, refill_rate);
    ...
    if (strcmp(request->command, "check") == 0) {
        ...
        if (request->code_content) {
            // Check code snippet
            check_result = fast_check_code_snippet(server->fast_check, request->code_content,
                                                   request->file_path);
        ...
        if (request->code_content && request->line > 0 && request->column > 0) {
            ...
            if (server->fast_check && server->fast_check->analyzer) {
                ...
                const char *keywords[] = {"fn",  "let",   "const",  "if",    "else",    "while",
                                          "for", "match", "struct", "enum",  "impl",    "trait",
                                          "use", "pub",   "return", "break", "continue"};
                ...
                const char *builtins[] = {"print",  "println", "log",  "panic",
                                          "assert", "len",     "range"};
            ...
            const char *json_string =
                json_object_to_json_string_ext(completions_response, JSON_C_TO_STRING_PLAIN);
        ...
        if (request->code_content) {
            ...
            analysis_result = fast_check_code_snippet(server->fast_check, request->code_content,
                                                      request->file_path);
        } else if (request->file_path) {
            // Analyze file
        ...
        if (analysis_result) {
            ...
            if (analysis_result->warning_count > 0) {
                score -=
                    (analysis_result->warning_count * 0.1); // Each warning reduces score by 0.1
            }
            ...
            if (analysis_result->error_count > 0) {
                ...
                json_object *message = json_object_new_string(
                    "Fix syntax and semantic errors to improve code quality");
            ...
            if (analysis_result->warning_count > 0) {
                ...
                json_object *message =
                    json_object_new_string("Address warnings to improve code maintainability");
            ...
            if (analysis_result->check_time_ms > 100.0) {
                ...
                json_object *message = json_object_new_string(
                    "Consider refactoring for better compilation performance");
            ...
            if (score > 0.8) {
                ...
                json_object *message = json_object_new_string(
                    "Great code quality! Consider adding documentation comments");
                json_object *priority = json_object_new_string("low");
            ...
            const char *json_string =
                json_object_to_json_string_ext(analysis_response, JSON_C_TO_STRING_PLAIN);
            response->result_json = strdup(json_string);
            json_object_put(analysis_response);
            fast_check_result_destroy(analysis_result);
        } else {
            response->result_json = strdup(
                "{\"suggestions\": [], \"quality_score\": 0.0, \"error\": \"Analysis failed\"}");
        }
        ...
        for (size_t i = 0; i < server->connection_count; i++) {
            if (request->client_id &&
                strcmp(server->connections[i].client_id, request->client_id) == 0) {
        ...
        if (target_conn && request->file_path) {
            ...
            response->error_message =
                strdup("Failed to subscribe: connection not found or missing file_path");
            server->error_count++;
        }
        ...
        if (target_conn) {
            ...
            send_websocket_text_frame(
                target_conn->socket_fd,
                "{\"type\":\"ai_suggestion\", \"id\":\"SIM_001\", \"description\":\"Refactor this "
                "loop for clarity.\", \"confidence\":0.8\"}");
            send_websocket_text_frame(
                target_conn->socket_fd,
                "{\"type\":\"ai_suggestion\", \"id\":\"SIM_002\", \"description\":\"Consider using "
                "a more idiomatic pattern here.\", \"confidence\":0.7\"}");
