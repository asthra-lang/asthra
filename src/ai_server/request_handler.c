...
char *dev_server_response_to_json(const DevServerResponse *response) {
    ...
    if (response->request_id) {
        json_object_object_add(json_response, "request_id",
    ...
    if (response->error_message) {
        json_object_object_add(json_response, "error_message",
                               json_object_new_string(response->error_message));
    }
    json_object_object_add(json_response, "processing_time_ms",
                           json_object_new_double(response->processing_time_ms));
    ...
    if (response->result_json) {
        ...
        if (nested_result) {
            ...
            json_object_object_add(json_response, "result",
                                   json_object_new_string(response->result_json));
        }
    }

    json_object_object_add(json_response, "timestamp", json_object_new_int64(response->timestamp));
    json_object_object_add(json_response, "response_size",
                           json_object_new_int(response->response_size));

    const char *json_string =
        json_object_to_json_string_ext(json_response, JSON_C_TO_STRING_PRETTY);
    char *result = NULL;
    if (json_string) {
...
const char *dev_server_error_to_string(DevServerErrorCode error) {
    ...
    case DEV_SERVER_OK:
        return "Success";
    case DEV_SERVER_ERROR_INVALID_REQUEST:
        return "Invalid request";
    case DEV_SERVER_ERROR_FILE_NOT_FOUND:
        return "File not found";
    case DEV_SERVER_ERROR_PARSE_ERROR:
        return "Parse error";
    case DEV_SERVER_ERROR_TIMEOUT:
        return "Request timeout";
    case DEV_SERVER_ERROR_SERVER_ERROR:
        return "Internal server error";
    case DEV_SERVER_ERROR_RATE_LIMITED:
        return "Rate limit exceeded";
    case DEV_SERVER_ERROR_CONNECTION_CLOSED:
        return "Connection closed";
    case DEV_SERVER_ERROR_INVALID_PARAMS:
