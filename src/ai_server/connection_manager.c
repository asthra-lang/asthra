...
void asthra_dev_server_add_connection(AsthraDevelopmentServer *server, int client_fd, const char *client_id, const char *websocket_key) {
    ...
    if (server->connection_count >= server->connection_capacity) {
        size_t new_capacity = server->connection_capacity == 0 ? 4 : server->connection_capacity * 2;
        DevServerConnection *new_connections = realloc(server->connections, sizeof(DevServerConnection) * new_capacity);
        if (!new_connections) {
...
void asthra_dev_server_broadcast_notification(AsthraDevelopmentServer *server, const char *notification_json) {
    if (!server || !notification_json) return;
