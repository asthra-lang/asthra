...
void handle_real_time_analysis(AsthraDevelopmentServer *server, DevServerConnection *conn) {
    if (!server || !conn || !conn->is_subscribed_for_analysis || !conn->subscribed_file_path) return;
    ...
    if (check_result) {
        ...
        json_object_object_add(analysis_response, "type", json_object_new_string("real_time_analysis"));
        json_object_object_add(analysis_response, "file_path", json_object_new_string(conn->subscribed_file_path));
        json_object_object_add(analysis_response, "success", json_object_new_boolean(check_result->success));
        json_object_object_add(analysis_response, "check_time_ms", json_object_new_double(check_result->check_time_ms));
        ...
        if (!check_result->success) {
            ...
            json_object_object_add(suggestion_error, "description", json_object_new_string("Consider fixing the syntax error or missing import."));
            ...
            json_object_object_add(info_diag, "message", json_object_new_string("Analysis complete: No issues found."));
            ...
            json_object_object_add(suggestion_perf, "description", json_object_new_string("Function might be optimized for better performance."));
            ...
            json_object_object_add(suggestion_doc, "description", json_object_new_string("Add comprehensive documentation for this function."));
        ...
        const char *json_string = json_object_to_json_string_ext(analysis_response, JSON_C_TO_STRING_PRETTY);
        if (json_string) {
