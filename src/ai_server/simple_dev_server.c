...
SimpleResponse *handle_request(SimpleDevServer *server, SimpleRequest *request) {
    ...
    if (strcmp(request->command, "check") == 0) {
        ...
        snprintf(stats_json, 512,
                 ...
                 "}",
                 server->total_requests, server->total_requests - server->error_count,
                 server->error_count,
                 server->total_requests > 0 ? server->total_response_time / server->total_requests
                                            : 0.0);
...
char *response_to_json(SimpleResponse *response) {
    ...
    snprintf(result, 4096,
             ...
             response->request_id ? response->request_id : "", response->success ? "true" : "false",
