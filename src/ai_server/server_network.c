    ...
    while (!server->should_stop) {
                    ...
                    if (strstr(buffer, "Upgrade: websocket") &&
                        ...
                        if (websocket_key_start && websocket_key_end) {
                            ...
                            if (accept_key) {
                                ...
                                asthra_dev_server_add_connection(server, client_fd, client_id,
                                                                 websocket_key);

                                // Keep connection open for WebSocket communication (loop below)
                                // For now, we will break after handshake for simplicity
                                // A full implementation would enter a WebSocket message handling
                                // loop here For the scope of this task, we will just handle the
                                // handshake.
                                continue; // Continue to next select() call to handle other
                                          // connections

                            } else {
                                fprintf(stderr,
                                        "Error: Failed to generate WebSocket accept key.\n");
                                close(client_fd);
                            }
                        } else {
        ...
        for (size_t i = 0; i < server->connection_count;) {
            ...
            if (conn->is_active) {
                ...
                int conn_activity =
                    select(conn->socket_fd + 1, &conn_readfds, NULL, NULL, &conn_timeout);
                ...
                if (conn_activity < 0 && errno != EINTR) {
                    ...
                    char ws_buffer[4096];
                    ssize_t ws_bytes_read =
                        recv(conn->socket_fd, ws_buffer, sizeof(ws_buffer) - 1, 0);
                    ...
                    if (ws_bytes_read <= 0) {
                        ...
                        if (parse_websocket_frame(ws_buffer, ws_bytes_read, &payload,
                                                  &payload_len)) {
                            // Parse JSON request from WebSocket payload
                            DevServerRequest *request = dev_server_request_create(payload);
                            if (request) {
                                // Handle the request and send response
                                DevServerResponse *response =
                                    process_development_server_request(server, request);
                                if (response) {
