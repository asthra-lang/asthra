...
int main(int argc, char *argv[]) {
    ...
    output_filename = platform_build_output_filename(cli_options.compiler_options.input_file,
    ...
    if (cli_options.test_mode) {
        ...
        // Compile the file
        result = asthra_compile_file(ctx, cli_options.compiler_options.input_file,
                                     cli_options.compiler_options.output_file);
        ...
        if (result != 0) {
            ...
            for (size_t i = 0; i < error_count; i++) {
                ...
                fprintf(stderr, "%s:%zu:%zu: %s: %s\n", error->file ? error->file : "<unknown>",
                        error->line, error->column, error->is_warning ? "warning" : "error",
                        error->message);
            }
