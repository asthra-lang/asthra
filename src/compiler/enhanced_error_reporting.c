...
static bool ensure_reporter_capacity(EnhancedErrorReporter *reporter) {
    if (reporter->diagnostic_count >= reporter->capacity) {
        ...
        EnhancedDiagnostic **new_diagnostics =
            realloc(reporter->diagnostics, sizeof(EnhancedDiagnostic *) * new_capacity);
        if (!new_diagnostics)
            return false;
...
void enhanced_error_reporter_report_undefined_variable(EnhancedErrorReporter *reporter,
    ...
    if (!reporter || !var_name || !file_path)
        return;
    if (!ensure_reporter_capacity(reporter))
        return;
    ...
    EnhancedDiagnostic *diagnostic =
        enhanced_diagnostic_create("ASTHRA_E001", DIAGNOSTIC_ERROR, message);
    if (!diagnostic)
        return;

    // Add span
    DiagnosticSpan span = {.start_line = line,
                           .start_column = column,
                           .end_line = line,
...
void enhanced_error_reporter_report_type_mismatch(EnhancedErrorReporter *reporter,
                                                  ...
                                                  size_t column, const char *file_path) {
    if (!reporter || !expected_type || !actual_type || !file_path)
        return;
    if (!ensure_reporter_capacity(reporter))
        return;
    ...
    // Create diagnostic message
    char message[512];
    snprintf(message, sizeof(message), "type mismatch: expected '%s', found '%s'", expected_type,
             actual_type);

    EnhancedDiagnostic *diagnostic =
        enhanced_diagnostic_create("ASTHRA_E003", DIAGNOSTIC_ERROR, message);
    if (!diagnostic)
        return;
...
void enhanced_error_reporter_report_missing_visibility(EnhancedErrorReporter *reporter,
                                                       ...
                                                       size_t column, const char *file_path) {
    if (!reporter || !decl_type || !file_path)
        return;
    if (!ensure_reporter_capacity(reporter))
        return;
    ...
    char message[256];
    snprintf(message, sizeof(message), "missing required visibility modifier for %s declaration",
             decl_type);

    EnhancedDiagnostic *diagnostic =
        enhanced_diagnostic_create("ASTHRA_E002", DIAGNOSTIC_ERROR, message);
    if (!diagnostic)
        return;
    ...
    if (reporter->enable_suggestions) {
        DiagnosticSuggestion suggestion = {
            ...
            .confidence = CONFIDENCE_HIGH,
            .rationale = safe_strdup(
                "Current PEG grammar requires explicit visibility modifiers (pub/priv)")};
        enhanced_diagnostic_add_suggestion(diagnostic, &suggestion);
    }
...
void enhanced_error_reporter_report_missing_parameter_list(EnhancedErrorReporter *reporter,
                                                           ...
                                                           size_t column, const char *file_path) {
    if (!reporter || !func_name || !file_path)
        return;
    if (!ensure_reporter_capacity(reporter))
        return;
    ...
    // Create diagnostic for current PEG grammar compliance
    char message[256];
    snprintf(message, sizeof(message), "missing required parameter list for function '%s'",
             func_name);

    EnhancedDiagnostic *diagnostic =
        enhanced_diagnostic_create("ASTHRA_E004", DIAGNOSTIC_ERROR, message);
    if (!diagnostic)
        return;
    ...
    if (reporter->enable_suggestions) {
        DiagnosticSuggestion suggestion = {
            ...
            .span = span,
            .text =
                safe_strdup("(none)"), // Use 'none' for empty parameter lists per current grammar
            .confidence = CONFIDENCE_HIGH,
            .rationale = safe_strdup("Current PEG grammar requires explicit parameter lists - use "
...
void enhanced_error_reporter_report_missing_struct_content(EnhancedErrorReporter *reporter,
                                                           ...
                                                           size_t column, const char *file_path) {
    if (!reporter || !struct_name || !file_path)
        return;
    if (!ensure_reporter_capacity(reporter))
        return;
    ...
    EnhancedDiagnostic *diagnostic =
        enhanced_diagnostic_create("ASTHRA_E005", DIAGNOSTIC_ERROR, message);
    if (!diagnostic)
        return;
    ...
    if (reporter->enable_suggestions) {
        DiagnosticSuggestion suggestion = {
            ...
            .span = span,
            .text =
                safe_strdup("{ none }"), // Use 'none' for empty struct content per current grammar
            .confidence = CONFIDENCE_HIGH,
            .rationale = safe_strdup("Current PEG grammar requires explicit struct content - use "
                                     "'none' for empty structs")};
        enhanced_diagnostic_add_suggestion(diagnostic, &suggestion);
    }
...
char *enhanced_error_reporter_to_json(EnhancedErrorReporter *reporter) {
    if (!reporter || reporter->diagnostic_count == 0)
        return NULL;
    ...
    for (size_t i = 0; i < reporter->diagnostic_count; i++) {
        ...
        for (size_t j = 0; j < diagnostic->suggestion_count; j++) {
            ...
            printf("  suggestion (%s confidence): %s\n",
                   ...
                   : (suggestion->confidence == CONFIDENCE_MEDIUM) ? "medium"
                                                                   : "low",
