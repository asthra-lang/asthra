...
bool dependency_graph_add_file(DependencyGraph *graph, const char *file_path) {
    if (!graph || !file_path) return false;
    ...
    if (graph->node_count >= graph->capacity) {
        ...
        FileDependencyNode *new_nodes = realloc(graph->nodes, 
                                               sizeof(FileDependencyNode) * new_capacity);
        if (!new_nodes) {
            pthread_rwlock_unlock(&graph->graph_lock);
            return false;
        }
...
bool dependency_graph_add_dependency(DependencyGraph *graph, const char *file, const char *dependency) {
    if (!graph || !file || !dependency) return false;
    
    pthread_rwlock_wrlock(&graph->graph_lock);
    
    // Ensure both files exist in the graph
    if (!dependency_graph_add_file(graph, file) || 
        !dependency_graph_add_file(graph, dependency)) {
        pthread_rwlock_unlock(&graph->graph_lock);
        return false;
    }
    ...
    char **new_deps = realloc(file_node->dependencies, 
                             sizeof(char*) * (file_node->dependency_count + 1));
    if (!new_deps) {
        pthread_rwlock_unlock(&graph->graph_lock);
        return false;
    }
    ...
    char **new_dependents = realloc(dep_node->dependents, 
                                   sizeof(char*) * (dep_node->dependent_count + 1));
    if (!new_dependents) {
        pthread_rwlock_unlock(&graph->graph_lock);
        return false;
    }
...
char **dependency_graph_get_changed_files(DependencyGraph *graph, const char *file, size_t *count) {
    if (!graph || !file || !count) return NULL;
