...
BottleneckAnalysis *performance_analyze_bottlenecks(const PerformanceProfile *profile,
    ...
    if (!profile || !bottleneck_count)
        return NULL;

    BottleneckAnalysis *bottlenecks =
        calloc(8, sizeof(BottleneckAnalysis)); // Max 8 bottleneck types
    if (!bottlenecks)
        return NULL;
    ...
    if (profile->memory_stats.peak_memory_bytes > 100 * 1024 * 1024) { // > 100MB
        ...
        snprintf(bottlenecks[count].suggested_optimization,
                 sizeof(bottlenecks[count].suggested_optimization),
    ...
    if (profile->parallel_stats.thread_utilization_percentage < 50.0 &&
        profile->parallel_stats.thread_pool_size > 1) {
        bottlenecks[count].type = BOTTLENECK_THREAD_CONTENTION;
        bottlenecks[count].impact_percentage = 20.0;
        snprintf(bottlenecks[count].description, sizeof(bottlenecks[count].description),
                 "Low thread utilization: %.1f%%",
...
performance_get_optimization_recommendations(const PerformanceProfile *profile,
    ...
    if (profile->memory_stats.peak_memory_bytes > 50 * 1024 * 1024) {
        strncpy(recommendations[count].optimization_type, "Memory Pool",
                sizeof(recommendations[count].optimization_type) - 1);
        ...
        snprintf(recommendations[count].implementation_steps,
                 sizeof(recommendations[count].implementation_steps),
    ...
    if (profile->cache_stats.hit_rate_percentage < 80.0) {
        strncpy(recommendations[count].optimization_type, "Cache Tuning",
                sizeof(recommendations[count].optimization_type) - 1);
        snprintf(recommendations[count].description, sizeof(recommendations[count].description),
    ...
    if (profile->parallel_stats.thread_utilization_percentage < 70.0) {
        strncpy(recommendations[count].optimization_type, "Parallelization",
                sizeof(recommendations[count].optimization_type) - 1);
        snprintf(recommendations[count].description, sizeof(recommendations[count].description),
