...
static SemanticAnalysisCache *find_file_entry(FastSemanticCacheManager *cache, const char *file_path) {
    ...
    for (size_t i = 0; i < cache->file_entry_count; i++) {
        if (cache->file_entries[i] && 
            cache->file_entries[i]->file_content_hash == target_hash) {
...
bool fast_semantic_cache_check_memory_limit(FastSemanticCacheManager *cache, size_t additional_bytes) {
...
bool fast_semantic_cache_store_file(FastSemanticCacheManager *cache, const char *file_path, SemanticAnalysisCache *result) {
    if (!cache || !file_path || !result) return false;
    ...
    if (cache->file_entry_count >= cache->file_capacity) {
        ...
        SemanticAnalysisCache **new_entries = realloc(cache->file_entries, 
                                                      sizeof(SemanticAnalysisCache*) * new_capacity);
        if (!new_entries) {
            pthread_rwlock_unlock(&cache->fast_cache_lock);
            return false;
        }
...
bool fast_semantic_cache_is_file_valid(FastSemanticCacheManager *cache, const char *file_path, time_t file_mtime) {
...
FastCacheStatistics fast_semantic_cache_get_statistics(FastSemanticCacheManager *cache) {
    ...
    stats.hit_rate = (cache->total_file_checks > 0) ? 
                     (double)cache->cache_hits / (double)cache->total_file_checks : 0.0;
    stats.memory_usage_bytes = cache->current_memory_usage;
    stats.average_analysis_time_ms = (cache->total_file_checks > 0) ?
                                    (double)cache->total_analysis_time / (double)cache->total_file_checks : 0.0;
    
    // Calculate cache efficiency (hit rate * memory efficiency)
    double memory_efficiency = (cache->max_memory_usage > 0) ?
                              1.0 - ((double)cache->current_memory_usage / (double)cache->max_memory_usage) : 0.0;
...
bool semantic_analysis_cache_populate(SemanticAnalysisCache *cache, 
                                     SymbolTable *symbols, 
