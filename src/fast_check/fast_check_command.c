...
FastCheckCommandResult *fast_check_run_command(FastCheckEngine *engine, const char **files,
    ...
    if (cmd_result->files_checked > 0) {
        cmd_result->avg_time_per_file =
            cmd_result->total_time_ms / (double)cmd_result->files_checked;
    }
    ...
    snprintf(summary_buffer, sizeof(summary_buffer),
             ...
             cmd_result->files_checked, cmd_result->total_time_ms, cmd_result->avg_time_per_file,
             cmd_result->total_errors, cmd_result->total_warnings, cmd_result->cache_hits);
...
FastCheckCommandResult *fast_check_run_on_directory(FastCheckEngine *engine,
    ...
    if (!engine || !directory_path || !config)
        return NULL;

    // Discover files in directory
    size_t file_count;
    char **files =
        fast_check_discover_files(directory_path, config->include_patterns, config->include_count,
                                  config->exclude_patterns, config->exclude_count, &file_count);
    ...
    FastCheckCommandResult *result =
        fast_check_run_command(engine, (const char **)files, file_count, config);
