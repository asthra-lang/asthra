...
void performance_profiler_record_history(PerformanceProfile *profile) {
    ...
    profile->historical_total_times[profile->history_index] =
        performance_timer_get_duration_ms(&profile->overall_timer);
    profile->historical_avg_file_times[profile->history_index] =
        profile->file_stats.average_file_time_ms;
    profile->historical_cache_hit_rates[profile->history_index] =
        profile->cache_stats.hit_rate_percentage;
    profile->historical_peak_memory[profile->history_index] =
        profile->memory_stats.peak_memory_bytes;
...
bool performance_meets_single_file_target(const PerformanceProfile *profile,
...
bool performance_detect_regression(const PerformanceProfile *profile,
    ...
    double avg_total_time = calculate_historical_average_double(profile->historical_total_times,
                                                                profile->history_count);
    double avg_avg_file_time = calculate_historical_average_double(
        profile->historical_avg_file_times, profile->history_count);
    double avg_cache_hit_rate = calculate_historical_average_double(
        profile->historical_cache_hit_rates, profile->history_count);
    size_t avg_peak_memory = calculate_historical_average_size_t(profile->historical_peak_memory,
                                                                 profile->history_count);
    ...
    if (current_total_time > avg_total_time * (1.0 + REGRESSION_THRESHOLD_PERCENT / 100.0)) {
        snprintf(details + strlen(details), sizeof(details) - strlen(details),
                 ...
                 (current_total_time - avg_total_time) / avg_total_time * 100.0, current_total_time,
                 avg_total_time);
        regressed = true;
    }
