...
FastCheckStatistics *fast_check_get_statistics(FastCheckEngine *engine) {
    ...
    stats->average_check_time_ms = engine->total_checks > 0 ? 
        (double)engine->total_check_time / (double)engine->total_checks : 0.0;
    
    size_t total_operations = engine->cache_hits + engine->cache_misses;
    stats->cache_hit_rate_percent = total_operations > 0 ? 
        (engine->cache_hits * 100) / total_operations : 0;
