...
char** fast_check_discover_files(const char *directory, 
                                const char **include_patterns, size_t include_count,
                                const char **exclude_patterns, size_t exclude_count,
                                size_t *file_count) {
...
FileCheckResult* fast_check_single_file_detailed(FastCheckEngine *engine, const char *file_path, FastCheckConfig *config) {
    (void)engine;
    (void)config;
...
FastCheckCommandResult* fast_check_run_command(FastCheckEngine *engine, char **files, size_t file_count, FastCheckConfig *config) {
    (void)engine;
    (void)config;
...
FastCheckCommandResult* fast_check_run_on_directory(FastCheckEngine *engine, const char *directory, FastCheckConfig *config) {
    (void)engine;
    (void)directory;
    (void)config;
...
char* fast_check_format_result_human(FileCheckResult *result) {
    ...
    snprintf(output, 256, "%s: %s (errors: %d, warnings: %d, %.1fms)",
             result->file_path,
             (result->status == FILE_STATUS_OK) ? "OK" : "ERROR",
             result->error_count,
...
FastCheckStatistics* fast_check_get_statistics(FastCheckEngine *engine) {
    ...
    stats->total_analysis_time_ms = engine->stats.average_check_time_ms * engine->stats.total_checks;
...
char* fast_check_get_relative_path(const char *file_path, const char *base_path) {
    if (!file_path || !base_path) return NULL;
