...
void performance_record_file_complete(PerformanceProfile *profile, const char *filename,
    ...
    if (profile->file_stats.files_processed == 1 ||
        duration_ms < profile->file_stats.fastest_time_ms) {
        strncpy(profile->file_stats.fastest_file, filename,
                sizeof(profile->file_stats.fastest_file) - 1);
        profile->file_stats.fastest_time_ms = duration_ms;
    }

    if (profile->file_stats.files_processed == 1 ||
        duration_ms > profile->file_stats.slowest_time_ms) {
        strncpy(profile->file_stats.slowest_file, filename,
                sizeof(profile->file_stats.slowest_file) - 1);
