...
void performance_record_cache_hit(PerformanceProfile *profile, double lookup_time_ms) {
    ...
    double total_time = profile->cache_stats.average_lookup_time_ms *
                        (double)(profile->cache_stats.total_requests - 1);
    profile->cache_stats.average_lookup_time_ms =
        (total_time + lookup_time_ms) / (double)profile->cache_stats.total_requests;

    // Update hit rate
    profile->cache_stats.hit_rate_percentage = (double)profile->cache_stats.cache_hits /
                                               (double)profile->cache_stats.total_requests * 100.0;
