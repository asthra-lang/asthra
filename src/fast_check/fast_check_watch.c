...
int fast_check_start_watch_mode(FastCheckEngine *engine, const char **watch_paths,
                                ...
                                void *user_data) {
    if (!engine || !watch_paths || !callback)
        return -1;
    ...
    while (watch_mode_active) {
        for (size_t i = 0; i < path_count && watch_mode_active; i++) {
            ...
            size_t file_count;
            char **modified_files =
                fast_check_get_modified_files(watch_paths[i], time(NULL) - 1, &file_count);

            if (modified_files) {
                for (size_t j = 0; j < file_count; j++) {
                    FastCheckConfig config = {0};
                    FileCheckResult *result =
                        fast_check_single_file_detailed(engine, modified_files[j], &config);
