...
static int llvm_backend_generate(AsthraBackend *backend, AsthraCompilerContext *ctx,
                                 const ASTNode *ast, const char *output_file) {
    if (!backend || !backend->private_data || !ast) {
        if (backend)
            backend->last_error = "Invalid parameters to generate";
        return -1;
    }
...
static bool llvm_backend_supports_feature(AsthraBackend *backend, const char *feature) {
    ...
    if (strcmp(feature, "debug-info") == 0)
        return true;
    if (strcmp(feature, "optimization") == 0)
        return true;
    if (strcmp(feature, "cross-compilation") == 0)
        return true;
    if (strcmp(feature, "llvm-ir") == 0)
        return true;
    if (strcmp(feature, "bitcode") == 0)
        return true;

    // Target-specific features
    if (strcmp(feature, "x86_64") == 0)
        return true;
    if (strcmp(feature, "aarch64") == 0)
        return true;
    if (strcmp(feature, "arm64") == 0)
        return true;
...
static const char *llvm_backend_get_version(AsthraBackend *backend) {
    ...
    snprintf(version, sizeof(version), "LLVM Backend 1.0 (LLVM %d.%d)", LLVM_VERSION_MAJOR,
             LLVM_VERSION_MINOR);
    return version;
}
...
void llvm_backend_report_error(LLVMBackendData *data, const ASTNode *node, const char *message) {
    ...
    if (!error)
        return; // Out of memory, can't report error
