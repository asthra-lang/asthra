...
void generate_statement(LLVMBackendData *data, const ASTNode *node) {
    ...
    case AST_UNSAFE_BLOCK:
        // When unsafe block is used as a statement, generate it as an expression and discard the result
    ...
    case AST_SPAWN_STMT: {
        ...
        if (node->data.spawn_stmt.call_expr) {
            ...
            if (!function) {
                llvm_backend_report_error_printf(data, node, "Undefined function in spawn: %s", func_name);
                break;
            }
    ...
    case AST_SPAWN_WITH_HANDLE_STMT: {
        ...
        if (!handle_name) {
            llvm_backend_report_error(data, node, "Invalid spawn_with_handle statement: missing handle name");
            break;
        }
        ...
        if (node->data.spawn_with_handle_stmt.call_expr) {
            ...
            if (result_type == data->void_type) {
                // For void functions, create a dummy i32 handle that stores 0 to indicate completion
            ...
            if (!func_name) {
                llvm_backend_report_error(data, node, "Invalid spawn_with_handle statement: missing function name");
                break;
            }
            ...
            LLVMValueRef function = LLVMGetNamedFunction(data->module, func_name);
            if (!function) {
                llvm_backend_report_error_printf(data, node, "Undefined function in spawn_with_handle: %s", func_name);
                break;
            }
            ...
            if (return_type == data->void_type) {
                ...
                LLVMBuildCall2(data->builder, func_type, function, 
                ...
                // For non-void functions, store the result
                LLVMValueRef result = LLVMBuildCall2(data->builder, func_type, function, 
                                                    arg_values, arg_count, "spawn_result");
