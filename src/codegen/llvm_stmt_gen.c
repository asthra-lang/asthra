...
static bool generate_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value, LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block);
static bool generate_literal_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value, LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block);
static bool generate_enum_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value, LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block);
static bool generate_wildcard_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value, LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block);
static bool generate_identifier_pattern_binding(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value);

// Forward declarations for for loop iterable helpers
static bool generate_range_loop(LLVMBackendData *data, const ASTNode *node, const ASTNode *range_call);
...
static bool get_range_bounds(const ASTNode *range_call, LLVMValueRef *start, LLVMValueRef *end, LLVMBackendData *data);
        ...
        case AST_FOR_STMT:
            {
                ...
                if (is_range_function_call(node->data.for_stmt.iterable)) {
                    ...
                    if (type_kind == LLVMArrayTypeKind) {
                        ...
                        llvm_backend_report_error_printf(data, node, "Unsupported iterable type kind: %d", type_kind);
                        break;
                    }
                }
        ...
        case AST_MATCH_STMT:
            {
                ...
                LLVMValueRef function = data->current_function;
                LLVMBasicBlockRef match_end_bb = LLVMAppendBasicBlockInContext(
                    data->context, function, "match_end");
                ...
                for (size_t i = 0; i < node->data.match_stmt.arms->count; i++) {
                    ...
                    LLVMBasicBlockRef arm_block = LLVMAppendBasicBlockInContext(
                        data->context, function, arm_name);
                    
                    char next_name[64];
                    snprintf(next_name, sizeof(next_name), "match_next_%zu", i);
                    LLVMBasicBlockRef next_block = (i == node->data.match_stmt.arms->count - 1) ? 
                        match_end_bb : LLVMAppendBasicBlockInContext(data->context, function, next_name);
                    
                    // Generate pattern check
                    if (!generate_pattern_check(data, arm->data.match_arm.pattern, 
                                               match_value, arm_block, next_block)) {
                    ...
                    if (arm->data.match_arm.guard) {
                        ...
                        if (guard_value) {
                            ...
                            LLVMBasicBlockRef guard_true_bb = LLVMAppendBasicBlockInContext(
                                data->context, function, guard_true_name);
...
static bool generate_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value, 
    ...
    switch (pattern->type) {
        ...
        default:
            llvm_backend_report_error_printf(data, pattern, "Unsupported pattern type: %d", pattern->type);
            return false;
    }
}

// Generate literal pattern check
static bool generate_literal_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value,
                                          LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block) {
    if (!data || !pattern || !value || !match_block || !next_block) {
        return false;
    }
...
static bool generate_enum_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value,
                                       LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block) {
    if (!data || !pattern || !value || !match_block || !next_block) {
        return false;
    }
    ...
    LLVMValueRef tag_ptr = LLVMBuildGEP2(data->builder, enum_type, value, indices, 2, "enum_tag_ptr");
    ...
    LLVMValueRef cmp_result = LLVMBuildICmp(data->builder, LLVMIntEQ, tag_value, expected_tag, "enum_pattern_cmp");
    LLVMBuildCondBr(data->builder, cmp_result, match_block, next_block);
...
static bool generate_wildcard_pattern_check(LLVMBackendData *data, const ASTNode *pattern, LLVMValueRef value,
                                           LLVMBasicBlockRef match_block, LLVMBasicBlockRef next_block) {
    if (!data || !match_block) {
        return false;
    }
...
static bool generate_range_loop(LLVMBackendData *data, const ASTNode *node, const ASTNode *range_call) {
    ...
    LLVMValueRef cond = LLVMBuildICmp(data->builder, LLVMIntSLT, current_val, end_val, "range_cond");
...
static bool generate_array_loop(LLVMBackendData *data, const ASTNode *node, LLVMValueRef array) {
    ...
    LLVMValueRef function = data->current_function;
    LLVMBasicBlockRef loop_bb = LLVMAppendBasicBlockInContext(
        data->context, function, "array_loop");
    LLVMBasicBlockRef loop_body_bb = LLVMAppendBasicBlockInContext(
        data->context, function, "array_body");
    LLVMBasicBlockRef loop_end_bb = LLVMAppendBasicBlockInContext(
        data->context, function, "array_end");
    ...
    LLVMValueRef element_var = LLVMBuildAlloca(data->builder, element_type, node->data.for_stmt.variable);
    ...
    // Loop header - check condition
    LLVMPositionBuilderAtEnd(data->builder, loop_bb);
    LLVMValueRef current_index = LLVMBuildLoad2(data->builder, index_type, index_var, "array_index");
    LLVMValueRef cond = LLVMBuildICmp(data->builder, LLVMIntSLT, current_index, array_length, "array_cond");
    LLVMBuildCondBr(data->builder, cond, loop_body_bb, loop_end_bb);
    ...
    LLVMValueRef element_ptr = LLVMBuildGEP2(data->builder, array_type, array, indices, 2, "array_elem_ptr");
    LLVMValueRef element_value = LLVMBuildLoad2(data->builder, element_type, element_ptr, "array_elem");
...
static bool generate_slice_loop(LLVMBackendData *data, const ASTNode *node, LLVMValueRef slice) {
    ...
    LLVMValueRef ptr_ptr = LLVMBuildGEP2(data->builder, slice_type, slice, ptr_indices, 2, "slice_ptr_ptr");
    LLVMValueRef len_ptr = LLVMBuildGEP2(data->builder, slice_type, slice, len_indices, 2, "slice_len_ptr");
    ...
    LLVMValueRef length_i32 = LLVMBuildTrunc(data->builder, array_length, data->i32_type, "slice_len_i32");
    
    // Create basic blocks
    LLVMValueRef function = data->current_function;
    LLVMBasicBlockRef loop_bb = LLVMAppendBasicBlockInContext(
        data->context, function, "slice_loop");
    LLVMBasicBlockRef loop_body_bb = LLVMAppendBasicBlockInContext(
        data->context, function, "slice_body");
    LLVMBasicBlockRef loop_end_bb = LLVMAppendBasicBlockInContext(
        data->context, function, "slice_end");
    ...
    LLVMValueRef element_var = LLVMBuildAlloca(data->builder, data->ptr_type, node->data.for_stmt.variable);
    ...
    // Loop header - check condition
    LLVMPositionBuilderAtEnd(data->builder, loop_bb);
    LLVMValueRef current_index = LLVMBuildLoad2(data->builder, index_type, index_var, "slice_index");
    LLVMValueRef cond = LLVMBuildICmp(data->builder, LLVMIntSLT, current_index, length_i32, "slice_cond");
    LLVMBuildCondBr(data->builder, cond, loop_body_bb, loop_end_bb);
    ...
    LLVMValueRef current_index_64 = LLVMBuildZExt(data->builder, current_index, data->i64_type, "slice_index_64");
    LLVMValueRef element_ptr = LLVMBuildGEP2(data->builder, data->ptr_type, array_ptr, &current_index_64, 1, "slice_elem_ptr");
