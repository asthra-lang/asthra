...
static void initialize_builtin_backends(void) {
    ...
    backend_registry[0] = (BackendRegistryEntry){
        .type = ASTHRA_BACKEND_LLVM_IR, .name = "LLVM IR Generator", .ops = &llvm_backend_ops};
...
void asthra_backend_get_stats(AsthraBackend *backend, size_t *lines_generated,
    ...
    if (lines_generated)
        *lines_generated = backend->stats.lines_generated;
    if (functions_processed)
        *functions_processed = backend->stats.functions_processed;
    if (generation_time)
        *generation_time = backend->stats.generation_time;
}
...
int asthra_backend_register(AsthraBackendType type, const char *name, const AsthraBackendOps *ops) {
    ...
    // Add to registry
    backend_registry[registry_count++] =
        (BackendRegistryEntry){.type = type, .name = name, .ops = ops};
