...
static const TestTemplate BASIC_TEMPLATES[] = {
    ...
    },
    {
        .name = "function_call",
        .category = "semantic",
        .template_code = "fn add(a: i32, b: i32) -> i32 {\n    return a + b;\n}\nlet result = add(5, 3);",
...
int main(int argc, char **argv) {
    ...
    if (category) opts.test_category = category;
    
    const char *mode = cli_get_string_option(values, 16, "mode");
    if (mode) {
        if (strcmp(mode, "parser") == 0) opts.mode = GEN_MODE_PARSER_TESTS;
        else if (strcmp(mode, "semantic") == 0) opts.mode = GEN_MODE_SEMANTIC_TESTS;
        else if (strcmp(mode, "edge") == 0) opts.mode = GEN_MODE_EDGE_CASES;
        else if (strcmp(mode, "performance") == 0) opts.mode = GEN_MODE_PERFORMANCE_TESTS;
        else if (strcmp(mode, "all") == 0) opts.mode = GEN_MODE_ALL;
    }
    
    const char *complexity = cli_get_string_option(values, 16, "complexity");
    if (complexity) {
        if (strcmp(complexity, "simple") == 0) opts.complexity = COMPLEXITY_SIMPLE;
        else if (strcmp(complexity, "medium") == 0) opts.complexity = COMPLEXITY_MEDIUM;
        else if (strcmp(complexity, "complex") == 0) opts.complexity = COMPLEXITY_COMPLEX;
        else if (strcmp(complexity, "random") == 0) opts.complexity = COMPLEXITY_RANDOM;
    }
    ...
    if (!cli_validate_file_exists(opts.grammar_file)) {
        fprintf(stderr, "Warning: Grammar file '%s' does not exist, proceeding with template-based generation\n", 
                opts.grammar_file);
    }
...
static ToolResult setup_cli_options(CliConfig *config) {
    ...
    // No makefile option
    if (cli_add_option(config, "no-makefile", 'M', false, false,
                       "Skip generating test makefile") != 0) {
        result.success = false;
...
static ToolResult generate_from_templates(GeneratorOptions *opts) {
    ...
    for (int i = 0; i < opts->test_count && i < TEMPLATE_COUNT * 3; i++) {
        ...
        if (opts->mode != GEN_MODE_ALL) {
            if (opts->mode == GEN_MODE_PARSER_TESTS && strcmp(template->category, "parser") != 0) continue;
            if (opts->mode == GEN_MODE_SEMANTIC_TESTS && strcmp(template->category, "semantic") != 0) continue;
        }
        ...
        if (!test_content) continue;
        
        // Create filename
        char filename[256];
        snprintf(filename, sizeof(filename), "%s/test_%s_%d.c", 
                 opts->output_dir, template->name, i / TEMPLATE_COUNT + 1);
...
static ToolResult generate_edge_cases(GeneratorOptions *opts) {
    ...
    const char *edge_cases[] = {
        ...
        "// Edge case: Very long identifier\nfn very_long_function_name_that_tests_identifier_length_limits() -> i32 { return 0; }",
...
static char* expand_template(const TestTemplate *template, int variation_id) {
    ...
    snprintf(expanded, content_size, "// Template: %s (variation %d)\n%s", 
             template->name, variation_id, template->template_code);
...
static char* generate_random_test(GenerationMode mode, ComplexityLevel complexity) {
    ...
    const char *random_tests[] = {
        "fn random_function() -> i32 { return 42; }",
        "let x: i32 = 10;\nlet y: i32 = x + 5;",
        "struct RandomStruct { field: i32, }",
        "enum RandomEnum { A, B, C, }"
    };
