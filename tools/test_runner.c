...
#define run_test(test) _Generic((test), \
    char*: run_test_file, \
    const char*: run_test_file, \
    TestCase*: run_test_case, \
    default: run_test_generic \
)(test)
...
static TestResult run_test_file(const char *file_path) {
    if (!file_path) return TEST_ERROR;
...
static TestResult run_test_case(TestCase *test_case) {
    if (!test_case) return TEST_ERROR;
...
static TestCase* discover_tests(const char *directory, const char *pattern, size_t *test_count) {
    ...
    static TestCase mock_tests[] = {
        {.name = "basic_syntax", .file_path = "tests/basic_syntax.asthra", .result = TEST_PASS, .duration_ms = 0, .error_message = NULL},
        {.name = "arithmetic", .file_path = "tests/arithmetic.asthra", .result = TEST_PASS, .duration_ms = 0, .error_message = NULL},
        {.name = "functions", .file_path = "tests/functions.asthra", .result = TEST_PASS, .duration_ms = 0, .error_message = NULL},
        {.name = "control_flow", .file_path = "tests/control_flow.asthra", .result = TEST_PASS, .duration_ms = 0, .error_message = NULL}
    };
    
    (void)directory; (void)pattern; // Suppress unused parameter warnings
...
static void print_results_text(const TestCase *tests, size_t test_count, const TestRunnerOptions *options) {
    printf("\nTest Results:\n");
    printf("=============\n");
...
static void print_results_json(const TestCase *tests, size_t test_count, const TestRunnerOptions *options) {
    printf("{\n");
    printf("  \"summary\": {\n");
