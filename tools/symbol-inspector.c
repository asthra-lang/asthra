...
static ToolResult trace_type_resolution(SymbolTable *symbols, const char *type_name,
...
static void handle_interactive_command(const char *command, SymbolTable *symbols,
                                       InspectorOptions *opts);
...
int main(int argc, char **argv) {
    ...
    InspectorOptions opts = {.input_file = NULL,
                             .output_file = NULL,
                             .query_symbol = NULL,
    ...
    CliConfig *config =
        cli_create_config("Asthra Symbol Inspector", "[options] <input_file>",
                          "Debug semantic analysis by examining symbol tables and type resolution");
    ...
    if (mode) {
        if (strcmp(mode, "interactive") == 0)
            ...
            opts.mode = MODE_DUMP_ALL;
        else if (strcmp(mode, "scope-tree") == 0)
            opts.mode = MODE_SCOPE_TREE;
        else if (strcmp(mode, "type-trace") == 0)
            opts.mode = MODE_TYPE_TRACE;
    }
...
static ToolResult setup_cli_options(CliConfig *config) {
    ...
    // Output file option
    if (cli_add_option(config, "output", 'o', true, false, "Output file for inspection results") !=
        0) {
    ...
    // Query symbol option
    if (cli_add_option(config, "query", 'q', true, false, "Query specific symbol by name") != 0) {
        result.success = false;
    ...
    if (cli_add_option(config, "no-types", 't', false, false, "Hide type information") != 0) {
        result.success = false;
    ...
    if (cli_add_option(config, "show-unused", 'u', false, false, "Show unused symbols") != 0) {
        result.success = false;
    ...
    // JSON export option
    if (cli_add_option(config, "json", 'j', false, false, "Export results in JSON format") != 0) {
        result.success = false;
...
static void print_symbol_entry(SymbolEntry *entry, InspectorOptions *opts, int indent) {
    ...
    if (entry->flags.is_exported)
        printf("exported ");
    if (entry->flags.is_mutable)
        printf("mutable ");
    if (entry->flags.is_initialized)
        printf("initialized ");
    if (entry->flags.is_predeclared)
        printf("predeclared ");
    printf("\n");
