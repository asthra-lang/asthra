...
int main(int argc, char **argv) {
    ...
    ValidatorOptions opts = {.grammar_file = "grammar.txt",
    ...
    CliConfig *config =
        cli_create_config("Asthra Grammar Validator", "[options]",
    ...
    if (parser_dir)
        opts.parser_source_dir = parser_dir;
...
static ToolResult setup_cli_options(CliConfig *config) {
    ...
    // Strict mode option
    if (cli_add_option(config, "strict", 's', false, false, "Enable strict validation mode") != 0) {
        result.success = false;
...
static bool is_rule_implemented_in_parser(const char *rule_name, const char *parser_dir) {
    ...
    char command[512];
    snprintf(command, sizeof(command), "grep -r \"parse_%s\\|%s\" %s > /dev/null 2>&1", rule_name,
             rule_name, parser_dir);
