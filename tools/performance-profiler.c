...
static void record_hot_path(ProfilingContext *ctx, const char *function, const char *file, int line,
                            uint64_t duration);
...
int main(int argc, char **argv) {
    ...
    ProfilerOptions opts = {.input_file = NULL,
    ...
    CliConfig *config =
        cli_create_config("Asthra Performance Profiler", "[options] <input_file>",
                          "Extend existing benchmark infrastructure with detailed profiling");
    ...
    if (format)
        opts.report_format = format;

    const char *mode = cli_get_string_option(values, 16, "mode");
    if (mode) {
        if (strcmp(mode, "compilation") == 0)
            opts.mode = PROFILE_MODE_COMPILATION;
        else if (strcmp(mode, "phases") == 0)
            opts.mode = PROFILE_MODE_PHASES;
        else if (strcmp(mode, "memory") == 0)
            opts.mode = PROFILE_MODE_MEMORY;
        else if (strcmp(mode, "hot-paths") == 0)
            opts.mode = PROFILE_MODE_HOT_PATHS;
        else if (strcmp(mode, "full") == 0)
            opts.mode = PROFILE_MODE_FULL;
    }
...
static ToolResult setup_cli_options(CliConfig *config) {
    ...
    if (cli_add_option(config, "no-memory", 'M', false, false, "Disable memory usage tracking") !=
        0) {
        result.success = false;
    ...
    // Flamegraph option
    if (cli_add_option(config, "flamegraph", 'F', false, false, "Generate flamegraph data") != 0) {
        result.success = false;
    ...
    // No phases option
    if (cli_add_option(config, "no-phases", 'P', false, false, "Disable detailed phase analysis") !=
        0) {
        result.success = false;
...
static ToolResult profile_compilation_phases(const char *input_file, ProfilingContext *ctx) {
    ...
    if (opt_engine && opt_engine->profiler) {
        ...
        BottleneckType *bottlenecks =
            performance_analyze_bottlenecks(opt_engine->profiler, &ctx->bottleneck_count);

        OptimizationRecommendationType *recommendations =
            performance_get_optimization_recommendations(opt_engine->profiler,
    ...
    ctx->phases[PHASE_TOTAL].duration_ns =
        ctx->phases[PHASE_TOTAL].end_time_ns - ctx->phases[PHASE_TOTAL].start_time_ns;
    ...
    if (prof_engine) {
        ...
        if (prof_result && prof_engine->profiler) {
            ...
            if (profile->overall_timer.duration_ms > 0) {
                double lexer_time =
                    profile->overall_timer.duration_ms * 0.3; // Estimate 30% in lexer
            ...
            if (profile->overall_timer.duration_ms > 0) {
                double parser_time =
                    profile->overall_timer.duration_ms * 0.4; // Estimate 40% in parser
            ...
            if (profile->file_stats.average_file_time_ms > 0) {
                double semantic_time =
                    profile->file_stats.average_file_time_ms * 0.2; // Estimate 20% in semantic
            ...
            if (profile->file_stats.average_file_time_ms > 0) {
                double codegen_time =
                    profile->file_stats.average_file_time_ms * 0.1; // Estimate 10% in codegen
...
static ToolResult generate_performance_report(ProfilerOptions *opts, ProfilingContext *ctx) {
    ...
    if (ctx->phases[PHASE_TOTAL].completed) {
        ...
        for (int phase = 0; phase < PHASE_COUNT; phase++) {
            if (ctx->phases[phase].completed) {
                ...
                fprintf(report, "  %-20s: %8.3f ms\n", get_phase_name((CompilationPhase)phase),
                        duration_ms);
            }
        }
        fprintf(report, "\n");
    ...
    if (opts->enable_hot_path_tracking && ctx->hot_path_count > 0) {
        ...
        for (int i = 0; i < ctx->hot_path_count; i++) {
            ...
            fprintf(report, "  %-30s: %8.3f ms avg (%llu calls)\n", path->function_name, avg_ms,
                    (unsigned long long)path->call_count);
        }
        fprintf(report, "\n");
    }
...
static void end_phase_timing(ProfilingContext *ctx, CompilationPhase phase) {
    ...
    ctx->phases[phase].duration_ns =
        ctx->phases[phase].end_time_ns - ctx->phases[phase].start_time_ns;
