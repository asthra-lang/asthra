...
static bool parse_arguments(int argc, char *argv[], AsthreLinkOptions *options) {
    ...
    while ((c = getopt_long(argc, argv, "o:l:L:sgSvfh", long_options, &option_index)) != -1) {
        ...
        case 'L':
            if (!add_string_to_array(&options->library_paths, &options->library_path_count,
                                     optarg)) {
...
static bool perform_linking(const AsthreLinkOptions *options) {
    ...
    for (size_t i = 0; i < options->input_count; i++) {
        ...
        Asthra_ObjectFile *object_file =
            asthra_object_manager_load_file(manager, options->input_files[i]);
        if (!object_file) {
    ...
    if (options->validate_output) {
        ...
        Asthra_ExecutableGenerator *generator =
            asthra_executable_generator_create(options->output_format);
        if (generator) {
            Asthra_ExecutableValidation validation;
            if (asthra_executable_generator_validate(generator, options->output_file,
                                                     &validation)) {
...
int main(int argc, char *argv[]) {
    ...
    if (options.verbose) {
        ...
        printf("Output format: %s\n", options.output_format == ASTHRA_EXEC_FORMAT_ELF ? "ELF"
                                      : options.output_format == ASTHRA_EXEC_FORMAT_MACH_O
                                          ? "Mach-O"
                                      : options.output_format == ASTHRA_EXEC_FORMAT_PE ? "PE"
                                                                                       : "Unknown");
