...
static ToolResult traverse_ast_node(ASTNode *node, FILE *output, VisualizerOptions *opts,
                                    int depth);
...
int main(int argc, char **argv) {
    ...
    VisualizerOptions opts = {.input_file = NULL,
    ...
    CliConfig *config = cli_create_config(
        "Asthra AST Visualizer", "[options] <input_file>",
...
static ToolResult setup_cli_options(CliConfig *config) {
    ...
    // Compact view option
    if (cli_add_option(config, "compact", 'c', false, false, "Generate compact visualization") !=
        0) {
        result.success = false;
