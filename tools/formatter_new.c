...
static int write_file(const char *filename, const char *content, StatsFramework *stats) {
    ...
    // Update statistics
    stats_increment(stats, "bytes_processed", bytes_written);
...
static char* format_content(const char *content, const FormatterOptions *options, 
    ...
    for (size_t i = 0; i < content_len; i++) {
        ...
        // Find matching format rule
        const FormatRule *rule = NULL;
        for (const FormatRule *r = format_rules; r->trigger != 0; r++) {
            if (c == r->trigger) {
                rule = r;
                break;
            }
        ...
        if (rule) {
            ...
            const char *replacement = rule->replacement;
            while (*replacement && output_pos < output_capacity - 1) {
                output[output_pos++] = *replacement++;
            }
            ...
            stats_increment_by_one(stats, "indentation_changes");
        } else {
    ...
    // Update statistics
    stats_set(stats, "lines_formatted", line_count);
...
int main(int argc, char *argv[]) {
    ...
    stats_add_counter(stats, "indentation_changes", "Indentation changes", false);
    ...
    if (options.check_only) {
        ...
        if (strcmp(content, formatted_content) != 0) {
        ...
        // In-place mode: write back to input file
        if (write_file(options.input_file, formatted_content, stats) != 0) {
            ERROR_REPORT_ERROR(errors, "Failed to write formatted content to input file");
            error_print_all(errors);
            free(content);
            free(formatted_content);
            goto cleanup;
        ...
        // Output file mode: write to specified file
        if (write_file(options.output_file, formatted_content, stats) != 0) {
            ERROR_REPORT_ERROR(errors, "Failed to write formatted content to output file");
            error_print_all(errors);
            free(content);
            free(formatted_content);
            goto cleanup;
